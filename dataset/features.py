...
ME: matmul shape (16-d). VE: vector length + chain depth (12-d).
Scalar: loop nest + body size (10-d).
"""
...
def feature_vector_me(s: MatmulShape) -> list[float]:
    """16-d ME feature vector."""
    M, N, K = s.M, s.N, s.K
    tiles_m = math.ceil(M / 128)
    tiles_n = math.ceil(N / 128)
    ...
    return [
        ...
        *onehot,                       # 4
        ...
        float(tiles_m * 128 - M),      # pad rows
        float(tiles_n * 128 - N),      # pad cols
        acc_is_f32,
    ]


ME_FEATURE_DIM = 16   # 12 shape/log/tile features + 4-way dtype one-hot
...
def feature_vector_ve(s: GenericShape) -> list[float]:
    """20-d VE feature vector (12 shape/dtype + 8 op-class multi-hot)."""
    onehot = [1.0 if s.dtype == d else 0.0 for d in _DTYPES]
    return [
        float(s.n_elems),
...
def feature_vector_scalar(s: GenericShape) -> list[float]:
    """18-d scalar feature vector (10 shape/dtype + 8 op-class multi-hot)."""
...
def feature_vector_contraction(kind: str, B: int, M: int, K: int, N: int,
    ...
    """17-d contraction feature vector."""
    kind_hot = [1.0 if kind == k else 0.0 for k in _CONTRACTION_KINDS]   # 4
    dt_hot = [1.0 if dtype == d else 0.0 for d in _DTYPES]               # 4
    flops = max(1, B * M * K * N)
    return [
...
CONTRACTION_FEATURE_DIM = 9 + len(_CONTRACTION_KINDS) + len(_DTYPES)   # 17
...
def feature_vector_transpose(in_shape: list[int], perm: list[int],
                             dtype: str) -> list[float]:
    """15-d transpose feature vector: rank, size, padded dims + permutation."""
    rank = len(in_shape)
    elems = 1
    for d in in_shape:
        elems *= d
...
TRANSPOSE_FEATURE_DIM = 3 + 2 * _TRANSPOSE_MAXRANK + len(_DTYPES)      # 15
...
def feature_vector_structured(op_kind: str, in_elems: int, out_elems: int,
    ...
    """16-d feature vector for the structured Tier-2 ops."""
...
STRUCTURED_FEATURE_DIM = 7 + len(_STRUCTURED_KINDS) + len(_DTYPES)    # 16
