...
Dtypes: i8 (integer arith), bf16, f32 (float arith). f8E5M2 is excluded — its
elementwise/reduction path does not lower (unrealized_conversion_cast).
argmax is omitted (needs a 2-result generic + linalg.index).
"""
...
def build_generic_corpus(out_dir: Path, n_samples: int, seed: int, *,
    ...
    while written < n_samples and attempts < n_samples * 5:
        ...
        if roll < 0.25:
            ...
            dtype = rng.choice(_FLOAT_DTYPES)
        ...
        if shape is None:
            continue
        uid = f"{engine}_{written:05d}_{rhash[:8]}"
