...
weight relayouts). No ME kernel; lowers on VE and Scalar, so this targets
those engines. Dtypes {i8, bf16, f32} (transpose is dtype-agnostic data
movement). Covers rank-2/3/4 permutations; the cost knobs are the shape and
the permutation, not arithmetic.
"""
...
from dataset.features import feature_vector_transpose
...
def _transpose_module(shape, perm, dtype, proc):
    ...
    module = (
        "module {\n"
        f"  func.func @main(%arg0: {in_t}, %arg1: {out_t}) -> {out_t} {{\n"
