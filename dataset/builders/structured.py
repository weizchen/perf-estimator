...
Dtypes {i8, bf16, f32}; div / unary transcendental elementwise kinds are
float-only. (map, pack, unpack were dropped: map is redundant with generic +
elementwise; pack/unpack fail bufferization in the dispatch path.)
"""
...
from dataset.features import feature_vector_structured
...
def build_structured_corpus(out_dir: Path, n_samples: int, seed: int = 0,
    ...
    while written < n_samples and attempts < n_samples * 6:
        ...
        module, body, dt, ine, oute, rank, n_in, extra = _make(kind, rng, proc)
