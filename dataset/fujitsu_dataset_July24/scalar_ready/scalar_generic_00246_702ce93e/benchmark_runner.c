...
static uint16_t input0_storage[111] __attribute__((aligned(64)));
static uint16_t input1_storage[111] __attribute__((aligned(64)));
static uint16_t output0_storage[111] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 111, 1);
  fill_f16(input1_storage, 111, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_generic_00246_702ce93e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 111));
  return 0;
}
