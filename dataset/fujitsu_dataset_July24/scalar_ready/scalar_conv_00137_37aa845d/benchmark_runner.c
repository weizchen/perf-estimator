...
static int8_t input0_storage[361] __attribute__((aligned(64)));
static int8_t input1_storage[98] __attribute__((aligned(64)));
static int32_t input2_storage[98] __attribute__((aligned(64)));
static int32_t output0_storage[98] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 361, 1);
  fill_i8(input1_storage, 98, 2);
  fill_i32(input2_storage, 98, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 19, 19, 1, 361, 19, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 2, 14, 2, 2, 1, input2_storage, input2_storage, 0, 1, 7, 7, 2, 98, 14, 2, 1, output0_storage, output0_storage, 0, 1, 7, 7, 2, 98, 14, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00137_37aa845d: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 98));
  return 0;
}
