...
static float input0_storage[405504] __attribute__((aligned(64)));
static float input1_storage[180224] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 405504, 1);
  fill_f32(input1_storage, 180224, 2);
  fill_f32(input2_storage, 147456, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 704, 704, 1, input1_storage, input1_storage, 0, 704, 256, 256, 1, input2_storage, input2_storage, 0, 576, 256, 256, 1, output0_storage, output0_storage, 0, 576, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00370_4fdc09fd: benchmark completed");
