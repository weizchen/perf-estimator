...
static int8_t input0_storage[4571136] __attribute__((aligned(64)));
static int8_t input1_storage[294912] __attribute__((aligned(64)));
static int32_t input2_storage[1015808] __attribute__((aligned(64)));
static int32_t output0_storage[1015808] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 4571136, 1);
  fill_i8(input1_storage, 294912, 2);
  fill_i32(input2_storage, 1015808, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 1152, 1152, 1, input1_storage, input1_storage, 0, 1152, 256, 256, 1, input2_storage, input2_storage, 0, 3968, 256, 256, 1, output0_storage, output0_storage, 0, 3968, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00087_a220e411: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1015808));
  return 0;
}
