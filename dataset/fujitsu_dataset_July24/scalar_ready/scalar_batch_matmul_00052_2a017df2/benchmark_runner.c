...
static uint16_t input0_storage[8847360] __attribute__((aligned(64)));
static uint16_t input1_storage[7077888] __attribute__((aligned(64)));
static uint16_t input2_storage[11796480] __attribute__((aligned(64)));
static uint16_t output0_storage[11796480] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 8847360, 1);
  fill_f16(input1_storage, 7077888, 2);
  fill_f16(input2_storage, 11796480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 960, 576, 552960, 576, 1, input1_storage, input1_storage, 0, 16, 576, 768, 442368, 768, 1, input2_storage, input2_storage, 0, 16, 960, 768, 737280, 768, 1, output0_storage, output0_storage, 0, 16, 960, 768, 737280, 768, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00052_2a017df2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 11796480));
  return 0;
}
