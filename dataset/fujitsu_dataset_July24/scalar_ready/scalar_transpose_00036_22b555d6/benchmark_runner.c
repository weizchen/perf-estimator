...
static uint16_t input0_storage[10336] __attribute__((aligned(64)));
static uint16_t input1_storage[10336] __attribute__((aligned(64)));
static uint16_t output0_storage[10336] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 10336, 1);
  fill_f16(input1_storage, 10336, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 38, 17, 4, 4, 272, 16, 4, 1, input1_storage, input1_storage, 0, 38, 17, 4, 4, 272, 16, 4, 1, output0_storage, output0_storage, 0, 38, 17, 4, 4, 272, 16, 4, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00036_22b555d6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 10336));
  return 0;
}
