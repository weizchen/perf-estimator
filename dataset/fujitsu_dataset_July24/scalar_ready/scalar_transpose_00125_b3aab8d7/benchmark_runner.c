...
static int8_t input0_storage[14760] __attribute__((aligned(64)));
static int8_t input1_storage[14760] __attribute__((aligned(64)));
static int8_t output0_storage[14760] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 14760, 1);
  fill_i8(input1_storage, 14760, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 10, 6, 6, 41, 1476, 246, 41, 1, input1_storage, input1_storage, 0, 10, 41, 6, 6, 1476, 36, 6, 1, output0_storage, output0_storage, 0, 10, 41, 6, 6, 1476, 36, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00125_b3aab8d7: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 14760));
  return 0;
}
