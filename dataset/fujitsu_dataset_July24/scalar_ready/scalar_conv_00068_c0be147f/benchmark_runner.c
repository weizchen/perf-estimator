...
static uint16_t input0_storage[65268] __attribute__((aligned(64)));
static uint16_t input1_storage[5550] __attribute__((aligned(64)));
static float input2_storage[8664] __attribute__((aligned(64)));
static float output0_storage[8664] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 65268, 1);
  fill_f16(input1_storage, 5550, 2);
  fill_f32(input2_storage, 8664, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 42, 42, 37, 65268, 1554, 37, 1, input1_storage, input1_storage, 0, 5, 5, 37, 6, 1110, 222, 6, 1, input2_storage, input2_storage, 0, 1, 38, 38, 6, 8664, 228, 6, 1, output0_storage, output0_storage, 0, 1, 38, 38, 6, 8664, 228, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00068_c0be147f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8664));
  return 0;
}
