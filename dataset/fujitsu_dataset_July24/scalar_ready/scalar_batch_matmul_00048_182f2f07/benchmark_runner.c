...
static uint16_t input1_storage[196608] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 196608, 2);
  fill_f32(input2_storage, 196608, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 384, 384, 147456, 384, 1, input1_storage, input1_storage, 0, 4, 384, 128, 49152, 128, 1, input2_storage, input2_storage, 0, 4, 384, 128, 49152, 128, 1, output0_storage, output0_storage, 0, 4, 384, 128, 49152, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00048_182f2f07: benchmark completed");
