...
static float input0_storage[180224] __attribute__((aligned(64)));
static float input1_storage[5316608] __attribute__((aligned(64)));
static float input2_storage[483328] __attribute__((aligned(64)));
static float output0_storage[483328] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 180224, 1);
  fill_f32(input1_storage, 5316608, 2);
  fill_f32(input2_storage, 483328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1408, 1408, 1, input1_storage, input1_storage, 0, 1408, 3776, 3776, 1, input2_storage, input2_storage, 0, 128, 3776, 3776, 1, output0_storage, output0_storage, 0, 128, 3776, 3776, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00335_9db18927: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 483328));
  return 0;
}
