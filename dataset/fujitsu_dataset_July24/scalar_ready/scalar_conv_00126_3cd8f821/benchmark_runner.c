...
static float input0_storage[432] __attribute__((aligned(64)));
static float input1_storage[3] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 432, 1);
  fill_f32(input1_storage, 3, 2);
  fill_f32(input2_storage, 144, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 12, 12, 3, 432, 36, 3, 1, input1_storage, input1_storage, 0, 1, 1, 3, 1, 3, 3, 1, 1, input2_storage, input2_storage, 0, 1, 12, 12, 1, 144, 12, 1, 1, output0_storage, output0_storage, 0, 1, 12, 12, 1, 144, 12, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00126_3cd8f821: benchmark completed");
