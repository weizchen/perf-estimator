...
static float input0_storage[3969] __attribute__((aligned(64)));
static float input1_storage[1050] __attribute__((aligned(64)));
static float input2_storage[37800] __attribute__((aligned(64)));
static float output0_storage[37800] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3969, 1);
  fill_f32(input1_storage, 1050, 2);
  fill_f32(input2_storage, 37800, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 63, 63, 1, 3969, 63, 1, 1, input1_storage, input1_storage, 0, 5, 5, 1, 42, 210, 42, 42, 1, input2_storage, input2_storage, 0, 1, 30, 30, 42, 37800, 1260, 42, 1, output0_storage, output0_storage, 0, 1, 30, 30, 42, 37800, 1260, 42, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00140_505046be: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 37800));
  return 0;
}
