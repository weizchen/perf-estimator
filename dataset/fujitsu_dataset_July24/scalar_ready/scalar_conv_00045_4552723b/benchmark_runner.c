...
static float input0_storage[134848] __attribute__((aligned(64)));
static float input1_storage[1075] __attribute__((aligned(64)));
static float input2_storage[676] __attribute__((aligned(64)));
static float output0_storage[676] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 134848, 1);
  fill_f32(input1_storage, 1075, 2);
  fill_f32(input2_storage, 676, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 56, 56, 43, 134848, 2408, 43, 1, input1_storage, input1_storage, 0, 5, 5, 43, 1, 215, 43, 1, 1, input2_storage, input2_storage, 0, 1, 26, 26, 1, 676, 26, 1, 1, output0_storage, output0_storage, 0, 1, 26, 26, 1, 676, 26, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00045_4552723b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 676));
  return 0;
}
