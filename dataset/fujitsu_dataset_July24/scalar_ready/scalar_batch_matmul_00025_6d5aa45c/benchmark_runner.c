...
static uint16_t input1_storage[2359296] __attribute__((aligned(64)));
static float input2_storage[442368] __attribute__((aligned(64)));
static float output0_storage[442368] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 2359296, 2);
  fill_f32(input2_storage, 442368, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 192, 1024, 196608, 1024, 1, input1_storage, input1_storage, 0, 4, 1024, 576, 589824, 576, 1, input2_storage, input2_storage, 0, 4, 192, 576, 110592, 576, 1, output0_storage, output0_storage, 0, 4, 192, 576, 110592, 576, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00025_6d5aa45c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 442368));
  return 0;
}
