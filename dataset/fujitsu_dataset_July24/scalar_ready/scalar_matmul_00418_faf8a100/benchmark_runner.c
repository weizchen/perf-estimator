...
static float input0_storage[5529600] __attribute__((aligned(64)));
static float input1_storage[14155776] __attribute__((aligned(64)));
static float input2_storage[6553600] __attribute__((aligned(64)));
static float output0_storage[6553600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 5529600, 1);
  fill_f32(input1_storage, 14155776, 2);
  fill_f32(input2_storage, 6553600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 3456, 3456, 1, input1_storage, input1_storage, 0, 3456, 4096, 4096, 1, input2_storage, input2_storage, 0, 1600, 4096, 4096, 1, output0_storage, output0_storage, 0, 1600, 4096, 4096, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00418_faf8a100: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 6553600));
  return 0;
}
