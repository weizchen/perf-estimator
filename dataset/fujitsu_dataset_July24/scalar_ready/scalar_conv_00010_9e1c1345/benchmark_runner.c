...
static float input0_storage[11250] __attribute__((aligned(64)));
static float input1_storage[162] __attribute__((aligned(64)));
static float input2_storage[1521] __attribute__((aligned(64)));
static float output0_storage[1521] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 11250, 1);
  fill_f32(input1_storage, 162, 2);
  fill_f32(input2_storage, 1521, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 25, 25, 18, 11250, 450, 18, 1, input1_storage, input1_storage, 0, 1, 1, 18, 9, 162, 162, 9, 1, input2_storage, input2_storage, 0, 1, 13, 13, 9, 1521, 117, 9, 1, output0_storage, output0_storage, 0, 1, 13, 13, 9, 1521, 117, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00010_9e1c1345: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1521));
  return 0;
}
