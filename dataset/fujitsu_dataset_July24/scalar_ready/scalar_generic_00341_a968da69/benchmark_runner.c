...
static uint16_t input0_storage[166] __attribute__((aligned(64)));
static uint16_t input1_storage[166] __attribute__((aligned(64)));
static uint16_t output0_storage[166] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 166, 1);
  fill_f16(input1_storage, 166, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 166, 1, input1_storage, input1_storage, 0, 166, 1, output0_storage, output0_storage, 0, 166, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00341_a968da69: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 166));
  return 0;
}
