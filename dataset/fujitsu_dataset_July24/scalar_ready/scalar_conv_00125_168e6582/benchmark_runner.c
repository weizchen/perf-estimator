...
static uint16_t input0_storage[832] __attribute__((aligned(64)));
static uint16_t input1_storage[481] __attribute__((aligned(64)));
static float input2_storage[592] __attribute__((aligned(64)));
static float output0_storage[592] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 832, 1);
  fill_f16(input1_storage, 481, 2);
  fill_f32(input2_storage, 592, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 13, 832, 104, 13, 1, input1_storage, input1_storage, 0, 1, 1, 13, 37, 481, 481, 37, 1, input2_storage, input2_storage, 0, 1, 4, 4, 37, 592, 148, 37, 1, output0_storage, output0_storage, 0, 1, 4, 4, 37, 592, 148, 37, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00125_168e6582: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 592));
  return 0;
}
