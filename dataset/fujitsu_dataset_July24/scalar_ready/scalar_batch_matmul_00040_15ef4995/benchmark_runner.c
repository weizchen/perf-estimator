...
static uint16_t input0_storage[1048576] __attribute__((aligned(64)));
static uint16_t input1_storage[393216] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1048576, 1);
  fill_f16(input1_storage, 393216, 2);
  fill_f32(input2_storage, 3145728, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 512, 64, 32768, 64, 1, input1_storage, input1_storage, 0, 32, 64, 192, 12288, 192, 1, input2_storage, input2_storage, 0, 32, 512, 192, 98304, 192, 1, output0_storage, output0_storage, 0, 32, 512, 192, 98304, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00040_15ef4995: benchmark completed");
