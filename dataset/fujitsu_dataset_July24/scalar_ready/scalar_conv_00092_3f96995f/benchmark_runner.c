...
static int8_t input0_storage[400] __attribute__((aligned(64)));
static int8_t input1_storage[1944] __attribute__((aligned(64)));
static int32_t input2_storage[864] __attribute__((aligned(64)));
static int32_t output0_storage[864] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 400, 1);
  fill_i8(input1_storage, 1944, 2);
  fill_i32(input2_storage, 864, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 10, 10, 4, 400, 40, 4, 1, input1_storage, input1_storage, 0, 3, 3, 4, 54, 648, 216, 54, 1, input2_storage, input2_storage, 0, 1, 4, 4, 54, 864, 216, 54, 1, output0_storage, output0_storage, 0, 1, 4, 4, 54, 864, 216, 54, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00092_3f96995f: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 864));
  return 0;
}
