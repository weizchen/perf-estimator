...
static uint16_t input0_storage[2179072] __attribute__((aligned(64)));
static uint16_t input1_storage[630784] __attribute__((aligned(64)));
static uint16_t input2_storage[1712128] __attribute__((aligned(64)));
static uint16_t output0_storage[1712128] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2179072, 1);
  fill_f16(input1_storage, 630784, 2);
  fill_f16(input2_storage, 1712128, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2432, 896, 896, 1, input1_storage, input1_storage, 0, 896, 704, 704, 1, input2_storage, input2_storage, 0, 2432, 704, 704, 1, output0_storage, output0_storage, 0, 2432, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00299_8a307af4: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1712128));
  return 0;
}
