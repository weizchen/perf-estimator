...
static float input0_storage[614400] __attribute__((aligned(64)));
static float input1_storage[184320] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 614400, 1);
  fill_f32(input1_storage, 184320, 2);
  fill_f32(input2_storage, 122880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 960, 960, 1, input1_storage, input1_storage, 0, 960, 192, 192, 1, input2_storage, input2_storage, 0, 640, 192, 192, 1, output0_storage, output0_storage, 0, 640, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00417_a8be8648: benchmark completed");
