...
static float input1_storage[1764] __attribute__((aligned(64)));
static float input2_storage[3600] __attribute__((aligned(64)));
static float output0_storage[3600] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 1764, 2);
  fill_f32(input2_storage, 3600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 16, 16, 1, 256, 16, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 36, 252, 36, 36, 1, input2_storage, input2_storage, 0, 1, 10, 10, 36, 3600, 360, 36, 1, output0_storage, output0_storage, 0, 1, 10, 10, 36, 3600, 360, 36, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00058_90ba587f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3600));
  return 0;
}
