...
static uint16_t input1_storage[1998848] __attribute__((aligned(64)));
static uint16_t input2_storage[1249280] __attribute__((aligned(64)));
static uint16_t output0_storage[1249280] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1998848, 2);
  fill_f16(input2_storage, 1249280, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 512, 512, 1, input1_storage, input1_storage, 0, 512, 3904, 3904, 1, input2_storage, input2_storage, 0, 320, 3904, 3904, 1, output0_storage, output0_storage, 0, 320, 3904, 3904, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00286_f2a4108a: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1249280));
  return 0;
}
