...
static uint16_t input0_storage[753664] __attribute__((aligned(64)));
static uint16_t input1_storage[2260992] __attribute__((aligned(64)));
static uint16_t input2_storage[196608] __attribute__((aligned(64)));
static uint16_t output0_storage[196608] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 753664, 1);
  fill_f16(input1_storage, 2260992, 2);
  fill_f16(input2_storage, 196608, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 768, 768, 1, input2_storage, input2_storage, 0, 256, 768, 768, 1, output0_storage, output0_storage, 0, 256, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00244_1a84e17e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 196608));
  return 0;
}
