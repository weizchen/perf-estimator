...
static int8_t input0_storage[2129920] __attribute__((aligned(64)));
static int8_t input1_storage[2293760] __attribute__((aligned(64)));
static int32_t input2_storage[2981888] __attribute__((aligned(64)));
static int32_t output0_storage[2981888] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 2129920, 1);
  fill_i8(input1_storage, 2293760, 2);
  fill_i32(input2_storage, 2981888, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 1792, 1792, 1, input2_storage, input2_storage, 0, 1664, 1792, 1792, 1, output0_storage, output0_storage, 0, 1664, 1792, 1792, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00100_59d4e47a: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2981888));
  return 0;
}
