...
static int8_t input1_storage[925] __attribute__((aligned(64)));
static int32_t input2_storage[333] __attribute__((aligned(64)));
static int32_t output0_storage[333] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 925, 2);
  fill_i32(input2_storage, 333, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 9, 9, 1, 81, 9, 1, 1, input1_storage, input1_storage, 0, 5, 5, 1, 37, 185, 37, 37, 1, input2_storage, input2_storage, 0, 1, 3, 3, 37, 333, 111, 37, 1, output0_storage, output0_storage, 0, 1, 3, 3, 37, 333, 111, 37, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00066_6c7ad01b: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 333));
  return 0;
}
