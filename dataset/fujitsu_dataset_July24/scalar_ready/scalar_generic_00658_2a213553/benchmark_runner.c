...
static int8_t input1_storage[36] __attribute__((aligned(64)));
static int8_t output0_storage[36] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 36, 1);
  fill_i8(input1_storage, 36, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_generic_00658_2a213553: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 36));
  return 0;
}
