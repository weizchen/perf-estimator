...
static float input0_storage[242] __attribute__((aligned(64)));
static float input1_storage[108] __attribute__((aligned(64)));
static float input2_storage[150] __attribute__((aligned(64)));
static float output0_storage[150] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 242, 1);
  fill_f32(input1_storage, 108, 2);
  fill_f32(input2_storage, 150, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 11, 11, 2, 242, 22, 2, 1, input1_storage, input1_storage, 0, 3, 3, 2, 6, 36, 12, 6, 1, input2_storage, input2_storage, 0, 1, 5, 5, 6, 150, 30, 6, 1, output0_storage, output0_storage, 0, 1, 5, 5, 6, 150, 30, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00034_3811a4d2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 150));
  return 0;
}
