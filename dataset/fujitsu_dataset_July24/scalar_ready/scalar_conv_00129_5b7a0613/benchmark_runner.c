...
static int32_t input2_storage[192] __attribute__((aligned(64)));
static int32_t output0_storage[192] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 192, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 4, 256, 32, 4, 1, input1_storage, input1_storage, 0, 1, 1, 4, 3, 12, 12, 3, 1, input2_storage, input2_storage, 0, 1, 8, 8, 3, 192, 24, 3, 1, output0_storage, output0_storage, 0, 1, 8, 8, 3, 192, 24, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00129_5b7a0613: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 192));
  return 0;
}
