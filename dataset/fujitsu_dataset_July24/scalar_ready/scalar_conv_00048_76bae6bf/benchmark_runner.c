...
static int8_t input1_storage[3] __attribute__((aligned(64)));
static int32_t input2_storage[1200] __attribute__((aligned(64)));
static int32_t output0_storage[1200] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 3, 2);
  fill_i32(input2_storage, 1200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 40, 40, 1, 1600, 40, 1, 1, input1_storage, input1_storage, 0, 1, 1, 1, 3, 3, 3, 3, 1, input2_storage, input2_storage, 0, 1, 20, 20, 3, 1200, 60, 3, 1, output0_storage, output0_storage, 0, 1, 20, 20, 3, 1200, 60, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00048_76bae6bf: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1200));
  return 0;
}
