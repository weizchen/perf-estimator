...
static uint16_t input0_storage[57500] __attribute__((aligned(64)));
static uint16_t input1_storage[23] __attribute__((aligned(64)));
static float input2_storage[2500] __attribute__((aligned(64)));
static float output0_storage[2500] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 57500, 1);
  fill_f16(input1_storage, 23, 2);
  fill_f32(input2_storage, 2500, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 50, 50, 23, 57500, 1150, 23, 1, input1_storage, input1_storage, 0, 1, 1, 23, 1, 23, 23, 1, 1, input2_storage, input2_storage, 0, 1, 50, 50, 1, 2500, 50, 1, 1, output0_storage, output0_storage, 0, 1, 50, 50, 1, 2500, 50, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00111_ba6948e8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2500));
  return 0;
}
