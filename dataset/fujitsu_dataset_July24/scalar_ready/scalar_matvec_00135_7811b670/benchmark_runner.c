...
static uint16_t input0_storage[286720] __attribute__((aligned(64)));
static uint16_t input1_storage[896] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 286720, 1);
  fill_f16(input1_storage, 896, 2);
  fill_f32(input2_storage, 320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 896, 896, 1, input1_storage, input1_storage, 0, 896, 1, input2_storage, input2_storage, 0, 320, 1, output0_storage, output0_storage, 0, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_matvec_00135_7811b670: benchmark completed");
