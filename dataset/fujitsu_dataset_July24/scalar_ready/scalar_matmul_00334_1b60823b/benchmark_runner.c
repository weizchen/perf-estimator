...
static float input0_storage[2949120] __attribute__((aligned(64)));
static float input1_storage[3391488] __attribute__((aligned(64)));
static float input2_storage[1884160] __attribute__((aligned(64)));
static float output0_storage[1884160] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 2949120, 1);
  fill_f32(input1_storage, 3391488, 2);
  fill_f32(input2_storage, 1884160, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 2304, 2304, 1, input1_storage, input1_storage, 0, 2304, 1472, 1472, 1, input2_storage, input2_storage, 0, 1280, 1472, 1472, 1, output0_storage, output0_storage, 0, 1280, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00334_1b60823b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1884160));
  return 0;
}
