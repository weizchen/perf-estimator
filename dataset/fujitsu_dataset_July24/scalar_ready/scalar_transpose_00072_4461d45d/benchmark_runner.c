...
static uint16_t input0_storage[24000] __attribute__((aligned(64)));
static uint16_t input1_storage[24000] __attribute__((aligned(64)));
static uint16_t output0_storage[24000] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 24000, 1);
  fill_f16(input1_storage, 24000, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 10, 10, 6, 40, 2400, 240, 40, 1, input1_storage, input1_storage, 0, 10, 6, 10, 40, 2400, 400, 40, 1, output0_storage, output0_storage, 0, 10, 6, 10, 40, 2400, 400, 40, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00072_4461d45d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 24000));
  return 0;
}
