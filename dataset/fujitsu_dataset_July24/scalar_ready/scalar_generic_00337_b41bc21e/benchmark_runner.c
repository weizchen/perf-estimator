...
static uint16_t input1_storage[38] __attribute__((aligned(64)));
static uint16_t output0_storage[38] __attribute__((aligned(64)));
...
int main(void) {
  ...
  ofa_dma_memory_fence();
  puts("scalar_generic_00337_b41bc21e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 38));
  return 0;
}
