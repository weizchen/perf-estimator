...
static float input0_storage[160] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 160, 1);
  fill_f32(input1_storage, 163840, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 160, 1, input1_storage, input1_storage, 0, 1, 160, 32, 32, 163840, 1024, 32, 1, output0_storage, output0_storage, 0, 1, 160, 32, 32, 163840, 1024, 32, 1);
  ofa_dma_memory_fence();
  puts("nvidia_mit-b0_start176_end181_32__forward__op0__scalar: benchmark completed");
