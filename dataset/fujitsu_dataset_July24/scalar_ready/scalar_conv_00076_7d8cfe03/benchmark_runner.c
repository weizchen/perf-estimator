...
static uint16_t input0_storage[7396] __attribute__((aligned(64)));
static uint16_t input1_storage[4] __attribute__((aligned(64)));
static float input2_storage[1849] __attribute__((aligned(64)));
static float output0_storage[1849] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 7396, 1);
  fill_f16(input1_storage, 4, 2);
  fill_f32(input2_storage, 1849, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 43, 43, 4, 7396, 172, 4, 1, input1_storage, input1_storage, 0, 1, 1, 4, 1, 4, 4, 1, 1, input2_storage, input2_storage, 0, 1, 43, 43, 1, 1849, 43, 1, 1, output0_storage, output0_storage, 0, 1, 43, 43, 1, 1849, 43, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00076_7d8cfe03: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1849));
  return 0;
}
