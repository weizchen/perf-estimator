...
static float input0_storage[54] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 54, 1);
  fill_f32(input1_storage, 54, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 54, 1, input1_storage, input1_storage, 0, 54, 1, output0_storage, output0_storage, 0, 54, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00681_cd73ae20: benchmark completed");
