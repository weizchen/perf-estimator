...
static uint16_t input0_storage[278528] __attribute__((aligned(64)));
static uint16_t input1_storage[32768] __attribute__((aligned(64)));
static uint16_t input2_storage[139264] __attribute__((aligned(64)));
static uint16_t output0_storage[139264] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 278528, 1);
  fill_f16(input1_storage, 32768, 2);
  fill_f16(input2_storage, 139264, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 256, 256, 1, input1_storage, input1_storage, 0, 256, 128, 128, 1, input2_storage, input2_storage, 0, 1088, 128, 128, 1, output0_storage, output0_storage, 0, 1088, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00214_ec7783f4: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 139264));
  return 0;
}
