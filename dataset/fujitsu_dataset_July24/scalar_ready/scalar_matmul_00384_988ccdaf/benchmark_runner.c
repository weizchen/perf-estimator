...
static float input0_storage[1396736] __attribute__((aligned(64)));
static float input1_storage[1015808] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1396736, 1);
  fill_f32(input1_storage, 1015808, 2);
  fill_f32(input2_storage, 360448, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 1984, 1984, 1, input1_storage, input1_storage, 0, 1984, 512, 512, 1, input2_storage, input2_storage, 0, 704, 512, 512, 1, output0_storage, output0_storage, 0, 704, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00384_988ccdaf: benchmark completed");
