...
static uint16_t input0_storage[10400] __attribute__((aligned(64)));
static uint16_t input1_storage[1170] __attribute__((aligned(64)));
static float input2_storage[1620] __attribute__((aligned(64)));
static float output0_storage[1620] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 10400, 1);
  fill_f16(input1_storage, 1170, 2);
  fill_f32(input2_storage, 1620, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 20, 20, 26, 10400, 520, 26, 1, input1_storage, input1_storage, 0, 3, 3, 26, 5, 390, 130, 5, 1, input2_storage, input2_storage, 0, 1, 18, 18, 5, 1620, 90, 5, 1, output0_storage, output0_storage, 0, 1, 18, 18, 5, 1620, 90, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00123_afef322e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1620));
  return 0;
}
