...
static uint16_t input0_storage[491520] __attribute__((aligned(64)));
static uint16_t input1_storage[5242880] __attribute__((aligned(64)));
static float input2_storage[3145728] __attribute__((aligned(64)));
static float output0_storage[3145728] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 491520, 1);
  fill_f16(input1_storage, 5242880, 2);
  fill_f32(input2_storage, 3145728, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 192, 320, 61440, 320, 1, input1_storage, input1_storage, 0, 8, 320, 2048, 655360, 2048, 1, input2_storage, input2_storage, 0, 8, 192, 2048, 393216, 2048, 1, output0_storage, output0_storage, 0, 8, 192, 2048, 393216, 2048, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00033_44c1a7b7: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3145728));
  return 0;
}
