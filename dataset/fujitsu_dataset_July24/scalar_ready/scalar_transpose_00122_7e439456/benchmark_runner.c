...
static float input0_storage[7128] __attribute__((aligned(64)));
static float input1_storage[7128] __attribute__((aligned(64)));
static float output0_storage[7128] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 7128, 1);
  fill_f32(input1_storage, 7128, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 12, 9, 6, 11, 594, 66, 11, 1, input1_storage, input1_storage, 0, 12, 11, 6, 9, 594, 54, 9, 1, output0_storage, output0_storage, 0, 12, 11, 6, 9, 594, 54, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00122_7e439456: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7128));
  return 0;
}
