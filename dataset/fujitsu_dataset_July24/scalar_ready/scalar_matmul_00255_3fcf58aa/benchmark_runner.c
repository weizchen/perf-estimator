...
static uint16_t input0_storage[176128] __attribute__((aligned(64)));
static uint16_t input1_storage[12288] __attribute__((aligned(64)));
static uint16_t input2_storage[528384] __attribute__((aligned(64)));
static uint16_t output0_storage[528384] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 176128, 1);
  fill_f16(input1_storage, 12288, 2);
  fill_f16(input2_storage, 528384, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 64, 64, 1, input1_storage, input1_storage, 0, 64, 192, 192, 1, input2_storage, input2_storage, 0, 2752, 192, 192, 1, output0_storage, output0_storage, 0, 2752, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00255_3fcf58aa: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 528384));
  return 0;
}
