...
static float input0_storage[841] __attribute__((aligned(64)));
static float input1_storage[147] __attribute__((aligned(64)));
static float input2_storage[432] __attribute__((aligned(64)));
static float output0_storage[432] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 841, 1);
  fill_f32(input1_storage, 147, 2);
  fill_f32(input2_storage, 432, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 29, 29, 1, 841, 29, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 3, 21, 3, 3, 1, input2_storage, input2_storage, 0, 1, 12, 12, 3, 432, 36, 3, 1, output0_storage, output0_storage, 0, 1, 12, 12, 3, 432, 36, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00107_5f58ddca: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 432));
  return 0;
}
