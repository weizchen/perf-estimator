...
static uint16_t input0_storage[8000] __attribute__((aligned(64)));
static uint16_t input1_storage[20] __attribute__((aligned(64)));
static float input2_storage[400] __attribute__((aligned(64)));
static float output0_storage[400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 8000, 1);
  fill_f16(input1_storage, 20, 2);
  fill_f32(input2_storage, 400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 20, 20, 20, 8000, 400, 20, 1, input1_storage, input1_storage, 0, 1, 1, 20, 1, 20, 20, 1, 1, input2_storage, input2_storage, 0, 1, 20, 20, 1, 400, 20, 1, 1, output0_storage, output0_storage, 0, 1, 20, 20, 1, 400, 20, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00136_4134e1fd: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 400));
  return 0;
}
