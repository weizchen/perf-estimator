...
static int8_t input0_storage[2187] __attribute__((aligned(64)));
static int8_t input1_storage[588] __attribute__((aligned(64)));
static int32_t input2_storage[1764] __attribute__((aligned(64)));
static int32_t output0_storage[1764] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 2187, 1);
  fill_i8(input1_storage, 588, 2);
  fill_i32(input2_storage, 1764, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 27, 27, 3, 2187, 81, 3, 1, input1_storage, input1_storage, 0, 7, 7, 3, 4, 84, 12, 4, 1, input2_storage, input2_storage, 0, 1, 21, 21, 4, 1764, 84, 4, 1, output0_storage, output0_storage, 0, 1, 21, 21, 4, 1764, 84, 4, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00098_ceb93ae3: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1764));
  return 0;
}
