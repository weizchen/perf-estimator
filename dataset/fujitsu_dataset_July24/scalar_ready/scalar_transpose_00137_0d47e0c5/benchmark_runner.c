...
static float input0_storage[125280] __attribute__((aligned(64)));
static float input1_storage[125280] __attribute__((aligned(64)));
static float output0_storage[125280] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 125280, 1);
  fill_f32(input1_storage, 125280, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 24, 4, 29, 45, 5220, 1305, 45, 1, input1_storage, input1_storage, 0, 24, 4, 45, 29, 5220, 1305, 29, 1, output0_storage, output0_storage, 0, 24, 4, 45, 29, 5220, 1305, 29, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00137_0d47e0c5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 125280));
  return 0;
}
