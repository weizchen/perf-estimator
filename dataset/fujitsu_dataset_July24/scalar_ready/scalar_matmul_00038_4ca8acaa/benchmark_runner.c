...
static int8_t input1_storage[507904] __attribute__((aligned(64)));
static int32_t input2_storage[2031616] __attribute__((aligned(64)));
static int32_t output0_storage[2031616] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 507904, 2);
  fill_i32(input2_storage, 2031616, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 128, 128, 1, input1_storage, input1_storage, 0, 128, 3968, 3968, 1, input2_storage, input2_storage, 0, 512, 3968, 3968, 1, output0_storage, output0_storage, 0, 512, 3968, 3968, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00038_4ca8acaa: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2031616));
  return 0;
}
