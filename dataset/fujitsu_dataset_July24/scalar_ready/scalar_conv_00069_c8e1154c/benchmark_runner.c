...
static float input0_storage[10816] __attribute__((aligned(64)));
static float input1_storage[21952] __attribute__((aligned(64)));
static float input2_storage[11200] __attribute__((aligned(64)));
static float output0_storage[11200] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 10816, 1);
  fill_f32(input1_storage, 21952, 2);
  fill_f32(input2_storage, 11200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 26, 26, 16, 10816, 416, 16, 1, input1_storage, input1_storage, 0, 7, 7, 16, 28, 3136, 448, 28, 1, input2_storage, input2_storage, 0, 1, 20, 20, 28, 11200, 560, 28, 1, output0_storage, output0_storage, 0, 1, 20, 20, 28, 11200, 560, 28, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00069_c8e1154c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 11200));
  return 0;
}
