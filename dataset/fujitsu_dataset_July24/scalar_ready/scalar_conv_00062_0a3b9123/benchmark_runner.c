...
static float input0_storage[8750] __attribute__((aligned(64)));
static float input1_storage[3430] __attribute__((aligned(64)));
static float input2_storage[1805] __attribute__((aligned(64)));
static float output0_storage[1805] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 8750, 1);
  fill_f32(input1_storage, 3430, 2);
  fill_f32(input2_storage, 1805, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 25, 25, 14, 8750, 350, 14, 1, input1_storage, input1_storage, 0, 7, 7, 14, 5, 490, 70, 5, 1, input2_storage, input2_storage, 0, 1, 19, 19, 5, 1805, 95, 5, 1, output0_storage, output0_storage, 0, 1, 19, 19, 5, 1805, 95, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00062_0a3b9123: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1805));
  return 0;
}
