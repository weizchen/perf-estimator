...
static uint16_t input0_storage[139264] __attribute__((aligned(64)));
static uint16_t input1_storage[835584] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 139264, 1);
  fill_f16(input1_storage, 835584, 2);
  fill_f32(input2_storage, 49152, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 64, 1088, 69632, 1088, 1, input1_storage, input1_storage, 0, 2, 1088, 384, 417792, 384, 1, input2_storage, input2_storage, 0, 2, 64, 384, 24576, 384, 1, output0_storage, output0_storage, 0, 2, 64, 384, 24576, 384, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00036_db0244e1: benchmark completed");
