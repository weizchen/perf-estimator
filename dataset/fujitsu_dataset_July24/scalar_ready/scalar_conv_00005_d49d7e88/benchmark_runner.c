...
static float input0_storage[4608] __attribute__((aligned(64)));
static float input1_storage[2744] __attribute__((aligned(64)));
static float input2_storage[12348] __attribute__((aligned(64)));
static float output0_storage[12348] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 4608, 1);
  fill_f32(input1_storage, 2744, 2);
  fill_f32(input2_storage, 12348, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 48, 48, 2, 4608, 96, 2, 1, input1_storage, input1_storage, 0, 7, 7, 2, 28, 392, 56, 28, 1, input2_storage, input2_storage, 0, 1, 21, 21, 28, 12348, 588, 28, 1, output0_storage, output0_storage, 0, 1, 21, 21, 28, 12348, 588, 28, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00005_d49d7e88: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 12348));
  return 0;
}
