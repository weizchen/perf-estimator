...
static uint16_t input0_storage[8216576] __attribute__((aligned(64)));
static uint16_t input1_storage[483328] __attribute__((aligned(64)));
static uint16_t input2_storage[278528] __attribute__((aligned(64)));
static uint16_t output0_storage[278528] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 8216576, 1);
  fill_f16(input1_storage, 483328, 2);
  fill_f16(input2_storage, 278528, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 3776, 3776, 1, input1_storage, input1_storage, 0, 3776, 128, 128, 1, input2_storage, input2_storage, 0, 2176, 128, 128, 1, output0_storage, output0_storage, 0, 2176, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00304_c0974349: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 278528));
  return 0;
}
