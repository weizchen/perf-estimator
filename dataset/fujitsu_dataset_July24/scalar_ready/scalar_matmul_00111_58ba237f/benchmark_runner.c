...
static uint16_t input1_storage[315392] __attribute__((aligned(64)));
static float input2_storage[86016] __attribute__((aligned(64)));
static float output0_storage[86016] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 315392, 2);
  fill_f32(input2_storage, 86016, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 704, 704, 1, input1_storage, input1_storage, 0, 704, 448, 448, 1, input2_storage, input2_storage, 0, 192, 448, 448, 1, output0_storage, output0_storage, 0, 192, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00111_58ba237f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 86016));
  return 0;
}
