...
static int8_t input0_storage[1452] __attribute__((aligned(64)));
static int8_t input1_storage[1296] __attribute__((aligned(64)));
static int32_t input2_storage[300] __attribute__((aligned(64)));
static int32_t output0_storage[300] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1452, 1);
  fill_i8(input1_storage, 1296, 2);
  fill_i32(input2_storage, 300, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 11, 11, 12, 1452, 132, 12, 1, input1_storage, input1_storage, 0, 3, 3, 12, 12, 432, 144, 12, 1, input2_storage, input2_storage, 0, 1, 5, 5, 12, 300, 60, 12, 1, output0_storage, output0_storage, 0, 1, 5, 5, 12, 300, 60, 12, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00131_93360548: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 300));
  return 0;
}
