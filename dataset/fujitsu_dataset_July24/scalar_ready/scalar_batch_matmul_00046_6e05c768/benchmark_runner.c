...
static uint16_t input0_storage[10223616] __attribute__((aligned(64)));
static uint16_t input1_storage[13631488] __attribute__((aligned(64)));
static float input2_storage[1572864] __attribute__((aligned(64)));
static float output0_storage[1572864] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 10223616, 1);
  fill_f16(input1_storage, 13631488, 2);
  fill_f32(input2_storage, 1572864, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 192, 1664, 319488, 1664, 1, input1_storage, input1_storage, 0, 32, 1664, 256, 425984, 256, 1, input2_storage, input2_storage, 0, 32, 192, 256, 49152, 256, 1, output0_storage, output0_storage, 0, 32, 192, 256, 49152, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00046_6e05c768: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1572864));
  return 0;
}
