...
static uint16_t input0_storage[1664] __attribute__((aligned(64)));
static uint16_t input1_storage[702] __attribute__((aligned(64)));
static float input2_storage[108] __attribute__((aligned(64)));
static float output0_storage[108] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1664, 1);
  fill_f16(input1_storage, 702, 2);
  fill_f32(input2_storage, 108, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 26, 1664, 208, 26, 1, input1_storage, input1_storage, 0, 3, 3, 26, 3, 234, 78, 3, 1, input2_storage, input2_storage, 0, 1, 6, 6, 3, 108, 18, 3, 1, output0_storage, output0_storage, 0, 1, 6, 6, 3, 108, 18, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00110_babd6d82: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 108));
  return 0;
}
