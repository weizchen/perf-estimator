...
static int8_t input0_storage[5618] __attribute__((aligned(64)));
static int8_t input1_storage[56] __attribute__((aligned(64)));
static int32_t input2_storage[78652] __attribute__((aligned(64)));
static int32_t output0_storage[78652] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 5618, 1);
  fill_i8(input1_storage, 56, 2);
  fill_i32(input2_storage, 78652, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 53, 53, 2, 5618, 106, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 28, 56, 56, 28, 1, input2_storage, input2_storage, 0, 1, 53, 53, 28, 78652, 1484, 28, 1, output0_storage, output0_storage, 0, 1, 53, 53, 28, 78652, 1484, 28, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00008_4cbdf93c: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 78652));
  return 0;
}
