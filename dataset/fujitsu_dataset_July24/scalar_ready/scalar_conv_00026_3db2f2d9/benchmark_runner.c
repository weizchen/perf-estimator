...
static float input1_storage[80] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 80, 2);
  fill_f32(input2_storage, 720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 12, 12, 4, 576, 48, 4, 1, input1_storage, input1_storage, 0, 1, 1, 4, 20, 80, 80, 20, 1, input2_storage, input2_storage, 0, 1, 6, 6, 20, 720, 120, 20, 1, output0_storage, output0_storage, 0, 1, 6, 6, 20, 720, 120, 20, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00026_3db2f2d9: benchmark completed");
