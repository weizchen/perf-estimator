...
static float input0_storage[110592] __attribute__((aligned(64)));
static float input1_storage[552960] __attribute__((aligned(64)));
static float input2_storage[20480] __attribute__((aligned(64)));
static float output0_storage[20480] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 110592, 1);
  fill_f32(input1_storage, 552960, 2);
  fill_f32(input2_storage, 20480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 320, 320, 1, input2_storage, input2_storage, 0, 64, 320, 320, 1, output0_storage, output0_storage, 0, 64, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00363_e71ba7d1: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 20480));
  return 0;
}
