...
static uint16_t input0_storage[1937408] __attribute__((aligned(64)));
static uint16_t input1_storage[270336] __attribute__((aligned(64)));
static float input2_storage[1056768] __attribute__((aligned(64)));
static float output0_storage[1056768] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1937408, 1);
  fill_f16(input1_storage, 270336, 2);
  fill_f32(input2_storage, 1056768, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 704, 704, 1, input1_storage, input1_storage, 0, 704, 384, 384, 1, input2_storage, input2_storage, 0, 2752, 384, 384, 1, output0_storage, output0_storage, 0, 2752, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00202_075c1e74: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1056768));
  return 0;
}
