...
static uint16_t input0_storage[12615680] __attribute__((aligned(64)));
static uint16_t input1_storage[1146880] __attribute__((aligned(64)));
static uint16_t input2_storage[1126400] __attribute__((aligned(64)));
static uint16_t output0_storage[1126400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 12615680, 1);
  fill_f16(input1_storage, 1146880, 2);
  fill_f16(input2_storage, 1126400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3520, 3584, 3584, 1, input1_storage, input1_storage, 0, 3584, 320, 320, 1, input2_storage, input2_storage, 0, 3520, 320, 320, 1, output0_storage, output0_storage, 0, 3520, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00282_36f7fed8: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1126400));
  return 0;
}
