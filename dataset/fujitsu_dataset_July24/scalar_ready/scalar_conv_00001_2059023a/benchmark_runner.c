...
static uint16_t input0_storage[29068] __attribute__((aligned(64)));
static uint16_t input1_storage[16856] __attribute__((aligned(64)));
static float input2_storage[800] __attribute__((aligned(64)));
static float output0_storage[800] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 29068, 1);
  fill_f16(input1_storage, 16856, 2);
  fill_f32(input2_storage, 800, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 26, 26, 43, 29068, 1118, 43, 1, input1_storage, input1_storage, 0, 7, 7, 43, 8, 2408, 344, 8, 1, input2_storage, input2_storage, 0, 1, 10, 10, 8, 800, 80, 8, 1, output0_storage, output0_storage, 0, 1, 10, 10, 8, 800, 80, 8, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00001_2059023a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 800));
  return 0;
}
