...
static float input1_storage[983040] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 983040, 2);
  fill_f32(input2_storage, 983040, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 960, 960, 1, input2_storage, input2_storage, 0, 1024, 960, 960, 1, output0_storage, output0_storage, 0, 1024, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00405_cd828280: benchmark completed");
