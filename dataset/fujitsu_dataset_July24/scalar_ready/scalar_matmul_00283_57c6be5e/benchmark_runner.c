...
static uint16_t input0_storage[1277952] __attribute__((aligned(64)));
static uint16_t input1_storage[1769472] __attribute__((aligned(64)));
static uint16_t input2_storage[958464] __attribute__((aligned(64)));
static uint16_t output0_storage[958464] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1277952, 1);
  fill_f16(input1_storage, 1769472, 2);
  fill_f16(input2_storage, 958464, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 1536, 1536, 1, input1_storage, input1_storage, 0, 1536, 1152, 1152, 1, input2_storage, input2_storage, 0, 832, 1152, 1152, 1, output0_storage, output0_storage, 0, 832, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00283_57c6be5e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 958464));
  return 0;
}
