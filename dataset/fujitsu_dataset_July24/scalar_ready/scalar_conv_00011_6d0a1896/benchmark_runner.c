...
static float input0_storage[4400] __attribute__((aligned(64)));
static float input1_storage[396] __attribute__((aligned(64)));
static float input2_storage[324] __attribute__((aligned(64)));
static float output0_storage[324] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 4400, 1);
  fill_f32(input1_storage, 396, 2);
  fill_f32(input2_storage, 324, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 20, 20, 11, 4400, 220, 11, 1, input1_storage, input1_storage, 0, 3, 3, 11, 4, 132, 44, 4, 1, input2_storage, input2_storage, 0, 1, 9, 9, 4, 324, 36, 4, 1, output0_storage, output0_storage, 0, 1, 9, 9, 4, 324, 36, 4, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00011_6d0a1896: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 324));
  return 0;
}
