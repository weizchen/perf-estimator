...
static float input0_storage[492] __attribute__((aligned(64)));
static float input1_storage[492] __attribute__((aligned(64)));
static float output0_storage[492] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 492, 1);
  fill_f32(input1_storage, 492, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_transpose_00060_fa88d9ed: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 492));
  return 0;
}
