...
static float input0_storage[324] __attribute__((aligned(64)));
static float input1_storage[24] __attribute__((aligned(64)));
static float input2_storage[7776] __attribute__((aligned(64)));
static float output0_storage[7776] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 324, 1);
  fill_f32(input1_storage, 24, 2);
  fill_f32(input2_storage, 7776, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 18, 18, 1, 324, 18, 1, 1, input1_storage, input1_storage, 0, 1, 1, 1, 24, 24, 24, 24, 1, input2_storage, input2_storage, 0, 1, 18, 18, 24, 7776, 432, 24, 1, output0_storage, output0_storage, 0, 1, 18, 18, 24, 7776, 432, 24, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00086_7a86c057: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7776));
  return 0;
}
