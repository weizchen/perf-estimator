...
static int8_t input0_storage[573440] __attribute__((aligned(64)));
static int8_t input1_storage[2752512] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 573440, 1);
  fill_i8(input1_storage, 2752512, 2);
  fill_i32(input2_storage, 1966080, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 896, 896, 1, input1_storage, input1_storage, 0, 896, 3072, 3072, 1, input2_storage, input2_storage, 0, 640, 3072, 3072, 1, output0_storage, output0_storage, 0, 640, 3072, 3072, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00019_f6caf100: benchmark completed");
