...
static int8_t input0_storage[120612] __attribute__((aligned(64)));
static int8_t input1_storage[33516] __attribute__((aligned(64)));
static int32_t input2_storage[4800] __attribute__((aligned(64)));
static int32_t output0_storage[4800] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 120612, 1);
  fill_i8(input1_storage, 33516, 2);
  fill_i32(input2_storage, 4800, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 46, 46, 57, 120612, 2622, 57, 1, input1_storage, input1_storage, 0, 7, 7, 57, 12, 4788, 684, 12, 1, input2_storage, input2_storage, 0, 1, 20, 20, 12, 4800, 240, 12, 1, output0_storage, output0_storage, 0, 1, 20, 20, 12, 4800, 240, 12, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00071_207e1cef: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 4800));
  return 0;
}
