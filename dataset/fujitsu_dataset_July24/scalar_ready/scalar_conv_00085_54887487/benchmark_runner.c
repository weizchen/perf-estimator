...
static uint16_t input0_storage[3249] __attribute__((aligned(64)));
static uint16_t input1_storage[1960] __attribute__((aligned(64)));
static float input2_storage[27040] __attribute__((aligned(64)));
static float output0_storage[27040] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3249, 1);
  fill_f16(input1_storage, 1960, 2);
  fill_f32(input2_storage, 27040, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 57, 57, 1, 3249, 57, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 40, 280, 40, 40, 1, input2_storage, input2_storage, 0, 1, 26, 26, 40, 27040, 1040, 40, 1, output0_storage, output0_storage, 0, 1, 26, 26, 40, 27040, 1040, 40, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00085_54887487: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 27040));
  return 0;
}
