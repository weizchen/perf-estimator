...
static uint16_t input0_storage[2916] __attribute__((aligned(64)));
static uint16_t input1_storage[675] __attribute__((aligned(64)));
static float input2_storage[147] __attribute__((aligned(64)));
static float output0_storage[147] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2916, 1);
  fill_f16(input1_storage, 675, 2);
  fill_f32(input2_storage, 147, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 18, 18, 9, 2916, 162, 9, 1, input1_storage, input1_storage, 0, 5, 5, 9, 3, 135, 27, 3, 1, input2_storage, input2_storage, 0, 1, 7, 7, 3, 147, 21, 3, 1, output0_storage, output0_storage, 0, 1, 7, 7, 3, 147, 21, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00049_338eecaa: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 147));
  return 0;
}
