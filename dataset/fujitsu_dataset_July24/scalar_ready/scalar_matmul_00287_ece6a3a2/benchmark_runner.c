...
static uint16_t input0_storage[593920] __attribute__((aligned(64)));
static uint16_t input1_storage[901120] __attribute__((aligned(64)));
static uint16_t input2_storage[5226496] __attribute__((aligned(64)));
static uint16_t output0_storage[5226496] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 593920, 1);
  fill_f16(input1_storage, 901120, 2);
  fill_f16(input2_storage, 5226496, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1856, 320, 320, 1, input1_storage, input1_storage, 0, 320, 2816, 2816, 1, input2_storage, input2_storage, 0, 1856, 2816, 2816, 1, output0_storage, output0_storage, 0, 1856, 2816, 2816, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00287_ece6a3a2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5226496));
  return 0;
}
