...
static float input0_storage[90] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 90, 1);
  fill_f32(input1_storage, 90, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 90, 1, input1_storage, input1_storage, 0, 90, 1, output0_storage, output0_storage, 0, 90, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00440_4770fa16: benchmark completed");
