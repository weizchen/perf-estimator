...
static uint16_t input0_storage[798720] __attribute__((aligned(64)));
static uint16_t input1_storage[2457600] __attribute__((aligned(64)));
static uint16_t input2_storage[2129920] __attribute__((aligned(64)));
static uint16_t output0_storage[2129920] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 798720, 1);
  fill_f16(input1_storage, 2457600, 2);
  fill_f16(input2_storage, 2129920, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 960, 960, 1, input1_storage, input1_storage, 0, 960, 2560, 2560, 1, input2_storage, input2_storage, 0, 832, 2560, 2560, 1, output0_storage, output0_storage, 0, 832, 2560, 2560, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00213_bed40e91: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2129920));
  return 0;
}
