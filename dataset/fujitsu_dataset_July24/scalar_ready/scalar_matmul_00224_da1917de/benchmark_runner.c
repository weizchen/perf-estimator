...
static uint16_t input0_storage[675840] __attribute__((aligned(64)));
static uint16_t input1_storage[10588160] __attribute__((aligned(64)));
static uint16_t input2_storage[577536] __attribute__((aligned(64)));
static uint16_t output0_storage[577536] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 675840, 1);
  fill_f16(input1_storage, 10588160, 2);
  fill_f16(input2_storage, 577536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 3520, 3520, 1, input1_storage, input1_storage, 0, 3520, 3008, 3008, 1, input2_storage, input2_storage, 0, 192, 3008, 3008, 1, output0_storage, output0_storage, 0, 192, 3008, 3008, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00224_da1917de: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 577536));
  return 0;
}
