...
static uint16_t input0_storage[17661] __attribute__((aligned(64)));
static uint16_t input1_storage[1050] __attribute__((aligned(64)));
static float input2_storage[338] __attribute__((aligned(64)));
static float output0_storage[338] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 17661, 1);
  fill_f16(input1_storage, 1050, 2);
  fill_f32(input2_storage, 338, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 29, 29, 21, 17661, 609, 21, 1, input1_storage, input1_storage, 0, 5, 5, 21, 2, 210, 42, 2, 1, input2_storage, input2_storage, 0, 1, 13, 13, 2, 338, 26, 2, 1, output0_storage, output0_storage, 0, 1, 13, 13, 2, 338, 26, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00115_1873b03c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 338));
  return 0;
}
