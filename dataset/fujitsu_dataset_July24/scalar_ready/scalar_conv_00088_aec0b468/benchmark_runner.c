...
static uint16_t input1_storage[425] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 425, 2);
  fill_f32(input2_storage, 2448, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 16, 16, 1, 256, 16, 1, 1, input1_storage, input1_storage, 0, 5, 5, 1, 17, 85, 17, 17, 1, input2_storage, input2_storage, 0, 1, 12, 12, 17, 2448, 204, 17, 1, output0_storage, output0_storage, 0, 1, 12, 12, 17, 2448, 204, 17, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00088_aec0b468: benchmark completed");
