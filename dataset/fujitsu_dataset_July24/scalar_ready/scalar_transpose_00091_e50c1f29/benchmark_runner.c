...
static uint16_t input0_storage[9675] __attribute__((aligned(64)));
static uint16_t input1_storage[9675] __attribute__((aligned(64)));
static uint16_t output0_storage[9675] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9675, 1);
  fill_f16(input1_storage, 9675, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 5, 9, 43, 5, 1935, 215, 5, 1, input1_storage, input1_storage, 0, 5, 43, 9, 5, 1935, 45, 5, 1, output0_storage, output0_storage, 0, 5, 43, 9, 5, 1935, 45, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00091_e50c1f29: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 9675));
  return 0;
}
