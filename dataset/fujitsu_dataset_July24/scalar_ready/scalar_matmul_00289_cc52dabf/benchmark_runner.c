...
static uint16_t input0_storage[4472832] __attribute__((aligned(64)));
static uint16_t input1_storage[2752512] __attribute__((aligned(64)));
static uint16_t input2_storage[1703936] __attribute__((aligned(64)));
static uint16_t output0_storage[1703936] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4472832, 1);
  fill_f16(input1_storage, 2752512, 2);
  fill_f16(input2_storage, 1703936, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 1024, 1024, 1, input2_storage, input2_storage, 0, 1664, 1024, 1024, 1, output0_storage, output0_storage, 0, 1664, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00289_cc52dabf: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1703936));
  return 0;
}
