...
static float input0_storage[659456] __attribute__((aligned(64)));
static float input1_storage[1433600] __attribute__((aligned(64)));
static float input2_storage[4710400] __attribute__((aligned(64)));
static float output0_storage[4710400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 659456, 1);
  fill_f32(input1_storage, 1433600, 2);
  fill_f32(input2_storage, 4710400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1472, 448, 448, 1, input1_storage, input1_storage, 0, 448, 3200, 3200, 1, input2_storage, input2_storage, 0, 1472, 3200, 3200, 1, output0_storage, output0_storage, 0, 1472, 3200, 3200, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00398_cfabc8ff: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4710400));
  return 0;
}
