...
static float input0_storage[217088] __attribute__((aligned(64)));
static float input1_storage[2387968] __attribute__((aligned(64)));
static float input2_storage[45056] __attribute__((aligned(64)));
static float output0_storage[45056] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 217088, 1);
  fill_f32(input1_storage, 2387968, 2);
  fill_f32(input2_storage, 45056, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3392, 3392, 1, input1_storage, input1_storage, 0, 3392, 704, 704, 1, input2_storage, input2_storage, 0, 64, 704, 704, 1, output0_storage, output0_storage, 0, 64, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00411_b81753e2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 45056));
  return 0;
}
