...
static int16_t input0_storage[190] __attribute__((aligned(64)));
static int16_t input1_storage[38] __attribute__((aligned(64)));
static int16_t output0_storage[38] __attribute__((aligned(64)));
...
int main(void) {
  fill_i16(input0_storage, 190, 1);
  fill_i16(input1_storage, 38, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 5, 38, 38, 1, input1_storage, input1_storage, 0, 38, 1, output0_storage, output0_storage, 0, 38, 1);
  ofa_dma_memory_fence();
  puts("scalar_reduce_00288_726bde1e: benchmark completed");
  printf("output0_i16_checksum=%lld\n", checksum_i16(output0_storage, 38));
  return 0;
}
