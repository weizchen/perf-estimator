...
static float input0_storage[507] __attribute__((aligned(64)));
static float input1_storage[567] __attribute__((aligned(64)));
static float input2_storage[756] __attribute__((aligned(64)));
static float output0_storage[756] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 507, 1);
  fill_f32(input1_storage, 567, 2);
  fill_f32(input2_storage, 756, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 13, 13, 3, 507, 39, 3, 1, input1_storage, input1_storage, 0, 3, 3, 3, 21, 189, 63, 21, 1, input2_storage, input2_storage, 0, 1, 6, 6, 21, 756, 126, 21, 1, output0_storage, output0_storage, 0, 1, 6, 6, 21, 756, 126, 21, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00143_4e853620: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 756));
  return 0;
}
