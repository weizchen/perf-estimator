...
static float input0_storage[567] __attribute__((aligned(64)));
static float input1_storage[18522] __attribute__((aligned(64)));
static float input2_storage[216] __attribute__((aligned(64)));
static float output0_storage[216] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 567, 1);
  fill_f32(input1_storage, 18522, 2);
  fill_f32(input2_storage, 216, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 9, 9, 7, 567, 63, 7, 1, input1_storage, input1_storage, 0, 7, 7, 7, 54, 2646, 378, 54, 1, input2_storage, input2_storage, 0, 1, 2, 2, 54, 216, 108, 54, 1, output0_storage, output0_storage, 0, 1, 2, 2, 54, 216, 108, 54, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00106_2b4b197b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 216));
  return 0;
}
