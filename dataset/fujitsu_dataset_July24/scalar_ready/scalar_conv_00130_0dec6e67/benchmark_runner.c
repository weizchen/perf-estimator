...
static uint16_t input0_storage[26896] __attribute__((aligned(64)));
static uint16_t input1_storage[8064] __attribute__((aligned(64)));
static float input2_storage[85176] __attribute__((aligned(64)));
static float output0_storage[85176] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 26896, 1);
  fill_f16(input1_storage, 8064, 2);
  fill_f32(input2_storage, 85176, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 41, 41, 16, 26896, 656, 16, 1, input1_storage, input1_storage, 0, 3, 3, 16, 56, 2688, 896, 56, 1, input2_storage, input2_storage, 0, 1, 39, 39, 56, 85176, 2184, 56, 1, output0_storage, output0_storage, 0, 1, 39, 39, 56, 85176, 2184, 56, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00130_0dec6e67: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 85176));
  return 0;
}
