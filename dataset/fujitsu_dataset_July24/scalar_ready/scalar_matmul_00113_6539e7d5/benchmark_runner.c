...
static float input2_storage[135168] __attribute__((aligned(64)));
static float output0_storage[135168] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 135168, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 512, 512, 1, input1_storage, input1_storage, 0, 512, 2112, 2112, 1, input2_storage, input2_storage, 0, 64, 2112, 2112, 1, output0_storage, output0_storage, 0, 64, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00113_6539e7d5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 135168));
  return 0;
}
