...
static float input0_storage[1585152] __attribute__((aligned(64)));
static float input1_storage[176128] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1585152, 1);
  fill_f32(input1_storage, 176128, 2);
  fill_f32(input2_storage, 36864, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 2752, 2752, 1, input1_storage, input1_storage, 0, 2752, 64, 64, 1, input2_storage, input2_storage, 0, 576, 64, 64, 1, output0_storage, output0_storage, 0, 576, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00350_d24db696: benchmark completed");
