...
static uint16_t input0_storage[115425] __attribute__((aligned(64)));
static uint16_t input1_storage[11172] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 115425, 1);
  fill_f16(input1_storage, 11172, 2);
  fill_f32(input2_storage, 1600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 45, 45, 57, 115425, 2565, 57, 1, input1_storage, input1_storage, 0, 7, 7, 57, 4, 1596, 228, 4, 1, input2_storage, input2_storage, 0, 1, 20, 20, 4, 1600, 80, 4, 1, output0_storage, output0_storage, 0, 1, 20, 20, 4, 1600, 80, 4, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00052_f7950216: benchmark completed");
