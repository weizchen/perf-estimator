...
static uint16_t input1_storage[3784704] __attribute__((aligned(64)));
static uint16_t input2_storage[720896] __attribute__((aligned(64)));
static uint16_t output0_storage[720896] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 3784704, 2);
  fill_f16(input2_storage, 720896, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 1408, 1408, 1, input2_storage, input2_storage, 0, 512, 1408, 1408, 1, output0_storage, output0_storage, 0, 512, 1408, 1408, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00279_cc1c236e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 720896));
  return 0;
}
