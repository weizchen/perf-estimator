...
static float input0_storage[429] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 429, 1);
  fill_f32(input1_storage, 39, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 39, 11, 11, 1, input1_storage, input1_storage, 0, 39, 1, output0_storage, output0_storage, 0, 39, 1);
  ofa_dma_memory_fence();
  puts("scalar_reduce_00240_f556438d: benchmark completed");
