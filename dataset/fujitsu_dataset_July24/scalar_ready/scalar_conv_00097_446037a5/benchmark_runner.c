...
static uint16_t input0_storage[5832] __attribute__((aligned(64)));
static uint16_t input1_storage[162] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5832, 1);
  fill_f16(input1_storage, 162, 2);
  fill_f32(input2_storage, 256, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 18, 18, 18, 5832, 324, 18, 1, input1_storage, input1_storage, 0, 3, 3, 18, 1, 54, 18, 1, 1, input2_storage, input2_storage, 0, 1, 16, 16, 1, 256, 16, 1, 1, output0_storage, output0_storage, 0, 1, 16, 16, 1, 256, 16, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00097_446037a5: benchmark completed");
