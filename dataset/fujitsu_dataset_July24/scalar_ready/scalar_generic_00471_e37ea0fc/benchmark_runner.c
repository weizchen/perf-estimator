...
static int16_t input0_storage[2740] __attribute__((aligned(64)));
static int16_t input1_storage[2740] __attribute__((aligned(64)));
static int16_t output0_storage[2740] __attribute__((aligned(64)));
...
int main(void) {
  fill_i16(input0_storage, 2740, 1);
  fill_i16(input1_storage, 2740, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2740, 1, input1_storage, input1_storage, 0, 2740, 1, output0_storage, output0_storage, 0, 2740, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00471_e37ea0fc: benchmark completed");
  printf("output0_i16_checksum=%lld\n", checksum_i16(output0_storage, 2740));
  return 0;
}
