...
static float input0_storage[4644864] __attribute__((aligned(64)));
static float input1_storage[8429568] __attribute__((aligned(64)));
static float input2_storage[5419008] __attribute__((aligned(64)));
static float output0_storage[5419008] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 4644864, 1);
  fill_f32(input1_storage, 8429568, 2);
  fill_f32(input2_storage, 5419008, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1728, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 3136, 3136, 1, input2_storage, input2_storage, 0, 1728, 3136, 3136, 1, output0_storage, output0_storage, 0, 1728, 3136, 3136, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00383_932ad667: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5419008));
  return 0;
}
