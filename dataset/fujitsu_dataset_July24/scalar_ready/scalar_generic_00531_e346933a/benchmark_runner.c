...
static int16_t input0_storage[2960] __attribute__((aligned(64)));
...
int main(void) {
  fill_i16(input0_storage, 2960, 1);
  fill_i16(input1_storage, 2960, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2960, 1, input1_storage, input1_storage, 0, 2960, 1, output0_storage, output0_storage, 0, 2960, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00531_e346933a: benchmark completed");
