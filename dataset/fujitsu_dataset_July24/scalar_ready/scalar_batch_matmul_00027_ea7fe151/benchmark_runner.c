...
static uint16_t input0_storage[32768] __attribute__((aligned(64)));
static uint16_t input1_storage[819200] __attribute__((aligned(64)));
static float input2_storage[102400] __attribute__((aligned(64)));
static float output0_storage[102400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 32768, 1);
  fill_f16(input1_storage, 819200, 2);
  fill_f32(input2_storage, 102400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 64, 512, 32768, 512, 1, input1_storage, input1_storage, 0, 1, 512, 1600, 819200, 1600, 1, input2_storage, input2_storage, 0, 1, 64, 1600, 102400, 1600, 1, output0_storage, output0_storage, 0, 1, 64, 1600, 102400, 1600, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00027_ea7fe151: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 102400));
  return 0;
}
