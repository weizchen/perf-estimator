...
static uint16_t input2_storage[135168] __attribute__((aligned(64)));
static uint16_t output0_storage[135168] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 135168, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 256, 256, 1, input1_storage, input1_storage, 0, 256, 2112, 2112, 1, input2_storage, input2_storage, 0, 64, 2112, 2112, 1, output0_storage, output0_storage, 0, 64, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00248_fbed36bf: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 135168));
  return 0;
}
