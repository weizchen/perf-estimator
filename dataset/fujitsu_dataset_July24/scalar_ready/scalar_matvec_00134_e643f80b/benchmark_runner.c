...
static uint16_t input0_storage[90112] __attribute__((aligned(64)));
static uint16_t input1_storage[1408] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 90112, 1);
  fill_f16(input1_storage, 1408, 2);
  fill_f32(input2_storage, 64, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 1408, 1408, 1, input1_storage, input1_storage, 0, 1408, 1, input2_storage, input2_storage, 0, 64, 1, output0_storage, output0_storage, 0, 64, 1);
  ofa_dma_memory_fence();
  puts("scalar_matvec_00134_e643f80b: benchmark completed");
