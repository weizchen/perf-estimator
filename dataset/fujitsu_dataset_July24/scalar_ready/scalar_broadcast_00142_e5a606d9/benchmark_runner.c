...
static int8_t input1_storage[68820] __attribute__((aligned(64)));
static int8_t output0_storage[68820] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 68820, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 372, 5, 5, 1, input1_storage, input1_storage, 0, 372, 5, 37, 185, 37, 1, output0_storage, output0_storage, 0, 372, 5, 37, 185, 37, 1);
  ofa_dma_memory_fence();
  puts("scalar_broadcast_00142_e5a606d9: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 68820));
  return 0;
}
