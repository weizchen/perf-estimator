...
static uint16_t input1_storage[622592] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 622592, 2);
  fill_f16(input2_storage, 16384, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 2432, 2432, 1, input1_storage, input1_storage, 0, 2432, 256, 256, 1, input2_storage, input2_storage, 0, 64, 256, 256, 1, output0_storage, output0_storage, 0, 64, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00315_57036280: benchmark completed");
