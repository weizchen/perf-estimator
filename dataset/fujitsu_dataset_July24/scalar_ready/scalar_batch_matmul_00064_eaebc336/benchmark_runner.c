...
static uint16_t input2_storage[327680] __attribute__((aligned(64)));
static uint16_t output0_storage[327680] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 327680, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 320, 128, 40960, 128, 1, input1_storage, input1_storage, 0, 4, 128, 256, 32768, 256, 1, input2_storage, input2_storage, 0, 4, 320, 256, 81920, 256, 1, output0_storage, output0_storage, 0, 4, 320, 256, 81920, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00064_eaebc336: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 327680));
  return 0;
}
