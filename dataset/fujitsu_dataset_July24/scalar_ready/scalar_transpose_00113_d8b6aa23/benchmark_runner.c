...
static uint16_t input0_storage[582750] __attribute__((aligned(64)));
static uint16_t input1_storage[582750] __attribute__((aligned(64)));
static uint16_t output0_storage[582750] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 582750, 1);
  fill_f16(input1_storage, 582750, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 42, 15, 25, 37, 13875, 925, 37, 1, input1_storage, input1_storage, 0, 42, 15, 37, 25, 13875, 925, 25, 1, output0_storage, output0_storage, 0, 42, 15, 37, 25, 13875, 925, 25, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00113_d8b6aa23: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 582750));
  return 0;
}
