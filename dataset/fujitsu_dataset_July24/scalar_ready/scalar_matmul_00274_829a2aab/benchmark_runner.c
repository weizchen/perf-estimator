...
static uint16_t input0_storage[450560] __attribute__((aligned(64)));
static uint16_t input1_storage[765952] __attribute__((aligned(64)));
static uint16_t input2_storage[696320] __attribute__((aligned(64)));
static uint16_t output0_storage[696320] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 450560, 1);
  fill_f16(input1_storage, 765952, 2);
  fill_f16(input2_storage, 696320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 704, 704, 1, input1_storage, input1_storage, 0, 704, 1088, 1088, 1, input2_storage, input2_storage, 0, 640, 1088, 1088, 1, output0_storage, output0_storage, 0, 640, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00274_829a2aab: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 696320));
  return 0;
}
