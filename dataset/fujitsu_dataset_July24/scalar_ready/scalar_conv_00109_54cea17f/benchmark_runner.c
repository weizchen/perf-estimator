...
static float input0_storage[5476] __attribute__((aligned(64)));
static float input1_storage[7252] __attribute__((aligned(64)));
static float input2_storage[35557] __attribute__((aligned(64)));
static float output0_storage[35557] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 5476, 1);
  fill_f32(input1_storage, 7252, 2);
  fill_f32(input2_storage, 35557, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 37, 37, 4, 5476, 148, 4, 1, input1_storage, input1_storage, 0, 7, 7, 4, 37, 1036, 148, 37, 1, input2_storage, input2_storage, 0, 1, 31, 31, 37, 35557, 1147, 37, 1, output0_storage, output0_storage, 0, 1, 31, 31, 37, 35557, 1147, 37, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00109_54cea17f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 35557));
  return 0;
}
