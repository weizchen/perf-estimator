...
static float input0_storage[1960] __attribute__((aligned(64)));
static float input1_storage[1530] __attribute__((aligned(64)));
static float input2_storage[2448] __attribute__((aligned(64)));
static float output0_storage[2448] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1960, 1);
  fill_f32(input1_storage, 1530, 2);
  fill_f32(input2_storage, 2448, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 14, 14, 10, 1960, 140, 10, 1, input1_storage, input1_storage, 0, 3, 3, 10, 17, 510, 170, 17, 1, input2_storage, input2_storage, 0, 1, 12, 12, 17, 2448, 204, 17, 1, output0_storage, output0_storage, 0, 1, 12, 12, 17, 2448, 204, 17, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00101_b9fea176: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2448));
  return 0;
}
