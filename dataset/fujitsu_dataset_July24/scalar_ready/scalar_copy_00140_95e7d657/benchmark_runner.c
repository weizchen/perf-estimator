...
static int16_t input0_storage[13] __attribute__((aligned(64)));
...
int main(void) {
  fill_i16(input0_storage, 13, 1);
  fill_i16(input1_storage, 13, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 13, 1, input1_storage, input1_storage, 0, 13, 1, output0_storage, output0_storage, 0, 13, 1);
  ofa_dma_memory_fence();
  puts("scalar_copy_00140_95e7d657: benchmark completed");
