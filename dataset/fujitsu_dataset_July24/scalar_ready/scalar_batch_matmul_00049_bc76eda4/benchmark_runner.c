...
static uint16_t input0_storage[2129920] __attribute__((aligned(64)));
static uint16_t input1_storage[901120] __attribute__((aligned(64)));
static float input2_storage[4685824] __attribute__((aligned(64)));
static float output0_storage[4685824] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2129920, 1);
  fill_f16(input1_storage, 901120, 2);
  fill_f32(input2_storage, 4685824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 1664, 320, 532480, 320, 1, input1_storage, input1_storage, 0, 4, 320, 704, 225280, 704, 1, input2_storage, input2_storage, 0, 4, 1664, 704, 1171456, 704, 1, output0_storage, output0_storage, 0, 4, 1664, 704, 1171456, 704, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00049_bc76eda4: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4685824));
  return 0;
}
