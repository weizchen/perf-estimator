...
static int8_t input0_storage[79] __attribute__((aligned(64)));
static int8_t input1_storage[79] __attribute__((aligned(64)));
static int8_t output0_storage[79] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 79, 1);
  fill_i8(input1_storage, 79, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_generic_00085_11d5012b: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 79));
  return 0;
}
