...
static uint16_t input0_storage[1808] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1808, 1);
  fill_f16(input1_storage, 4, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 452, 452, 1, input1_storage, input1_storage, 0, 4, 1, output0_storage, output0_storage, 0, 4, 1);
  ofa_dma_memory_fence();
  puts("scalar_reduce_00090_385c9d2a: benchmark completed");
