...
static int8_t input0_storage[13713408] __attribute__((aligned(64)));
static int8_t input1_storage[3096576] __attribute__((aligned(64)));
static int32_t input2_storage[3555328] __attribute__((aligned(64)));
static int32_t output0_storage[3555328] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 13713408, 1);
  fill_i8(input1_storage, 3096576, 2);
  fill_i32(input2_storage, 3555328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 3456, 3456, 1, input1_storage, input1_storage, 0, 3456, 896, 896, 1, input2_storage, input2_storage, 0, 3968, 896, 896, 1, output0_storage, output0_storage, 0, 3968, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00005_06df1048: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 3555328));
  return 0;
}
