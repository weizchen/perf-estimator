...
static uint16_t input0_storage[5996544] __attribute__((aligned(64)));
static uint16_t input1_storage[749568] __attribute__((aligned(64)));
static float input2_storage[294912] __attribute__((aligned(64)));
static float output0_storage[294912] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5996544, 1);
  fill_f16(input1_storage, 749568, 2);
  fill_f32(input2_storage, 294912, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 3904, 3904, 1, input1_storage, input1_storage, 0, 3904, 192, 192, 1, input2_storage, input2_storage, 0, 1536, 192, 192, 1, output0_storage, output0_storage, 0, 1536, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00190_1bb771bb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 294912));
  return 0;
}
