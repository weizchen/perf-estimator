...
static float input2_storage[61440] __attribute__((aligned(64)));
static float output0_storage[61440] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 61440, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 320, 320, 1, input2_storage, input2_storage, 0, 192, 320, 320, 1, output0_storage, output0_storage, 0, 192, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00153_c1c275a1: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 61440));
  return 0;
}
