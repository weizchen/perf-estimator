...
static uint16_t input0_storage[638976] __attribute__((aligned(64)));
static uint16_t input1_storage[98304] __attribute__((aligned(64)));
static uint16_t input2_storage[425984] __attribute__((aligned(64)));
static uint16_t output0_storage[425984] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 638976, 1);
  fill_f16(input1_storage, 98304, 2);
  fill_f16(input2_storage, 425984, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 832, 192, 159744, 192, 1, input1_storage, input1_storage, 0, 4, 192, 128, 24576, 128, 1, input2_storage, input2_storage, 0, 4, 832, 128, 106496, 128, 1, output0_storage, output0_storage, 0, 4, 832, 128, 106496, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00071_b227b505: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 425984));
  return 0;
}
