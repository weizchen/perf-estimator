...
static int8_t input0_storage[2209] __attribute__((aligned(64)));
static int8_t input1_storage[14] __attribute__((aligned(64)));
static int32_t input2_storage[8064] __attribute__((aligned(64)));
static int32_t output0_storage[8064] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 2209, 1);
  fill_i8(input1_storage, 14, 2);
  fill_i32(input2_storage, 8064, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 47, 47, 1, 2209, 47, 1, 1, input1_storage, input1_storage, 0, 1, 1, 1, 14, 14, 14, 14, 1, input2_storage, input2_storage, 0, 1, 24, 24, 14, 8064, 336, 14, 1, output0_storage, output0_storage, 0, 1, 24, 24, 14, 8064, 336, 14, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00144_0c7e2c82: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 8064));
  return 0;
}
