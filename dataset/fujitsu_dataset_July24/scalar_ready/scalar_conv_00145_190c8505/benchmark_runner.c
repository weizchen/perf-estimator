...
static int8_t input0_storage[18432] __attribute__((aligned(64)));
static int8_t input1_storage[6750] __attribute__((aligned(64)));
static int32_t input2_storage[11760] __attribute__((aligned(64)));
static int32_t output0_storage[11760] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 18432, 1);
  fill_i8(input1_storage, 6750, 2);
  fill_i32(input2_storage, 11760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 32, 32, 18, 18432, 576, 18, 1, input1_storage, input1_storage, 0, 5, 5, 18, 15, 1350, 270, 15, 1, input2_storage, input2_storage, 0, 1, 28, 28, 15, 11760, 420, 15, 1, output0_storage, output0_storage, 0, 1, 28, 28, 15, 11760, 420, 15, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00145_190c8505: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 11760));
  return 0;
}
