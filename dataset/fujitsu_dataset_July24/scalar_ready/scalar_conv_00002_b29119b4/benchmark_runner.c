...
static float input0_storage[1568] __attribute__((aligned(64)));
static float input1_storage[774] __attribute__((aligned(64)));
static float input2_storage[29068] __attribute__((aligned(64)));
static float output0_storage[29068] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1568, 1);
  fill_f32(input1_storage, 774, 2);
  fill_f32(input2_storage, 29068, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 28, 28, 2, 1568, 56, 2, 1, input1_storage, input1_storage, 0, 3, 3, 2, 43, 258, 86, 43, 1, input2_storage, input2_storage, 0, 1, 26, 26, 43, 29068, 1118, 43, 1, output0_storage, output0_storage, 0, 1, 26, 26, 43, 29068, 1118, 43, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00002_b29119b4: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 29068));
  return 0;
}
