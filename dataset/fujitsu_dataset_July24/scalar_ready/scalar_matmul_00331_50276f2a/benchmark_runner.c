...
static float input0_storage[212992] __attribute__((aligned(64)));
static float input1_storage[884736] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 212992, 1);
  fill_f32(input1_storage, 884736, 2);
  fill_f32(input2_storage, 2875392, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 256, 256, 1, input1_storage, input1_storage, 0, 256, 3456, 3456, 1, input2_storage, input2_storage, 0, 832, 3456, 3456, 1, output0_storage, output0_storage, 0, 832, 3456, 3456, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00331_50276f2a: benchmark completed");
