...
static uint16_t input0_storage[2785280] __attribute__((aligned(64)));
static uint16_t input1_storage[655360] __attribute__((aligned(64)));
static uint16_t input2_storage[1114112] __attribute__((aligned(64)));
static uint16_t output0_storage[1114112] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2785280, 1);
  fill_f16(input1_storage, 655360, 2);
  fill_f16(input2_storage, 1114112, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 1088, 640, 696320, 640, 1, input1_storage, input1_storage, 0, 4, 640, 256, 163840, 256, 1, input2_storage, input2_storage, 0, 4, 1088, 256, 278528, 256, 1, output0_storage, output0_storage, 0, 4, 1088, 256, 278528, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00066_b9b60b84: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1114112));
  return 0;
}
