...
static float input0_storage[8464] __attribute__((aligned(64)));
static float input1_storage[288] __attribute__((aligned(64)));
static float input2_storage[882] __attribute__((aligned(64)));
static float output0_storage[882] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 8464, 1);
  fill_f32(input1_storage, 288, 2);
  fill_f32(input2_storage, 882, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 23, 23, 16, 8464, 368, 16, 1, input1_storage, input1_storage, 0, 3, 3, 16, 2, 96, 32, 2, 1, input2_storage, input2_storage, 0, 1, 21, 21, 2, 882, 42, 2, 1, output0_storage, output0_storage, 0, 1, 21, 21, 2, 882, 42, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00146_51ad1dd5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 882));
  return 0;
}
