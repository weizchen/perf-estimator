...
static int8_t input1_storage[344064] __attribute__((aligned(64)));
static int32_t input2_storage[32768] __attribute__((aligned(64)));
static int32_t output0_storage[32768] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 344064, 2);
  fill_i32(input2_storage, 32768, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 128, 128, 1, input2_storage, input2_storage, 0, 256, 128, 128, 1, output0_storage, output0_storage, 0, 256, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00030_572a75b9: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 32768));
  return 0;
}
