...
static float input0_storage[6525] __attribute__((aligned(64)));
static float input1_storage[6525] __attribute__((aligned(64)));
static float output0_storage[6525] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 6525, 1);
  fill_f32(input1_storage, 6525, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_transpose_00019_d5d76b8a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 6525));
  return 0;
}
