...
static uint16_t input0_storage[8788] __attribute__((aligned(64)));
static uint16_t input1_storage[2925] __attribute__((aligned(64)));
static float input2_storage[4356] __attribute__((aligned(64)));
static float output0_storage[4356] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 8788, 1);
  fill_f16(input1_storage, 2925, 2);
  fill_f32(input2_storage, 4356, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 26, 26, 13, 8788, 338, 13, 1, input1_storage, input1_storage, 0, 5, 5, 13, 9, 585, 117, 9, 1, input2_storage, input2_storage, 0, 1, 22, 22, 9, 4356, 198, 9, 1, output0_storage, output0_storage, 0, 1, 22, 22, 9, 4356, 198, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00112_0c9a9352: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4356));
  return 0;
}
