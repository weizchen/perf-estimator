...
static uint16_t input0_storage[1352] __attribute__((aligned(64)));
static uint16_t input1_storage[88] __attribute__((aligned(64)));
static float input2_storage[1859] __attribute__((aligned(64)));
static float output0_storage[1859] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1352, 1);
  fill_f16(input1_storage, 88, 2);
  fill_f32(input2_storage, 1859, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 13, 13, 8, 1352, 104, 8, 1, input1_storage, input1_storage, 0, 1, 1, 8, 11, 88, 88, 11, 1, input2_storage, input2_storage, 0, 1, 13, 13, 11, 1859, 143, 11, 1, output0_storage, output0_storage, 0, 1, 13, 13, 11, 1859, 143, 11, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00108_c60ec3da: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1859));
  return 0;
}
