...
static int8_t input0_storage[27869] __attribute__((aligned(64)));
static int8_t input1_storage[10150] __attribute__((aligned(64)));
static int32_t input2_storage[10206] __attribute__((aligned(64)));
static int32_t output0_storage[10206] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 27869, 1);
  fill_i8(input1_storage, 10150, 2);
  fill_i32(input2_storage, 10206, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 31, 31, 29, 27869, 899, 29, 1, input1_storage, input1_storage, 0, 5, 5, 29, 14, 2030, 406, 14, 1, input2_storage, input2_storage, 0, 1, 27, 27, 14, 10206, 378, 14, 1, output0_storage, output0_storage, 0, 1, 27, 27, 14, 10206, 378, 14, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00043_fe0548aa: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 10206));
  return 0;
}
