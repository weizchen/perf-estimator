...
static uint16_t input0_storage[29] __attribute__((aligned(64)));
static uint16_t input1_storage[29] __attribute__((aligned(64)));
static uint16_t output0_storage[29] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 29, 1);
  fill_f16(input1_storage, 29, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_generic_00309_e341176f: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 29));
  return 0;
}
