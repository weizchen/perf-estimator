...
static uint16_t input0_storage[4513792] __attribute__((aligned(64)));
static uint16_t input1_storage[3325952] __attribute__((aligned(64)));
static uint16_t input2_storage[1089536] __attribute__((aligned(64)));
static uint16_t output0_storage[1089536] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4513792, 1);
  fill_f16(input1_storage, 3325952, 2);
  fill_f16(input2_storage, 1089536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 896, 896, 1, input2_storage, input2_storage, 0, 1216, 896, 896, 1, output0_storage, output0_storage, 0, 1216, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00268_2b1b27a2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1089536));
  return 0;
}
