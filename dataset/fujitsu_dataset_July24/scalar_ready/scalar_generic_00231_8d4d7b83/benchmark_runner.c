...
static float input1_storage[122] __attribute__((aligned(64)));
static float output0_storage[122] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 122, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 122, 1, input1_storage, input1_storage, 0, 122, 1, output0_storage, output0_storage, 0, 122, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00231_8d4d7b83: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 122));
  return 0;
}
