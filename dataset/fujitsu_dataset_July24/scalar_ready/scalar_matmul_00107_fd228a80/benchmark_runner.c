...
static float input2_storage[4915200] __attribute__((aligned(64)));
static float output0_storage[4915200] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 4915200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2560, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1920, 1920, 1, input2_storage, input2_storage, 0, 2560, 1920, 1920, 1, output0_storage, output0_storage, 0, 2560, 1920, 1920, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00107_fd228a80: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4915200));
  return 0;
}
