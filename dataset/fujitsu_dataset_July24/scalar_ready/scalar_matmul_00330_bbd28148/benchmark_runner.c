...
static float input0_storage[10928128] __attribute__((aligned(64)));
static float input1_storage[12353536] __attribute__((aligned(64)));
static float input2_storage[9797632] __attribute__((aligned(64)));
static float output0_storage[9797632] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 10928128, 1);
  fill_f32(input1_storage, 12353536, 2);
  fill_f32(input2_storage, 9797632, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2944, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 3328, 3328, 1, input2_storage, input2_storage, 0, 2944, 3328, 3328, 1, output0_storage, output0_storage, 0, 2944, 3328, 3328, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00330_bbd28148: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9797632));
  return 0;
}
