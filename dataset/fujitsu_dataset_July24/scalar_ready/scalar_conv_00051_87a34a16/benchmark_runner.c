...
static uint16_t input1_storage[490] __attribute__((aligned(64)));
static float input2_storage[160] __attribute__((aligned(64)));
static float output0_storage[160] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 490, 2);
  fill_f32(input2_storage, 160, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 14, 14, 1, 196, 14, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 10, 70, 10, 10, 1, input2_storage, input2_storage, 0, 1, 4, 4, 10, 160, 40, 10, 1, output0_storage, output0_storage, 0, 1, 4, 4, 10, 160, 40, 10, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00051_87a34a16: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 160));
  return 0;
}
