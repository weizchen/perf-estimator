...
static uint16_t input0_storage[605] __attribute__((aligned(64)));
static uint16_t input1_storage[490] __attribute__((aligned(64)));
static float input2_storage[18] __attribute__((aligned(64)));
static float output0_storage[18] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 605, 1);
  fill_f16(input1_storage, 490, 2);
  fill_f32(input2_storage, 18, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 11, 11, 5, 605, 55, 5, 1, input1_storage, input1_storage, 0, 7, 7, 5, 2, 70, 10, 2, 1, input2_storage, input2_storage, 0, 1, 3, 3, 2, 18, 6, 2, 1, output0_storage, output0_storage, 0, 1, 3, 3, 2, 18, 6, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00102_ff201f93: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 18));
  return 0;
}
