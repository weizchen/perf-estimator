...
static float input0_storage[867] __attribute__((aligned(64)));
static float input1_storage[2058] __attribute__((aligned(64)));
static float input2_storage[504] __attribute__((aligned(64)));
static float output0_storage[504] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 867, 1);
  fill_f32(input1_storage, 2058, 2);
  fill_f32(input2_storage, 504, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 17, 17, 3, 867, 51, 3, 1, input1_storage, input1_storage, 0, 7, 7, 3, 14, 294, 42, 14, 1, input2_storage, input2_storage, 0, 1, 6, 6, 14, 504, 84, 14, 1, output0_storage, output0_storage, 0, 1, 6, 6, 14, 504, 84, 14, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00121_4d930525: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 504));
  return 0;
}
