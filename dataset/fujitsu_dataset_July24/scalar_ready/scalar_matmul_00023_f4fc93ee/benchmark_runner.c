...
static int8_t input0_storage[1376256] __attribute__((aligned(64)));
static int8_t input1_storage[5505024] __attribute__((aligned(64)));
static int32_t input2_storage[589824] __attribute__((aligned(64)));
static int32_t output0_storage[589824] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1376256, 1);
  fill_i8(input1_storage, 5505024, 2);
  fill_i32(input2_storage, 589824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 3584, 3584, 1, input1_storage, input1_storage, 0, 3584, 1536, 1536, 1, input2_storage, input2_storage, 0, 384, 1536, 1536, 1, output0_storage, output0_storage, 0, 384, 1536, 1536, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00023_f4fc93ee: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 589824));
  return 0;
}
