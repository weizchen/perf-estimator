...
static uint16_t input0_storage[442368] __attribute__((aligned(64)));
static uint16_t input1_storage[147456] __attribute__((aligned(64)));
static float input2_storage[221184] __attribute__((aligned(64)));
static float output0_storage[221184] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 442368, 1);
  fill_f16(input1_storage, 147456, 2);
  fill_f32(input2_storage, 221184, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 576, 384, 221184, 384, 1, input1_storage, input1_storage, 0, 2, 384, 192, 73728, 192, 1, input2_storage, input2_storage, 0, 2, 576, 192, 110592, 192, 1, output0_storage, output0_storage, 0, 2, 576, 192, 110592, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00037_b25024d8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 221184));
  return 0;
}
