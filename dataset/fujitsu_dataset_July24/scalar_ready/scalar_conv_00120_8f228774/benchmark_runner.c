...
static uint16_t input0_storage[1000] __attribute__((aligned(64)));
static uint16_t input1_storage[500] __attribute__((aligned(64)));
static float input2_storage[72] __attribute__((aligned(64)));
static float output0_storage[72] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1000, 1);
  fill_f16(input1_storage, 500, 2);
  fill_f32(input2_storage, 72, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 10, 10, 10, 1000, 100, 10, 1, input1_storage, input1_storage, 0, 5, 5, 10, 2, 100, 20, 2, 1, input2_storage, input2_storage, 0, 1, 6, 6, 2, 72, 12, 2, 1, output0_storage, output0_storage, 0, 1, 6, 6, 2, 72, 12, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00120_8f228774: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 72));
  return 0;
}
