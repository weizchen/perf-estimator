...
static float input0_storage[2523136] __attribute__((aligned(64)));
static float input1_storage[45056] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 2523136, 1);
  fill_f32(input1_storage, 45056, 2);
  fill_f32(input2_storage, 229376, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3584, 704, 704, 1, input1_storage, input1_storage, 0, 704, 64, 64, 1, input2_storage, input2_storage, 0, 3584, 64, 64, 1, output0_storage, output0_storage, 0, 3584, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00396_a96df93d: benchmark completed");
