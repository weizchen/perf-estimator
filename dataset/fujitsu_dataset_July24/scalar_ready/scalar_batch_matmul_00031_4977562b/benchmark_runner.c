...
static uint16_t input0_storage[1720320] __attribute__((aligned(64)));
static uint16_t input1_storage[114688] __attribute__((aligned(64)));
static float input2_storage[491520] __attribute__((aligned(64)));
static float output0_storage[491520] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1720320, 1);
  fill_f16(input1_storage, 114688, 2);
  fill_f32(input2_storage, 491520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 1920, 448, 860160, 448, 1, input1_storage, input1_storage, 0, 2, 448, 128, 57344, 128, 1, input2_storage, input2_storage, 0, 2, 1920, 128, 245760, 128, 1, output0_storage, output0_storage, 0, 2, 1920, 128, 245760, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00031_4977562b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 491520));
  return 0;
}
