...
static uint16_t input0_storage[786432] __attribute__((aligned(64)));
static uint16_t input1_storage[14155776] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 786432, 1);
  fill_f16(input1_storage, 14155776, 2);
  fill_f32(input2_storage, 2359296, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 64, 384, 24576, 384, 1, input1_storage, input1_storage, 0, 32, 384, 1152, 442368, 1152, 1, input2_storage, input2_storage, 0, 32, 64, 1152, 73728, 1152, 1, output0_storage, output0_storage, 0, 32, 64, 1152, 73728, 1152, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00030_ccfcc314: benchmark completed");
