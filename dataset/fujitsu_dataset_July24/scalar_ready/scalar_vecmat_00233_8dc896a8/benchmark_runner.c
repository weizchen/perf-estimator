...
static uint16_t input0_storage[1024] __attribute__((aligned(64)));
static uint16_t input1_storage[262144] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1024, 1);
  fill_f16(input1_storage, 262144, 2);
  fill_f32(input2_storage, 256, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 1, input1_storage, input1_storage, 0, 1024, 256, 256, 1, input2_storage, input2_storage, 0, 256, 1, output0_storage, output0_storage, 0, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_vecmat_00233_8dc896a8: benchmark completed");
