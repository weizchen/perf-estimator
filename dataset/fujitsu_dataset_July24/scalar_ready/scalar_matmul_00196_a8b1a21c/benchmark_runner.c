...
static float input2_storage[1523712] __attribute__((aligned(64)));
static float output0_storage[1523712] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4063232, 1);
  fill_f16(input1_storage, 1572864, 2);
  fill_f32(input2_storage, 1523712, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1984, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 768, 768, 1, input2_storage, input2_storage, 0, 1984, 768, 768, 1, output0_storage, output0_storage, 0, 1984, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00196_a8b1a21c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1523712));
  return 0;
}
