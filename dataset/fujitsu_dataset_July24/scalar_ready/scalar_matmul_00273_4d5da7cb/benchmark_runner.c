...
static uint16_t input0_storage[540672] __attribute__((aligned(64)));
static uint16_t input1_storage[7704576] __attribute__((aligned(64)));
static uint16_t input2_storage[933888] __attribute__((aligned(64)));
static uint16_t output0_storage[933888] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 540672, 1);
  fill_f16(input1_storage, 7704576, 2);
  fill_f16(input2_storage, 933888, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2112, 2112, 1, input1_storage, input1_storage, 0, 2112, 3648, 3648, 1, input2_storage, input2_storage, 0, 256, 3648, 3648, 1, output0_storage, output0_storage, 0, 256, 3648, 3648, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00273_4d5da7cb: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 933888));
  return 0;
}
