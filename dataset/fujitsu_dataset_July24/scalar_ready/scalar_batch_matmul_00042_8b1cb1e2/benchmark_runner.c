...
static uint16_t input0_storage[44040192] __attribute__((aligned(64)));
static uint16_t input1_storage[11010048] __attribute__((aligned(64)));
static float input2_storage[4718592] __attribute__((aligned(64)));
static float output0_storage[4718592] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 44040192, 1);
  fill_f16(input1_storage, 11010048, 2);
  fill_f32(input2_storage, 4718592, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 768, 1792, 1376256, 1792, 1, input1_storage, input1_storage, 0, 32, 1792, 192, 344064, 192, 1, input2_storage, input2_storage, 0, 32, 768, 192, 147456, 192, 1, output0_storage, output0_storage, 0, 32, 768, 192, 147456, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00042_8b1cb1e2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4718592));
  return 0;
}
