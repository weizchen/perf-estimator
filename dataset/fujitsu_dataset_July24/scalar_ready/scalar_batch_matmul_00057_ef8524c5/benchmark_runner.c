...
static uint16_t input0_storage[327680] __attribute__((aligned(64)));
static uint16_t input1_storage[1310720] __attribute__((aligned(64)));
static uint16_t input2_storage[262144] __attribute__((aligned(64)));
static uint16_t output0_storage[262144] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 327680, 1);
  fill_f16(input1_storage, 1310720, 2);
  fill_f16(input2_storage, 262144, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 64, 320, 20480, 320, 1, input1_storage, input1_storage, 0, 16, 320, 256, 81920, 256, 1, input2_storage, input2_storage, 0, 16, 64, 256, 16384, 256, 1, output0_storage, output0_storage, 0, 16, 64, 256, 16384, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00057_ef8524c5: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 262144));
  return 0;
}
