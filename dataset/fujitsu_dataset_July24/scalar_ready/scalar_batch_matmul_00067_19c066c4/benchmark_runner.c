...
static uint16_t input0_storage[917504] __attribute__((aligned(64)));
static uint16_t input1_storage[262144] __attribute__((aligned(64)));
static uint16_t input2_storage[229376] __attribute__((aligned(64)));
static uint16_t output0_storage[229376] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 917504, 1);
  fill_f16(input1_storage, 262144, 2);
  fill_f16(input2_storage, 229376, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 448, 512, 229376, 512, 1, input1_storage, input1_storage, 0, 4, 512, 128, 65536, 128, 1, input2_storage, input2_storage, 0, 4, 448, 128, 57344, 128, 1, output0_storage, output0_storage, 0, 4, 448, 128, 57344, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00067_19c066c4: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 229376));
  return 0;
}
