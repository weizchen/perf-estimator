...
static int8_t input0_storage[4587520] __attribute__((aligned(64)));
static int8_t input1_storage[3932160] __attribute__((aligned(64)));
static int32_t input2_storage[2752512] __attribute__((aligned(64)));
static int32_t output0_storage[2752512] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 4587520, 1);
  fill_i8(input1_storage, 3932160, 2);
  fill_i32(input2_storage, 2752512, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 2560, 2560, 1, input1_storage, input1_storage, 0, 2560, 1536, 1536, 1, input2_storage, input2_storage, 0, 1792, 1536, 1536, 1, output0_storage, output0_storage, 0, 1792, 1536, 1536, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00043_0d669a8c: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2752512));
  return 0;
}
