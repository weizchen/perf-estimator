...
static uint16_t input0_storage[1179648] __attribute__((aligned(64)));
static uint16_t input1_storage[1966080] __attribute__((aligned(64)));
static float input2_storage[1966080] __attribute__((aligned(64)));
static float output0_storage[1966080] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1179648, 1);
  fill_f16(input1_storage, 1966080, 2);
  fill_f32(input2_storage, 1966080, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 192, 192, 36864, 192, 1, input1_storage, input1_storage, 0, 32, 192, 320, 61440, 320, 1, input2_storage, input2_storage, 0, 32, 192, 320, 61440, 320, 1, output0_storage, output0_storage, 0, 32, 192, 320, 61440, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00035_b0cd907e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1966080));
  return 0;
}
