...
static int8_t input0_storage[59536] __attribute__((aligned(64)));
static int8_t input1_storage[192] __attribute__((aligned(64)));
static int32_t input2_storage[44652] __attribute__((aligned(64)));
static int32_t output0_storage[44652] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 59536, 1);
  fill_i8(input1_storage, 192, 2);
  fill_i32(input2_storage, 44652, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 61, 61, 16, 59536, 976, 16, 1, input1_storage, input1_storage, 0, 1, 1, 16, 12, 192, 192, 12, 1, input2_storage, input2_storage, 0, 1, 61, 61, 12, 44652, 732, 12, 1, output0_storage, output0_storage, 0, 1, 61, 61, 12, 44652, 732, 12, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00040_c3e9c1c5: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 44652));
  return 0;
}
