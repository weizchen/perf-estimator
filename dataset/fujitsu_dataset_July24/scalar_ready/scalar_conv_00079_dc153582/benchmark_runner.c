...
static float input0_storage[28812] __attribute__((aligned(64)));
static float input1_storage[3528] __attribute__((aligned(64)));
static float input2_storage[11094] __attribute__((aligned(64)));
static float output0_storage[11094] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 28812, 1);
  fill_f32(input1_storage, 3528, 2);
  fill_f32(input2_storage, 11094, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 49, 49, 12, 28812, 588, 12, 1, input1_storage, input1_storage, 0, 7, 7, 12, 6, 504, 72, 6, 1, input2_storage, input2_storage, 0, 1, 43, 43, 6, 11094, 258, 6, 1, output0_storage, output0_storage, 0, 1, 43, 43, 6, 11094, 258, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00079_dc153582: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 11094));
  return 0;
}
