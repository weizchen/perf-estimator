...
static uint16_t input0_storage[141288] __attribute__((aligned(64)));
static uint16_t input1_storage[1050] __attribute__((aligned(64)));
static float input2_storage[2916] __attribute__((aligned(64)));
static float output0_storage[2916] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 141288, 1);
  fill_f16(input1_storage, 1050, 2);
  fill_f32(input2_storage, 2916, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 58, 58, 42, 141288, 2436, 42, 1, input1_storage, input1_storage, 0, 5, 5, 42, 1, 210, 42, 1, 1, input2_storage, input2_storage, 0, 1, 54, 54, 1, 2916, 54, 1, 1, output0_storage, output0_storage, 0, 1, 54, 54, 1, 2916, 54, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00083_751ffa0b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2916));
  return 0;
}
