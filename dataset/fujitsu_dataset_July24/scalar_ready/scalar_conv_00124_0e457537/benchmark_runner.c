...
static float input0_storage[10086] __attribute__((aligned(64)));
static float input1_storage[588] __attribute__((aligned(64)));
static float input2_storage[2450] __attribute__((aligned(64)));
static float output0_storage[2450] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 10086, 1);
  fill_f32(input1_storage, 588, 2);
  fill_f32(input2_storage, 2450, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 41, 41, 6, 10086, 246, 6, 1, input1_storage, input1_storage, 0, 7, 7, 6, 2, 84, 12, 2, 1, input2_storage, input2_storage, 0, 1, 35, 35, 2, 2450, 70, 2, 1, output0_storage, output0_storage, 0, 1, 35, 35, 2, 2450, 70, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00124_0e457537: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2450));
  return 0;
}
