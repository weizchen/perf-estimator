...
static uint16_t input0_storage[119646] __attribute__((aligned(64)));
static uint16_t input1_storage[414] __attribute__((aligned(64)));
static float input2_storage[23409] __attribute__((aligned(64)));
static float output0_storage[23409] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 119646, 1);
  fill_f16(input1_storage, 414, 2);
  fill_f32(input2_storage, 23409, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 51, 51, 46, 119646, 2346, 46, 1, input1_storage, input1_storage, 0, 1, 1, 46, 9, 414, 414, 9, 1, input2_storage, input2_storage, 0, 1, 51, 51, 9, 23409, 459, 9, 1, output0_storage, output0_storage, 0, 1, 51, 51, 9, 23409, 459, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00014_757b6b3f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 23409));
  return 0;
}
