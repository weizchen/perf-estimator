...
static uint16_t input1_storage[106496] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 106496, 2);
  fill_f16(input2_storage, 851968, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 128, 128, 1, input1_storage, input1_storage, 0, 128, 832, 832, 1, input2_storage, input2_storage, 0, 1024, 832, 832, 1, output0_storage, output0_storage, 0, 1024, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00228_8d794131: benchmark completed");
