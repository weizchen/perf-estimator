...
static uint16_t input0_storage[1171456] __attribute__((aligned(64)));
static uint16_t input1_storage[704] __attribute__((aligned(64)));
static float input2_storage[1664] __attribute__((aligned(64)));
static float output0_storage[1664] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1171456, 1);
  fill_f16(input1_storage, 704, 2);
  fill_f32(input2_storage, 1664, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 704, 704, 1, input1_storage, input1_storage, 0, 704, 1, input2_storage, input2_storage, 0, 1664, 1, output0_storage, output0_storage, 0, 1664, 1);
  ofa_dma_memory_fence();
  puts("scalar_matvec_00145_c501f560: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1664));
  return 0;
}
