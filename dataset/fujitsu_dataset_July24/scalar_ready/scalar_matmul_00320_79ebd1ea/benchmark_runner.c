...
static float input0_storage[118784] __attribute__((aligned(64)));
static float input1_storage[1069056] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 118784, 1);
  fill_f32(input1_storage, 1069056, 2);
  fill_f32(input2_storage, 36864, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 576, 576, 1, input2_storage, input2_storage, 0, 64, 576, 576, 1, output0_storage, output0_storage, 0, 64, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00320_79ebd1ea: benchmark completed");
