...
static uint16_t input0_storage[315392] __attribute__((aligned(64)));
static uint16_t input1_storage[57344] __attribute__((aligned(64)));
static uint16_t input2_storage[90112] __attribute__((aligned(64)));
static uint16_t output0_storage[90112] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 315392, 1);
  fill_f16(input1_storage, 57344, 2);
  fill_f16(input2_storage, 90112, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 448, 448, 1, input1_storage, input1_storage, 0, 448, 128, 128, 1, input2_storage, input2_storage, 0, 704, 128, 128, 1, output0_storage, output0_storage, 0, 704, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00316_426d433c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 90112));
  return 0;
}
