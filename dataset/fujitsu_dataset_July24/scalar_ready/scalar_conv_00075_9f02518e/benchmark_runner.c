...
static float input0_storage[21952] __attribute__((aligned(64)));
static float input1_storage[175] __attribute__((aligned(64)));
static float input2_storage[2704] __attribute__((aligned(64)));
static float output0_storage[2704] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 21952, 1);
  fill_f32(input1_storage, 175, 2);
  fill_f32(input2_storage, 2704, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 56, 56, 7, 21952, 392, 7, 1, input1_storage, input1_storage, 0, 5, 5, 7, 1, 35, 7, 1, 1, input2_storage, input2_storage, 0, 1, 52, 52, 1, 2704, 52, 1, 1, output0_storage, output0_storage, 0, 1, 52, 52, 1, 2704, 52, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00075_9f02518e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2704));
  return 0;
}
