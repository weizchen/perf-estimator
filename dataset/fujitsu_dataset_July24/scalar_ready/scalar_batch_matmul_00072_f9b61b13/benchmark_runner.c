...
static uint16_t input0_storage[368640] __attribute__((aligned(64)));
static uint16_t input1_storage[614400] __attribute__((aligned(64)));
static uint16_t input2_storage[61440] __attribute__((aligned(64)));
static uint16_t output0_storage[61440] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 368640, 1);
  fill_f16(input1_storage, 614400, 2);
  fill_f16(input2_storage, 61440, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 192, 1920, 368640, 1920, 1, input1_storage, input1_storage, 0, 1, 1920, 320, 614400, 320, 1, input2_storage, input2_storage, 0, 1, 192, 320, 61440, 320, 1, output0_storage, output0_storage, 0, 1, 192, 320, 61440, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00072_f9b61b13: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 61440));
  return 0;
}
