...
static uint16_t input0_storage[13824] __attribute__((aligned(64)));
static uint16_t input1_storage[20412] __attribute__((aligned(64)));
static float input2_storage[8232] __attribute__((aligned(64)));
static float output0_storage[8232] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 13824, 1);
  fill_f16(input1_storage, 20412, 2);
  fill_f32(input2_storage, 8232, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 16, 16, 54, 13824, 864, 54, 1, input1_storage, input1_storage, 0, 3, 3, 54, 42, 6804, 2268, 42, 1, input2_storage, input2_storage, 0, 1, 14, 14, 42, 8232, 588, 42, 1, output0_storage, output0_storage, 0, 1, 14, 14, 42, 8232, 588, 42, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00064_45b96ed3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8232));
  return 0;
}
