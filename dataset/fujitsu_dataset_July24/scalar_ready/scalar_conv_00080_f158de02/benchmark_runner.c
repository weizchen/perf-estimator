...
static int8_t input0_storage[68400] __attribute__((aligned(64)));
static int8_t input1_storage[1425] __attribute__((aligned(64)));
static int32_t input2_storage[9408] __attribute__((aligned(64)));
static int32_t output0_storage[9408] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 68400, 1);
  fill_i8(input1_storage, 1425, 2);
  fill_i32(input2_storage, 9408, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 60, 60, 19, 68400, 1140, 19, 1, input1_storage, input1_storage, 0, 5, 5, 19, 3, 285, 57, 3, 1, input2_storage, input2_storage, 0, 1, 56, 56, 3, 9408, 168, 3, 1, output0_storage, output0_storage, 0, 1, 56, 56, 3, 9408, 168, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00080_f158de02: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 9408));
  return 0;
}
