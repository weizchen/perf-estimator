...
static uint16_t input0_storage[2162688] __attribute__((aligned(64)));
static uint16_t input1_storage[983040] __attribute__((aligned(64)));
static float input2_storage[3604480] __attribute__((aligned(64)));
static float output0_storage[3604480] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2162688, 1);
  fill_f16(input1_storage, 983040, 2);
  fill_f32(input2_storage, 3604480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 704, 192, 135168, 192, 1, input1_storage, input1_storage, 0, 16, 192, 320, 61440, 320, 1, input2_storage, input2_storage, 0, 16, 704, 320, 225280, 320, 1, output0_storage, output0_storage, 0, 16, 704, 320, 225280, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00041_a43e213c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3604480));
  return 0;
}
