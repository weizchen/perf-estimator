...
static uint16_t input0_storage[86016] __attribute__((aligned(64)));
static uint16_t input1_storage[36864] __attribute__((aligned(64)));
static uint16_t input2_storage[86016] __attribute__((aligned(64)));
static uint16_t output0_storage[86016] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 86016, 1);
  fill_f16(input1_storage, 36864, 2);
  fill_f16(input2_storage, 86016, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 448, 192, 86016, 192, 1, input1_storage, input1_storage, 0, 1, 192, 192, 36864, 192, 1, input2_storage, input2_storage, 0, 1, 448, 192, 86016, 192, 1, output0_storage, output0_storage, 0, 1, 448, 192, 86016, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00061_2cb9ec26: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 86016));
  return 0;
}
