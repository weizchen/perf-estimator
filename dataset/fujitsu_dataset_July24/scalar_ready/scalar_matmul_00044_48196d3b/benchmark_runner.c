...
static int32_t input2_storage[819200] __attribute__((aligned(64)));
static int32_t output0_storage[819200] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 819200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 256, 256, 1, input1_storage, input1_storage, 0, 256, 1280, 1280, 1, input2_storage, input2_storage, 0, 640, 1280, 1280, 1, output0_storage, output0_storage, 0, 640, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00044_48196d3b: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 819200));
  return 0;
}
