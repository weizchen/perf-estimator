...
static uint16_t input0_storage[163840] __attribute__((aligned(64)));
static uint16_t input1_storage[327680] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 163840, 1);
  fill_f16(input1_storage, 327680, 2);
  fill_f32(input2_storage, 65536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 64, 320, 20480, 320, 1, input1_storage, input1_storage, 0, 8, 320, 128, 40960, 128, 1, input2_storage, input2_storage, 0, 8, 64, 128, 8192, 128, 1, output0_storage, output0_storage, 0, 8, 64, 128, 8192, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00032_4aa50af5: benchmark completed");
