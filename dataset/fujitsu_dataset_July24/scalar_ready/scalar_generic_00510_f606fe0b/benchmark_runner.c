...
static uint16_t input0_storage[650] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 650, 1);
  fill_f16(input1_storage, 650, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 650, 1, input1_storage, input1_storage, 0, 650, 1, output0_storage, output0_storage, 0, 650, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00510_f606fe0b: benchmark completed");
