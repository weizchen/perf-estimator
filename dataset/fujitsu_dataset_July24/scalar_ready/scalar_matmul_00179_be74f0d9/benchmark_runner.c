...
static uint16_t input0_storage[626688] __attribute__((aligned(64)));
static uint16_t input1_storage[2646016] __attribute__((aligned(64)));
static float input2_storage[1400832] __attribute__((aligned(64)));
static float output0_storage[1400832] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 626688, 1);
  fill_f16(input1_storage, 2646016, 2);
  fill_f32(input2_storage, 1400832, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 1088, 1088, 1, input1_storage, input1_storage, 0, 1088, 2432, 2432, 1, input2_storage, input2_storage, 0, 576, 2432, 2432, 1, output0_storage, output0_storage, 0, 576, 2432, 2432, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00179_be74f0d9: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1400832));
  return 0;
}
