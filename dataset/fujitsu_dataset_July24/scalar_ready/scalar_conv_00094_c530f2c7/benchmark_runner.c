...
static uint16_t input0_storage[729] __attribute__((aligned(64)));
static uint16_t input1_storage[225] __attribute__((aligned(64)));
static float input2_storage[4761] __attribute__((aligned(64)));
static float output0_storage[4761] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 729, 1);
  fill_f16(input1_storage, 225, 2);
  fill_f32(input2_storage, 4761, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 27, 27, 1, 729, 27, 1, 1, input1_storage, input1_storage, 0, 5, 5, 1, 9, 45, 9, 9, 1, input2_storage, input2_storage, 0, 1, 23, 23, 9, 4761, 207, 9, 1, output0_storage, output0_storage, 0, 1, 23, 23, 9, 4761, 207, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00094_c530f2c7: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4761));
  return 0;
}
