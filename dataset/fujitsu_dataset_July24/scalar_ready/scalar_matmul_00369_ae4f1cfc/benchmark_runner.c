...
static float input0_storage[344064] __attribute__((aligned(64)));
static float input1_storage[135168] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 344064, 1);
  fill_f32(input1_storage, 135168, 2);
  fill_f32(input2_storage, 1261568, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 192, 192, 1, input1_storage, input1_storage, 0, 192, 704, 704, 1, input2_storage, input2_storage, 0, 1792, 704, 704, 1, output0_storage, output0_storage, 0, 1792, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00369_ae4f1cfc: benchmark completed");
