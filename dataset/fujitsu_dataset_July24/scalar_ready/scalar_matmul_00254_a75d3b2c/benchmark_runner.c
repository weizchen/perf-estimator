...
static uint16_t input0_storage[2031616] __attribute__((aligned(64)));
static uint16_t input1_storage[196608] __attribute__((aligned(64)));
static uint16_t input2_storage[380928] __attribute__((aligned(64)));
static uint16_t output0_storage[380928] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2031616, 1);
  fill_f16(input1_storage, 196608, 2);
  fill_f16(input2_storage, 380928, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1984, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 192, 192, 1, input2_storage, input2_storage, 0, 1984, 192, 192, 1, output0_storage, output0_storage, 0, 1984, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00254_a75d3b2c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 380928));
  return 0;
}
