...
static float input0_storage[2289664] __attribute__((aligned(64)));
static float input1_storage[638976] __attribute__((aligned(64)));
static float input2_storage[2113536] __attribute__((aligned(64)));
static float output0_storage[2113536] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 2289664, 1);
  fill_f32(input1_storage, 638976, 2);
  fill_f32(input2_storage, 2113536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 832, 832, 1, input1_storage, input1_storage, 0, 832, 768, 768, 1, input2_storage, input2_storage, 0, 2752, 768, 768, 1, output0_storage, output0_storage, 0, 2752, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00399_999bf6ec: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2113536));
  return 0;
}
