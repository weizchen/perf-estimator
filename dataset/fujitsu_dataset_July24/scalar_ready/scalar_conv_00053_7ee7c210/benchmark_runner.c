...
static int8_t input0_storage[1083] __attribute__((aligned(64)));
static int8_t input1_storage[63] __attribute__((aligned(64)));
static int32_t input2_storage[2100] __attribute__((aligned(64)));
static int32_t output0_storage[2100] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1083, 1);
  fill_i8(input1_storage, 63, 2);
  fill_i32(input2_storage, 2100, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 19, 19, 3, 1083, 57, 3, 1, input1_storage, input1_storage, 0, 1, 1, 3, 21, 63, 63, 21, 1, input2_storage, input2_storage, 0, 1, 10, 10, 21, 2100, 210, 21, 1, output0_storage, output0_storage, 0, 1, 10, 10, 21, 2100, 210, 21, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00053_7ee7c210: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2100));
  return 0;
}
