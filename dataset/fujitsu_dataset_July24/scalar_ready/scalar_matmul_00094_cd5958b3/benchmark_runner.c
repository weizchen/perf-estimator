...
static int8_t input0_storage[7618560] __attribute__((aligned(64)));
static int8_t input1_storage[1720320] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 7618560, 1);
  fill_i8(input1_storage, 1720320, 2);
  fill_i32(input2_storage, 3555328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 1920, 1920, 1, input1_storage, input1_storage, 0, 1920, 896, 896, 1, input2_storage, input2_storage, 0, 3968, 896, 896, 1, output0_storage, output0_storage, 0, 3968, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00094_cd5958b3: benchmark completed");
