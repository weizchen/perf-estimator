...
static uint16_t input0_storage[3465216] __attribute__((aligned(64)));
static uint16_t input1_storage[6352896] __attribute__((aligned(64)));
static uint16_t input2_storage[2433024] __attribute__((aligned(64)));
static uint16_t output0_storage[2433024] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3465216, 1);
  fill_f16(input1_storage, 6352896, 2);
  fill_f16(input2_storage, 2433024, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1152, 3008, 3008, 1, input1_storage, input1_storage, 0, 3008, 2112, 2112, 1, input2_storage, input2_storage, 0, 1152, 2112, 2112, 1, output0_storage, output0_storage, 0, 1152, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00310_1cb8a6f7: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2433024));
  return 0;
}
