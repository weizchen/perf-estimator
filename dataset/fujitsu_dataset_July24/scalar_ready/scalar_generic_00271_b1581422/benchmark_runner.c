...
static int8_t input0_storage[3501] __attribute__((aligned(64)));
static int8_t input1_storage[3501] __attribute__((aligned(64)));
static int8_t output0_storage[3501] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 3501, 1);
  fill_i8(input1_storage, 3501, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3501, 1, input1_storage, input1_storage, 0, 3501, 1, output0_storage, output0_storage, 0, 3501, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00271_b1581422: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 3501));
  return 0;
}
