...
static uint16_t input1_storage[1400832] __attribute__((aligned(64)));
static uint16_t input2_storage[1167360] __attribute__((aligned(64)));
static uint16_t output0_storage[1167360] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1400832, 2);
  fill_f16(input2_storage, 1167360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 384, 384, 1, input1_storage, input1_storage, 0, 384, 3648, 3648, 1, input2_storage, input2_storage, 0, 320, 3648, 3648, 1, output0_storage, output0_storage, 0, 320, 3648, 3648, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00229_93767d9d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1167360));
  return 0;
}
