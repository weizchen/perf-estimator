...
static int8_t input0_storage[26912] __attribute__((aligned(64)));
static int8_t input1_storage[32] __attribute__((aligned(64)));
static int32_t input2_storage[225] __attribute__((aligned(64)));
static int32_t output0_storage[225] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 26912, 1);
  fill_i8(input1_storage, 32, 2);
  fill_i32(input2_storage, 225, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 29, 29, 32, 26912, 928, 32, 1, input1_storage, input1_storage, 0, 1, 1, 32, 1, 32, 32, 1, 1, input2_storage, input2_storage, 0, 1, 15, 15, 1, 225, 15, 1, 1, output0_storage, output0_storage, 0, 1, 15, 15, 1, 225, 15, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00024_d5fbbb42: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 225));
  return 0;
}
