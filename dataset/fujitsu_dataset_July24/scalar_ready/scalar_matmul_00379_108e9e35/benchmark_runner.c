...
static float input0_storage[774144] __attribute__((aligned(64)));
static float input1_storage[1462272] __attribute__((aligned(64)));
static float input2_storage[626688] __attribute__((aligned(64)));
static float output0_storage[626688] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 774144, 1);
  fill_f32(input1_storage, 1462272, 2);
  fill_f32(input2_storage, 626688, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 1344, 1344, 1, input1_storage, input1_storage, 0, 1344, 1088, 1088, 1, input2_storage, input2_storage, 0, 576, 1088, 1088, 1, output0_storage, output0_storage, 0, 576, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00379_108e9e35: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 626688));
  return 0;
}
