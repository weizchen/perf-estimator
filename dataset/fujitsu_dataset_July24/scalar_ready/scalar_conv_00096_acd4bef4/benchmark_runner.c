...
static int8_t input0_storage[4332] __attribute__((aligned(64)));
static int8_t input1_storage[135] __attribute__((aligned(64)));
static int32_t input2_storage[6480] __attribute__((aligned(64)));
static int32_t output0_storage[6480] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 4332, 1);
  fill_i8(input1_storage, 135, 2);
  fill_i32(input2_storage, 6480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 38, 38, 3, 4332, 114, 3, 1, input1_storage, input1_storage, 0, 3, 3, 3, 5, 45, 15, 5, 1, input2_storage, input2_storage, 0, 1, 36, 36, 5, 6480, 180, 5, 1, output0_storage, output0_storage, 0, 1, 36, 36, 5, 6480, 180, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00096_acd4bef4: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 6480));
  return 0;
}
