...
static uint16_t input0_storage[243] __attribute__((aligned(64)));
static uint16_t input1_storage[1176] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 243, 1);
  fill_f16(input1_storage, 1176, 2);
  fill_f32(input2_storage, 72, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 9, 9, 3, 243, 27, 3, 1, input1_storage, input1_storage, 0, 7, 7, 3, 8, 168, 24, 8, 1, input2_storage, input2_storage, 0, 1, 3, 3, 8, 72, 24, 8, 1, output0_storage, output0_storage, 0, 1, 3, 3, 8, 72, 24, 8, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00009_ebb84e7d: benchmark completed");
