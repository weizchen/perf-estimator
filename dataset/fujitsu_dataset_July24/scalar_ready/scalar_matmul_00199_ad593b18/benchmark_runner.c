...
static uint16_t input0_storage[12591104] __attribute__((aligned(64)));
static uint16_t input1_storage[9977856] __attribute__((aligned(64)));
static float input2_storage[9117696] __attribute__((aligned(64)));
static float output0_storage[9117696] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 12591104, 1);
  fill_f16(input1_storage, 9977856, 2);
  fill_f32(input2_storage, 9117696, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3392, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 2688, 2688, 1, input2_storage, input2_storage, 0, 3392, 2688, 2688, 1, output0_storage, output0_storage, 0, 3392, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00199_ad593b18: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9117696));
  return 0;
}
