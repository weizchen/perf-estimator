...
static float input0_storage[24037] __attribute__((aligned(64)));
static float input1_storage[5733] __attribute__((aligned(64)));
static float input2_storage[82369] __attribute__((aligned(64)));
static float output0_storage[82369] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 24037, 1);
  fill_f32(input1_storage, 5733, 2);
  fill_f32(input2_storage, 82369, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 43, 43, 13, 24037, 559, 13, 1, input1_storage, input1_storage, 0, 3, 3, 13, 49, 1911, 637, 49, 1, input2_storage, input2_storage, 0, 1, 41, 41, 49, 82369, 2009, 49, 1, output0_storage, output0_storage, 0, 1, 41, 41, 49, 82369, 2009, 49, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00020_f1520029: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 82369));
  return 0;
}
