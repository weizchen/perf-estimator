...
static uint16_t input0_storage[1802240] __attribute__((aligned(64)));
static uint16_t input1_storage[2613248] __attribute__((aligned(64)));
static float input2_storage[4751360] __attribute__((aligned(64)));
static float output0_storage[4751360] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1802240, 1);
  fill_f16(input1_storage, 2613248, 2);
  fill_f32(input2_storage, 4751360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 1280, 704, 901120, 704, 1, input1_storage, input1_storage, 0, 2, 704, 1856, 1306624, 1856, 1, input2_storage, input2_storage, 0, 2, 1280, 1856, 2375680, 1856, 1, output0_storage, output0_storage, 0, 2, 1280, 1856, 2375680, 1856, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00047_2a98480b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4751360));
  return 0;
}
