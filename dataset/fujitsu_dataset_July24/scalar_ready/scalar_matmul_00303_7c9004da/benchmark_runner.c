...
static uint16_t input2_storage[98304] __attribute__((aligned(64)));
static uint16_t output0_storage[98304] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 98304, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 704, 704, 1, input1_storage, input1_storage, 0, 704, 192, 192, 1, input2_storage, input2_storage, 0, 512, 192, 192, 1, output0_storage, output0_storage, 0, 512, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00303_7c9004da: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 98304));
  return 0;
}
