...
static uint16_t input0_storage[17500] __attribute__((aligned(64)));
static uint16_t input1_storage[1029] __attribute__((aligned(64)));
static float input2_storage[5808] __attribute__((aligned(64)));
static float output0_storage[5808] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 17500, 1);
  fill_f16(input1_storage, 1029, 2);
  fill_f32(input2_storage, 5808, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 50, 50, 7, 17500, 350, 7, 1, input1_storage, input1_storage, 0, 7, 7, 7, 3, 147, 21, 3, 1, input2_storage, input2_storage, 0, 1, 44, 44, 3, 5808, 132, 3, 1, output0_storage, output0_storage, 0, 1, 44, 44, 3, 5808, 132, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00037_4784bffa: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5808));
  return 0;
}
