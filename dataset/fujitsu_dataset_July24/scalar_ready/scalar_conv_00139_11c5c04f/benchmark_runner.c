...
static uint16_t input0_storage[338] __attribute__((aligned(64)));
static uint16_t input1_storage[64] __attribute__((aligned(64)));
static float input2_storage[5408] __attribute__((aligned(64)));
static float output0_storage[5408] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 338, 1);
  fill_f16(input1_storage, 64, 2);
  fill_f32(input2_storage, 5408, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 13, 13, 2, 338, 26, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 32, 64, 64, 32, 1, input2_storage, input2_storage, 0, 1, 13, 13, 32, 5408, 416, 32, 1, output0_storage, output0_storage, 0, 1, 13, 13, 32, 5408, 416, 32, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00139_11c5c04f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5408));
  return 0;
}
