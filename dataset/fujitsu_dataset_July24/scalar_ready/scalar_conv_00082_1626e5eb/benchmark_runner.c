...
static uint16_t input0_storage[9600] __attribute__((aligned(64)));
static uint16_t input1_storage[1242] __attribute__((aligned(64)));
static float input2_storage[33212] __attribute__((aligned(64)));
static float output0_storage[33212] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9600, 1);
  fill_f16(input1_storage, 1242, 2);
  fill_f32(input2_storage, 33212, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 40, 40, 6, 9600, 240, 6, 1, input1_storage, input1_storage, 0, 3, 3, 6, 23, 414, 138, 23, 1, input2_storage, input2_storage, 0, 1, 38, 38, 23, 33212, 874, 23, 1, output0_storage, output0_storage, 0, 1, 38, 38, 23, 33212, 874, 23, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00082_1626e5eb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 33212));
  return 0;
}
