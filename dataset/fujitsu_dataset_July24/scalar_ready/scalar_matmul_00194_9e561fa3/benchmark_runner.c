...
static float input2_storage[1011712] __attribute__((aligned(64)));
static float output0_storage[1011712] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 1011712, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 1408, 1408, 1, input1_storage, input1_storage, 0, 1408, 1216, 1216, 1, input2_storage, input2_storage, 0, 832, 1216, 1216, 1, output0_storage, output0_storage, 0, 832, 1216, 1216, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00194_9e561fa3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1011712));
  return 0;
}
