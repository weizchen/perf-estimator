...
static float input0_storage[5808] __attribute__((aligned(64)));
static float input1_storage[60] __attribute__((aligned(64)));
static float input2_storage[9680] __attribute__((aligned(64)));
static float output0_storage[9680] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 5808, 1);
  fill_f32(input1_storage, 60, 2);
  fill_f32(input2_storage, 9680, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 44, 44, 3, 5808, 132, 3, 1, input1_storage, input1_storage, 0, 1, 1, 3, 20, 60, 60, 20, 1, input2_storage, input2_storage, 0, 1, 22, 22, 20, 9680, 440, 20, 1, output0_storage, output0_storage, 0, 1, 22, 22, 20, 9680, 440, 20, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00007_9073ef0d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9680));
  return 0;
}
