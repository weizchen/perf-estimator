...
static int8_t input0_storage[784] __attribute__((aligned(64)));
static int8_t input1_storage[2548] __attribute__((aligned(64)));
static int32_t input2_storage[208] __attribute__((aligned(64)));
static int32_t output0_storage[208] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 784, 1);
  fill_i8(input1_storage, 2548, 2);
  fill_i32(input2_storage, 208, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 14, 14, 4, 784, 56, 4, 1, input1_storage, input1_storage, 0, 7, 7, 4, 13, 364, 52, 13, 1, input2_storage, input2_storage, 0, 1, 4, 4, 13, 208, 52, 13, 1, output0_storage, output0_storage, 0, 1, 4, 4, 13, 208, 52, 13, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00113_dcb8a930: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 208));
  return 0;
}
