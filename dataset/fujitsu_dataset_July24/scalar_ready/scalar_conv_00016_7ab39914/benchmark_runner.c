...
static uint16_t input0_storage[6776] __attribute__((aligned(64)));
static uint16_t input1_storage[6300] __attribute__((aligned(64)));
static float input2_storage[1458] __attribute__((aligned(64)));
static float output0_storage[1458] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 6776, 1);
  fill_f16(input1_storage, 6300, 2);
  fill_f32(input2_storage, 1458, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 22, 22, 14, 6776, 308, 14, 1, input1_storage, input1_storage, 0, 5, 5, 14, 18, 1260, 252, 18, 1, input2_storage, input2_storage, 0, 1, 9, 9, 18, 1458, 162, 18, 1, output0_storage, output0_storage, 0, 1, 9, 9, 18, 1458, 162, 18, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00016_7ab39914: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1458));
  return 0;
}
