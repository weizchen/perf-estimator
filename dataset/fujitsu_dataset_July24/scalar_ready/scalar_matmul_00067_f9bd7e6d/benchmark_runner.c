...
static int8_t input0_storage[884736] __attribute__((aligned(64)));
static int8_t input1_storage[2949120] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 884736, 1);
  fill_i8(input1_storage, 2949120, 2);
  fill_i32(input2_storage, 491520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 2304, 2304, 1, input1_storage, input1_storage, 0, 2304, 1280, 1280, 1, input2_storage, input2_storage, 0, 384, 1280, 1280, 1, output0_storage, output0_storage, 0, 384, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00067_f9bd7e6d: benchmark completed");
