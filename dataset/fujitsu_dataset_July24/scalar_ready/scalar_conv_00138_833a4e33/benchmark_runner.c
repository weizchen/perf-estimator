...
static uint16_t input0_storage[361] __attribute__((aligned(64)));
static uint16_t input1_storage[245] __attribute__((aligned(64)));
static float input2_storage[845] __attribute__((aligned(64)));
static float output0_storage[845] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 361, 1);
  fill_f16(input1_storage, 245, 2);
  fill_f32(input2_storage, 845, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 19, 19, 1, 361, 19, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 5, 35, 5, 5, 1, input2_storage, input2_storage, 0, 1, 13, 13, 5, 845, 65, 5, 1, output0_storage, output0_storage, 0, 1, 13, 13, 5, 845, 65, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00138_833a4e33: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 845));
  return 0;
}
