...
static int8_t input0_storage[262144] __attribute__((aligned(64)));
static int8_t input1_storage[1179648] __attribute__((aligned(64)));
static int32_t input2_storage[294912] __attribute__((aligned(64)));
static int32_t output0_storage[294912] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 262144, 1);
  fill_i8(input1_storage, 1179648, 2);
  fill_i32(input2_storage, 294912, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 1152, 1152, 1, input2_storage, input2_storage, 0, 256, 1152, 1152, 1, output0_storage, output0_storage, 0, 256, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00053_ec555494: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 294912));
  return 0;
}
