...
static uint16_t input0_storage[4107] __attribute__((aligned(64)));
static uint16_t input1_storage[30] __attribute__((aligned(64)));
static float input2_storage[13690] __attribute__((aligned(64)));
static float output0_storage[13690] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4107, 1);
  fill_f16(input1_storage, 30, 2);
  fill_f32(input2_storage, 13690, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 37, 37, 3, 4107, 111, 3, 1, input1_storage, input1_storage, 0, 1, 1, 3, 10, 30, 30, 10, 1, input2_storage, input2_storage, 0, 1, 37, 37, 10, 13690, 370, 10, 1, output0_storage, output0_storage, 0, 1, 37, 37, 10, 13690, 370, 10, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00072_8728a709: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 13690));
  return 0;
}
