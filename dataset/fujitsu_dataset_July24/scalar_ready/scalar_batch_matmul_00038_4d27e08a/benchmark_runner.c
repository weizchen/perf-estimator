...
static uint16_t input0_storage[122880] __attribute__((aligned(64)));
static uint16_t input1_storage[172032] __attribute__((aligned(64)));
static float input2_storage[143360] __attribute__((aligned(64)));
static float output0_storage[143360] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 122880, 1);
  fill_f16(input1_storage, 172032, 2);
  fill_f32(input2_storage, 143360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 320, 384, 122880, 384, 1, input1_storage, input1_storage, 0, 1, 384, 448, 172032, 448, 1, input2_storage, input2_storage, 0, 1, 320, 448, 143360, 448, 1, output0_storage, output0_storage, 0, 1, 320, 448, 143360, 448, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00038_4d27e08a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 143360));
  return 0;
}
