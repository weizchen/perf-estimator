...
static uint16_t input0_storage[4534272] __attribute__((aligned(64)));
static uint16_t input1_storage[839680] __attribute__((aligned(64)));
static uint16_t input2_storage[552960] __attribute__((aligned(64)));
static uint16_t output0_storage[552960] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4534272, 1);
  fill_f16(input1_storage, 839680, 2);
  fill_f16(input2_storage, 552960, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1728, 2624, 2624, 1, input1_storage, input1_storage, 0, 2624, 320, 320, 1, input2_storage, input2_storage, 0, 1728, 320, 320, 1, output0_storage, output0_storage, 0, 1728, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00256_ffa8b9c6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 552960));
  return 0;
}
