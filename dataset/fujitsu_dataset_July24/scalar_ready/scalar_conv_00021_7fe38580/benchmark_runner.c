...
static int8_t input1_storage[62] __attribute__((aligned(64)));
static int32_t input2_storage[3100] __attribute__((aligned(64)));
static int32_t output0_storage[3100] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 62, 2);
  fill_i32(input2_storage, 3100, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 10, 10, 2, 200, 20, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 31, 62, 62, 31, 1, input2_storage, input2_storage, 0, 1, 10, 10, 31, 3100, 310, 31, 1, output0_storage, output0_storage, 0, 1, 10, 10, 31, 3100, 310, 31, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00021_7fe38580: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 3100));
  return 0;
}
