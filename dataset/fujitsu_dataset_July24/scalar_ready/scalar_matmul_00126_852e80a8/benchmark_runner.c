...
static float input2_storage[1904640] __attribute__((aligned(64)));
static float output0_storage[1904640] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 1904640, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1984, 256, 256, 1, input1_storage, input1_storage, 0, 256, 960, 960, 1, input2_storage, input2_storage, 0, 1984, 960, 960, 1, output0_storage, output0_storage, 0, 1984, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00126_852e80a8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1904640));
  return 0;
}
