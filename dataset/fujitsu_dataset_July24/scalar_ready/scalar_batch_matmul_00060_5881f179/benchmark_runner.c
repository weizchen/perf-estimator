...
static uint16_t input0_storage[7208960] __attribute__((aligned(64)));
static uint16_t input1_storage[2523136] __attribute__((aligned(64)));
static uint16_t input2_storage[4587520] __attribute__((aligned(64)));
static uint16_t output0_storage[4587520] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 7208960, 1);
  fill_f16(input1_storage, 2523136, 2);
  fill_f16(input2_storage, 4587520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 1280, 704, 901120, 704, 1, input1_storage, input1_storage, 0, 8, 704, 448, 315392, 448, 1, input2_storage, input2_storage, 0, 8, 1280, 448, 573440, 448, 1, output0_storage, output0_storage, 0, 8, 1280, 448, 573440, 448, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00060_5881f179: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 4587520));
  return 0;
}
