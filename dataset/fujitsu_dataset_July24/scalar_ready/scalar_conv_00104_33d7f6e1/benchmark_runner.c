...
static int8_t input0_storage[11616] __attribute__((aligned(64)));
static int8_t input1_storage[900] __attribute__((aligned(64)));
static int32_t input2_storage[9600] __attribute__((aligned(64)));
static int32_t output0_storage[9600] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 11616, 1);
  fill_i8(input1_storage, 900, 2);
  fill_i32(input2_storage, 9600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 44, 44, 6, 11616, 264, 6, 1, input1_storage, input1_storage, 0, 5, 5, 6, 6, 180, 36, 6, 1, input2_storage, input2_storage, 0, 1, 40, 40, 6, 9600, 240, 6, 1, output0_storage, output0_storage, 0, 1, 40, 40, 6, 9600, 240, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00104_33d7f6e1: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 9600));
  return 0;
}
