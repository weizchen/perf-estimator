...
static uint16_t input0_storage[356352] __attribute__((aligned(64)));
static uint16_t input1_storage[2375680] __attribute__((aligned(64)));
static uint16_t input2_storage[245760] __attribute__((aligned(64)));
static uint16_t output0_storage[245760] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 356352, 1);
  fill_f16(input1_storage, 2375680, 2);
  fill_f16(input2_storage, 245760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 1280, 1280, 1, input2_storage, input2_storage, 0, 192, 1280, 1280, 1, output0_storage, output0_storage, 0, 192, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00305_adac0ed4: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 245760));
  return 0;
}
