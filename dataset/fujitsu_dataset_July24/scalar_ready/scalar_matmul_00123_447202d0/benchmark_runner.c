...
static uint16_t input1_storage[909312] __attribute__((aligned(64)));
static float input2_storage[757760] __attribute__((aligned(64)));
static float output0_storage[757760] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 909312, 2);
  fill_f32(input2_storage, 757760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 384, 384, 1, input1_storage, input1_storage, 0, 384, 2368, 2368, 1, input2_storage, input2_storage, 0, 320, 2368, 2368, 1, output0_storage, output0_storage, 0, 320, 2368, 2368, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00123_447202d0: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 757760));
  return 0;
}
