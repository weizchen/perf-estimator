...
static uint16_t input0_storage[1732608] __attribute__((aligned(64)));
static uint16_t input1_storage[385024] __attribute__((aligned(64)));
static float input2_storage[73728] __attribute__((aligned(64)));
static float output0_storage[73728] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1732608, 1);
  fill_f16(input1_storage, 385024, 2);
  fill_f32(input2_storage, 73728, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 3008, 3008, 1, input1_storage, input1_storage, 0, 3008, 128, 128, 1, input2_storage, input2_storage, 0, 576, 128, 128, 1, output0_storage, output0_storage, 0, 576, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00207_d92efa7d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 73728));
  return 0;
}
