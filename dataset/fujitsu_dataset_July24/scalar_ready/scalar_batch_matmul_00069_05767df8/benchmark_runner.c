...
static uint16_t input1_storage[491520] __attribute__((aligned(64)));
static uint16_t input2_storage[589824] __attribute__((aligned(64)));
static uint16_t output0_storage[589824] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 491520, 2);
  fill_f16(input2_storage, 589824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 384, 320, 122880, 320, 1, input1_storage, input1_storage, 0, 2, 320, 768, 245760, 768, 1, input2_storage, input2_storage, 0, 2, 384, 768, 294912, 768, 1, output0_storage, output0_storage, 0, 2, 384, 768, 294912, 768, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00069_05767df8: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 589824));
  return 0;
}
