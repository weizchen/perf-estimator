...
static uint16_t input0_storage[131072] __attribute__((aligned(64)));
static uint16_t input1_storage[1572864] __attribute__((aligned(64)));
static uint16_t input2_storage[786432] __attribute__((aligned(64)));
static uint16_t output0_storage[786432] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 131072, 1);
  fill_f16(input1_storage, 1572864, 2);
  fill_f16(input2_storage, 786432, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 64, 128, 8192, 128, 1, input1_storage, input1_storage, 0, 16, 128, 768, 98304, 768, 1, input2_storage, input2_storage, 0, 16, 64, 768, 49152, 768, 1, output0_storage, output0_storage, 0, 16, 64, 768, 49152, 768, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00058_beb7c6ea: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 786432));
  return 0;
}
