...
static int8_t input0_storage[960] __attribute__((aligned(64)));
static int8_t input1_storage[2835] __attribute__((aligned(64)));
static int32_t input2_storage[756] __attribute__((aligned(64)));
static int32_t output0_storage[756] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 960, 1);
  fill_i8(input1_storage, 2835, 2);
  fill_i32(input2_storage, 756, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 15, 960, 120, 15, 1, input1_storage, input1_storage, 0, 3, 3, 15, 21, 945, 315, 21, 1, input2_storage, input2_storage, 0, 1, 6, 6, 21, 756, 126, 21, 1, output0_storage, output0_storage, 0, 1, 6, 6, 21, 756, 126, 21, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00090_8e3fa11e: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 756));
  return 0;
}
