...
static float input0_storage[961] __attribute__((aligned(64)));
static float input1_storage[75] __attribute__((aligned(64)));
static float input2_storage[588] __attribute__((aligned(64)));
static float output0_storage[588] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 961, 1);
  fill_f32(input1_storage, 75, 2);
  fill_f32(input2_storage, 588, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 31, 31, 1, 961, 31, 1, 1, input1_storage, input1_storage, 0, 5, 5, 1, 3, 15, 3, 3, 1, input2_storage, input2_storage, 0, 1, 14, 14, 3, 588, 42, 3, 1, output0_storage, output0_storage, 0, 1, 14, 14, 3, 588, 42, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00116_17e0e0c0: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 588));
  return 0;
}
