...
static uint16_t input1_storage[208896] __attribute__((aligned(64)));
static uint16_t input2_storage[6266880] __attribute__((aligned(64)));
static uint16_t output0_storage[6266880] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 208896, 2);
  fill_f16(input2_storage, 6266880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 64, 64, 1, input1_storage, input1_storage, 0, 64, 3264, 3264, 1, input2_storage, input2_storage, 0, 1920, 3264, 3264, 1, output0_storage, output0_storage, 0, 1920, 3264, 3264, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00232_67a17e6c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 6266880));
  return 0;
}
