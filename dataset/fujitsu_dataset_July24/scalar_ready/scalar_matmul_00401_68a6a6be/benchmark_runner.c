...
static float input0_storage[2785280] __attribute__((aligned(64)));
static float input1_storage[3481600] __attribute__((aligned(64)));
static float input2_storage[2048000] __attribute__((aligned(64)));
static float output0_storage[2048000] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 2785280, 1);
  fill_f32(input1_storage, 3481600, 2);
  fill_f32(input2_storage, 2048000, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 1600, 1600, 1, input2_storage, input2_storage, 0, 1280, 1600, 1600, 1, output0_storage, output0_storage, 0, 1280, 1600, 1600, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00401_68a6a6be: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2048000));
  return 0;
}
