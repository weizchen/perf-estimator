...
static uint16_t input0_storage[401408] __attribute__((aligned(64)));
static uint16_t input1_storage[3411968] __attribute__((aligned(64)));
static float input2_storage[139264] __attribute__((aligned(64)));
static float output0_storage[139264] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 401408, 1);
  fill_f16(input1_storage, 3411968, 2);
  fill_f32(input2_storage, 139264, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 1088, 1088, 1, input2_storage, input2_storage, 0, 128, 1088, 1088, 1, output0_storage, output0_storage, 0, 128, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00170_306b698e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 139264));
  return 0;
}
