...
static uint16_t input0_storage[46137344] __attribute__((aligned(64)));
static uint16_t input1_storage[8388608] __attribute__((aligned(64)));
static float input2_storage[2883584] __attribute__((aligned(64)));
static float output0_storage[2883584] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 46137344, 1);
  fill_f16(input1_storage, 8388608, 2);
  fill_f32(input2_storage, 2883584, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 704, 2048, 1441792, 2048, 1, input1_storage, input1_storage, 0, 32, 2048, 128, 262144, 128, 1, input2_storage, input2_storage, 0, 32, 704, 128, 90112, 128, 1, output0_storage, output0_storage, 0, 32, 704, 128, 90112, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00026_9889af7b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2883584));
  return 0;
}
