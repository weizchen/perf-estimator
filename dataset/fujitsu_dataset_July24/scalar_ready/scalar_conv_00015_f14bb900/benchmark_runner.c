...
static uint16_t input0_storage[86756] __attribute__((aligned(64)));
static uint16_t input1_storage[11275] __attribute__((aligned(64)));
static float input2_storage[19404] __attribute__((aligned(64)));
static float output0_storage[19404] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 86756, 1);
  fill_f16(input1_storage, 11275, 2);
  fill_f32(input2_storage, 19404, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 46, 46, 41, 86756, 1886, 41, 1, input1_storage, input1_storage, 0, 5, 5, 41, 11, 2255, 451, 11, 1, input2_storage, input2_storage, 0, 1, 42, 42, 11, 19404, 462, 11, 1, output0_storage, output0_storage, 0, 1, 42, 42, 11, 19404, 462, 11, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00015_f14bb900: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 19404));
  return 0;
}
