...
static uint16_t input0_storage[5242880] __attribute__((aligned(64)));
static uint16_t input1_storage[4194304] __attribute__((aligned(64)));
static uint16_t input2_storage[10485760] __attribute__((aligned(64)));
static uint16_t output0_storage[10485760] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5242880, 1);
  fill_f16(input1_storage, 4194304, 2);
  fill_f16(input2_storage, 10485760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 640, 256, 163840, 256, 1, input1_storage, input1_storage, 0, 32, 256, 512, 131072, 512, 1, input2_storage, input2_storage, 0, 32, 640, 512, 327680, 512, 1, output0_storage, output0_storage, 0, 32, 640, 512, 327680, 512, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00050_5fb25433: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 10485760));
  return 0;
}
