...
static uint16_t input2_storage[2048000] __attribute__((aligned(64)));
static uint16_t output0_storage[2048000] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 2048000, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3200, 128, 128, 1, input1_storage, input1_storage, 0, 128, 640, 640, 1, input2_storage, input2_storage, 0, 3200, 640, 640, 1, output0_storage, output0_storage, 0, 3200, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00240_ba622140: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2048000));
  return 0;
}
