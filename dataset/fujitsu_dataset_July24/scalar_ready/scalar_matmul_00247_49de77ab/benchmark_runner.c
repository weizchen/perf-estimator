...
static uint16_t input1_storage[1835008] __attribute__((aligned(64)));
static uint16_t input2_storage[9633792] __attribute__((aligned(64)));
static uint16_t output0_storage[9633792] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1835008, 2);
  fill_f16(input2_storage, 9633792, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2688, 512, 512, 1, input1_storage, input1_storage, 0, 512, 3584, 3584, 1, input2_storage, input2_storage, 0, 2688, 3584, 3584, 1, output0_storage, output0_storage, 0, 2688, 3584, 3584, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00247_49de77ab: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 9633792));
  return 0;
}
