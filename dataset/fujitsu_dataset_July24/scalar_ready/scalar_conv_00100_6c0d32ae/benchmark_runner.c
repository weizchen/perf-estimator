...
static int8_t input0_storage[1600] __attribute__((aligned(64)));
static int8_t input1_storage[10192] __attribute__((aligned(64)));
static int32_t input2_storage[2548] __attribute__((aligned(64)));
static int32_t output0_storage[2548] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1600, 1);
  fill_i8(input1_storage, 10192, 2);
  fill_i32(input2_storage, 2548, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 20, 20, 4, 1600, 80, 4, 1, input1_storage, input1_storage, 0, 7, 7, 4, 52, 1456, 208, 52, 1, input2_storage, input2_storage, 0, 1, 7, 7, 52, 2548, 364, 52, 1, output0_storage, output0_storage, 0, 1, 7, 7, 52, 2548, 364, 52, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00100_6c0d32ae: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2548));
  return 0;
}
