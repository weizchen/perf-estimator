...
static float input0_storage[1548288] __attribute__((aligned(64)));
static float input1_storage[368640] __attribute__((aligned(64)));
static float input2_storage[3870720] __attribute__((aligned(64)));
static float output0_storage[3870720] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1548288, 1);
  fill_f32(input1_storage, 368640, 2);
  fill_f32(input2_storage, 3870720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4032, 384, 384, 1, input1_storage, input1_storage, 0, 384, 960, 960, 1, input2_storage, input2_storage, 0, 4032, 960, 960, 1, output0_storage, output0_storage, 0, 4032, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00422_89124191: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3870720));
  return 0;
}
