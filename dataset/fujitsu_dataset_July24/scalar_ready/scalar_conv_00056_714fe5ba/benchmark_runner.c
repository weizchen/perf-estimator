...
static int8_t input0_storage[75816] __attribute__((aligned(64)));
static int8_t input1_storage[12350] __attribute__((aligned(64)));
static int32_t input2_storage[11875] __attribute__((aligned(64)));
static int32_t output0_storage[11875] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 75816, 1);
  fill_i8(input1_storage, 12350, 2);
  fill_i32(input2_storage, 11875, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 54, 54, 26, 75816, 1404, 26, 1, input1_storage, input1_storage, 0, 5, 5, 26, 19, 2470, 494, 19, 1, input2_storage, input2_storage, 0, 1, 25, 25, 19, 11875, 475, 19, 1, output0_storage, output0_storage, 0, 1, 25, 25, 19, 11875, 475, 19, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00056_714fe5ba: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 11875));
  return 0;
}
