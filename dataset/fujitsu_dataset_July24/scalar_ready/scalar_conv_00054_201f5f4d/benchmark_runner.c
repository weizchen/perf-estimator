...
static int8_t input0_storage[107604] __attribute__((aligned(64)));
static int8_t input1_storage[549] __attribute__((aligned(64)));
static int32_t input2_storage[400] __attribute__((aligned(64)));
static int32_t output0_storage[400] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 107604, 1);
  fill_i8(input1_storage, 549, 2);
  fill_i32(input2_storage, 400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 42, 42, 61, 107604, 2562, 61, 1, input1_storage, input1_storage, 0, 3, 3, 61, 1, 183, 61, 1, 1, input2_storage, input2_storage, 0, 1, 20, 20, 1, 400, 20, 1, 1, output0_storage, output0_storage, 0, 1, 20, 20, 1, 400, 20, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00054_201f5f4d: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 400));
  return 0;
}
