...
static uint16_t input0_storage[4616192] __attribute__((aligned(64)));
static uint16_t input1_storage[5558272] __attribute__((aligned(64)));
static float input2_storage[11841536] __attribute__((aligned(64)));
static float output0_storage[11841536] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4616192, 1);
  fill_f16(input1_storage, 5558272, 2);
  fill_f32(input2_storage, 11841536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3136, 1472, 1472, 1, input1_storage, input1_storage, 0, 1472, 3776, 3776, 1, input2_storage, input2_storage, 0, 3136, 3776, 3776, 1, output0_storage, output0_storage, 0, 3136, 3776, 3776, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00145_f44050b0: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 11841536));
  return 0;
}
