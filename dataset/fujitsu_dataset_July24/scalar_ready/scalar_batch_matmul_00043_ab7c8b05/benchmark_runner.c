...
static uint16_t input0_storage[57344] __attribute__((aligned(64)));
static uint16_t input1_storage[57344] __attribute__((aligned(64)));
static float input2_storage[401408] __attribute__((aligned(64)));
static float output0_storage[401408] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 57344, 1);
  fill_f16(input1_storage, 57344, 2);
  fill_f32(input2_storage, 401408, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 448, 64, 28672, 64, 1, input1_storage, input1_storage, 0, 2, 64, 448, 28672, 448, 1, input2_storage, input2_storage, 0, 2, 448, 448, 200704, 448, 1, output0_storage, output0_storage, 0, 2, 448, 448, 200704, 448, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00043_ab7c8b05: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 401408));
  return 0;
}
