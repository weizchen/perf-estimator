...
static uint16_t input0_storage[94471] __attribute__((aligned(64)));
static uint16_t input1_storage[94471] __attribute__((aligned(64)));
static uint16_t output0_storage[94471] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 94471, 1);
  fill_f16(input1_storage, 94471, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 13, 13, 13, 43, 7267, 559, 43, 1, input1_storage, input1_storage, 0, 13, 13, 43, 13, 7267, 559, 13, 1, output0_storage, output0_storage, 0, 13, 13, 43, 13, 7267, 559, 13, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00116_7a0a899c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 94471));
  return 0;
}
