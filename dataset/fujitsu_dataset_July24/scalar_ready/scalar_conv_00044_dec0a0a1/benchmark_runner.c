...
static uint16_t input0_storage[2116] __attribute__((aligned(64)));
static uint16_t input1_storage[18] __attribute__((aligned(64)));
static float input2_storage[968] __attribute__((aligned(64)));
static float output0_storage[968] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2116, 1);
  fill_f16(input1_storage, 18, 2);
  fill_f32(input2_storage, 968, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 46, 46, 1, 2116, 46, 1, 1, input1_storage, input1_storage, 0, 3, 3, 1, 2, 6, 2, 2, 1, input2_storage, input2_storage, 0, 1, 22, 22, 2, 968, 44, 2, 1, output0_storage, output0_storage, 0, 1, 22, 22, 2, 968, 44, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00044_dec0a0a1: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 968));
  return 0;
}
