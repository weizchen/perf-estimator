...
static float input0_storage[2580480] __attribute__((aligned(64)));
static float input1_storage[5160960] __attribute__((aligned(64)));
static float input2_storage[7372800] __attribute__((aligned(64)));
static float output0_storage[7372800] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 2580480, 1);
  fill_f32(input1_storage, 5160960, 2);
  fill_f32(input2_storage, 7372800, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 1344, 1344, 1, input1_storage, input1_storage, 0, 1344, 3840, 3840, 1, input2_storage, input2_storage, 0, 1920, 3840, 3840, 1, output0_storage, output0_storage, 0, 1920, 3840, 3840, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00375_f20f1a54: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7372800));
  return 0;
}
