...
static int32_t input2_storage[786432] __attribute__((aligned(64)));
static int32_t output0_storage[786432] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 786432, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 768, 3072, 3072, 1, input1_storage, input1_storage, 0, 3072, 1024, 1024, 1, input2_storage, input2_storage, 0, 768, 1024, 1024, 1, output0_storage, output0_storage, 0, 768, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00064_5b0c854c: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 786432));
  return 0;
}
