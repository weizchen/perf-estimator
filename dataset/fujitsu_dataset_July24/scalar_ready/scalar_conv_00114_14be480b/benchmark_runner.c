...
static float input0_storage[3179] __attribute__((aligned(64)));
static float input1_storage[11] __attribute__((aligned(64)));
static float input2_storage[289] __attribute__((aligned(64)));
static float output0_storage[289] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3179, 1);
  fill_f32(input1_storage, 11, 2);
  fill_f32(input2_storage, 289, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 17, 17, 11, 3179, 187, 11, 1, input1_storage, input1_storage, 0, 1, 1, 11, 1, 11, 11, 1, 1, input2_storage, input2_storage, 0, 1, 17, 17, 1, 289, 17, 1, 1, output0_storage, output0_storage, 0, 1, 17, 17, 1, 289, 17, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00114_14be480b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 289));
  return 0;
}
