...
static uint16_t input2_storage[131072] __attribute__((aligned(64)));
static uint16_t output0_storage[131072] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 131072, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 128, 64, 8192, 64, 1, input1_storage, input1_storage, 0, 8, 64, 128, 8192, 128, 1, input2_storage, input2_storage, 0, 8, 128, 128, 16384, 128, 1, output0_storage, output0_storage, 0, 8, 128, 128, 16384, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00074_f253f08c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 131072));
  return 0;
}
