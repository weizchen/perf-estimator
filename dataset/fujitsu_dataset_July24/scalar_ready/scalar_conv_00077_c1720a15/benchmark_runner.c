...
static int8_t input1_storage[343] __attribute__((aligned(64)));
static int32_t input2_storage[1] __attribute__((aligned(64)));
static int32_t output0_storage[1] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 343, 2);
  fill_i32(input2_storage, 1, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 7, 448, 56, 7, 1, input1_storage, input1_storage, 0, 7, 7, 7, 1, 49, 7, 1, 1, input2_storage, input2_storage, 0, 1, 1, 1, 1, 1, 1, 1, 1, output0_storage, output0_storage, 0, 1, 1, 1, 1, 1, 1, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00077_c1720a15: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1));
  return 0;
}
