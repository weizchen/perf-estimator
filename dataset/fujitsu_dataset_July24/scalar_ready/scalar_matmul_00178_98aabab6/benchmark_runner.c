...
static uint16_t input1_storage[5840896] __attribute__((aligned(64)));
static float input2_storage[253952] __attribute__((aligned(64)));
static float output0_storage[253952] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 5840896, 2);
  fill_f32(input2_storage, 253952, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 1984, 1984, 1, input2_storage, input2_storage, 0, 128, 1984, 1984, 1, output0_storage, output0_storage, 0, 128, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00178_98aabab6: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 253952));
  return 0;
}
