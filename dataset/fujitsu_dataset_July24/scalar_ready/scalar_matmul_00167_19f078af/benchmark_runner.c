...
static uint16_t input0_storage[14991360] __attribute__((aligned(64)));
static uint16_t input1_storage[8245248] __attribute__((aligned(64)));
static float input2_storage[8110080] __attribute__((aligned(64)));
static float output0_storage[8110080] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 14991360, 1);
  fill_f16(input1_storage, 8245248, 2);
  fill_f32(input2_storage, 8110080, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3840, 3904, 3904, 1, input1_storage, input1_storage, 0, 3904, 2112, 2112, 1, input2_storage, input2_storage, 0, 3840, 2112, 2112, 1, output0_storage, output0_storage, 0, 3840, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00167_19f078af: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8110080));
  return 0;
}
