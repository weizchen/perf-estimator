...
static uint16_t input0_storage[557056] __attribute__((aligned(64)));
static uint16_t input1_storage[1253376] __attribute__((aligned(64)));
static uint16_t input2_storage[294912] __attribute__((aligned(64)));
static uint16_t output0_storage[294912] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 557056, 1);
  fill_f16(input1_storage, 1253376, 2);
  fill_f16(input2_storage, 294912, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 256, 1088, 278528, 1088, 1, input1_storage, input1_storage, 0, 2, 1088, 576, 626688, 576, 1, input2_storage, input2_storage, 0, 2, 256, 576, 147456, 576, 1, output0_storage, output0_storage, 0, 2, 256, 576, 147456, 576, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00063_3ab8e53d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 294912));
  return 0;
}
