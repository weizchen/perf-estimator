...
static float input0_storage[676] __attribute__((aligned(64)));
static float input1_storage[49] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 676, 1);
  fill_f32(input1_storage, 49, 2);
  fill_f32(input2_storage, 100, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 26, 26, 1, 676, 26, 1, 1, input1_storage, input1_storage, 0, 7, 7, 1, 1, 7, 1, 1, 1, input2_storage, input2_storage, 0, 1, 10, 10, 1, 100, 10, 1, 1, output0_storage, output0_storage, 0, 1, 10, 10, 1, 100, 10, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00147_0b1bbc60: benchmark completed");
