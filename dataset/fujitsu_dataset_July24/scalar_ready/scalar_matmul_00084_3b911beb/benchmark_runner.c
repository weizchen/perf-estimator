...
static int32_t input2_storage[835584] __attribute__((aligned(64)));
static int32_t output0_storage[835584] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 835584, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 128, 128, 1, input1_storage, input1_storage, 0, 128, 384, 384, 1, input2_storage, input2_storage, 0, 2176, 384, 384, 1, output0_storage, output0_storage, 0, 2176, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00084_3b911beb: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 835584));
  return 0;
}
