...
static float input0_storage[12544] __attribute__((aligned(64)));
static float input1_storage[8624] __attribute__((aligned(64)));
static float input2_storage[1331] __attribute__((aligned(64)));
static float output0_storage[1331] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 12544, 1);
  fill_f32(input1_storage, 8624, 2);
  fill_f32(input2_storage, 1331, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 28, 28, 16, 12544, 448, 16, 1, input1_storage, input1_storage, 0, 7, 7, 16, 11, 1232, 176, 11, 1, input2_storage, input2_storage, 0, 1, 11, 11, 11, 1331, 121, 11, 1, output0_storage, output0_storage, 0, 1, 11, 11, 11, 1331, 121, 11, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00032_78933fba: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1331));
  return 0;
}
