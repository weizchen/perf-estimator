...
static uint16_t input0_storage[155648] __attribute__((aligned(64)));
static uint16_t input1_storage[2957312] __attribute__((aligned(64)));
static uint16_t input2_storage[311296] __attribute__((aligned(64)));
static uint16_t output0_storage[311296] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 155648, 1);
  fill_f16(input1_storage, 2957312, 2);
  fill_f16(input2_storage, 311296, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 2432, 2432, 1, input2_storage, input2_storage, 0, 128, 2432, 2432, 1, output0_storage, output0_storage, 0, 128, 2432, 2432, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00225_11ec1bf4: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 311296));
  return 0;
}
