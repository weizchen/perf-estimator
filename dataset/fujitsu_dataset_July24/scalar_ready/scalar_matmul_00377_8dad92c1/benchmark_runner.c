...
static float input0_storage[1343488] __attribute__((aligned(64)));
static float input1_storage[335872] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1343488, 1);
  fill_f32(input1_storage, 335872, 2);
  fill_f32(input2_storage, 65536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 2624, 2624, 1, input1_storage, input1_storage, 0, 2624, 128, 128, 1, input2_storage, input2_storage, 0, 512, 128, 128, 1, output0_storage, output0_storage, 0, 512, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00377_8dad92c1: benchmark completed");
