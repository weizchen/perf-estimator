...
static uint16_t input0_storage[2752512] __attribute__((aligned(64)));
static uint16_t input1_storage[786432] __attribute__((aligned(64)));
static float input2_storage[229376] __attribute__((aligned(64)));
static float output0_storage[229376] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2752512, 1);
  fill_f16(input1_storage, 786432, 2);
  fill_f32(input2_storage, 229376, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 3072, 3072, 1, input1_storage, input1_storage, 0, 3072, 256, 256, 1, input2_storage, input2_storage, 0, 896, 256, 256, 1, output0_storage, output0_storage, 0, 896, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00137_2b811fbb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 229376));
  return 0;
}
