...
static float input0_storage[5832] __attribute__((aligned(64)));
static float input1_storage[4410] __attribute__((aligned(64)));
static float input2_storage[720] __attribute__((aligned(64)));
static float output0_storage[720] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 5832, 1);
  fill_f32(input1_storage, 4410, 2);
  fill_f32(input2_storage, 720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 18, 18, 18, 5832, 324, 18, 1, input1_storage, input1_storage, 0, 7, 7, 18, 5, 630, 90, 5, 1, input2_storage, input2_storage, 0, 1, 12, 12, 5, 720, 60, 5, 1, output0_storage, output0_storage, 0, 1, 12, 12, 5, 720, 60, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00035_4ed2d7a6: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 720));
  return 0;
}
