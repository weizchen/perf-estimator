...
static int8_t input0_storage[4600] __attribute__((aligned(64)));
static int8_t input1_storage[17250] __attribute__((aligned(64)));
static int32_t input2_storage[135] __attribute__((aligned(64)));
static int32_t output0_storage[135] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 4600, 1);
  fill_i8(input1_storage, 17250, 2);
  fill_i32(input2_storage, 135, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 10, 10, 46, 4600, 460, 46, 1, input1_storage, input1_storage, 0, 5, 5, 46, 15, 3450, 690, 15, 1, input2_storage, input2_storage, 0, 1, 3, 3, 15, 135, 45, 15, 1, output0_storage, output0_storage, 0, 1, 3, 3, 15, 135, 45, 15, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00028_a733c5db: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 135));
  return 0;
}
