...
static uint16_t input0_storage[331776] __attribute__((aligned(64)));
static uint16_t input1_storage[356352] __attribute__((aligned(64)));
static float input2_storage[3207168] __attribute__((aligned(64)));
static float output0_storage[3207168] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 331776, 1);
  fill_f16(input1_storage, 356352, 2);
  fill_f32(input2_storage, 3207168, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1728, 192, 192, 1, input1_storage, input1_storage, 0, 192, 1856, 1856, 1, input2_storage, input2_storage, 0, 1728, 1856, 1856, 1, output0_storage, output0_storage, 0, 1728, 1856, 1856, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00140_7b788839: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3207168));
  return 0;
}
