...
static float input0_storage[901120] __attribute__((aligned(64)));
static float input1_storage[2928640] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 901120, 1);
  fill_f32(input1_storage, 2928640, 2);
  fill_f32(input2_storage, 212992, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 3520, 3520, 1, input1_storage, input1_storage, 0, 3520, 832, 832, 1, input2_storage, input2_storage, 0, 256, 832, 832, 1, output0_storage, output0_storage, 0, 256, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00421_fe5e6c9b: benchmark completed");
