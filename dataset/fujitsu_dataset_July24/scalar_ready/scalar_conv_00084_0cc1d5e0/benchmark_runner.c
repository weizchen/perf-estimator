...
static float input0_storage[3600] __attribute__((aligned(64)));
static float input1_storage[975] __attribute__((aligned(64)));
static float input2_storage[5616] __attribute__((aligned(64)));
static float output0_storage[5616] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3600, 1);
  fill_f32(input1_storage, 975, 2);
  fill_f32(input2_storage, 5616, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 12, 12, 25, 3600, 300, 25, 1, input1_storage, input1_storage, 0, 1, 1, 25, 39, 975, 975, 39, 1, input2_storage, input2_storage, 0, 1, 12, 12, 39, 5616, 468, 39, 1, output0_storage, output0_storage, 0, 1, 12, 12, 39, 5616, 468, 39, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00084_0cc1d5e0: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5616));
  return 0;
}
