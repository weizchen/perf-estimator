...
static uint16_t input0_storage[1805] __attribute__((aligned(64)));
static uint16_t input1_storage[35] __attribute__((aligned(64)));
static float input2_storage[700] __attribute__((aligned(64)));
static float output0_storage[700] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1805, 1);
  fill_f16(input1_storage, 35, 2);
  fill_f32(input2_storage, 700, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 19, 19, 5, 1805, 95, 5, 1, input1_storage, input1_storage, 0, 1, 1, 5, 7, 35, 35, 7, 1, input2_storage, input2_storage, 0, 1, 10, 10, 7, 700, 70, 7, 1, output0_storage, output0_storage, 0, 1, 10, 10, 7, 700, 70, 7, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00087_fb543055: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 700));
  return 0;
}
