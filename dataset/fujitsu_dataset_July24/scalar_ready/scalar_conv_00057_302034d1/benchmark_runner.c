...
static int8_t input0_storage[17629] __attribute__((aligned(64)));
static int8_t input1_storage[5978] __attribute__((aligned(64)));
static int32_t input2_storage[242] __attribute__((aligned(64)));
static int32_t output0_storage[242] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 17629, 1);
  fill_i8(input1_storage, 5978, 2);
  fill_i32(input2_storage, 242, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 17, 17, 61, 17629, 1037, 61, 1, input1_storage, input1_storage, 0, 7, 7, 61, 2, 854, 122, 2, 1, input2_storage, input2_storage, 0, 1, 11, 11, 2, 242, 22, 2, 1, output0_storage, output0_storage, 0, 1, 11, 11, 2, 242, 22, 2, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00057_302034d1: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 242));
  return 0;
}
