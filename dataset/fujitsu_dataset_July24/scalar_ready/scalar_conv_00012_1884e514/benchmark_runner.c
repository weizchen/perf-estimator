...
static float input0_storage[3630] __attribute__((aligned(64)));
static float input1_storage[270] __attribute__((aligned(64)));
static float input2_storage[25] __attribute__((aligned(64)));
static float output0_storage[25] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3630, 1);
  fill_f32(input1_storage, 270, 2);
  fill_f32(input2_storage, 25, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 11, 11, 30, 3630, 330, 30, 1, input1_storage, input1_storage, 0, 3, 3, 30, 1, 90, 30, 1, 1, input2_storage, input2_storage, 0, 1, 5, 5, 1, 25, 5, 1, 1, output0_storage, output0_storage, 0, 1, 5, 5, 1, 25, 5, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00012_1884e514: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 25));
  return 0;
}
