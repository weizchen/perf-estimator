...
static int8_t input0_storage[127575] __attribute__((aligned(64)));
static int8_t input1_storage[43218] __attribute__((aligned(64)));
static int32_t input2_storage[5600] __attribute__((aligned(64)));
static int32_t output0_storage[5600] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 127575, 1);
  fill_i8(input1_storage, 43218, 2);
  fill_i32(input2_storage, 5600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 45, 45, 63, 127575, 2835, 63, 1, input1_storage, input1_storage, 0, 7, 7, 63, 14, 6174, 882, 14, 1, input2_storage, input2_storage, 0, 1, 20, 20, 14, 5600, 280, 14, 1, output0_storage, output0_storage, 0, 1, 20, 20, 14, 5600, 280, 14, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00063_fa4308d6: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 5600));
  return 0;
}
