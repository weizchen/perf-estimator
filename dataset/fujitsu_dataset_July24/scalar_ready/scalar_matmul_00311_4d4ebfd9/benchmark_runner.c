...
static uint16_t input0_storage[1437696] __attribute__((aligned(64)));
static uint16_t input1_storage[221184] __attribute__((aligned(64)));
static uint16_t input2_storage[106496] __attribute__((aligned(64)));
static uint16_t output0_storage[106496] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1437696, 1);
  fill_f16(input1_storage, 221184, 2);
  fill_f16(input2_storage, 106496, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 128, 128, 1, input2_storage, input2_storage, 0, 832, 128, 128, 1, output0_storage, output0_storage, 0, 832, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00311_4d4ebfd9: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 106496));
  return 0;
}
