...
static uint16_t input0_storage[51] __attribute__((aligned(64)));
static uint16_t input1_storage[51] __attribute__((aligned(64)));
static uint16_t output0_storage[51] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 51, 1);
  fill_f16(input1_storage, 51, 2);
  ...
  ofa_dma_memory_fence();
  puts("scalar_generic_00498_4bfe864f: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 51));
  return 0;
}
