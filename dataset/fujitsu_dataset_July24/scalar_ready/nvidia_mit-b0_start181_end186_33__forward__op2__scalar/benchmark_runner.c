...
static float input0_storage[25600] __attribute__((aligned(64)));
static float input1_storage[25600] __attribute__((aligned(64)));
static float output0_storage[25600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 25600, 1);
  fill_f32(input1_storage, 25600, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 160, 160, 160, 1, input1_storage, input1_storage, 0, 1, 160, 160, 25600, 160, 1, output0_storage, output0_storage, 0, 1, 160, 160, 25600, 160, 1);
  ofa_dma_memory_fence();
  puts("nvidia_mit-b0_start181_end186_33__forward__op2__scalar: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 25600));
  return 0;
}
