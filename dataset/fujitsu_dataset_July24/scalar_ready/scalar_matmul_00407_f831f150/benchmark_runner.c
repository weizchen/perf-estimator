...
static float input0_storage[3522560] __attribute__((aligned(64)));
static float input1_storage[819200] __attribute__((aligned(64)));
static float input2_storage[1761280] __attribute__((aligned(64)));
static float output0_storage[1761280] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3522560, 1);
  fill_f32(input1_storage, 819200, 2);
  fill_f32(input2_storage, 1761280, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 640, 640, 1, input2_storage, input2_storage, 0, 2752, 640, 640, 1, output0_storage, output0_storage, 0, 2752, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00407_f831f150: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1761280));
  return 0;
}
