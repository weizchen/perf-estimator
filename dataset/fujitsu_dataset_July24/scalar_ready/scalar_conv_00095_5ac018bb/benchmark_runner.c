...
static uint16_t input0_storage[132651] __attribute__((aligned(64)));
static uint16_t input1_storage[12852] __attribute__((aligned(64)));
static float input2_storage[17500] __attribute__((aligned(64)));
static float output0_storage[17500] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 132651, 1);
  fill_f16(input1_storage, 12852, 2);
  fill_f32(input2_storage, 17500, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 51, 51, 51, 132651, 2601, 51, 1, input1_storage, input1_storage, 0, 3, 3, 51, 28, 4284, 1428, 28, 1, input2_storage, input2_storage, 0, 1, 25, 25, 28, 17500, 700, 28, 1, output0_storage, output0_storage, 0, 1, 25, 25, 28, 17500, 700, 28, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00095_5ac018bb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 17500));
  return 0;
}
