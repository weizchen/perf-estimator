...
static uint16_t input0_storage[53482] __attribute__((aligned(64)));
static uint16_t input1_storage[53482] __attribute__((aligned(64)));
static uint16_t output0_storage[53482] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 53482, 1);
  fill_f16(input1_storage, 53482, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 11, 11, 13, 34, 4862, 442, 34, 1, input1_storage, input1_storage, 0, 11, 11, 34, 13, 4862, 442, 13, 1, output0_storage, output0_storage, 0, 11, 11, 34, 13, 4862, 442, 13, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00086_36e41a6b: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 53482));
  return 0;
}
