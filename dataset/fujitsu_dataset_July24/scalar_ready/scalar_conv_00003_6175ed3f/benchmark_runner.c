...
static uint16_t input0_storage[81] __attribute__((aligned(64)));
static uint16_t input1_storage[306] __attribute__((aligned(64)));
static float input2_storage[544] __attribute__((aligned(64)));
static float output0_storage[544] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 81, 1);
  fill_f16(input1_storage, 306, 2);
  fill_f32(input2_storage, 544, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 9, 9, 1, 81, 9, 1, 1, input1_storage, input1_storage, 0, 3, 3, 1, 34, 102, 34, 34, 1, input2_storage, input2_storage, 0, 1, 4, 4, 34, 544, 136, 34, 1, output0_storage, output0_storage, 0, 1, 4, 4, 34, 544, 136, 34, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00003_6175ed3f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 544));
  return 0;
}
