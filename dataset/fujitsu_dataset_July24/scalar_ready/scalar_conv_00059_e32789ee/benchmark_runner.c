...
static uint16_t input0_storage[2888] __attribute__((aligned(64)));
static uint16_t input1_storage[14] __attribute__((aligned(64)));
static float input2_storage[2527] __attribute__((aligned(64)));
static float output0_storage[2527] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2888, 1);
  fill_f16(input1_storage, 14, 2);
  fill_f32(input2_storage, 2527, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 38, 38, 2, 2888, 76, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 7, 14, 14, 7, 1, input2_storage, input2_storage, 0, 1, 19, 19, 7, 2527, 133, 7, 1, output0_storage, output0_storage, 0, 1, 19, 19, 7, 2527, 133, 7, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00059_e32789ee: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2527));
  return 0;
}
