...
static int16_t input1_storage[42] __attribute__((aligned(64)));
static int16_t input2_storage[42] __attribute__((aligned(64)));
static int16_t input3_storage[42] __attribute__((aligned(64)));
static int16_t output0_storage[42] __attribute__((aligned(64)));
...
int main(void) {
  fill_i1(input0_storage, 42, 1);
  fill_i16(input1_storage, 42, 2);
  fill_i16(input2_storage, 42, 3);
  fill_i16(input3_storage, 42, 4);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 7, 6, 6, 1, input1_storage, input1_storage, 0, 7, 6, 6, 1, input2_storage, input2_storage, 0, 7, 6, 6, 1, input3_storage, input3_storage, 0, 7, 6, 6, 1, output0_storage, output0_storage, 0, 7, 6, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_select_00002_d9e04041: benchmark completed");
  printf("output0_i16_checksum=%lld\n", checksum_i16(output0_storage, 42));
  return 0;
}
