...
static uint16_t input0_storage[65536] __attribute__((aligned(64)));
static uint16_t input1_storage[589824] __attribute__((aligned(64)));
static float input2_storage[147456] __attribute__((aligned(64)));
static float output0_storage[147456] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 65536, 1);
  fill_f16(input1_storage, 589824, 2);
  fill_f32(input2_storage, 147456, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 128, 512, 65536, 512, 1, input1_storage, input1_storage, 0, 1, 512, 1152, 589824, 1152, 1, input2_storage, input2_storage, 0, 1, 128, 1152, 147456, 1152, 1, output0_storage, output0_storage, 0, 1, 128, 1152, 147456, 1152, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00028_ed411e60: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 147456));
  return 0;
}
