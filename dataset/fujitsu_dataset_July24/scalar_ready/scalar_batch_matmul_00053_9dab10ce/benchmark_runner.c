...
static uint16_t input1_storage[294912] __attribute__((aligned(64)));
static uint16_t input2_storage[73728] __attribute__((aligned(64)));
static uint16_t output0_storage[73728] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 294912, 2);
  fill_f16(input2_storage, 73728, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 128, 512, 65536, 512, 1, input1_storage, input1_storage, 0, 1, 512, 576, 294912, 576, 1, input2_storage, input2_storage, 0, 1, 128, 576, 73728, 576, 1, output0_storage, output0_storage, 0, 1, 128, 576, 73728, 576, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00053_9dab10ce: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 73728));
  return 0;
}
