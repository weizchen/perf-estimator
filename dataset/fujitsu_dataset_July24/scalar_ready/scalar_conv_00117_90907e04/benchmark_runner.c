...
static uint16_t input0_storage[5000] __attribute__((aligned(64)));
static uint16_t input1_storage[392] __attribute__((aligned(64)));
static float input2_storage[7744] __attribute__((aligned(64)));
static float output0_storage[7744] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5000, 1);
  fill_f16(input1_storage, 392, 2);
  fill_f32(input2_storage, 7744, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 50, 50, 2, 5000, 100, 2, 1, input1_storage, input1_storage, 0, 7, 7, 2, 4, 56, 8, 4, 1, input2_storage, input2_storage, 0, 1, 44, 44, 4, 7744, 176, 4, 1, output0_storage, output0_storage, 0, 1, 44, 44, 4, 7744, 176, 4, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00117_90907e04: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7744));
  return 0;
}
