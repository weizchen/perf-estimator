...
static float input0_storage[5408] __attribute__((aligned(64)));
static float input1_storage[18] __attribute__((aligned(64)));
static float input2_storage[6084] __attribute__((aligned(64)));
static float output0_storage[6084] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 5408, 1);
  fill_f32(input1_storage, 18, 2);
  fill_f32(input2_storage, 6084, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 52, 52, 2, 5408, 104, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 9, 18, 18, 9, 1, input2_storage, input2_storage, 0, 1, 26, 26, 9, 6084, 234, 9, 1, output0_storage, output0_storage, 0, 1, 26, 26, 9, 6084, 234, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00070_b1ff21e6: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 6084));
  return 0;
}
