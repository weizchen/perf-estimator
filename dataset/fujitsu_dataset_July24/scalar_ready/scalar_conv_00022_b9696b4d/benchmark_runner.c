...
static int8_t input0_storage[17051] __attribute__((aligned(64)));
static int8_t input1_storage[2891] __attribute__((aligned(64)));
static int32_t input2_storage[121] __attribute__((aligned(64)));
static int32_t output0_storage[121] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 17051, 1);
  fill_i8(input1_storage, 2891, 2);
  fill_i32(input2_storage, 121, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 17, 17, 59, 17051, 1003, 59, 1, input1_storage, input1_storage, 0, 7, 7, 59, 1, 413, 59, 1, 1, input2_storage, input2_storage, 0, 1, 11, 11, 1, 121, 11, 1, 1, output0_storage, output0_storage, 0, 1, 11, 11, 1, 121, 11, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00022_b9696b4d: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 121));
  return 0;
}
