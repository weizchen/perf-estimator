...
static float input0_storage[1032192] __attribute__((aligned(64)));
static float input1_storage[5419008] __attribute__((aligned(64)));
static float input2_storage[344064] __attribute__((aligned(64)));
static float output0_storage[344064] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1032192, 1);
  fill_f32(input1_storage, 5419008, 2);
  fill_f32(input2_storage, 344064, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 4032, 4032, 1, input1_storage, input1_storage, 0, 4032, 1344, 1344, 1, input2_storage, input2_storage, 0, 256, 1344, 1344, 1, output0_storage, output0_storage, 0, 256, 1344, 1344, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00382_4a19df65: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 344064));
  return 0;
}
