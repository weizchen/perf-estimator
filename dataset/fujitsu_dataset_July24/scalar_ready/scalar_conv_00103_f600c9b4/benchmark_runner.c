...
static int8_t input0_storage[3328] __attribute__((aligned(64)));
static int8_t input1_storage[325] __attribute__((aligned(64)));
static int32_t input2_storage[144] __attribute__((aligned(64)));
static int32_t output0_storage[144] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 3328, 1);
  fill_i8(input1_storage, 325, 2);
  fill_i32(input2_storage, 144, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 16, 16, 13, 3328, 208, 13, 1, input1_storage, input1_storage, 0, 5, 5, 13, 1, 65, 13, 1, 1, input2_storage, input2_storage, 0, 1, 12, 12, 1, 144, 12, 1, 1, output0_storage, output0_storage, 0, 1, 12, 12, 1, 144, 12, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00103_f600c9b4: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 144));
  return 0;
}
