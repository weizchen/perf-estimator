...
static uint16_t input0_storage[188416] __attribute__((aligned(64)));
static uint16_t input1_storage[4993024] __attribute__((aligned(64)));
static uint16_t input2_storage[434176] __attribute__((aligned(64)));
static uint16_t output0_storage[434176] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 188416, 1);
  fill_f16(input1_storage, 4993024, 2);
  fill_f16(input2_storage, 434176, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1472, 1472, 1, input1_storage, input1_storage, 0, 1472, 3392, 3392, 1, input2_storage, input2_storage, 0, 128, 3392, 3392, 1, output0_storage, output0_storage, 0, 128, 3392, 3392, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00271_430b02f6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 434176));
  return 0;
}
