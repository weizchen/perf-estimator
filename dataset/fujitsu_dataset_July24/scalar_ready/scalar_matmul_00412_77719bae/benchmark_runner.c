...
static float input0_storage[184320] __attribute__((aligned(64)));
static float input1_storage[221184] __attribute__((aligned(64)));
static float input2_storage[1105920] __attribute__((aligned(64)));
static float output0_storage[1105920] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 184320, 1);
  fill_f32(input1_storage, 221184, 2);
  fill_f32(input2_storage, 1105920, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 960, 192, 192, 1, input1_storage, input1_storage, 0, 192, 1152, 1152, 1, input2_storage, input2_storage, 0, 960, 1152, 1152, 1, output0_storage, output0_storage, 0, 960, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00412_77719bae: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1105920));
  return 0;
}
