...
static float input0_storage[19176] __attribute__((aligned(64)));
static float input1_storage[19176] __attribute__((aligned(64)));
static float output0_storage[19176] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 19176, 1);
  fill_f32(input1_storage, 19176, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 6, 47, 17, 4794, 799, 17, 1, input1_storage, input1_storage, 0, 4, 47, 6, 17, 4794, 102, 17, 1, output0_storage, output0_storage, 0, 4, 47, 6, 17, 4794, 102, 17, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00057_a5c0036a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 19176));
  return 0;
}
