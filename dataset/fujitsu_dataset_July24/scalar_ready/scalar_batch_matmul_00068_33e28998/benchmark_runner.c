...
static uint16_t input0_storage[19267584] __attribute__((aligned(64)));
static uint16_t input1_storage[3670016] __attribute__((aligned(64)));
static uint16_t input2_storage[11010048] __attribute__((aligned(64)));
static uint16_t output0_storage[11010048] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 19267584, 1);
  fill_f16(input1_storage, 3670016, 2);
  fill_f16(input2_storage, 11010048, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 32, 1344, 448, 602112, 448, 1, input1_storage, input1_storage, 0, 32, 448, 256, 114688, 256, 1, input2_storage, input2_storage, 0, 32, 1344, 256, 344064, 256, 1, output0_storage, output0_storage, 0, 32, 1344, 256, 344064, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00068_33e28998: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 11010048));
  return 0;
}
