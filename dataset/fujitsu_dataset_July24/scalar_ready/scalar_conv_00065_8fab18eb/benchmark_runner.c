...
static uint16_t input0_storage[28812] __attribute__((aligned(64)));
static uint16_t input1_storage[12] __attribute__((aligned(64)));
static float input2_storage[2401] __attribute__((aligned(64)));
static float output0_storage[2401] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 28812, 1);
  fill_f16(input1_storage, 12, 2);
  fill_f32(input2_storage, 2401, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 49, 49, 12, 28812, 588, 12, 1, input1_storage, input1_storage, 0, 1, 1, 12, 1, 12, 12, 1, 1, input2_storage, input2_storage, 0, 1, 49, 49, 1, 2401, 49, 1, 1, output0_storage, output0_storage, 0, 1, 49, 49, 1, 2401, 49, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00065_8fab18eb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2401));
  return 0;
}
