...
static int8_t input0_storage[7600] __attribute__((aligned(64)));
static int8_t input1_storage[7448] __attribute__((aligned(64)));
static int32_t input2_storage[1568] __attribute__((aligned(64)));
static int32_t output0_storage[1568] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 7600, 1);
  fill_i8(input1_storage, 7448, 2);
  fill_i32(input2_storage, 1568, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 20, 20, 19, 7600, 380, 19, 1, input1_storage, input1_storage, 0, 7, 7, 19, 8, 1064, 152, 8, 1, input2_storage, input2_storage, 0, 1, 14, 14, 8, 1568, 112, 8, 1, output0_storage, output0_storage, 0, 1, 14, 14, 8, 1568, 112, 8, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00122_679d1bc8: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1568));
  return 0;
}
