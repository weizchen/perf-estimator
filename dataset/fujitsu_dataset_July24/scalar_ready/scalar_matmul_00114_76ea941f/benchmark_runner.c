...
static uint16_t input0_storage[1413120] __attribute__((aligned(64)));
static uint16_t input1_storage[753664] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1413120, 1);
  fill_f16(input1_storage, 753664, 2);
  fill_f32(input2_storage, 491520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 960, 1472, 1472, 1, input1_storage, input1_storage, 0, 1472, 512, 512, 1, input2_storage, input2_storage, 0, 960, 512, 512, 1, output0_storage, output0_storage, 0, 960, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00114_76ea941f: benchmark completed");
