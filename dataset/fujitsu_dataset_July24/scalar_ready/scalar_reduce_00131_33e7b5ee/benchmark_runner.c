...
static int16_t input0_storage[6279] __attribute__((aligned(64)));
...
int main(void) {
  fill_i16(input0_storage, 6279, 1);
  fill_i16(input1_storage, 23, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 273, 23, 23, 1, input1_storage, input1_storage, 0, 23, 1, output0_storage, output0_storage, 0, 23, 1);
  ofa_dma_memory_fence();
  puts("scalar_reduce_00131_33e7b5ee: benchmark completed");
