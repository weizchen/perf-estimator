...
static uint16_t input1_storage[5111808] __attribute__((aligned(64)));
static uint16_t input2_storage[1179648] __attribute__((aligned(64)));
static uint16_t output0_storage[1179648] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 5111808, 2);
  fill_f16(input2_storage, 1179648, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 384, 1664, 638976, 1664, 1, input1_storage, input1_storage, 0, 16, 1664, 192, 319488, 192, 1, input2_storage, input2_storage, 0, 16, 384, 192, 73728, 192, 1, output0_storage, output0_storage, 0, 16, 384, 192, 73728, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00051_24e4a792: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1179648));
  return 0;
}
