...
static uint16_t input0_storage[1134] __attribute__((aligned(64)));
static uint16_t input1_storage[686] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1134, 1);
  fill_f16(input1_storage, 686, 2);
  fill_f32(input2_storage, 4, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 9, 9, 14, 1134, 126, 14, 1, input1_storage, input1_storage, 0, 7, 7, 14, 1, 98, 14, 1, 1, input2_storage, input2_storage, 0, 1, 2, 2, 1, 4, 2, 1, 1, output0_storage, output0_storage, 0, 1, 2, 2, 1, 4, 2, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00074_19fc8038: benchmark completed");
