...
static uint16_t input0_storage[529] __attribute__((aligned(64)));
static uint16_t input1_storage[216] __attribute__((aligned(64)));
static float input2_storage[10584] __attribute__((aligned(64)));
static float output0_storage[10584] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 529, 1);
  fill_f16(input1_storage, 216, 2);
  fill_f32(input2_storage, 10584, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 23, 23, 1, 529, 23, 1, 1, input1_storage, input1_storage, 0, 3, 3, 1, 24, 72, 24, 24, 1, input2_storage, input2_storage, 0, 1, 21, 21, 24, 10584, 504, 24, 1, output0_storage, output0_storage, 0, 1, 21, 21, 24, 10584, 504, 24, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00149_4704a02c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 10584));
  return 0;
}
