...
static uint16_t input0_storage[1462272] __attribute__((aligned(64)));
static uint16_t input1_storage[835584] __attribute__((aligned(64)));
static uint16_t input2_storage[114688] __attribute__((aligned(64)));
static uint16_t output0_storage[114688] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1462272, 1);
  fill_f16(input1_storage, 835584, 2);
  fill_f16(input2_storage, 114688, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 3264, 3264, 1, input1_storage, input1_storage, 0, 3264, 256, 256, 1, input2_storage, input2_storage, 0, 448, 256, 256, 1, output0_storage, output0_storage, 0, 448, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00243_b07f6ae5: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 114688));
  return 0;
}
