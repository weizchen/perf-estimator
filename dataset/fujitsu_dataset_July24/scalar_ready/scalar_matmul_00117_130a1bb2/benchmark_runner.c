...
static uint16_t input1_storage[188416] __attribute__((aligned(64)));
static float input2_storage[2166784] __attribute__((aligned(64)));
static float output0_storage[2166784] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 188416, 2);
  fill_f32(input2_storage, 2166784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1472, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1472, 1472, 1, input2_storage, input2_storage, 0, 1472, 1472, 1472, 1, output0_storage, output0_storage, 0, 1472, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00117_130a1bb2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2166784));
  return 0;
}
