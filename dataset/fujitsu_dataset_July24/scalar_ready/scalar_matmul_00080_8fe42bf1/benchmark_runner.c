...
static int8_t input0_storage[7077888] __attribute__((aligned(64)));
static int8_t input1_storage[6553600] __attribute__((aligned(64)));
static int32_t input2_storage[11059200] __attribute__((aligned(64)));
static int32_t output0_storage[11059200] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 7077888, 1);
  fill_i8(input1_storage, 6553600, 2);
  fill_i32(input2_storage, 11059200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3456, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 3200, 3200, 1, input2_storage, input2_storage, 0, 3456, 3200, 3200, 1, output0_storage, output0_storage, 0, 3456, 3200, 3200, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00080_8fe42bf1: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 11059200));
  return 0;
}
