...
static int8_t input0_storage[8304] __attribute__((aligned(64)));
static int16_t input1_storage[8304] __attribute__((aligned(64)));
static int16_t input2_storage[8304] __attribute__((aligned(64)));
static int16_t input3_storage[8304] __attribute__((aligned(64)));
static int16_t output0_storage[8304] __attribute__((aligned(64)));
...
int main(void) {
  fill_i1(input0_storage, 8304, 1);
  fill_i16(input1_storage, 8304, 2);
  fill_i16(input2_storage, 8304, 3);
  fill_i16(input3_storage, 8304, 4);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 173, 48, 48, 1, input1_storage, input1_storage, 0, 173, 48, 48, 1, input2_storage, input2_storage, 0, 173, 48, 48, 1, input3_storage, input3_storage, 0, 173, 48, 48, 1, output0_storage, output0_storage, 0, 173, 48, 48, 1);
  ofa_dma_memory_fence();
  puts("scalar_select_00148_ff2236f6: benchmark completed");
  printf("output0_i16_checksum=%lld\n", checksum_i16(output0_storage, 8304));
  return 0;
}
