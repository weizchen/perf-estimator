...
static float input0_storage[22420] __attribute__((aligned(64)));
static float input1_storage[190] __attribute__((aligned(64)));
static float output0_storage[190] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 22420, 1);
  fill_f32(input1_storage, 190, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 118, 190, 190, 1, input1_storage, input1_storage, 0, 190, 1, output0_storage, output0_storage, 0, 190, 1);
  ofa_dma_memory_fence();
  puts("scalar_reduce_00220_e3f56ae3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 190));
  return 0;
}
