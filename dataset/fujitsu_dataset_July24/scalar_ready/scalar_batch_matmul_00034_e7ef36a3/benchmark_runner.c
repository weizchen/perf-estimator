...
static uint16_t input0_storage[1187840] __attribute__((aligned(64)));
static uint16_t input1_storage[204800] __attribute__((aligned(64)));
static float input2_storage[1187840] __attribute__((aligned(64)));
static float output0_storage[1187840] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1187840, 1);
  fill_f16(input1_storage, 204800, 2);
  fill_f32(input2_storage, 1187840, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 1856, 320, 593920, 320, 1, input1_storage, input1_storage, 0, 2, 320, 320, 102400, 320, 1, input2_storage, input2_storage, 0, 2, 1856, 320, 593920, 320, 1, output0_storage, output0_storage, 0, 2, 1856, 320, 593920, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00034_e7ef36a3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1187840));
  return 0;
}
