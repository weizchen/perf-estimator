...
static uint16_t input0_storage[1597440] __attribute__((aligned(64)));
static uint16_t input1_storage[53248] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1597440, 1);
  fill_f16(input1_storage, 53248, 2);
  fill_f16(input2_storage, 122880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 832, 832, 1, input1_storage, input1_storage, 0, 832, 64, 64, 1, input2_storage, input2_storage, 0, 1920, 64, 64, 1, output0_storage, output0_storage, 0, 1920, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00272_3ca178d8: benchmark completed");
