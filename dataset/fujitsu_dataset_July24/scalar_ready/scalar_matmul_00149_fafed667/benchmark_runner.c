...
static uint16_t input0_storage[335872] __attribute__((aligned(64)));
static uint16_t input1_storage[442368] __attribute__((aligned(64)));
static float input2_storage[9068544] __attribute__((aligned(64)));
static float output0_storage[9068544] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 335872, 1);
  fill_f16(input1_storage, 442368, 2);
  fill_f32(input2_storage, 9068544, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2624, 128, 128, 1, input1_storage, input1_storage, 0, 128, 3456, 3456, 1, input2_storage, input2_storage, 0, 2624, 3456, 3456, 1, output0_storage, output0_storage, 0, 2624, 3456, 3456, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00149_fafed667: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9068544));
  return 0;
}
