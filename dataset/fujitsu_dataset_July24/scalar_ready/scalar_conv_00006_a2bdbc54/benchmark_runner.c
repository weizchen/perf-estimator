...
static float input0_storage[6125] __attribute__((aligned(64)));
static float input1_storage[150] __attribute__((aligned(64)));
static float input2_storage[36750] __attribute__((aligned(64)));
static float output0_storage[36750] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 6125, 1);
  fill_f32(input1_storage, 150, 2);
  fill_f32(input2_storage, 36750, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 35, 35, 5, 6125, 175, 5, 1, input1_storage, input1_storage, 0, 1, 1, 5, 30, 150, 150, 30, 1, input2_storage, input2_storage, 0, 1, 35, 35, 30, 36750, 1050, 30, 1, output0_storage, output0_storage, 0, 1, 35, 35, 30, 36750, 1050, 30, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00006_a2bdbc54: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 36750));
  return 0;
}
