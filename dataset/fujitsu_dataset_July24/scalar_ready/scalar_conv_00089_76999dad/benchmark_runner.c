...
static int8_t input0_storage[3648] __attribute__((aligned(64)));
static int8_t input1_storage[42750] __attribute__((aligned(64)));
static int32_t input2_storage[480] __attribute__((aligned(64)));
static int32_t output0_storage[480] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 3648, 1);
  fill_i8(input1_storage, 42750, 2);
  fill_i32(input2_storage, 480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 57, 3648, 456, 57, 1, input1_storage, input1_storage, 0, 5, 5, 57, 30, 8550, 1710, 30, 1, input2_storage, input2_storage, 0, 1, 4, 4, 30, 480, 120, 30, 1, output0_storage, output0_storage, 0, 1, 4, 4, 30, 480, 120, 30, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00089_76999dad: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 480));
  return 0;
}
