...
static float input0_storage[800] __attribute__((aligned(64)));
static float input1_storage[2] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 800, 1);
  fill_f32(input1_storage, 2, 2);
  fill_f32(input2_storage, 400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 20, 20, 2, 800, 40, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 1, 2, 2, 1, 1, input2_storage, input2_storage, 0, 1, 20, 20, 1, 400, 20, 1, 1, output0_storage, output0_storage, 0, 1, 20, 20, 1, 400, 20, 1, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00036_4e615eea: benchmark completed");
