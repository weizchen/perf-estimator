...
static uint16_t input1_storage[1187840] __attribute__((aligned(64)));
static uint16_t input2_storage[40960] __attribute__((aligned(64)));
static uint16_t output0_storage[40960] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1187840, 2);
  fill_f16(input2_storage, 40960, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2, 64, 1856, 118784, 1856, 1, input1_storage, input1_storage, 0, 2, 1856, 320, 593920, 320, 1, input2_storage, input2_storage, 0, 2, 64, 320, 20480, 320, 1, output0_storage, output0_storage, 0, 2, 64, 320, 20480, 320, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00056_5cf79221: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 40960));
  return 0;
}
