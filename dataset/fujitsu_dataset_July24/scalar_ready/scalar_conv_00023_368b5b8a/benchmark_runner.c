...
static int8_t input0_storage[8575] __attribute__((aligned(64)));
static int8_t input1_storage[1260] __attribute__((aligned(64)));
static int32_t input2_storage[21780] __attribute__((aligned(64)));
static int32_t output0_storage[21780] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 8575, 1);
  fill_i8(input1_storage, 1260, 2);
  fill_i32(input2_storage, 21780, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 35, 35, 7, 8575, 245, 7, 1, input1_storage, input1_storage, 0, 3, 3, 7, 20, 420, 140, 20, 1, input2_storage, input2_storage, 0, 1, 33, 33, 20, 21780, 660, 20, 1, output0_storage, output0_storage, 0, 1, 33, 33, 20, 21780, 660, 20, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00023_368b5b8a: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 21780));
  return 0;
}
