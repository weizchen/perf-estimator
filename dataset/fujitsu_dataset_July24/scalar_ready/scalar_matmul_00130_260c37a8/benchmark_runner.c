...
static float input2_storage[458752] __attribute__((aligned(64)));
static float output0_storage[458752] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1605632, 1);
  fill_f16(input1_storage, 229376, 2);
  fill_f32(input2_storage, 458752, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 896, 896, 1, input1_storage, input1_storage, 0, 896, 256, 256, 1, input2_storage, input2_storage, 0, 1792, 256, 256, 1, output0_storage, output0_storage, 0, 1792, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00130_260c37a8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 458752));
  return 0;
}
