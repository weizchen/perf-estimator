...
static uint16_t input0_storage[9437184] __attribute__((aligned(64)));
static uint16_t input1_storage[3145728] __attribute__((aligned(64)));
static float input2_storage[1769472] __attribute__((aligned(64)));
static float output0_storage[1769472] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9437184, 1);
  fill_f16(input1_storage, 3145728, 2);
  fill_f32(input2_storage, 1769472, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 576, 1024, 589824, 1024, 1, input1_storage, input1_storage, 0, 16, 1024, 192, 196608, 192, 1, input2_storage, input2_storage, 0, 16, 576, 192, 110592, 192, 1, output0_storage, output0_storage, 0, 16, 576, 192, 110592, 192, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00045_6c723f15: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1769472));
  return 0;
}
