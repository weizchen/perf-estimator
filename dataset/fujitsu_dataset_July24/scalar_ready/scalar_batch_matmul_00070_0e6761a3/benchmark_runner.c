...
static uint16_t input0_storage[22020096] __attribute__((aligned(64)));
static uint16_t input1_storage[6291456] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 22020096, 1);
  fill_f16(input1_storage, 6291456, 2);
  fill_f16(input2_storage, 7340032, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 1792, 1536, 2752512, 1536, 1, input1_storage, input1_storage, 0, 8, 1536, 512, 786432, 512, 1, input2_storage, input2_storage, 0, 8, 1792, 512, 917504, 512, 1, output0_storage, output0_storage, 0, 8, 1792, 512, 917504, 512, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00070_0e6761a3: benchmark completed");
