...
static int8_t input0_storage[4050] __attribute__((aligned(64)));
static int8_t input1_storage[500] __attribute__((aligned(64)));
static int32_t input2_storage[4410] __attribute__((aligned(64)));
static int32_t output0_storage[4410] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 4050, 1);
  fill_i8(input1_storage, 500, 2);
  fill_i32(input2_storage, 4410, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 45, 45, 2, 4050, 90, 2, 1, input1_storage, input1_storage, 0, 5, 5, 2, 10, 100, 20, 10, 1, input2_storage, input2_storage, 0, 1, 21, 21, 10, 4410, 210, 10, 1, output0_storage, output0_storage, 0, 1, 21, 21, 10, 4410, 210, 10, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00030_00ed39cb: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 4410));
  return 0;
}
