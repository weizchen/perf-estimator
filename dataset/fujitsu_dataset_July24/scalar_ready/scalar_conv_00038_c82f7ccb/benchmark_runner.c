...
static int8_t input0_storage[7623] __attribute__((aligned(64)));
static int8_t input1_storage[819] __attribute__((aligned(64)));
static int32_t input2_storage[468] __attribute__((aligned(64)));
static int32_t output0_storage[468] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 7623, 1);
  fill_i8(input1_storage, 819, 2);
  fill_i32(input2_storage, 468, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 11, 11, 63, 7623, 693, 63, 1, input1_storage, input1_storage, 0, 1, 1, 63, 13, 819, 819, 13, 1, input2_storage, input2_storage, 0, 1, 6, 6, 13, 468, 78, 13, 1, output0_storage, output0_storage, 0, 1, 6, 6, 13, 468, 78, 13, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00038_c82f7ccb: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 468));
  return 0;
}
