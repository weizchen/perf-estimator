...
static float input0_storage[540800] __attribute__((aligned(64)));
static float input1_storage[18432] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 540800, 1);
  fill_f32(input1_storage, 18432, 2);
  fill_f32(input2_storage, 262144, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 32, 130, 130, 540800, 16900, 130, 1, input1_storage, input1_storage, 0, 64, 32, 3, 3, 288, 9, 3, 1, input2_storage, input2_storage, 0, 1, 64, 64, 64, 262144, 4096, 64, 1, output0_storage, output0_storage, 0, 1, 64, 64, 64, 262144, 4096, 64, 1);
  ofa_dma_memory_fence();
  puts("nvidia_mit-b0_start88_end93_18__forward__op1__scalar: benchmark completed");
