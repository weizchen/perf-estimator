...
static uint16_t input0_storage[1376256] __attribute__((aligned(64)));
static uint16_t input1_storage[12845056] __attribute__((aligned(64)));
static uint16_t input2_storage[5505024] __attribute__((aligned(64)));
static uint16_t output0_storage[5505024] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1376256, 1);
  fill_f16(input1_storage, 12845056, 2);
  fill_f16(input2_storage, 5505024, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 192, 448, 86016, 448, 1, input1_storage, input1_storage, 0, 16, 448, 1792, 802816, 1792, 1, input2_storage, input2_storage, 0, 16, 192, 1792, 344064, 1792, 1, output0_storage, output0_storage, 0, 16, 192, 1792, 344064, 1792, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00062_45ecd21e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5505024));
  return 0;
}
