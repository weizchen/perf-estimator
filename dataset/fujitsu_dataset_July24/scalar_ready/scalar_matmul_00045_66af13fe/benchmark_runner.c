...
static int8_t input1_storage[262144] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 262144, 2);
  fill_i32(input2_storage, 2752512, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2688, 256, 256, 1, input1_storage, input1_storage, 0, 256, 1024, 1024, 1, input2_storage, input2_storage, 0, 2688, 1024, 1024, 1, output0_storage, output0_storage, 0, 2688, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00045_66af13fe: benchmark completed");
