...
static int8_t input0_storage[289] __attribute__((aligned(64)));
static int8_t input1_storage[12] __attribute__((aligned(64)));
static int32_t input2_storage[972] __attribute__((aligned(64)));
static int32_t output0_storage[972] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 289, 1);
  fill_i8(input1_storage, 12, 2);
  fill_i32(input2_storage, 972, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 17, 17, 1, 289, 17, 1, 1, input1_storage, input1_storage, 0, 1, 1, 1, 12, 12, 12, 12, 1, input2_storage, input2_storage, 0, 1, 9, 9, 12, 972, 108, 12, 1, output0_storage, output0_storage, 0, 1, 9, 9, 12, 972, 108, 12, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00091_56c88563: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 972));
  return 0;
}
