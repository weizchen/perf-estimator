...
static float input0_storage[12800] __attribute__((aligned(64)));
static float input1_storage[432] __attribute__((aligned(64)));
static float input2_storage[2166] __attribute__((aligned(64)));
static float output0_storage[2166] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 12800, 1);
  fill_f32(input1_storage, 432, 2);
  fill_f32(input2_storage, 2166, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 40, 40, 8, 12800, 320, 8, 1, input1_storage, input1_storage, 0, 3, 3, 8, 6, 144, 48, 6, 1, input2_storage, input2_storage, 0, 1, 19, 19, 6, 2166, 114, 6, 1, output0_storage, output0_storage, 0, 1, 19, 19, 6, 2166, 114, 6, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00141_f2b2cd0f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2166));
  return 0;
}
