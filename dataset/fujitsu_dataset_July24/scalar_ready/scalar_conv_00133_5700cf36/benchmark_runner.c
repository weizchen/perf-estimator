...
static int8_t input0_storage[578] __attribute__((aligned(64)));
static int8_t input1_storage[16] __attribute__((aligned(64)));
static int32_t input2_storage[2312] __attribute__((aligned(64)));
static int32_t output0_storage[2312] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 578, 1);
  fill_i8(input1_storage, 16, 2);
  fill_i32(input2_storage, 2312, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 17, 17, 2, 578, 34, 2, 1, input1_storage, input1_storage, 0, 1, 1, 2, 8, 16, 16, 8, 1, input2_storage, input2_storage, 0, 1, 17, 17, 8, 2312, 136, 8, 1, output0_storage, output0_storage, 0, 1, 17, 17, 8, 2312, 136, 8, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00133_5700cf36: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2312));
  return 0;
}
