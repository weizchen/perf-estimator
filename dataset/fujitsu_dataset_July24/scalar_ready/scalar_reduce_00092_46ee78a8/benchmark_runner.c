...
static uint16_t output0_storage[12] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4428, 1);
  fill_f16(input1_storage, 12, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 369, 12, 12, 1, input1_storage, input1_storage, 0, 12, 1, output0_storage, output0_storage, 0, 12, 1);
  ofa_dma_memory_fence();
  puts("scalar_reduce_00092_46ee78a8: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 12));
  return 0;
}
