...
static uint16_t input0_storage[7875] __attribute__((aligned(64)));
static uint16_t input1_storage[6300] __attribute__((aligned(64)));
static float input2_storage[3380] __attribute__((aligned(64)));
static float output0_storage[3380] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 7875, 1);
  fill_f16(input1_storage, 6300, 2);
  fill_f32(input2_storage, 3380, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 15, 15, 35, 7875, 525, 35, 1, input1_storage, input1_storage, 0, 3, 3, 35, 20, 2100, 700, 20, 1, input2_storage, input2_storage, 0, 1, 13, 13, 20, 3380, 260, 20, 1, output0_storage, output0_storage, 0, 1, 13, 13, 20, 3380, 260, 20, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00050_3ffd17d5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3380));
  return 0;
}
