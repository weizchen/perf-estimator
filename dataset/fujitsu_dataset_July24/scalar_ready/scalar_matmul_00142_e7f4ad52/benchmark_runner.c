...
static uint16_t input1_storage[7507968] __attribute__((aligned(64)));
static float input2_storage[770048] __attribute__((aligned(64)));
static float output0_storage[770048] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 7507968, 2);
  fill_f32(input2_storage, 770048, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2496, 2496, 1, input1_storage, input1_storage, 0, 2496, 3008, 3008, 1, input2_storage, input2_storage, 0, 256, 3008, 3008, 1, output0_storage, output0_storage, 0, 256, 3008, 3008, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00142_e7f4ad52: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 770048));
  return 0;
}
