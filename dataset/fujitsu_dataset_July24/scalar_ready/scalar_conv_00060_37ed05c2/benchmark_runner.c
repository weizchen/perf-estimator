...
static int8_t input0_storage[107163] __attribute__((aligned(64)));
static int8_t input1_storage[3159] __attribute__((aligned(64)));
static int32_t input2_storage[48373] __attribute__((aligned(64)));
static int32_t output0_storage[48373] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 107163, 1);
  fill_i8(input1_storage, 3159, 2);
  fill_i32(input2_storage, 48373, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 63, 63, 27, 107163, 1701, 27, 1, input1_storage, input1_storage, 0, 3, 3, 27, 13, 1053, 351, 13, 1, input2_storage, input2_storage, 0, 1, 61, 61, 13, 48373, 793, 13, 1, output0_storage, output0_storage, 0, 1, 61, 61, 13, 48373, 793, 13, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00060_37ed05c2: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 48373));
  return 0;
}
