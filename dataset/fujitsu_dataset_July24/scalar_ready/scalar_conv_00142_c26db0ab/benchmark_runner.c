...
static int8_t input1_storage[650] __attribute__((aligned(64)));
static int32_t input2_storage[416] __attribute__((aligned(64)));
static int32_t output0_storage[416] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 144, 1);
  fill_i8(input1_storage, 650, 2);
  fill_i32(input2_storage, 416, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 12, 12, 1, 144, 12, 1, 1, input1_storage, input1_storage, 0, 5, 5, 1, 26, 130, 26, 26, 1, input2_storage, input2_storage, 0, 1, 4, 4, 26, 416, 104, 26, 1, output0_storage, output0_storage, 0, 1, 4, 4, 26, 416, 104, 26, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00142_c26db0ab: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 416));
  return 0;
}
