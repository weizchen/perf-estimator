...
static int8_t input0_storage[3600] __attribute__((aligned(64)));
static int8_t input1_storage[1764] __attribute__((aligned(64)));
static int32_t input2_storage[1296] __attribute__((aligned(64)));
static int32_t output0_storage[1296] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 3600, 1);
  fill_i8(input1_storage, 1764, 2);
  fill_i32(input2_storage, 1296, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 30, 30, 4, 3600, 120, 4, 1, input1_storage, input1_storage, 0, 7, 7, 4, 9, 252, 36, 9, 1, input2_storage, input2_storage, 0, 1, 12, 12, 9, 1296, 108, 9, 1, output0_storage, output0_storage, 0, 1, 12, 12, 9, 1296, 108, 9, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00046_0e3929a2: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1296));
  return 0;
}
