...
static int8_t input0_storage[768] __attribute__((aligned(64)));
static int8_t input1_storage[2058] __attribute__((aligned(64)));
static int32_t input2_storage[1400] __attribute__((aligned(64)));
static int32_t output0_storage[1400] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 768, 1);
  fill_i8(input1_storage, 2058, 2);
  fill_i32(input2_storage, 1400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 16, 16, 3, 768, 48, 3, 1, input1_storage, input1_storage, 0, 7, 7, 3, 14, 294, 42, 14, 1, input2_storage, input2_storage, 0, 1, 10, 10, 14, 1400, 140, 14, 1, output0_storage, output0_storage, 0, 1, 10, 10, 14, 1400, 140, 14, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00004_b8a22998: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1400));
  return 0;
}
