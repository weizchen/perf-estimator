...
static uint16_t input1_storage[233472] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 233472, 2);
  fill_f32(input2_storage, 3735552, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 64, 64, 1, input1_storage, input1_storage, 0, 64, 3648, 3648, 1, input2_storage, input2_storage, 0, 1024, 3648, 3648, 1, output0_storage, output0_storage, 0, 1024, 3648, 3648, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00174_d249c281: benchmark completed");
