...
static uint16_t input0_storage[26607616] __attribute__((aligned(64)));
static uint16_t input1_storage[7602176] __attribute__((aligned(64)));
static uint16_t input2_storage[7340032] __attribute__((aligned(64)));
static uint16_t output0_storage[7340032] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 26607616, 1);
  fill_f16(input1_storage, 7602176, 2);
  fill_f16(input2_storage, 7340032, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 1792, 1856, 3325952, 1856, 1, input1_storage, input1_storage, 0, 8, 1856, 512, 950272, 512, 1, input2_storage, input2_storage, 0, 8, 1792, 512, 917504, 512, 1, output0_storage, output0_storage, 0, 8, 1792, 512, 917504, 512, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00065_13c7da3b: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 7340032));
  return 0;
}
