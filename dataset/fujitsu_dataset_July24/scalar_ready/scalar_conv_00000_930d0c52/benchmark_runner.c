...
static uint16_t input0_storage[3520] __attribute__((aligned(64)));
static uint16_t input1_storage[18865] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3520, 1);
  fill_f16(input1_storage, 18865, 2);
  fill_f32(input2_storage, 7, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 8, 8, 55, 3520, 440, 55, 1, input1_storage, input1_storage, 0, 7, 7, 55, 7, 2695, 385, 7, 1, input2_storage, input2_storage, 0, 1, 1, 1, 7, 7, 7, 7, 1, output0_storage, output0_storage, 0, 1, 1, 1, 7, 7, 7, 7, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00000_930d0c52: benchmark completed");
