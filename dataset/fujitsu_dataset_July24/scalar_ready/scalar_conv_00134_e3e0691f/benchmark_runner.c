...
static uint16_t input0_storage[21504] __attribute__((aligned(64)));
static uint16_t input1_storage[567] __attribute__((aligned(64)));
static float input2_storage[675] __attribute__((aligned(64)));
static float output0_storage[675] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 21504, 1);
  fill_f16(input1_storage, 567, 2);
  fill_f32(input2_storage, 675, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 32, 32, 21, 21504, 672, 21, 1, input1_storage, input1_storage, 0, 3, 3, 21, 3, 189, 63, 3, 1, input2_storage, input2_storage, 0, 1, 15, 15, 3, 675, 45, 3, 1, output0_storage, output0_storage, 0, 1, 15, 15, 3, 675, 45, 3, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00134_e3e0691f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 675));
  return 0;
}
