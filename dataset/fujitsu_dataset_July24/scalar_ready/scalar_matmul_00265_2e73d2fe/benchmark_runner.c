...
static uint16_t input1_storage[5963776] __attribute__((aligned(64)));
static uint16_t input2_storage[688128] __attribute__((aligned(64)));
static uint16_t output0_storage[688128] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 5963776, 2);
  fill_f16(input2_storage, 688128, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 3328, 3328, 1, input1_storage, input1_storage, 0, 3328, 1792, 1792, 1, input2_storage, input2_storage, 0, 384, 1792, 1792, 1, output0_storage, output0_storage, 0, 384, 1792, 1792, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00265_2e73d2fe: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 688128));
  return 0;
}
