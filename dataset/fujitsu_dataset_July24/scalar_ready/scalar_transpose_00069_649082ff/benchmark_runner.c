...
static float input0_storage[56610] __attribute__((aligned(64)));
static float input1_storage[56610] __attribute__((aligned(64)));
static float output0_storage[56610] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 56610, 1);
  fill_f32(input1_storage, 56610, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 37, 6, 15, 17, 1530, 255, 17, 1, input1_storage, input1_storage, 0, 37, 6, 17, 15, 1530, 255, 15, 1, output0_storage, output0_storage, 0, 37, 6, 17, 15, 1530, 255, 15, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00069_649082ff: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 56610));
  return 0;
}
