...
static uint16_t input0_storage[1253376] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1253376, 1);
  fill_f16(input1_storage, 557056, 2);
  fill_f32(input2_storage, 147456, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 256, 256, 1, input2_storage, input2_storage, 0, 576, 256, 256, 1, output0_storage, output0_storage, 0, 576, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00162_ff0b8f06: benchmark completed");
