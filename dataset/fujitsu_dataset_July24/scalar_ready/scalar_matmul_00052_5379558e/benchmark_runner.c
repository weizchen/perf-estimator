...
static int32_t input2_storage[2539520] __attribute__((aligned(64)));
static int32_t output0_storage[2539520] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 2539520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 256, 256, 1, input1_storage, input1_storage, 0, 256, 640, 640, 1, input2_storage, input2_storage, 0, 3968, 640, 640, 1, output0_storage, output0_storage, 0, 3968, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00052_5379558e: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2539520));
  return 0;
}
