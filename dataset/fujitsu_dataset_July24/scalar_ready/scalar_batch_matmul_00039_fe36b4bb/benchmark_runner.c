...
static uint16_t input0_storage[2949120] __attribute__((aligned(64)));
static uint16_t input1_storage[14745600] __attribute__((aligned(64)));
static float input2_storage[2949120] __attribute__((aligned(64)));
static float output0_storage[2949120] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2949120, 1);
  fill_f16(input1_storage, 14745600, 2);
  fill_f32(input2_storage, 2949120, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 192, 960, 184320, 960, 1, input1_storage, input1_storage, 0, 16, 960, 960, 921600, 960, 1, input2_storage, input2_storage, 0, 16, 192, 960, 184320, 960, 1, output0_storage, output0_storage, 0, 16, 192, 960, 184320, 960, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00039_fe36b4bb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2949120));
  return 0;
}
