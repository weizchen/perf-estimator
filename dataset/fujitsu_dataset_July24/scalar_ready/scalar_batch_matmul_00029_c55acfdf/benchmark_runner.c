...
static uint16_t input0_storage[655360] __attribute__((aligned(64)));
static uint16_t input1_storage[1966080] __attribute__((aligned(64)));
static float input2_storage[9830400] __attribute__((aligned(64)));
static float output0_storage[9830400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 655360, 1);
  fill_f16(input1_storage, 1966080, 2);
  fill_f32(input2_storage, 9830400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 8, 640, 128, 81920, 128, 1, input1_storage, input1_storage, 0, 8, 128, 1920, 245760, 1920, 1, input2_storage, input2_storage, 0, 8, 640, 1920, 1228800, 1920, 1, output0_storage, output0_storage, 0, 8, 640, 1920, 1228800, 1920, 1);
  ofa_dma_memory_fence();
  puts("scalar_batch_matmul_00029_c55acfdf: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9830400));
  return 0;
}
