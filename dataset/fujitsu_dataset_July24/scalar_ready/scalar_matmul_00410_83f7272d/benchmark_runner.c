...
static float input0_storage[139264] __attribute__((aligned(64)));
static float input1_storage[352256] __attribute__((aligned(64)));
static float input2_storage[2994176] __attribute__((aligned(64)));
static float output0_storage[2994176] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 139264, 1);
  fill_f32(input1_storage, 352256, 2);
  fill_f32(input2_storage, 2994176, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2752, 2752, 1, input2_storage, input2_storage, 0, 1088, 2752, 2752, 1, output0_storage, output0_storage, 0, 1088, 2752, 2752, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00410_83f7272d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2994176));
  return 0;
}
