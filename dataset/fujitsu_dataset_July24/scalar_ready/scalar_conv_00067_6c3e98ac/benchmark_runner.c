...
static uint16_t input0_storage[300] __attribute__((aligned(64)));
static uint16_t input1_storage[69] __attribute__((aligned(64)));
static float input2_storage[2300] __attribute__((aligned(64)));
static float output0_storage[2300] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 300, 1);
  fill_f16(input1_storage, 69, 2);
  fill_f32(input2_storage, 2300, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 10, 10, 3, 300, 30, 3, 1, input1_storage, input1_storage, 0, 1, 1, 3, 23, 69, 69, 23, 1, input2_storage, input2_storage, 0, 1, 10, 10, 23, 2300, 230, 23, 1, output0_storage, output0_storage, 0, 1, 10, 10, 23, 2300, 230, 23, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00067_6c3e98ac: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2300));
  return 0;
}
