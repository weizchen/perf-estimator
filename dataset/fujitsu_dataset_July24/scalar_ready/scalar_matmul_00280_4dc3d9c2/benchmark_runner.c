...
static uint16_t input0_storage[3317760] __attribute__((aligned(64)));
static uint16_t input1_storage[442368] __attribute__((aligned(64)));
static uint16_t input2_storage[122880] __attribute__((aligned(64)));
static uint16_t output0_storage[122880] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3317760, 1);
  fill_f16(input1_storage, 442368, 2);
  fill_f16(input2_storage, 122880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 960, 3456, 3456, 1, input1_storage, input1_storage, 0, 3456, 128, 128, 1, input2_storage, input2_storage, 0, 960, 128, 128, 1, output0_storage, output0_storage, 0, 960, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00280_4dc3d9c2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 122880));
  return 0;
}
