...
static int8_t input0_storage[12558] __attribute__((aligned(64)));
static int8_t input1_storage[12558] __attribute__((aligned(64)));
static int8_t output0_storage[12558] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 12558, 1);
  fill_i8(input1_storage, 12558, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 13, 23, 6, 7, 966, 42, 7, 1, input1_storage, input1_storage, 0, 13, 6, 23, 7, 966, 161, 7, 1, output0_storage, output0_storage, 0, 13, 6, 23, 7, 966, 161, 7, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00129_d55d3ea1: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 12558));
  return 0;
}
