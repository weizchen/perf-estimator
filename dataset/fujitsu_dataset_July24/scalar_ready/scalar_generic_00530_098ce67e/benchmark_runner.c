...
static uint16_t input0_storage[36] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 36, 1);
  fill_f16(input1_storage, 36, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 36, 1, input1_storage, input1_storage, 0, 36, 1, output0_storage, output0_storage, 0, 36, 1);
  ofa_dma_memory_fence();
  puts("scalar_generic_00530_098ce67e: benchmark completed");
