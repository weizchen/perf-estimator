...
static float input0_storage[81] __attribute__((aligned(64)));
static float input1_storage[45] __attribute__((aligned(64)));
static float input2_storage[245] __attribute__((aligned(64)));
static float output0_storage[245] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 81, 1);
  fill_f32(input1_storage, 45, 2);
  fill_f32(input2_storage, 245, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1, 9, 9, 1, 81, 9, 1, 1, input1_storage, input1_storage, 0, 3, 3, 1, 5, 15, 5, 5, 1, input2_storage, input2_storage, 0, 1, 7, 7, 5, 245, 35, 5, 1, output0_storage, output0_storage, 0, 1, 7, 7, 5, 245, 35, 5, 1);
  ofa_dma_memory_fence();
  puts("scalar_conv_00055_5e248df2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 245));
  return 0;
}
