...
static uint16_t input0_storage[17100] __attribute__((aligned(64)));
static uint16_t input1_storage[17100] __attribute__((aligned(64)));
static uint16_t output0_storage[17100] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 17100, 1);
  fill_f16(input1_storage, 17100, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 19, 10, 15, 6, 900, 90, 6, 1, input1_storage, input1_storage, 0, 19, 6, 15, 10, 900, 150, 10, 1, output0_storage, output0_storage, 0, 19, 6, 15, 10, 900, 150, 10, 1);
  ofa_dma_memory_fence();
  puts("scalar_transpose_00046_ce7ae3de: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 17100));
  return 0;
}
