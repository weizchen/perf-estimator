...
static float input0_storage[3256320] __attribute__((aligned(64)));
static float input1_storage[61440] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3256320, 1);
  fill_f32(input1_storage, 61440, 2);
  fill_f32(input2_storage, 217088, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3392, 960, 960, 1, input1_storage, input1_storage, 0, 960, 64, 64, 1, input2_storage, input2_storage, 0, 3392, 64, 64, 1, output0_storage, output0_storage, 0, 3392, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("scalar_matmul_00347_8965f5ed: benchmark completed");
