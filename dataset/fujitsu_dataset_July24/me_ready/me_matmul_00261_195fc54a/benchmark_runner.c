...
static uint8_t input0_storage[655360] __attribute__((aligned(64)));
static uint8_t input1_storage[294912] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 655360, 1);
  fill_f8(input1_storage, 294912, 2);
  fill_f32(input2_storage, 2949120, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2560, 256, 256, 1, input1_storage, input1_storage, 0, 256, 1152, 1152, 1, input2_storage, input2_storage, 0, 2560, 1152, 1152, 1, output0_storage, output0_storage, 0, 2560, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00261_195fc54a: benchmark completed");
