...
static uint16_t input0_storage[507904] __attribute__((aligned(64)));
static uint16_t input1_storage[868352] __attribute__((aligned(64)));
static float input2_storage[6729728] __attribute__((aligned(64)));
static float output0_storage[6729728] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 507904, 1);
  fill_f16(input1_storage, 868352, 2);
  fill_f32(input2_storage, 6729728, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1984, 256, 256, 1, input1_storage, input1_storage, 0, 256, 3392, 3392, 1, input2_storage, input2_storage, 0, 1984, 3392, 3392, 1, output0_storage, output0_storage, 0, 1984, 3392, 3392, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00399_946399fa: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 6729728));
  return 0;
}
