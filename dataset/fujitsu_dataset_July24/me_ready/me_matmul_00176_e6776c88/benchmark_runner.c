...
static uint8_t input0_storage[9732096] __attribute__((aligned(64)));
static uint8_t input1_storage[7077888] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 9732096, 1);
  fill_f8(input1_storage, 7077888, 2);
  fill_f32(input2_storage, 5767168, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2816, 3456, 3456, 1, input1_storage, input1_storage, 0, 3456, 2048, 2048, 1, input2_storage, input2_storage, 0, 2816, 2048, 2048, 1, output0_storage, output0_storage, 0, 2816, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00176_e6776c88: benchmark completed");
