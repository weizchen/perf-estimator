...
static uint8_t input0_storage[1900544] __attribute__((aligned(64)));
static uint8_t input1_storage[4276224] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 1900544, 1);
  fill_f8(input1_storage, 4276224, 2);
  fill_f32(input2_storage, 589824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 1152, 1152, 1, input2_storage, input2_storage, 0, 512, 1152, 1152, 1, output0_storage, output0_storage, 0, 512, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00101_76bfb226: benchmark completed");
