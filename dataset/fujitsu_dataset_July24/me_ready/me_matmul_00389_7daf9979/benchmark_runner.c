...
static uint16_t input0_storage[704512] __attribute__((aligned(64)));
static uint16_t input1_storage[4050944] __attribute__((aligned(64)));
static float input2_storage[376832] __attribute__((aligned(64)));
static float output0_storage[376832] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 704512, 1);
  fill_f16(input1_storage, 4050944, 2);
  fill_f32(input2_storage, 376832, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2752, 2752, 1, input1_storage, input1_storage, 0, 2752, 1472, 1472, 1, input2_storage, input2_storage, 0, 256, 1472, 1472, 1, output0_storage, output0_storage, 0, 256, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00389_7daf9979: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 376832));
  return 0;
}
