...
static uint8_t input0_storage[4358144] __attribute__((aligned(64)));
static uint8_t input1_storage[917504] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 4358144, 1);
  fill_f8(input1_storage, 917504, 2);
  fill_f32(input2_storage, 1245184, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2432, 1792, 1792, 1, input1_storage, input1_storage, 0, 1792, 512, 512, 1, input2_storage, input2_storage, 0, 2432, 512, 512, 1, output0_storage, output0_storage, 0, 2432, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00000_bef03e2f: benchmark completed");
