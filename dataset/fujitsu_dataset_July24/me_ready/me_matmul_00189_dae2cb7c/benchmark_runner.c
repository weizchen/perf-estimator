...
static uint8_t input0_storage[2129920] __attribute__((aligned(64)));
static uint8_t input1_storage[851968] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 2129920, 1);
  fill_f8(input1_storage, 851968, 2);
  fill_f32(input2_storage, 163840, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 3328, 3328, 1, input1_storage, input1_storage, 0, 3328, 256, 256, 1, input2_storage, input2_storage, 0, 640, 256, 256, 1, output0_storage, output0_storage, 0, 640, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00189_dae2cb7c: benchmark completed");
