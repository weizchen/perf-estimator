...
static uint16_t input1_storage[380928] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 380928, 2);
  fill_f16(input2_storage, 49152, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1984, 1984, 1, input1_storage, input1_storage, 0, 1984, 192, 192, 1, input2_storage, input2_storage, 0, 256, 192, 192, 1, output0_storage, output0_storage, 0, 256, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00567_2b1f20f2: benchmark completed");
