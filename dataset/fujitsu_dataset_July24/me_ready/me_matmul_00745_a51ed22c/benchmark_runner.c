...
static uint16_t input0_storage[5160960] __attribute__((aligned(64)));
static uint16_t input1_storage[6967296] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5160960, 1);
  fill_f16(input1_storage, 6967296, 2);
  fill_f16(input2_storage, 2211840, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 4032, 4032, 1, input1_storage, input1_storage, 0, 4032, 1728, 1728, 1, input2_storage, input2_storage, 0, 1280, 1728, 1728, 1, output0_storage, output0_storage, 0, 1280, 1728, 1728, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00745_a51ed22c: benchmark completed");
