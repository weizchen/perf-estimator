...
static uint8_t input0_storage[131072] __attribute__((aligned(64)));
static uint8_t input1_storage[262144] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 131072, 1);
  fill_f8(input1_storage, 262144, 2);
  fill_f32(input2_storage, 2097152, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2048, 2048, 1, input2_storage, input2_storage, 0, 1024, 2048, 2048, 1, output0_storage, output0_storage, 0, 1024, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00201_60b57a61: benchmark completed");
