...
static uint8_t input0_storage[589824] __attribute__((aligned(64)));
static uint8_t input1_storage[442368] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 589824, 1);
  fill_f8(input1_storage, 442368, 2);
  fill_f32(input2_storage, 1769472, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 384, 384, 1, input1_storage, input1_storage, 0, 384, 1152, 1152, 1, input2_storage, input2_storage, 0, 1536, 1152, 1152, 1, output0_storage, output0_storage, 0, 1536, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00090_94919e3d: benchmark completed");
