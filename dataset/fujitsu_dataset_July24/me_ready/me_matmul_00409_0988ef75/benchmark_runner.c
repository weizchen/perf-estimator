...
static uint16_t input0_storage[1089536] __attribute__((aligned(64)));
static uint16_t input1_storage[311296] __attribute__((aligned(64)));
static float input2_storage[57344] __attribute__((aligned(64)));
static float output0_storage[57344] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1089536, 1);
  fill_f16(input1_storage, 311296, 2);
  fill_f32(input2_storage, 57344, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 2432, 2432, 1, input1_storage, input1_storage, 0, 2432, 128, 128, 1, input2_storage, input2_storage, 0, 448, 128, 128, 1, output0_storage, output0_storage, 0, 448, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00409_0988ef75: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 57344));
  return 0;
}
