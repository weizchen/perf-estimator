...
static uint16_t input1_storage[167936] __attribute__((aligned(64)));
static float input2_storage[4534272] __attribute__((aligned(64)));
static float output0_storage[4534272] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 167936, 2);
  fill_f32(input2_storage, 4534272, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1728, 64, 64, 1, input1_storage, input1_storage, 0, 64, 2624, 2624, 1, input2_storage, input2_storage, 0, 1728, 2624, 2624, 1, output0_storage, output0_storage, 0, 1728, 2624, 2624, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00458_1f30ce48: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4534272));
  return 0;
}
