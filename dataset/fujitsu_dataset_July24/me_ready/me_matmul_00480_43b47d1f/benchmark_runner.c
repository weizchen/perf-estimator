...
static uint16_t input0_storage[1474560] __attribute__((aligned(64)));
static uint16_t input1_storage[942080] __attribute__((aligned(64)));
static float input2_storage[3391488] __attribute__((aligned(64)));
static float output0_storage[3391488] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1474560, 1);
  fill_f16(input1_storage, 942080, 2);
  fill_f32(input2_storage, 3391488, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2304, 640, 640, 1, input1_storage, input1_storage, 0, 640, 1472, 1472, 1, input2_storage, input2_storage, 0, 2304, 1472, 1472, 1, output0_storage, output0_storage, 0, 2304, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00480_43b47d1f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3391488));
  return 0;
}
