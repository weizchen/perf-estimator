...
static uint16_t input0_storage[778240] __attribute__((aligned(64)));
static uint16_t input1_storage[737280] __attribute__((aligned(64)));
static uint16_t input2_storage[5603328] __attribute__((aligned(64)));
static uint16_t output0_storage[5603328] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 778240, 1);
  fill_f16(input1_storage, 737280, 2);
  fill_f16(input2_storage, 5603328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2432, 320, 320, 1, input1_storage, input1_storage, 0, 320, 2304, 2304, 1, input2_storage, input2_storage, 0, 2432, 2304, 2304, 1, output0_storage, output0_storage, 0, 2432, 2304, 2304, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00740_6d890b5b: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5603328));
  return 0;
}
