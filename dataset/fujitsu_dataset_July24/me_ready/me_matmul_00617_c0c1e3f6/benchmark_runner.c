...
static uint16_t input2_storage[479232] __attribute__((aligned(64)));
static uint16_t output0_storage[479232] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1597440, 2);
  fill_f16(input2_storage, 479232, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 640, 640, 1, input1_storage, input1_storage, 0, 640, 2496, 2496, 1, input2_storage, input2_storage, 0, 192, 2496, 2496, 1, output0_storage, output0_storage, 0, 192, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00617_c0c1e3f6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 479232));
  return 0;
}
