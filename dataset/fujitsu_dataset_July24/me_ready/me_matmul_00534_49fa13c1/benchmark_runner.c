...
static uint16_t input0_storage[151552] __attribute__((aligned(64)));
static uint16_t input1_storage[4243456] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 151552, 1);
  fill_f16(input1_storage, 4243456, 2);
  fill_f16(input2_storage, 114688, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 2368, 2368, 1, input1_storage, input1_storage, 0, 2368, 1792, 1792, 1, input2_storage, input2_storage, 0, 64, 1792, 1792, 1, output0_storage, output0_storage, 0, 64, 1792, 1792, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00534_49fa13c1: benchmark completed");
