...
static uint16_t input1_storage[2256896] __attribute__((aligned(64)));
static float input2_storage[622592] __attribute__((aligned(64)));
static float output0_storage[622592] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 2256896, 2);
  fill_f32(input2_storage, 622592, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 1216, 1216, 1, input2_storage, input2_storage, 0, 512, 1216, 1216, 1, output0_storage, output0_storage, 0, 512, 1216, 1216, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00347_6a077fa7: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 622592));
  return 0;
}
