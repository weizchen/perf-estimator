...
static uint16_t input1_storage[1007616] __attribute__((aligned(64)));
static float input2_storage[9572352] __attribute__((aligned(64)));
static float output0_storage[9572352] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1007616, 2);
  fill_f32(input2_storage, 9572352, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3648, 384, 384, 1, input1_storage, input1_storage, 0, 384, 2624, 2624, 1, input2_storage, input2_storage, 0, 3648, 2624, 2624, 1, output0_storage, output0_storage, 0, 3648, 2624, 2624, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00289_5caeb853: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9572352));
  return 0;
}
