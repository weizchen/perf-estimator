...
static uint16_t input2_storage[1376256] __attribute__((aligned(64)));
static uint16_t output0_storage[1376256] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 1376256, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 128, 128, 1, input1_storage, input1_storage, 0, 128, 896, 896, 1, input2_storage, input2_storage, 0, 1536, 896, 896, 1, output0_storage, output0_storage, 0, 1536, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00632_ad82ca3e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1376256));
  return 0;
}
