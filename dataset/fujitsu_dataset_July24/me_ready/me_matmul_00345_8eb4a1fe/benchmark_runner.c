...
static uint16_t input0_storage[9818112] __attribute__((aligned(64)));
static uint16_t input1_storage[192512] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9818112, 1);
  fill_f16(input1_storage, 192512, 2);
  fill_f32(input2_storage, 208896, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3264, 3008, 3008, 1, input1_storage, input1_storage, 0, 3008, 64, 64, 1, input2_storage, input2_storage, 0, 3264, 64, 64, 1, output0_storage, output0_storage, 0, 3264, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00345_8eb4a1fe: benchmark completed");
