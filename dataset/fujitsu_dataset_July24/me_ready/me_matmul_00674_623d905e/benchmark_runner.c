...
static uint16_t input0_storage[1671168] __attribute__((aligned(64)));
static uint16_t input1_storage[2019328] __attribute__((aligned(64)));
static uint16_t input2_storage[2850816] __attribute__((aligned(64)));
static uint16_t output0_storage[2850816] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1671168, 1);
  fill_f16(input1_storage, 2019328, 2);
  fill_f16(input2_storage, 2850816, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 1088, 1088, 1, input1_storage, input1_storage, 0, 1088, 1856, 1856, 1, input2_storage, input2_storage, 0, 1536, 1856, 1856, 1, output0_storage, output0_storage, 0, 1536, 1856, 1856, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00674_623d905e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2850816));
  return 0;
}
