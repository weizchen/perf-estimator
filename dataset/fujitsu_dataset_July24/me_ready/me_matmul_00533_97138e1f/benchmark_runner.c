...
static uint16_t input0_storage[3342336] __attribute__((aligned(64)));
static uint16_t input1_storage[417792] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3342336, 1);
  fill_f16(input1_storage, 417792, 2);
  fill_f16(input2_storage, 131072, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 3264, 3264, 1, input1_storage, input1_storage, 0, 3264, 128, 128, 1, input2_storage, input2_storage, 0, 1024, 128, 128, 1, output0_storage, output0_storage, 0, 1024, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00533_97138e1f: benchmark completed");
