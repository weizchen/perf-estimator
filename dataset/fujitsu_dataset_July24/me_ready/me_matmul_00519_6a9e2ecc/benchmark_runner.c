...
static float input2_storage[675840] __attribute__((aligned(64)));
static float output0_storage[675840] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 675840, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 960, 320, 320, 1, input1_storage, input1_storage, 0, 320, 704, 704, 1, input2_storage, input2_storage, 0, 960, 704, 704, 1, output0_storage, output0_storage, 0, 960, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00519_6a9e2ecc: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 675840));
  return 0;
}
