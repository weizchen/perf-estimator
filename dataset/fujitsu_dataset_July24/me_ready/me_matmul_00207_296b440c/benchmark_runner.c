...
static uint8_t input0_storage[3686400] __attribute__((aligned(64)));
static uint8_t input1_storage[737280] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 3686400, 1);
  fill_f8(input1_storage, 737280, 2);
  fill_f32(input2_storage, 737280, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 1920, 1920, 1, input1_storage, input1_storage, 0, 1920, 384, 384, 1, input2_storage, input2_storage, 0, 1920, 384, 384, 1, output0_storage, output0_storage, 0, 1920, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00207_296b440c: benchmark completed");
