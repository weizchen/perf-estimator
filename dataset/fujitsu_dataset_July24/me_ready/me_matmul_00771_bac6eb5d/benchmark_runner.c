...
static uint16_t input0_storage[180224] __attribute__((aligned(64)));
static uint16_t input1_storage[143360] __attribute__((aligned(64)));
static uint16_t input2_storage[6307840] __attribute__((aligned(64)));
static uint16_t output0_storage[6307840] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 180224, 1);
  fill_f16(input1_storage, 143360, 2);
  fill_f16(input2_storage, 6307840, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2816, 64, 64, 1, input1_storage, input1_storage, 0, 64, 2240, 2240, 1, input2_storage, input2_storage, 0, 2816, 2240, 2240, 1, output0_storage, output0_storage, 0, 2816, 2240, 2240, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00771_bac6eb5d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 6307840));
  return 0;
}
