...
static uint8_t input1_storage[3604480] __attribute__((aligned(64)));
static float input2_storage[180224] __attribute__((aligned(64)));
static float output0_storage[180224] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 3604480, 2);
  fill_f32(input2_storage, 180224, 3);
  ...
  ofa_dma_memory_fence();
  puts("me_matmul_00108_f131dfcb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 180224));
  return 0;
}
