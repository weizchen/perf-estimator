...
static uint16_t input0_storage[5939200] __attribute__((aligned(64)));
static uint16_t input1_storage[1126400] __attribute__((aligned(64)));
static float input2_storage[2613248] __attribute__((aligned(64)));
static float output0_storage[2613248] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5939200, 1);
  fill_f16(input1_storage, 1126400, 2);
  fill_f32(input2_storage, 2613248, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3712, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 704, 704, 1, input2_storage, input2_storage, 0, 3712, 704, 704, 1, output0_storage, output0_storage, 0, 3712, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00310_7557a74c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2613248));
  return 0;
}
