...
static uint8_t input0_storage[1572864] __attribute__((aligned(64)));
static uint8_t input1_storage[1966080] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 1572864, 1);
  fill_f8(input1_storage, 1966080, 2);
  fill_f32(input2_storage, 327680, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 3072, 3072, 1, input1_storage, input1_storage, 0, 3072, 640, 640, 1, input2_storage, input2_storage, 0, 512, 640, 640, 1, output0_storage, output0_storage, 0, 512, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00240_d2505d35: benchmark completed");
