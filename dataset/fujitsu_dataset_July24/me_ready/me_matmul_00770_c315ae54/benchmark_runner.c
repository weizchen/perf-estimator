...
static uint16_t input1_storage[1085440] __attribute__((aligned(64)));
static uint16_t input2_storage[217088] __attribute__((aligned(64)));
static uint16_t output0_storage[217088] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1085440, 2);
  fill_f16(input2_storage, 217088, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 320, 320, 1, input1_storage, input1_storage, 0, 320, 3392, 3392, 1, input2_storage, input2_storage, 0, 64, 3392, 3392, 1, output0_storage, output0_storage, 0, 64, 3392, 3392, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00770_c315ae54: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 217088));
  return 0;
}
