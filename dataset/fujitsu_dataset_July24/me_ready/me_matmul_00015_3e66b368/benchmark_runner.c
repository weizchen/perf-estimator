...
static uint8_t input1_storage[229376] __attribute__((aligned(64)));
static float input2_storage[2293760] __attribute__((aligned(64)));
static float output0_storage[2293760] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 229376, 2);
  fill_f32(input2_storage, 2293760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2560, 256, 256, 1, input1_storage, input1_storage, 0, 256, 896, 896, 1, input2_storage, input2_storage, 0, 2560, 896, 896, 1, output0_storage, output0_storage, 0, 2560, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00015_3e66b368: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2293760));
  return 0;
}
