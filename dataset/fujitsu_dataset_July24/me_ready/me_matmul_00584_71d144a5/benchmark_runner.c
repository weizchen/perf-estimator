...
static uint16_t input0_storage[4902912] __attribute__((aligned(64)));
static uint16_t input1_storage[4644864] __attribute__((aligned(64)));
static uint16_t input2_storage[1400832] __attribute__((aligned(64)));
static uint16_t output0_storage[1400832] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4902912, 1);
  fill_f16(input1_storage, 4644864, 2);
  fill_f16(input2_storage, 1400832, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 4032, 4032, 1, input1_storage, input1_storage, 0, 4032, 1152, 1152, 1, input2_storage, input2_storage, 0, 1216, 1152, 1152, 1, output0_storage, output0_storage, 0, 1216, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00584_71d144a5: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1400832));
  return 0;
}
