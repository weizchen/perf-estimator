...
static uint16_t input1_storage[851968] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 851968, 2);
  fill_f32(input2_storage, 229376, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 3328, 3328, 1, input1_storage, input1_storage, 0, 3328, 256, 256, 1, input2_storage, input2_storage, 0, 896, 256, 256, 1, output0_storage, output0_storage, 0, 896, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00368_45a4d7b6: benchmark completed");
