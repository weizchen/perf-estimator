...
static uint16_t input0_storage[434176] __attribute__((aligned(64)));
static uint16_t input1_storage[1736704] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 434176, 1);
  fill_f16(input1_storage, 1736704, 2);
  fill_f32(input2_storage, 65536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3392, 3392, 1, input1_storage, input1_storage, 0, 3392, 512, 512, 1, input2_storage, input2_storage, 0, 128, 512, 512, 1, output0_storage, output0_storage, 0, 128, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00497_8629b4ba: benchmark completed");
