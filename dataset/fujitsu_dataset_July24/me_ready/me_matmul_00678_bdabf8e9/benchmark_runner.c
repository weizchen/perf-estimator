...
static uint16_t input0_storage[319488] __attribute__((aligned(64)));
static uint16_t input1_storage[1490944] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 319488, 1);
  fill_f16(input1_storage, 1490944, 2);
  fill_f16(input2_storage, 172032, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 1664, 1664, 1, input1_storage, input1_storage, 0, 1664, 896, 896, 1, input2_storage, input2_storage, 0, 192, 896, 896, 1, output0_storage, output0_storage, 0, 192, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00678_bdabf8e9: benchmark completed");
