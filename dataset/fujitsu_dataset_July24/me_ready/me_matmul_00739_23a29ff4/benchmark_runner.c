...
static uint16_t input1_storage[3211264] __attribute__((aligned(64)));
static uint16_t input2_storage[802816] __attribute__((aligned(64)));
static uint16_t output0_storage[802816] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 3211264, 2);
  fill_f16(input2_storage, 802816, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 3136, 3136, 1, input2_storage, input2_storage, 0, 256, 3136, 3136, 1, output0_storage, output0_storage, 0, 256, 3136, 3136, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00739_23a29ff4: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 802816));
  return 0;
}
