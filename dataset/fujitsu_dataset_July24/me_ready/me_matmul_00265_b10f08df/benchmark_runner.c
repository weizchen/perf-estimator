...
static float input2_storage[110592] __attribute__((aligned(64)));
static float output0_storage[110592] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 110592, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 1088, 1088, 1, input1_storage, input1_storage, 0, 1088, 192, 192, 1, input2_storage, input2_storage, 0, 576, 192, 192, 1, output0_storage, output0_storage, 0, 576, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00265_b10f08df: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 110592));
  return 0;
}
