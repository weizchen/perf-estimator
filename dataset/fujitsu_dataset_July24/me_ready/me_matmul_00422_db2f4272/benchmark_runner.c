...
static uint16_t input0_storage[1441792] __attribute__((aligned(64)));
static uint16_t input1_storage[8257536] __attribute__((aligned(64)));
static float input2_storage[2838528] __attribute__((aligned(64)));
static float output0_storage[2838528] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1441792, 1);
  fill_f16(input1_storage, 8257536, 2);
  fill_f32(input2_storage, 2838528, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 4032, 4032, 1, input2_storage, input2_storage, 0, 704, 4032, 4032, 1, output0_storage, output0_storage, 0, 704, 4032, 4032, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00422_db2f4272: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2838528));
  return 0;
}
