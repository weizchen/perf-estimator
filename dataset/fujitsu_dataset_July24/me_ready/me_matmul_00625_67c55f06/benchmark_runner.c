...
static uint16_t input0_storage[225280] __attribute__((aligned(64)));
static uint16_t input1_storage[13516800] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 225280, 1);
  fill_f16(input1_storage, 13516800, 2);
  fill_f16(input2_storage, 245760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3520, 3520, 1, input1_storage, input1_storage, 0, 3520, 3840, 3840, 1, input2_storage, input2_storage, 0, 64, 3840, 3840, 1, output0_storage, output0_storage, 0, 64, 3840, 3840, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00625_67c55f06: benchmark completed");
