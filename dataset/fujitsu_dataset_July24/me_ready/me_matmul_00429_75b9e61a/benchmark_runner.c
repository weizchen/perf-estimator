...
static uint16_t input0_storage[3657728] __attribute__((aligned(64)));
static uint16_t input1_storage[933888] __attribute__((aligned(64)));
static float input2_storage[2310144] __attribute__((aligned(64)));
static float output0_storage[2310144] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3657728, 1);
  fill_f16(input1_storage, 933888, 2);
  fill_f32(input2_storage, 2310144, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3008, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 768, 768, 1, input2_storage, input2_storage, 0, 3008, 768, 768, 1, output0_storage, output0_storage, 0, 3008, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00429_75b9e61a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2310144));
  return 0;
}
