...
static uint16_t input0_storage[614400] __attribute__((aligned(64)));
static uint16_t input1_storage[2703360] __attribute__((aligned(64)));
static uint16_t input2_storage[450560] __attribute__((aligned(64)));
static uint16_t output0_storage[450560] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 614400, 1);
  fill_f16(input1_storage, 2703360, 2);
  fill_f16(input2_storage, 450560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 1920, 1920, 1, input1_storage, input1_storage, 0, 1920, 1408, 1408, 1, input2_storage, input2_storage, 0, 320, 1408, 1408, 1, output0_storage, output0_storage, 0, 320, 1408, 1408, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00706_dc65f03d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 450560));
  return 0;
}
