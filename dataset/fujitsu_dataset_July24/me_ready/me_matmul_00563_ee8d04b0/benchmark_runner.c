...
static uint16_t input0_storage[229376] __attribute__((aligned(64)));
static uint16_t input1_storage[147456] __attribute__((aligned(64)));
static uint16_t input2_storage[8257536] __attribute__((aligned(64)));
static uint16_t output0_storage[8257536] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 229376, 1);
  fill_f16(input1_storage, 147456, 2);
  fill_f16(input2_storage, 8257536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3584, 64, 64, 1, input1_storage, input1_storage, 0, 64, 2304, 2304, 1, input2_storage, input2_storage, 0, 3584, 2304, 2304, 1, output0_storage, output0_storage, 0, 3584, 2304, 2304, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00563_ee8d04b0: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 8257536));
  return 0;
}
