...
static uint16_t input0_storage[5406720] __attribute__((aligned(64)));
static uint16_t input1_storage[3932160] __attribute__((aligned(64)));
static uint16_t input2_storage[9011200] __attribute__((aligned(64)));
static uint16_t output0_storage[9011200] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5406720, 1);
  fill_f16(input1_storage, 3932160, 2);
  fill_f16(input2_storage, 9011200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3520, 1536, 1536, 1, input1_storage, input1_storage, 0, 1536, 2560, 2560, 1, input2_storage, input2_storage, 0, 3520, 2560, 2560, 1, output0_storage, output0_storage, 0, 3520, 2560, 2560, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00572_ec8bbee6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 9011200));
  return 0;
}
