...
static float input2_storage[2236416] __attribute__((aligned(64)));
static float output0_storage[2236416] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 479232, 2);
  fill_f32(input2_storage, 2236416, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 192, 192, 1, input1_storage, input1_storage, 0, 192, 2496, 2496, 1, input2_storage, input2_storage, 0, 896, 2496, 2496, 1, output0_storage, output0_storage, 0, 896, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00404_818cb98a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2236416));
  return 0;
}
