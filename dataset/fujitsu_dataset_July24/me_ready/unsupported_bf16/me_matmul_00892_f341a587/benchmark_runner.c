...
static uint16_t input1_storage[700416] __attribute__((aligned(64)));
static float input2_storage[5836800] __attribute__((aligned(64)));
static float output0_storage[5836800] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 307200, 1);
  fill_bf16(input1_storage, 700416, 2);
  fill_f32(input2_storage, 5836800, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 192, 192, 1, input1_storage, input1_storage, 0, 192, 3648, 3648, 1, input2_storage, input2_storage, 0, 1600, 3648, 3648, 1, output0_storage, output0_storage, 0, 1600, 3648, 3648, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00892_f341a587: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5836800));
  return 0;
}
