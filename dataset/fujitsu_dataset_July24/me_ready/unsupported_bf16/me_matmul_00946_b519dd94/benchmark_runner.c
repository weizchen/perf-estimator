...
static uint16_t input1_storage[5275648] __attribute__((aligned(64)));
static float input2_storage[565248] __attribute__((aligned(64)));
static float output0_storage[565248] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 5275648, 2);
  fill_f32(input2_storage, 565248, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 1792, 1792, 1, input1_storage, input1_storage, 0, 1792, 2944, 2944, 1, input2_storage, input2_storage, 0, 192, 2944, 2944, 1, output0_storage, output0_storage, 0, 192, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00946_b519dd94: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 565248));
  return 0;
}
