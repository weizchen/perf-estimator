...
static float input2_storage[921600] __attribute__((aligned(64)));
static float output0_storage[921600] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 1105920, 2);
  fill_f32(input2_storage, 921600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 384, 384, 1, input1_storage, input1_storage, 0, 384, 2880, 2880, 1, input2_storage, input2_storage, 0, 320, 2880, 2880, 1, output0_storage, output0_storage, 0, 320, 2880, 2880, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00898_324375dd: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 921600));
  return 0;
}
