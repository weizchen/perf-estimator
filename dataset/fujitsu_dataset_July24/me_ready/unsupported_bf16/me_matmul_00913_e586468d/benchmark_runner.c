...
static uint16_t input0_storage[253952] __attribute__((aligned(64)));
static uint16_t input1_storage[393216] __attribute__((aligned(64)));
static float input2_storage[6094848] __attribute__((aligned(64)));
static float output0_storage[6094848] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 253952, 1);
  fill_bf16(input1_storage, 393216, 2);
  fill_f32(input2_storage, 6094848, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1984, 128, 128, 1, input1_storage, input1_storage, 0, 128, 3072, 3072, 1, input2_storage, input2_storage, 0, 1984, 3072, 3072, 1, output0_storage, output0_storage, 0, 1984, 3072, 3072, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00913_e586468d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 6094848));
  return 0;
}
