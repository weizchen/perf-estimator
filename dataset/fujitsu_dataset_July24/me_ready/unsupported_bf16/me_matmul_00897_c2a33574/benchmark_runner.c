...
static float input2_storage[315392] __attribute__((aligned(64)));
static float output0_storage[315392] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 540672, 1);
  fill_bf16(input1_storage, 344064, 2);
  fill_f32(input2_storage, 315392, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 768, 768, 1, input1_storage, input1_storage, 0, 768, 448, 448, 1, input2_storage, input2_storage, 0, 704, 448, 448, 1, output0_storage, output0_storage, 0, 704, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00897_c2a33574: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 315392));
  return 0;
}
