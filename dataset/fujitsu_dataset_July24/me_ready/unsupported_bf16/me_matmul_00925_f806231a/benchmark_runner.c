...
static uint16_t input1_storage[696320] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 40960, 1);
  fill_bf16(input1_storage, 696320, 2);
  fill_f32(input2_storage, 278528, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 320, 320, 1, input1_storage, input1_storage, 0, 320, 2176, 2176, 1, input2_storage, input2_storage, 0, 128, 2176, 2176, 1, output0_storage, output0_storage, 0, 128, 2176, 2176, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00925_f806231a: benchmark completed");
