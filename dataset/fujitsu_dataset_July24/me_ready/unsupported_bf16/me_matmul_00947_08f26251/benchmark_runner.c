...
static uint16_t input1_storage[4833280] __attribute__((aligned(64)));
static float input2_storage[81920] __attribute__((aligned(64)));
static float output0_storage[81920] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 241664, 1);
  fill_bf16(input1_storage, 4833280, 2);
  fill_f32(input2_storage, 81920, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3776, 3776, 1, input1_storage, input1_storage, 0, 3776, 1280, 1280, 1, input2_storage, input2_storage, 0, 64, 1280, 1280, 1, output0_storage, output0_storage, 0, 64, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00947_08f26251: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 81920));
  return 0;
}
