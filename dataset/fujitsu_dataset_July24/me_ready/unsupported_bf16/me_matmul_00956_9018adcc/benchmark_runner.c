...
static float input2_storage[1638400] __attribute__((aligned(64)));
static float output0_storage[1638400] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 491520, 1);
  fill_bf16(input1_storage, 122880, 2);
  fill_f32(input2_storage, 1638400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2560, 192, 192, 1, input1_storage, input1_storage, 0, 192, 640, 640, 1, input2_storage, input2_storage, 0, 2560, 640, 640, 1, output0_storage, output0_storage, 0, 2560, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00956_9018adcc: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1638400));
  return 0;
}
