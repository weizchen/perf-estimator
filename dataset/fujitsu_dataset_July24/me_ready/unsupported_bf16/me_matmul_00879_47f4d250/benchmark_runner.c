...
static uint16_t input0_storage[6389760] __attribute__((aligned(64)));
static uint16_t input1_storage[2555904] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 6389760, 1);
  fill_bf16(input1_storage, 2555904, 2);
  fill_f32(input2_storage, 1474560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 3328, 3328, 1, input1_storage, input1_storage, 0, 3328, 768, 768, 1, input2_storage, input2_storage, 0, 1920, 768, 768, 1, output0_storage, output0_storage, 0, 1920, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00879_47f4d250: benchmark completed");
