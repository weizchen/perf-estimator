...
static uint16_t input1_storage[4669440] __attribute__((aligned(64)));
static float input2_storage[8716288] __attribute__((aligned(64)));
static float output0_storage[8716288] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 6881280, 1);
  fill_bf16(input1_storage, 4669440, 2);
  fill_f32(input2_storage, 8716288, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3584, 1920, 1920, 1, input1_storage, input1_storage, 0, 1920, 2432, 2432, 1, input2_storage, input2_storage, 0, 3584, 2432, 2432, 1, output0_storage, output0_storage, 0, 3584, 2432, 2432, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00937_8d183600: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8716288));
  return 0;
}
