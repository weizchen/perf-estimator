...
static uint16_t input1_storage[1024000] __attribute__((aligned(64)));
static float input2_storage[368640] __attribute__((aligned(64)));
static float output0_storage[368640] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 3686400, 1);
  fill_bf16(input1_storage, 1024000, 2);
  fill_f32(input2_storage, 368640, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1152, 3200, 3200, 1, input1_storage, input1_storage, 0, 3200, 320, 320, 1, input2_storage, input2_storage, 0, 1152, 320, 320, 1, output0_storage, output0_storage, 0, 1152, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00884_3694ae5d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 368640));
  return 0;
}
