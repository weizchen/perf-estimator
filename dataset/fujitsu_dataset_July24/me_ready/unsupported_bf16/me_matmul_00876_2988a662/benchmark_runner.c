...
static uint16_t input0_storage[774144] __attribute__((aligned(64)));
static uint16_t input1_storage[2101248] __attribute__((aligned(64)));
static float input2_storage[544768] __attribute__((aligned(64)));
static float output0_storage[544768] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 774144, 1);
  fill_bf16(input1_storage, 2101248, 2);
  fill_f32(input2_storage, 544768, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 1216, 1216, 1, input2_storage, input2_storage, 0, 448, 1216, 1216, 1, output0_storage, output0_storage, 0, 448, 1216, 1216, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00876_2988a662: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 544768));
  return 0;
}
