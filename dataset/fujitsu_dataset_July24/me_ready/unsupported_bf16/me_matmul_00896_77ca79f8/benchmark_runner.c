...
static uint16_t input0_storage[14475264] __attribute__((aligned(64)));
static uint16_t input1_storage[8126464] __attribute__((aligned(64)));
static float input2_storage[7471104] __attribute__((aligned(64)));
static float output0_storage[7471104] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 14475264, 1);
  fill_bf16(input1_storage, 8126464, 2);
  fill_f32(input2_storage, 7471104, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3648, 3968, 3968, 1, input1_storage, input1_storage, 0, 3968, 2048, 2048, 1, input2_storage, input2_storage, 0, 3648, 2048, 2048, 1, output0_storage, output0_storage, 0, 3648, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00896_77ca79f8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7471104));
  return 0;
}
