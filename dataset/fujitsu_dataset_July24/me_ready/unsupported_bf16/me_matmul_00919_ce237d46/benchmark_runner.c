...
int main(void) {
  fill_bf16(input0_storage, 8192, 1);
  fill_bf16(input1_storage, 417792, 2);
  fill_f32(input2_storage, 208896, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 128, 128, 1, input1_storage, input1_storage, 0, 128, 3264, 3264, 1, input2_storage, input2_storage, 0, 64, 3264, 3264, 1, output0_storage, output0_storage, 0, 64, 3264, 3264, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00919_ce237d46: benchmark completed");
