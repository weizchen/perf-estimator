...
static uint16_t input0_storage[4063232] __attribute__((aligned(64)));
static uint16_t input1_storage[1310720] __attribute__((aligned(64)));
static float input2_storage[5079040] __attribute__((aligned(64)));
static float output0_storage[5079040] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 4063232, 1);
  fill_bf16(input1_storage, 1310720, 2);
  fill_f32(input2_storage, 5079040, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 1280, 1280, 1, input2_storage, input2_storage, 0, 3968, 1280, 1280, 1, output0_storage, output0_storage, 0, 3968, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00883_e550cb48: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5079040));
  return 0;
}
