...
static uint16_t input0_storage[3862528] __attribute__((aligned(64)));
static uint16_t input1_storage[1175552] __attribute__((aligned(64)));
static float input2_storage[659456] __attribute__((aligned(64)));
static float output0_storage[659456] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 3862528, 1);
  fill_bf16(input1_storage, 1175552, 2);
  fill_f32(input2_storage, 659456, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1472, 2624, 2624, 1, input1_storage, input1_storage, 0, 2624, 448, 448, 1, input2_storage, input2_storage, 0, 1472, 448, 448, 1, output0_storage, output0_storage, 0, 1472, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00958_142e58ef: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 659456));
  return 0;
}
