...
static uint16_t input0_storage[5529600] __attribute__((aligned(64)));
static uint16_t input1_storage[6246400] __attribute__((aligned(64)));
static float input2_storage[13492224] __attribute__((aligned(64)));
static float output0_storage[13492224] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 5529600, 1);
  fill_bf16(input1_storage, 6246400, 2);
  fill_f32(input2_storage, 13492224, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3456, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 3904, 3904, 1, input2_storage, input2_storage, 0, 3456, 3904, 3904, 1, output0_storage, output0_storage, 0, 3456, 3904, 3904, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00943_efa01669: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 13492224));
  return 0;
}
