...
static uint16_t input0_storage[1130496] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1130496, 1);
  fill_bf16(input1_storage, 147456, 2);
  fill_f32(input2_storage, 1130496, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2944, 384, 384, 1, input1_storage, input1_storage, 0, 384, 384, 384, 1, input2_storage, input2_storage, 0, 2944, 384, 384, 1, output0_storage, output0_storage, 0, 2944, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00906_569958d5: benchmark completed");
