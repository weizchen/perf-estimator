...
static uint16_t input0_storage[6021120] __attribute__((aligned(64)));
static uint16_t input1_storage[1204224] __attribute__((aligned(64)));
static float input2_storage[737280] __attribute__((aligned(64)));
static float output0_storage[737280] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 6021120, 1);
  fill_bf16(input1_storage, 1204224, 2);
  fill_f32(input2_storage, 737280, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 384, 384, 1, input2_storage, input2_storage, 0, 1920, 384, 384, 1, output0_storage, output0_storage, 0, 1920, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00922_5af97e96: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 737280));
  return 0;
}
