...
static uint16_t input0_storage[1519616] __attribute__((aligned(64)));
static uint16_t input1_storage[1302528] __attribute__((aligned(64)));
static float input2_storage[172032] __attribute__((aligned(64)));
static float output0_storage[172032] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1519616, 1);
  fill_bf16(input1_storage, 1302528, 2);
  fill_f32(input2_storage, 172032, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 3392, 3392, 1, input1_storage, input1_storage, 0, 3392, 384, 384, 1, input2_storage, input2_storage, 0, 448, 384, 384, 1, output0_storage, output0_storage, 0, 448, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00881_a39a3469: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 172032));
  return 0;
}
