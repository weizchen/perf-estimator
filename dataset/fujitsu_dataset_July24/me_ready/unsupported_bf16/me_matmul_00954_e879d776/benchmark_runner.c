...
static uint16_t input0_storage[2019328] __attribute__((aligned(64)));
static uint16_t input1_storage[208896] __attribute__((aligned(64)));
static float input2_storage[356352] __attribute__((aligned(64)));
static float output0_storage[356352] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 2019328, 1);
  fill_bf16(input1_storage, 208896, 2);
  fill_f32(input2_storage, 356352, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1856, 1088, 1088, 1, input1_storage, input1_storage, 0, 1088, 192, 192, 1, input2_storage, input2_storage, 0, 1856, 192, 192, 1, output0_storage, output0_storage, 0, 1856, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00954_e879d776: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 356352));
  return 0;
}
