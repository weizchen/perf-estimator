...
static uint16_t input1_storage[3747840] __attribute__((aligned(64)));
static float input2_storage[499712] __attribute__((aligned(64)));
static float output0_storage[499712] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 3747840, 2);
  fill_f32(input2_storage, 499712, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 960, 960, 1, input1_storage, input1_storage, 0, 960, 3904, 3904, 1, input2_storage, input2_storage, 0, 128, 3904, 3904, 1, output0_storage, output0_storage, 0, 128, 3904, 3904, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00905_d0aa61d0: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 499712));
  return 0;
}
