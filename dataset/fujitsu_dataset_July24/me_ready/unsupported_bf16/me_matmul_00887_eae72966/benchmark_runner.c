...
static uint16_t input1_storage[1703936] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 1703936, 2);
  fill_f32(input2_storage, 131072, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 832, 832, 1, input1_storage, input1_storage, 0, 832, 2048, 2048, 1, input2_storage, input2_storage, 0, 64, 2048, 2048, 1, output0_storage, output0_storage, 0, 64, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00887_eae72966: benchmark completed");
