...
static uint16_t input0_storage[7077888] __attribute__((aligned(64)));
static uint16_t input1_storage[2801664] __attribute__((aligned(64)));
static float input2_storage[3735552] __attribute__((aligned(64)));
static float output0_storage[3735552] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 7077888, 1);
  fill_bf16(input1_storage, 2801664, 2);
  fill_f32(input2_storage, 3735552, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3072, 2304, 2304, 1, input1_storage, input1_storage, 0, 2304, 1216, 1216, 1, input2_storage, input2_storage, 0, 3072, 1216, 1216, 1, output0_storage, output0_storage, 0, 3072, 1216, 1216, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00950_8c2636b2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3735552));
  return 0;
}
