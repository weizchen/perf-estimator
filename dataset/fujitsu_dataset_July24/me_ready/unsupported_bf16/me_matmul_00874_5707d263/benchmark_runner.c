...
static float input2_storage[479232] __attribute__((aligned(64)));
static float output0_storage[479232] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 24576, 1);
  fill_bf16(input1_storage, 319488, 2);
  fill_f32(input2_storage, 479232, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2496, 2496, 1, input2_storage, input2_storage, 0, 192, 2496, 2496, 1, output0_storage, output0_storage, 0, 192, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00874_5707d263: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 479232));
  return 0;
}
