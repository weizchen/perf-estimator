...
static float input2_storage[114688] __attribute__((aligned(64)));
static float output0_storage[114688] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 458752, 1);
  fill_bf16(input1_storage, 3211264, 2);
  fill_f32(input2_storage, 114688, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3584, 3584, 1, input1_storage, input1_storage, 0, 3584, 896, 896, 1, input2_storage, input2_storage, 0, 128, 896, 896, 1, output0_storage, output0_storage, 0, 128, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00957_c8d7244e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 114688));
  return 0;
}
