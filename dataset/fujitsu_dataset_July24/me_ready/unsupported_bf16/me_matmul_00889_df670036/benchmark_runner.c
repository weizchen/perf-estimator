...
static float input2_storage[77824] __attribute__((aligned(64)));
static float output0_storage[77824] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1400832, 1);
  fill_bf16(input1_storage, 73728, 2);
  fill_f32(input2_storage, 77824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 1152, 1152, 1, input1_storage, input1_storage, 0, 1152, 64, 64, 1, input2_storage, input2_storage, 0, 1216, 64, 64, 1, output0_storage, output0_storage, 0, 1216, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00889_df670036: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 77824));
  return 0;
}
