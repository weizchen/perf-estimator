...
static uint16_t input0_storage[5419008] __attribute__((aligned(64)));
static uint16_t input1_storage[10838016] __attribute__((aligned(64)));
static float input2_storage[3612672] __attribute__((aligned(64)));
static float output0_storage[3612672] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 5419008, 1);
  fill_bf16(input1_storage, 10838016, 2);
  fill_f32(input2_storage, 3612672, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1344, 4032, 4032, 1, input1_storage, input1_storage, 0, 4032, 2688, 2688, 1, input2_storage, input2_storage, 0, 1344, 2688, 2688, 1, output0_storage, output0_storage, 0, 1344, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00928_d3f8d1e8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3612672));
  return 0;
}
