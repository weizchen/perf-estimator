...
static uint16_t input2_storage[1900544] __attribute__((aligned(64)));
static uint16_t output0_storage[1900544] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 950272, 1);
  fill_f16(input1_storage, 131072, 2);
  fill_f16(input2_storage, 1900544, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3712, 256, 256, 1, input1_storage, input1_storage, 0, 256, 512, 512, 1, input2_storage, input2_storage, 0, 3712, 512, 512, 1, output0_storage, output0_storage, 0, 3712, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00709_11411451: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1900544));
  return 0;
}
