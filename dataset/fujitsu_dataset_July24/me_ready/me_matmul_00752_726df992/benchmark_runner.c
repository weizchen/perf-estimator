...
static uint16_t input0_storage[9420800] __attribute__((aligned(64)));
static uint16_t input1_storage[1695744] __attribute__((aligned(64)));
static uint16_t input2_storage[1843200] __attribute__((aligned(64)));
static uint16_t output0_storage[1843200] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9420800, 1);
  fill_f16(input1_storage, 1695744, 2);
  fill_f16(input2_storage, 1843200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3200, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 576, 576, 1, input2_storage, input2_storage, 0, 3200, 576, 576, 1, output0_storage, output0_storage, 0, 3200, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00752_726df992: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1843200));
  return 0;
}
