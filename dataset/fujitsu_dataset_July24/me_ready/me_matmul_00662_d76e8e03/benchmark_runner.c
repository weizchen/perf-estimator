...
static uint16_t input0_storage[360448] __attribute__((aligned(64)));
static uint16_t input1_storage[2252800] __attribute__((aligned(64)));
static uint16_t input2_storage[1638400] __attribute__((aligned(64)));
static uint16_t output0_storage[1638400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 360448, 1);
  fill_f16(input1_storage, 2252800, 2);
  fill_f16(input2_storage, 1638400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 704, 704, 1, input1_storage, input1_storage, 0, 704, 3200, 3200, 1, input2_storage, input2_storage, 0, 512, 3200, 3200, 1, output0_storage, output0_storage, 0, 512, 3200, 3200, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00662_d76e8e03: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1638400));
  return 0;
}
