...
static uint8_t input1_storage[1179648] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 1179648, 2);
  fill_f32(input2_storage, 131072, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1152, 1152, 1, input1_storage, input1_storage, 0, 1152, 1024, 1024, 1, input2_storage, input2_storage, 0, 128, 1024, 1024, 1, output0_storage, output0_storage, 0, 128, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00039_3b992751: benchmark completed");
