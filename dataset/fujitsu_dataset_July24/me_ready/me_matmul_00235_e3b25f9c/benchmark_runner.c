...
static uint8_t input0_storage[10240000] __attribute__((aligned(64)));
static uint8_t input1_storage[2867200] __attribute__((aligned(64)));
static float input2_storage[2867200] __attribute__((aligned(64)));
static float output0_storage[2867200] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 10240000, 1);
  fill_f8(input1_storage, 2867200, 2);
  fill_f32(input2_storage, 2867200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3200, 3200, 3200, 1, input1_storage, input1_storage, 0, 3200, 896, 896, 1, input2_storage, input2_storage, 0, 3200, 896, 896, 1, output0_storage, output0_storage, 0, 3200, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00235_e3b25f9c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2867200));
  return 0;
}
