...
static uint8_t input0_storage[65536] __attribute__((aligned(64)));
static uint8_t input1_storage[376832] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 65536, 1);
  fill_f8(input1_storage, 376832, 2);
  fill_f32(input2_storage, 1507328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2944, 2944, 1, input2_storage, input2_storage, 0, 512, 2944, 2944, 1, output0_storage, output0_storage, 0, 512, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00096_1cc18baf: benchmark completed");
