...
static uint16_t input1_storage[2498560] __attribute__((aligned(64)));
static float input2_storage[2248704] __attribute__((aligned(64)));
static float output0_storage[2248704] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 2498560, 2);
  fill_f32(input2_storage, 2248704, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 640, 640, 1, input1_storage, input1_storage, 0, 640, 3904, 3904, 1, input2_storage, input2_storage, 0, 576, 3904, 3904, 1, output0_storage, output0_storage, 0, 576, 3904, 3904, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00335_e1e9faf6: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2248704));
  return 0;
}
