...
static uint16_t input0_storage[2715648] __attribute__((aligned(64)));
static uint16_t input1_storage[1462272] __attribute__((aligned(64)));
static float input2_storage[372736] __attribute__((aligned(64)));
static float output0_storage[372736] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2715648, 1);
  fill_f16(input1_storage, 1462272, 2);
  fill_f32(input2_storage, 372736, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 3264, 3264, 1, input1_storage, input1_storage, 0, 3264, 448, 448, 1, input2_storage, input2_storage, 0, 832, 448, 448, 1, output0_storage, output0_storage, 0, 832, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00405_ec5f84d9: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 372736));
  return 0;
}
