...
static uint16_t input1_storage[4710400] __attribute__((aligned(64)));
static float input2_storage[1507328] __attribute__((aligned(64)));
static float output0_storage[1507328] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 4710400, 2);
  fill_f32(input2_storage, 1507328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 2944, 2944, 1, input2_storage, input2_storage, 0, 512, 2944, 2944, 1, output0_storage, output0_storage, 0, 512, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00461_a6615058: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1507328));
  return 0;
}
