...
static uint8_t input0_storage[3014656] __attribute__((aligned(64)));
static uint8_t input1_storage[2260992] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 3014656, 1);
  fill_f8(input1_storage, 2260992, 2);
  fill_f32(input2_storage, 786432, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 768, 768, 1, input2_storage, input2_storage, 0, 1024, 768, 768, 1, output0_storage, output0_storage, 0, 1024, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00003_c0ff4ced: benchmark completed");
