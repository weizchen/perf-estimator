...
static uint16_t input0_storage[466944] __attribute__((aligned(64)));
static uint16_t input1_storage[303104] __attribute__((aligned(64)));
static uint16_t input2_storage[8638464] __attribute__((aligned(64)));
static uint16_t output0_storage[8638464] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 466944, 1);
  fill_f16(input1_storage, 303104, 2);
  fill_f16(input2_storage, 8638464, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3648, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2368, 2368, 1, input2_storage, input2_storage, 0, 3648, 2368, 2368, 1, output0_storage, output0_storage, 0, 3648, 2368, 2368, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00758_ce80a020: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 8638464));
  return 0;
}
