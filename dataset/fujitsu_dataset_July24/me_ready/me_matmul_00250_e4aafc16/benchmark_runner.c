...
static uint8_t input0_storage[917504] __attribute__((aligned(64)));
static uint8_t input1_storage[1572864] __attribute__((aligned(64)));
static float input2_storage[5505024] __attribute__((aligned(64)));
static float output0_storage[5505024] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 917504, 1);
  fill_f8(input1_storage, 1572864, 2);
  fill_f32(input2_storage, 5505024, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 512, 512, 1, input1_storage, input1_storage, 0, 512, 3072, 3072, 1, input2_storage, input2_storage, 0, 1792, 3072, 3072, 1, output0_storage, output0_storage, 0, 1792, 3072, 3072, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00250_e4aafc16: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5505024));
  return 0;
}
