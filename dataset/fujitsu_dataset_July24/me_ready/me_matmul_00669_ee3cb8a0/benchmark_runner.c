...
static uint16_t input0_storage[712704] __attribute__((aligned(64)));
static uint16_t input1_storage[3088384] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 712704, 1);
  fill_f16(input1_storage, 3088384, 2);
  fill_f16(input2_storage, 159744, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 832, 832, 1, input2_storage, input2_storage, 0, 192, 832, 832, 1, output0_storage, output0_storage, 0, 192, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00669_ee3cb8a0: benchmark completed");
