...
static uint8_t input0_storage[8650752] __attribute__((aligned(64)));
static uint8_t input1_storage[3538944] __attribute__((aligned(64)));
static float input2_storage[3244032] __attribute__((aligned(64)));
static float output0_storage[3244032] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 8650752, 1);
  fill_f8(input1_storage, 3538944, 2);
  fill_f32(input2_storage, 3244032, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2816, 3072, 3072, 1, input1_storage, input1_storage, 0, 3072, 1152, 1152, 1, input2_storage, input2_storage, 0, 2816, 1152, 1152, 1, output0_storage, output0_storage, 0, 2816, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00100_018b4b1d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3244032));
  return 0;
}
