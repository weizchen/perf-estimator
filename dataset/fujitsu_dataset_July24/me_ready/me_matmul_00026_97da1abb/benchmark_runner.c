...
static uint8_t input0_storage[7782400] __attribute__((aligned(64)));
static uint8_t input1_storage[6848512] __attribute__((aligned(64)));
static float input2_storage[9011200] __attribute__((aligned(64)));
static float output0_storage[9011200] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 7782400, 1);
  fill_f8(input1_storage, 6848512, 2);
  fill_f32(input2_storage, 9011200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3200, 2432, 2432, 1, input1_storage, input1_storage, 0, 2432, 2816, 2816, 1, input2_storage, input2_storage, 0, 3200, 2816, 2816, 1, output0_storage, output0_storage, 0, 3200, 2816, 2816, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00026_97da1abb: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9011200));
  return 0;
}
