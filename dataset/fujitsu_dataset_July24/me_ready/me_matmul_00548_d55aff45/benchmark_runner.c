...
static uint16_t input0_storage[9994240] __attribute__((aligned(64)));
static uint16_t input1_storage[655360] __attribute__((aligned(64)));
static uint16_t input2_storage[999424] __attribute__((aligned(64)));
static uint16_t output0_storage[999424] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9994240, 1);
  fill_f16(input1_storage, 655360, 2);
  fill_f16(input2_storage, 999424, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3904, 2560, 2560, 1, input1_storage, input1_storage, 0, 2560, 256, 256, 1, input2_storage, input2_storage, 0, 3904, 256, 256, 1, output0_storage, output0_storage, 0, 3904, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00548_d55aff45: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 999424));
  return 0;
}
