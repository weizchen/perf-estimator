...
static uint16_t input0_storage[565248] __attribute__((aligned(64)));
static uint16_t input1_storage[368640] __attribute__((aligned(64)));
static uint16_t input2_storage[1413120] __attribute__((aligned(64)));
static uint16_t output0_storage[1413120] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 565248, 1);
  fill_f16(input1_storage, 368640, 2);
  fill_f16(input2_storage, 1413120, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1472, 384, 384, 1, input1_storage, input1_storage, 0, 384, 960, 960, 1, input2_storage, input2_storage, 0, 1472, 960, 960, 1, output0_storage, output0_storage, 0, 1472, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00547_11b7bb06: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1413120));
  return 0;
}
