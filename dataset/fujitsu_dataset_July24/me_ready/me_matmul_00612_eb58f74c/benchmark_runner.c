...
static uint16_t input0_storage[2723840] __attribute__((aligned(64)));
static uint16_t input1_storage[573440] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2723840, 1);
  fill_f16(input1_storage, 573440, 2);
  fill_f16(input2_storage, 311296, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 2240, 2240, 1, input1_storage, input1_storage, 0, 2240, 256, 256, 1, input2_storage, input2_storage, 0, 1216, 256, 256, 1, output0_storage, output0_storage, 0, 1216, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00612_eb58f74c: benchmark completed");
