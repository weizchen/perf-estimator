...
static uint16_t input0_storage[933888] __attribute__((aligned(64)));
static uint16_t input1_storage[1048576] __attribute__((aligned(64)));
static uint16_t input2_storage[14942208] __attribute__((aligned(64)));
static uint16_t output0_storage[14942208] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 933888, 1);
  fill_f16(input1_storage, 1048576, 2);
  fill_f16(input2_storage, 14942208, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3648, 256, 256, 1, input1_storage, input1_storage, 0, 256, 4096, 4096, 1, input2_storage, input2_storage, 0, 3648, 4096, 4096, 1, output0_storage, output0_storage, 0, 3648, 4096, 4096, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00587_48702597: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 14942208));
  return 0;
}
