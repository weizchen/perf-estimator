...
static uint16_t input0_storage[1085440] __attribute__((aligned(64)));
static uint16_t input1_storage[143360] __attribute__((aligned(64)));
static uint16_t input2_storage[1519616] __attribute__((aligned(64)));
static uint16_t output0_storage[1519616] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1085440, 1);
  fill_f16(input1_storage, 143360, 2);
  fill_f16(input2_storage, 1519616, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3392, 320, 320, 1, input1_storage, input1_storage, 0, 320, 448, 448, 1, input2_storage, input2_storage, 0, 3392, 448, 448, 1, output0_storage, output0_storage, 0, 3392, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00553_452969b1: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1519616));
  return 0;
}
