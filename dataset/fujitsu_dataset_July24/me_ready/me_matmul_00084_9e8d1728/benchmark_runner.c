...
static uint8_t input0_storage[11501568] __attribute__((aligned(64)));
static uint8_t input1_storage[9797632] __attribute__((aligned(64)));
static float input2_storage[10174464] __attribute__((aligned(64)));
static float output0_storage[10174464] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 11501568, 1);
  fill_f8(input1_storage, 9797632, 2);
  fill_f32(input2_storage, 10174464, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3456, 3328, 3328, 1, input1_storage, input1_storage, 0, 3328, 2944, 2944, 1, input2_storage, input2_storage, 0, 3456, 2944, 2944, 1, output0_storage, output0_storage, 0, 3456, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00084_9e8d1728: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 10174464));
  return 0;
}
