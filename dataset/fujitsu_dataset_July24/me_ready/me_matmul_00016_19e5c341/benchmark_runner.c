...
static uint8_t input0_storage[950272] __attribute__((aligned(64)));
static uint8_t input1_storage[7602176] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 950272, 1);
  fill_f8(input1_storage, 7602176, 2);
  fill_f32(input2_storage, 524288, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 2048, 2048, 1, input2_storage, input2_storage, 0, 256, 2048, 2048, 1, output0_storage, output0_storage, 0, 256, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00016_19e5c341: benchmark completed");
