...
static uint16_t input1_storage[385024] __attribute__((aligned(64)));
static float input2_storage[10395648] __attribute__((aligned(64)));
static float output0_storage[10395648] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 385024, 2);
  fill_f32(input2_storage, 10395648, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3456, 128, 128, 1, input1_storage, input1_storage, 0, 128, 3008, 3008, 1, input2_storage, input2_storage, 0, 3456, 3008, 3008, 1, output0_storage, output0_storage, 0, 3456, 3008, 3008, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00348_29d5d222: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 10395648));
  return 0;
}
