...
static uint8_t input0_storage[1966080] __attribute__((aligned(64)));
static uint8_t input1_storage[4521984] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 1966080, 1);
  fill_f8(input1_storage, 4521984, 2);
  fill_f32(input2_storage, 3768320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 1536, 1536, 1, input1_storage, input1_storage, 0, 1536, 2944, 2944, 1, input2_storage, input2_storage, 0, 1280, 2944, 2944, 1, output0_storage, output0_storage, 0, 1280, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00086_01cd252e: benchmark completed");
