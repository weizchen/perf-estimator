...
static uint16_t input0_storage[528384] __attribute__((aligned(64)));
static uint16_t input1_storage[5812224] __attribute__((aligned(64)));
static uint16_t input2_storage[405504] __attribute__((aligned(64)));
static uint16_t output0_storage[405504] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 528384, 1);
  fill_f16(input1_storage, 5812224, 2);
  fill_f16(input2_storage, 405504, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 2752, 2752, 1, input1_storage, input1_storage, 0, 2752, 2112, 2112, 1, input2_storage, input2_storage, 0, 192, 2112, 2112, 1, output0_storage, output0_storage, 0, 192, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00543_dbabafe3: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 405504));
  return 0;
}
