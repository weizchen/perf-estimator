...
static uint16_t input0_storage[1327104] __attribute__((aligned(64)));
static uint16_t input1_storage[1130496] __attribute__((aligned(64)));
static float input2_storage[2543616] __attribute__((aligned(64)));
static float output0_storage[2543616] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1327104, 1);
  fill_f16(input1_storage, 1130496, 2);
  fill_f32(input2_storage, 2543616, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1728, 768, 768, 1, input1_storage, input1_storage, 0, 768, 1472, 1472, 1, input2_storage, input2_storage, 0, 1728, 1472, 1472, 1, output0_storage, output0_storage, 0, 1728, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00332_23292a0a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2543616));
  return 0;
}
