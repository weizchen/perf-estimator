...
static uint16_t input0_storage[9510912] __attribute__((aligned(64)));
static uint16_t input1_storage[221184] __attribute__((aligned(64)));
static uint16_t input2_storage[176128] __attribute__((aligned(64)));
static uint16_t output0_storage[176128] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 9510912, 1);
  fill_f16(input1_storage, 221184, 2);
  fill_f16(input2_storage, 176128, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 3456, 3456, 1, input1_storage, input1_storage, 0, 3456, 64, 64, 1, input2_storage, input2_storage, 0, 2752, 64, 64, 1, output0_storage, output0_storage, 0, 2752, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00609_9f8f6258: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 176128));
  return 0;
}
