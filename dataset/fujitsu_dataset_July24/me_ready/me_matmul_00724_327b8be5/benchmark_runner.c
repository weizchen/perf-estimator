...
static uint16_t input0_storage[2613248] __attribute__((aligned(64)));
static uint16_t input1_storage[2613248] __attribute__((aligned(64)));
static uint16_t input2_storage[495616] __attribute__((aligned(64)));
static uint16_t output0_storage[495616] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2613248, 1);
  fill_f16(input1_storage, 2613248, 2);
  fill_f16(input2_storage, 495616, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 704, 704, 1, input2_storage, input2_storage, 0, 704, 704, 704, 1, output0_storage, output0_storage, 0, 704, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00724_327b8be5: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 495616));
  return 0;
}
