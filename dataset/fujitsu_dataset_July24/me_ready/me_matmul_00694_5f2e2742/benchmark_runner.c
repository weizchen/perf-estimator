...
static uint16_t input0_storage[307200] __attribute__((aligned(64)));
static uint16_t input1_storage[3276800] __attribute__((aligned(64)));
static uint16_t input2_storage[393216] __attribute__((aligned(64)));
static uint16_t output0_storage[393216] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 307200, 1);
  fill_f16(input1_storage, 3276800, 2);
  fill_f16(input2_storage, 393216, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 2048, 2048, 1, input2_storage, input2_storage, 0, 192, 2048, 2048, 1, output0_storage, output0_storage, 0, 192, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00694_5f2e2742: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 393216));
  return 0;
}
