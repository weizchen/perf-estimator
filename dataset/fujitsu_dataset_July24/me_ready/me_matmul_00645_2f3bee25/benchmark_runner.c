...
static uint16_t input0_storage[819200] __attribute__((aligned(64)));
static uint16_t input1_storage[1015808] __attribute__((aligned(64)));
static uint16_t input2_storage[3174400] __attribute__((aligned(64)));
static uint16_t output0_storage[3174400] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 819200, 1);
  fill_f16(input1_storage, 1015808, 2);
  fill_f16(input2_storage, 3174400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 512, 512, 1, input1_storage, input1_storage, 0, 512, 1984, 1984, 1, input2_storage, input2_storage, 0, 1600, 1984, 1984, 1, output0_storage, output0_storage, 0, 1600, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00645_2f3bee25: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3174400));
  return 0;
}
