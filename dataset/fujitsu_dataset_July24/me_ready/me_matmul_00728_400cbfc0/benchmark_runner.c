...
static uint16_t input0_storage[6062080] __attribute__((aligned(64)));
static uint16_t input1_storage[8192000] __attribute__((aligned(64)));
static uint16_t input2_storage[7577600] __attribute__((aligned(64)));
static uint16_t output0_storage[7577600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 6062080, 1);
  fill_f16(input1_storage, 8192000, 2);
  fill_f16(input2_storage, 7577600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2368, 2560, 2560, 1, input1_storage, input1_storage, 0, 2560, 3200, 3200, 1, input2_storage, input2_storage, 0, 2368, 3200, 3200, 1, output0_storage, output0_storage, 0, 2368, 3200, 3200, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00728_400cbfc0: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 7577600));
  return 0;
}
