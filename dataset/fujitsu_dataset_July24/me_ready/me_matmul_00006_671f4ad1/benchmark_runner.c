...
static uint8_t input0_storage[6406144] __attribute__((aligned(64)));
static uint8_t input1_storage[1130496] __attribute__((aligned(64)));
static float input2_storage[835584] __attribute__((aligned(64)));
static float output0_storage[835584] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 6406144, 1);
  fill_f8(input1_storage, 1130496, 2);
  fill_f32(input2_storage, 835584, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 384, 384, 1, input2_storage, input2_storage, 0, 2176, 384, 384, 1, output0_storage, output0_storage, 0, 2176, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00006_671f4ad1: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 835584));
  return 0;
}
