...
static uint8_t input1_storage[11304960] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 11304960, 2);
  fill_f32(input2_storage, 983040, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 3840, 3840, 1, input2_storage, input2_storage, 0, 256, 3840, 3840, 1, output0_storage, output0_storage, 0, 256, 3840, 3840, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00005_067aae1d: benchmark completed");
