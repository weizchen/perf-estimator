...
static uint8_t input0_storage[2064384] __attribute__((aligned(64)));
static uint8_t input1_storage[1032192] __attribute__((aligned(64)));
static float input2_storage[1605632] __attribute__((aligned(64)));
static float output0_storage[1605632] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 2064384, 1);
  fill_f8(input1_storage, 1032192, 2);
  fill_f32(input2_storage, 1605632, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 1152, 1152, 1, input1_storage, input1_storage, 0, 1152, 896, 896, 1, input2_storage, input2_storage, 0, 1792, 896, 896, 1, output0_storage, output0_storage, 0, 1792, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00193_3fc4a16a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1605632));
  return 0;
}
