...
static uint8_t input0_storage[4980736] __attribute__((aligned(64)));
static uint8_t input1_storage[2621440] __attribute__((aligned(64)));
static float input2_storage[3112960] __attribute__((aligned(64)));
static float output0_storage[3112960] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 4980736, 1);
  fill_f8(input1_storage, 2621440, 2);
  fill_f32(input2_storage, 3112960, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2432, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 1280, 1280, 1, input2_storage, input2_storage, 0, 2432, 1280, 1280, 1, output0_storage, output0_storage, 0, 2432, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00133_e3eebd50: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3112960));
  return 0;
}
