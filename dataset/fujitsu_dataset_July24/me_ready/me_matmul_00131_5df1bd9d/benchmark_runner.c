...
static uint8_t input0_storage[2228224] __attribute__((aligned(64)));
static uint8_t input1_storage[1392640] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 2228224, 1);
  fill_f8(input1_storage, 1392640, 2);
  fill_f32(input2_storage, 655360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 640, 640, 1, input2_storage, input2_storage, 0, 1024, 640, 640, 1, output0_storage, output0_storage, 0, 1024, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00131_5df1bd9d: benchmark completed");
