...
static uint8_t input0_storage[1802240] __attribute__((aligned(64)));
static uint8_t input1_storage[409600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 1802240, 1);
  fill_f8(input1_storage, 409600, 2);
  fill_f32(input2_storage, 1802240, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2816, 640, 640, 1, input1_storage, input1_storage, 0, 640, 640, 640, 1, input2_storage, input2_storage, 0, 2816, 640, 640, 1, output0_storage, output0_storage, 0, 2816, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00223_78d8a8eb: benchmark completed");
