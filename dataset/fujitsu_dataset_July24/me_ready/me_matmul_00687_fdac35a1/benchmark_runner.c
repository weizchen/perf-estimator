...
static uint16_t input0_storage[389120] __attribute__((aligned(64)));
static uint16_t input1_storage[839680] __attribute__((aligned(64)));
static uint16_t input2_storage[3190784] __attribute__((aligned(64)));
static uint16_t output0_storage[3190784] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 389120, 1);
  fill_f16(input1_storage, 839680, 2);
  fill_f16(input2_storage, 3190784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 320, 320, 1, input1_storage, input1_storage, 0, 320, 2624, 2624, 1, input2_storage, input2_storage, 0, 1216, 2624, 2624, 1, output0_storage, output0_storage, 0, 1216, 2624, 2624, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00687_fdac35a1: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3190784));
  return 0;
}
