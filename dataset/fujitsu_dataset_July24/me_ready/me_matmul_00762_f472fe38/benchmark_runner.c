...
static uint16_t input0_storage[4091904] __attribute__((aligned(64)));
static uint16_t input1_storage[2322432] __attribute__((aligned(64)));
static uint16_t input2_storage[3182592] __attribute__((aligned(64)));
static uint16_t output0_storage[3182592] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4091904, 1);
  fill_f16(input1_storage, 2322432, 2);
  fill_f16(input2_storage, 3182592, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2368, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 1344, 1344, 1, input2_storage, input2_storage, 0, 2368, 1344, 1344, 1, output0_storage, output0_storage, 0, 2368, 1344, 1344, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00762_f472fe38: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3182592));
  return 0;
}
