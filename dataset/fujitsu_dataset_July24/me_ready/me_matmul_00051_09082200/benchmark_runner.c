...
static uint8_t input0_storage[3112960] __attribute__((aligned(64)));
static uint8_t input1_storage[933888] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 3112960, 1);
  fill_f8(input1_storage, 933888, 2);
  fill_f32(input2_storage, 491520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 2432, 2432, 1, input1_storage, input1_storage, 0, 2432, 384, 384, 1, input2_storage, input2_storage, 0, 1280, 384, 384, 1, output0_storage, output0_storage, 0, 1280, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00051_09082200: benchmark completed");
