...
static uint16_t input0_storage[2695168] __attribute__((aligned(64)));
static uint16_t input1_storage[2867200] __attribute__((aligned(64)));
static float input2_storage[9625600] __attribute__((aligned(64)));
static float output0_storage[9625600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2695168, 1);
  fill_f16(input1_storage, 2867200, 2);
  fill_f32(input2_storage, 9625600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3008, 896, 896, 1, input1_storage, input1_storage, 0, 896, 3200, 3200, 1, input2_storage, input2_storage, 0, 3008, 3200, 3200, 1, output0_storage, output0_storage, 0, 3008, 3200, 3200, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00349_46001dc4: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9625600));
  return 0;
}
