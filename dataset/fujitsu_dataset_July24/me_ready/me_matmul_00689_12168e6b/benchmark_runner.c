...
static uint16_t input0_storage[5111808] __attribute__((aligned(64)));
static uint16_t input1_storage[319488] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5111808, 1);
  fill_f16(input1_storage, 319488, 2);
  fill_f16(input2_storage, 262144, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2048, 2496, 2496, 1, input1_storage, input1_storage, 0, 2496, 128, 128, 1, input2_storage, input2_storage, 0, 2048, 128, 128, 1, output0_storage, output0_storage, 0, 2048, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00689_12168e6b: benchmark completed");
