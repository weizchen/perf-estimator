...
static uint16_t input0_storage[901120] __attribute__((aligned(64)));
static uint16_t input1_storage[1392640] __attribute__((aligned(64)));
static float input2_storage[765952] __attribute__((aligned(64)));
static float output0_storage[765952] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 901120, 1);
  fill_f16(input1_storage, 1392640, 2);
  fill_f32(input2_storage, 765952, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 1088, 1088, 1, input2_storage, input2_storage, 0, 704, 1088, 1088, 1, output0_storage, output0_storage, 0, 704, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00319_0a7ce50c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 765952));
  return 0;
}
