...
static uint16_t input0_storage[249856] __attribute__((aligned(64)));
static uint16_t input1_storage[8744960] __attribute__((aligned(64)));
static uint16_t input2_storage[143360] __attribute__((aligned(64)));
static uint16_t output0_storage[143360] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 249856, 1);
  fill_f16(input1_storage, 8744960, 2);
  fill_f16(input2_storage, 143360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3904, 3904, 1, input1_storage, input1_storage, 0, 3904, 2240, 2240, 1, input2_storage, input2_storage, 0, 64, 2240, 2240, 1, output0_storage, output0_storage, 0, 64, 2240, 2240, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00746_8aeb05fd: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 143360));
  return 0;
}
