...
static uint16_t input0_storage[630784] __attribute__((aligned(64)));
static uint16_t input1_storage[1118208] __attribute__((aligned(64)));
static float input2_storage[3514368] __attribute__((aligned(64)));
static float output0_storage[3514368] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 630784, 1);
  fill_f16(input1_storage, 1118208, 2);
  fill_f32(input2_storage, 3514368, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 448, 448, 1, input1_storage, input1_storage, 0, 448, 2496, 2496, 1, input2_storage, input2_storage, 0, 1408, 2496, 2496, 1, output0_storage, output0_storage, 0, 1408, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00489_ca669281: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3514368));
  return 0;
}
