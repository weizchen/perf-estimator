...
static uint16_t input0_storage[2981888] __attribute__((aligned(64)));
static uint16_t input1_storage[6193152] __attribute__((aligned(64)));
static uint16_t input2_storage[5750784] __attribute__((aligned(64)));
static uint16_t output0_storage[5750784] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2981888, 1);
  fill_f16(input1_storage, 6193152, 2);
  fill_f16(input2_storage, 5750784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 1792, 1792, 1, input1_storage, input1_storage, 0, 1792, 3456, 3456, 1, input2_storage, input2_storage, 0, 1664, 3456, 3456, 1, output0_storage, output0_storage, 0, 1664, 3456, 3456, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00537_aae94981: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5750784));
  return 0;
}
