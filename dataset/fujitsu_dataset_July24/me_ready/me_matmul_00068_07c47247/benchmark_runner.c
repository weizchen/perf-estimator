...
static uint8_t input0_storage[573440] __attribute__((aligned(64)));
static uint8_t input1_storage[1605632] __attribute__((aligned(64)));
static float input2_storage[1146880] __attribute__((aligned(64)));
static float output0_storage[1146880] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 573440, 1);
  fill_f8(input1_storage, 1605632, 2);
  fill_f32(input2_storage, 1146880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 896, 896, 1, input1_storage, input1_storage, 0, 896, 1792, 1792, 1, input2_storage, input2_storage, 0, 640, 1792, 1792, 1, output0_storage, output0_storage, 0, 640, 1792, 1792, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00068_07c47247: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1146880));
  return 0;
}
