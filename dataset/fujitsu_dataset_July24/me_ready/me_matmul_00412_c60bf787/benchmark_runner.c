...
static uint16_t input1_storage[126976] __attribute__((aligned(64)));
static float input2_storage[8126464] __attribute__((aligned(64)));
static float output0_storage[8126464] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 126976, 2);
  fill_f32(input2_storage, 8126464, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4096, 64, 64, 1, input1_storage, input1_storage, 0, 64, 1984, 1984, 1, input2_storage, input2_storage, 0, 4096, 1984, 1984, 1, output0_storage, output0_storage, 0, 4096, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00412_c60bf787: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8126464));
  return 0;
}
