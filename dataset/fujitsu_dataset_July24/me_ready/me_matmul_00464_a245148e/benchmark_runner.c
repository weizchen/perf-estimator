...
static uint16_t input0_storage[4317184] __attribute__((aligned(64)));
static uint16_t input1_storage[5431296] __attribute__((aligned(64)));
static float input2_storage[4952064] __attribute__((aligned(64)));
static float output0_storage[4952064] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4317184, 1);
  fill_f16(input1_storage, 5431296, 2);
  fill_f32(input2_storage, 4952064, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1984, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 2496, 2496, 1, input2_storage, input2_storage, 0, 1984, 2496, 2496, 1, output0_storage, output0_storage, 0, 1984, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00464_a245148e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4952064));
  return 0;
}
