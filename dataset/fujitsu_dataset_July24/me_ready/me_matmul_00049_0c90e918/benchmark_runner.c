...
static uint8_t input0_storage[7077888] __attribute__((aligned(64)));
static uint8_t input1_storage[786432] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 7077888, 1);
  fill_f8(input1_storage, 786432, 2);
  fill_f32(input2_storage, 589824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2304, 3072, 3072, 1, input1_storage, input1_storage, 0, 3072, 256, 256, 1, input2_storage, input2_storage, 0, 2304, 256, 256, 1, output0_storage, output0_storage, 0, 2304, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00049_0c90e918: benchmark completed");
