...
static uint8_t input0_storage[1949696] __attribute__((aligned(64)));
static uint8_t input1_storage[3211264] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 1949696, 1);
  fill_f8(input1_storage, 3211264, 2);
  fill_f32(input2_storage, 7798784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 896, 896, 1, input1_storage, input1_storage, 0, 896, 3584, 3584, 1, input2_storage, input2_storage, 0, 2176, 3584, 3584, 1, output0_storage, output0_storage, 0, 2176, 3584, 3584, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00020_4dd3662a: benchmark completed");
