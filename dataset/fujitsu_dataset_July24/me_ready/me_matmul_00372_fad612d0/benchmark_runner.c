...
static uint16_t input0_storage[12845056] __attribute__((aligned(64)));
static uint16_t input1_storage[524288] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 12845056, 1);
  fill_f16(input1_storage, 524288, 2);
  fill_f32(input2_storage, 401408, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3136, 4096, 4096, 1, input1_storage, input1_storage, 0, 4096, 128, 128, 1, input2_storage, input2_storage, 0, 3136, 128, 128, 1, output0_storage, output0_storage, 0, 3136, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00372_fad612d0: benchmark completed");
