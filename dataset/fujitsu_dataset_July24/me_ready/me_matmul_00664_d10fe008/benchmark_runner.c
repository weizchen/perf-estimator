...
static uint16_t input1_storage[61440] __attribute__((aligned(64)));
static uint16_t input2_storage[2580480] __attribute__((aligned(64)));
static uint16_t output0_storage[2580480] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 61440, 2);
  fill_f16(input2_storage, 2580480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2688, 64, 64, 1, input1_storage, input1_storage, 0, 64, 960, 960, 1, input2_storage, input2_storage, 0, 2688, 960, 960, 1, output0_storage, output0_storage, 0, 2688, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00664_d10fe008: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2580480));
  return 0;
}
