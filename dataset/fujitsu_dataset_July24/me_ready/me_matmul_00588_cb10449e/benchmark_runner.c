...
static uint16_t input1_storage[253952] __attribute__((aligned(64)));
static uint16_t input2_storage[4444160] __attribute__((aligned(64)));
static uint16_t output0_storage[4444160] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 253952, 2);
  fill_f16(input2_storage, 4444160, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2240, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1984, 1984, 1, input2_storage, input2_storage, 0, 2240, 1984, 1984, 1, output0_storage, output0_storage, 0, 2240, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00588_cb10449e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 4444160));
  return 0;
}
