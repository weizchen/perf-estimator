...
static uint16_t input0_storage[1703936] __attribute__((aligned(64)));
static uint16_t input1_storage[1802240] __attribute__((aligned(64)));
static float input2_storage[11714560] __attribute__((aligned(64)));
static float output0_storage[11714560] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1703936, 1);
  fill_f16(input1_storage, 1802240, 2);
  fill_f32(input2_storage, 11714560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3328, 512, 512, 1, input1_storage, input1_storage, 0, 512, 3520, 3520, 1, input2_storage, input2_storage, 0, 3328, 3520, 3520, 1, output0_storage, output0_storage, 0, 3328, 3520, 3520, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00451_a4e147ea: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 11714560));
  return 0;
}
