...
static uint16_t input1_storage[7741440] __attribute__((aligned(64)));
static float input2_storage[688128] __attribute__((aligned(64)));
static float output0_storage[688128] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 7741440, 2);
  fill_f32(input2_storage, 688128, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2880, 2880, 1, input1_storage, input1_storage, 0, 2880, 2688, 2688, 1, input2_storage, input2_storage, 0, 256, 2688, 2688, 1, output0_storage, output0_storage, 0, 256, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00320_5ec54f6c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 688128));
  return 0;
}
