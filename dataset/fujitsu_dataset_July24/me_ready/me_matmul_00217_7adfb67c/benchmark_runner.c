...
static uint8_t input0_storage[2342912] __attribute__((aligned(64)));
static uint8_t input1_storage[2129920] __attribute__((aligned(64)));
static float input2_storage[1802240] __attribute__((aligned(64)));
static float output0_storage[1802240] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 2342912, 1);
  fill_f8(input1_storage, 2129920, 2);
  fill_f32(input2_storage, 1802240, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 1664, 1664, 1, input1_storage, input1_storage, 0, 1664, 1280, 1280, 1, input2_storage, input2_storage, 0, 1408, 1280, 1280, 1, output0_storage, output0_storage, 0, 1408, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00217_7adfb67c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1802240));
  return 0;
}
