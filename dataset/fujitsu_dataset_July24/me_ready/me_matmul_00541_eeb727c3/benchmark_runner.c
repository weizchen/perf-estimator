...
static uint16_t input0_storage[745472] __attribute__((aligned(64)));
static uint16_t input1_storage[2236416] __attribute__((aligned(64)));
static uint16_t input2_storage[2408448] __attribute__((aligned(64)));
static uint16_t output0_storage[2408448] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 745472, 1);
  fill_f16(input1_storage, 2236416, 2);
  fill_f16(input2_storage, 2408448, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 832, 832, 1, input1_storage, input1_storage, 0, 832, 2688, 2688, 1, input2_storage, input2_storage, 0, 896, 2688, 2688, 1, output0_storage, output0_storage, 0, 896, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00541_eeb727c3: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2408448));
  return 0;
}
