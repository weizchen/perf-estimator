...
static int8_t input1_storage[819200] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 819200, 2);
  fill_i32(input2_storage, 327680, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 640, 640, 1, input2_storage, input2_storage, 0, 512, 640, 640, 1, output0_storage, output0_storage, 0, 512, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00857_dc18d313: benchmark completed");
