...
static int8_t input0_storage[7667712] __attribute__((aligned(64)));
static int8_t input1_storage[884736] __attribute__((aligned(64)));
static int32_t input2_storage[1277952] __attribute__((aligned(64)));
static int32_t output0_storage[1277952] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 7667712, 1);
  fill_i8(input1_storage, 884736, 2);
  fill_i32(input2_storage, 1277952, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3328, 2304, 2304, 1, input1_storage, input1_storage, 0, 2304, 384, 384, 1, input2_storage, input2_storage, 0, 3328, 384, 384, 1, output0_storage, output0_storage, 0, 3328, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00809_57a85284: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1277952));
  return 0;
}
