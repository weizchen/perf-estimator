...
static int32_t input2_storage[3440640] __attribute__((aligned(64)));
static int32_t output0_storage[3440640] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 3440640, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 1536, 1536, 1, input1_storage, input1_storage, 0, 1536, 2688, 2688, 1, input2_storage, input2_storage, 0, 1280, 2688, 2688, 1, output0_storage, output0_storage, 0, 1280, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00794_11d1ed45: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 3440640));
  return 0;
}
