...
static int8_t input0_storage[1261568] __attribute__((aligned(64)));
static int8_t input1_storage[802816] __attribute__((aligned(64)));
static int32_t input2_storage[1261568] __attribute__((aligned(64)));
static int32_t output0_storage[1261568] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1261568, 1);
  fill_i8(input1_storage, 802816, 2);
  fill_i32(input2_storage, 1261568, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 896, 896, 1, input1_storage, input1_storage, 0, 896, 896, 896, 1, input2_storage, input2_storage, 0, 1408, 896, 896, 1, output0_storage, output0_storage, 0, 1408, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00865_4c6bd2ac: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1261568));
  return 0;
}
