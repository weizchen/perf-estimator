...
static int32_t input2_storage[1507328] __attribute__((aligned(64)));
static int32_t output0_storage[1507328] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 1507328, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2944, 384, 384, 1, input1_storage, input1_storage, 0, 384, 512, 512, 1, input2_storage, input2_storage, 0, 2944, 512, 512, 1, output0_storage, output0_storage, 0, 2944, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00833_2f4729ed: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1507328));
  return 0;
}
