...
static int8_t input0_storage[4423680] __attribute__((aligned(64)));
static int8_t input1_storage[1474560] __attribute__((aligned(64)));
static int32_t input2_storage[3981312] __attribute__((aligned(64)));
static int32_t output0_storage[3981312] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 4423680, 1);
  fill_i8(input1_storage, 1474560, 2);
  fill_i32(input2_storage, 3981312, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3456, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 1152, 1152, 1, input2_storage, input2_storage, 0, 3456, 1152, 1152, 1, output0_storage, output0_storage, 0, 3456, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00817_cd5109ca: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 3981312));
  return 0;
}
