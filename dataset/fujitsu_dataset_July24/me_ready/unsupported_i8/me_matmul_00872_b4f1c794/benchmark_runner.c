...
static int8_t input0_storage[1556480] __attribute__((aligned(64)));
static int8_t input1_storage[4046848] __attribute__((aligned(64)));
static int32_t input2_storage[1064960] __attribute__((aligned(64)));
static int32_t output0_storage[1064960] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1556480, 1);
  fill_i8(input1_storage, 4046848, 2);
  fill_i32(input2_storage, 1064960, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 2432, 2432, 1, input1_storage, input1_storage, 0, 2432, 1664, 1664, 1, input2_storage, input2_storage, 0, 640, 1664, 1664, 1, output0_storage, output0_storage, 0, 640, 1664, 1664, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00872_b4f1c794: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1064960));
  return 0;
}
