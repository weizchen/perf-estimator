...
static int8_t input0_storage[1228800] __attribute__((aligned(64)));
static int8_t input1_storage[245760] __attribute__((aligned(64)));
static int32_t input2_storage[2048000] __attribute__((aligned(64)));
static int32_t output0_storage[2048000] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1228800, 1);
  fill_i8(input1_storage, 245760, 2);
  fill_i32(input2_storage, 2048000, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3200, 384, 384, 1, input1_storage, input1_storage, 0, 384, 640, 640, 1, input2_storage, input2_storage, 0, 3200, 640, 640, 1, output0_storage, output0_storage, 0, 3200, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00831_ace4aa74: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2048000));
  return 0;
}
