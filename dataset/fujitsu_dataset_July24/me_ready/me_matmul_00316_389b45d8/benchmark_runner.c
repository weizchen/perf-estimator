...
static uint16_t input1_storage[966656] __attribute__((aligned(64)));
static float input2_storage[2174976] __attribute__((aligned(64)));
static float output0_storage[2174976] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 966656, 2);
  fill_f32(input2_storage, 2174976, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 256, 256, 1, input1_storage, input1_storage, 0, 256, 3776, 3776, 1, input2_storage, input2_storage, 0, 576, 3776, 3776, 1, output0_storage, output0_storage, 0, 576, 3776, 3776, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00316_389b45d8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2174976));
  return 0;
}
