...
static uint16_t input0_storage[11010048] __attribute__((aligned(64)));
static uint16_t input1_storage[2064384] __attribute__((aligned(64)));
static uint16_t input2_storage[1769472] __attribute__((aligned(64)));
static uint16_t output0_storage[1769472] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 11010048, 1);
  fill_f16(input1_storage, 2064384, 2);
  fill_f16(input2_storage, 1769472, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3072, 3584, 3584, 1, input1_storage, input1_storage, 0, 3584, 576, 576, 1, input2_storage, input2_storage, 0, 3072, 576, 576, 1, output0_storage, output0_storage, 0, 3072, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00531_8c044c3e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1769472));
  return 0;
}
