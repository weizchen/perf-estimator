...
static uint16_t input0_storage[1232896] __attribute__((aligned(64)));
static uint16_t input1_storage[114688] __attribute__((aligned(64)));
static uint16_t input2_storage[704512] __attribute__((aligned(64)));
static uint16_t output0_storage[704512] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1232896, 1);
  fill_f16(input1_storage, 114688, 2);
  fill_f16(input2_storage, 704512, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 448, 448, 1, input1_storage, input1_storage, 0, 448, 256, 256, 1, input2_storage, input2_storage, 0, 2752, 256, 256, 1, output0_storage, output0_storage, 0, 2752, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00707_bf2c4668: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 704512));
  return 0;
}
