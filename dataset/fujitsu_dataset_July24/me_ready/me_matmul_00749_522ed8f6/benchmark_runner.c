...
static uint16_t input0_storage[110592] __attribute__((aligned(64)));
static uint16_t input1_storage[3428352] __attribute__((aligned(64)));
static uint16_t input2_storage[126976] __attribute__((aligned(64)));
static uint16_t output0_storage[126976] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 110592, 1);
  fill_f16(input1_storage, 3428352, 2);
  fill_f16(input2_storage, 126976, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 1984, 1984, 1, input2_storage, input2_storage, 0, 64, 1984, 1984, 1, output0_storage, output0_storage, 0, 64, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00749_522ed8f6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 126976));
  return 0;
}
