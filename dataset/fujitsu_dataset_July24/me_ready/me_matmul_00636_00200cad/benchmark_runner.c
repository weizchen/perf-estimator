...
static uint16_t input0_storage[602112] __attribute__((aligned(64)));
static uint16_t input1_storage[2752512] __attribute__((aligned(64)));
static uint16_t input2_storage[917504] __attribute__((aligned(64)));
static uint16_t output0_storage[917504] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 602112, 1);
  fill_f16(input1_storage, 2752512, 2);
  fill_f16(input2_storage, 917504, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 1344, 1344, 1, input1_storage, input1_storage, 0, 1344, 2048, 2048, 1, input2_storage, input2_storage, 0, 448, 2048, 2048, 1, output0_storage, output0_storage, 0, 448, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00636_00200cad: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 917504));
  return 0;
}
