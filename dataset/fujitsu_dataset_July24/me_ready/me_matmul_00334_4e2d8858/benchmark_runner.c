...
static uint16_t input0_storage[5079040] __attribute__((aligned(64)));
static uint16_t input1_storage[4177920] __attribute__((aligned(64)));
static float input2_storage[12951552] __attribute__((aligned(64)));
static float output0_storage[12951552] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5079040, 1);
  fill_f16(input1_storage, 4177920, 2);
  fill_f32(input2_storage, 12951552, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 3264, 3264, 1, input2_storage, input2_storage, 0, 3968, 3264, 3264, 1, output0_storage, output0_storage, 0, 3968, 3264, 3264, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00334_4e2d8858: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 12951552));
  return 0;
}
