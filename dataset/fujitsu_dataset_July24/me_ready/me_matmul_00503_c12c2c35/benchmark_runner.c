...
static uint16_t input0_storage[503808] __attribute__((aligned(64)));
static uint16_t input1_storage[688128] __attribute__((aligned(64)));
static float input2_storage[9404416] __attribute__((aligned(64)));
static float output0_storage[9404416] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 503808, 1);
  fill_f16(input1_storage, 688128, 2);
  fill_f32(input2_storage, 9404416, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2624, 192, 192, 1, input1_storage, input1_storage, 0, 192, 3584, 3584, 1, input2_storage, input2_storage, 0, 2624, 3584, 3584, 1, output0_storage, output0_storage, 0, 2624, 3584, 3584, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00503_c12c2c35: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9404416));
  return 0;
}
