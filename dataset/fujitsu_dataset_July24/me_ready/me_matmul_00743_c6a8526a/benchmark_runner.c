...
static uint16_t input1_storage[6021120] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 6021120, 2);
  fill_f16(input2_storage, 573440, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 2240, 2240, 1, input2_storage, input2_storage, 0, 256, 2240, 2240, 1, output0_storage, output0_storage, 0, 256, 2240, 2240, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00743_c6a8526a: benchmark completed");
