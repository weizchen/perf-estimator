...
static uint8_t input0_storage[2752512] __attribute__((aligned(64)));
static uint8_t input1_storage[12386304] __attribute__((aligned(64)));
static float input2_storage[2654208] __attribute__((aligned(64)));
static float output0_storage[2654208] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 2752512, 1);
  fill_f8(input1_storage, 12386304, 2);
  fill_f32(input2_storage, 2654208, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 768, 3584, 3584, 1, input1_storage, input1_storage, 0, 3584, 3456, 3456, 1, input2_storage, input2_storage, 0, 768, 3456, 3456, 1, output0_storage, output0_storage, 0, 768, 3456, 3456, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00234_d1654277: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2654208));
  return 0;
}
