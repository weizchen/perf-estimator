...
static uint8_t input1_storage[4751360] __attribute__((aligned(64)));
static float input2_storage[950272] __attribute__((aligned(64)));
static float output0_storage[950272] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 4751360, 2);
  fill_f32(input2_storage, 950272, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 3712, 3712, 1, input2_storage, input2_storage, 0, 256, 3712, 3712, 1, output0_storage, output0_storage, 0, 256, 3712, 3712, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00126_c85d93c8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 950272));
  return 0;
}
