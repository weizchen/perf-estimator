...
static uint16_t input1_storage[2367488] __attribute__((aligned(64)));
static uint16_t input2_storage[5570560] __attribute__((aligned(64)));
static uint16_t output0_storage[5570560] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 2367488, 2);
  fill_f16(input2_storage, 5570560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2560, 1088, 1088, 1, input1_storage, input1_storage, 0, 1088, 2176, 2176, 1, input2_storage, input2_storage, 0, 2560, 2176, 2176, 1, output0_storage, output0_storage, 0, 2560, 2176, 2176, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00717_72287676: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5570560));
  return 0;
}
