...
static uint8_t input1_storage[81920] __attribute__((aligned(64)));
static float input2_storage[1064960] __attribute__((aligned(64)));
static float output0_storage[1064960] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 81920, 2);
  fill_f32(input2_storage, 1064960, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 128, 128, 1, input1_storage, input1_storage, 0, 128, 640, 640, 1, input2_storage, input2_storage, 0, 1664, 640, 640, 1, output0_storage, output0_storage, 0, 1664, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00161_a746d583: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1064960));
  return 0;
}
