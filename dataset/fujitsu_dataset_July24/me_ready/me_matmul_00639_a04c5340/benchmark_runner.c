...
static uint16_t input0_storage[5345280] __attribute__((aligned(64)));
static uint16_t input1_storage[737280] __attribute__((aligned(64)));
static uint16_t input2_storage[475136] __attribute__((aligned(64)));
static uint16_t output0_storage[475136] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5345280, 1);
  fill_f16(input1_storage, 737280, 2);
  fill_f16(input2_storage, 475136, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1856, 2880, 2880, 1, input1_storage, input1_storage, 0, 2880, 256, 256, 1, input2_storage, input2_storage, 0, 1856, 256, 256, 1, output0_storage, output0_storage, 0, 1856, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00639_a04c5340: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 475136));
  return 0;
}
