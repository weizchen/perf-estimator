...
static uint16_t input1_storage[2412544] __attribute__((aligned(64)));
static float input2_storage[1396736] __attribute__((aligned(64)));
static float output0_storage[1396736] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 2412544, 2);
  fill_f32(input2_storage, 1396736, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 1984, 1984, 1, input2_storage, input2_storage, 0, 704, 1984, 1984, 1, output0_storage, output0_storage, 0, 704, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00515_b4bc056e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1396736));
  return 0;
}
