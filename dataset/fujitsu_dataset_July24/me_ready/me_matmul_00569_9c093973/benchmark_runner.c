...
static uint16_t input2_storage[2539520] __attribute__((aligned(64)));
static uint16_t output0_storage[2539520] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 2539520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 64, 64, 1, input1_storage, input1_storage, 0, 64, 3968, 3968, 1, input2_storage, input2_storage, 0, 640, 3968, 3968, 1, output0_storage, output0_storage, 0, 640, 3968, 3968, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00569_9c093973: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2539520));
  return 0;
}
