...
static uint8_t input0_storage[10551296] __attribute__((aligned(64)));
static uint8_t input1_storage[6406144] __attribute__((aligned(64)));
static float input2_storage[7798784] __attribute__((aligned(64)));
static float output0_storage[7798784] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 10551296, 1);
  fill_f8(input1_storage, 6406144, 2);
  fill_f32(input2_storage, 7798784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3584, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 2176, 2176, 1, input2_storage, input2_storage, 0, 3584, 2176, 2176, 1, output0_storage, output0_storage, 0, 3584, 2176, 2176, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00119_e90a813c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7798784));
  return 0;
}
