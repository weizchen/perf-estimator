...
static uint8_t input1_storage[147456] __attribute__((aligned(64)));
static float input2_storage[2506752] __attribute__((aligned(64)));
static float output0_storage[2506752] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f8(input1_storage, 147456, 2);
  fill_f32(input2_storage, 2506752, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1152, 1152, 1, input2_storage, input2_storage, 0, 2176, 1152, 1152, 1, output0_storage, output0_storage, 0, 2176, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00136_a8fb3488: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2506752));
  return 0;
}
