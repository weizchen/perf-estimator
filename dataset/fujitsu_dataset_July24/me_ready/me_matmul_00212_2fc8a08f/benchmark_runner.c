...
static uint8_t input0_storage[540672] __attribute__((aligned(64)));
static uint8_t input1_storage[491520] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 540672, 1);
  fill_f8(input1_storage, 491520, 2);
  fill_f32(input2_storage, 1802240, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 384, 384, 1, input1_storage, input1_storage, 0, 384, 1280, 1280, 1, input2_storage, input2_storage, 0, 1408, 1280, 1280, 1, output0_storage, output0_storage, 0, 1408, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00212_2fc8a08f: benchmark completed");
