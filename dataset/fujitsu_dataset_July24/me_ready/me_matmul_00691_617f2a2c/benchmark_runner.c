...
static uint16_t input0_storage[6946816] __attribute__((aligned(64)));
static uint16_t input1_storage[1441792] __attribute__((aligned(64)));
static uint16_t input2_storage[2387968] __attribute__((aligned(64)));
static uint16_t output0_storage[2387968] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 6946816, 1);
  fill_f16(input1_storage, 1441792, 2);
  fill_f16(input2_storage, 2387968, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3392, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 704, 704, 1, input2_storage, input2_storage, 0, 3392, 704, 704, 1, output0_storage, output0_storage, 0, 3392, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00691_617f2a2c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2387968));
  return 0;
}
