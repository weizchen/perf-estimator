...
static uint16_t input0_storage[585728] __attribute__((aligned(64)));
static uint16_t input1_storage[905216] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 585728, 1);
  fill_f16(input1_storage, 905216, 2);
  fill_f32(input2_storage, 765952, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 832, 832, 1, input1_storage, input1_storage, 0, 832, 1088, 1088, 1, input2_storage, input2_storage, 0, 704, 1088, 1088, 1, output0_storage, output0_storage, 0, 704, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00500_694dc369: benchmark completed");
