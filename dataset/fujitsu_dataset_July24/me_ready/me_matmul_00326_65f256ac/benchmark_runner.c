...
static uint16_t input0_storage[393216] __attribute__((aligned(64)));
static uint16_t input1_storage[10223616] __attribute__((aligned(64)));
static float input2_storage[425984] __attribute__((aligned(64)));
static float output0_storage[425984] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 393216, 1);
  fill_f16(input1_storage, 10223616, 2);
  fill_f32(input2_storage, 425984, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3072, 3072, 1, input1_storage, input1_storage, 0, 3072, 3328, 3328, 1, input2_storage, input2_storage, 0, 128, 3328, 3328, 1, output0_storage, output0_storage, 0, 128, 3328, 3328, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00326_65f256ac: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 425984));
  return 0;
}
