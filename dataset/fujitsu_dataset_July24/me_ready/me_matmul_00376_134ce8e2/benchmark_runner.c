...
static float input2_storage[184320] __attribute__((aligned(64)));
static float output0_storage[184320] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 184320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 960, 448, 448, 1, input1_storage, input1_storage, 0, 448, 192, 192, 1, input2_storage, input2_storage, 0, 960, 192, 192, 1, output0_storage, output0_storage, 0, 960, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00376_134ce8e2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 184320));
  return 0;
}
