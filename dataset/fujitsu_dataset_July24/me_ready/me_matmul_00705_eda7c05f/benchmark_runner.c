...
static uint16_t input0_storage[262144] __attribute__((aligned(64)));
static uint16_t input1_storage[1376256] __attribute__((aligned(64)));
static uint16_t input2_storage[344064] __attribute__((aligned(64)));
static uint16_t output0_storage[344064] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 262144, 1);
  fill_f16(input1_storage, 1376256, 2);
  fill_f16(input2_storage, 344064, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 1344, 1344, 1, input2_storage, input2_storage, 0, 256, 1344, 1344, 1, output0_storage, output0_storage, 0, 256, 1344, 1344, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00705_eda7c05f: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 344064));
  return 0;
}
