...
static uint16_t input0_storage[1118208] __attribute__((aligned(64)));
static uint16_t input1_storage[1290240] __attribute__((aligned(64)));
static uint16_t input2_storage[7188480] __attribute__((aligned(64)));
static uint16_t output0_storage[7188480] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1118208, 1);
  fill_f16(input1_storage, 1290240, 2);
  fill_f16(input2_storage, 7188480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2496, 448, 448, 1, input1_storage, input1_storage, 0, 448, 2880, 2880, 1, input2_storage, input2_storage, 0, 2496, 2880, 2880, 1, output0_storage, output0_storage, 0, 2496, 2880, 2880, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00721_0f2ec1fc: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 7188480));
  return 0;
}
