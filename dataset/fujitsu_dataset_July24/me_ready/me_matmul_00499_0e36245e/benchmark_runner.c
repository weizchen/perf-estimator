...
static uint16_t input0_storage[1933312] __attribute__((aligned(64)));
static uint16_t input1_storage[14016512] __attribute__((aligned(64)));
static float input2_storage[1900544] __attribute__((aligned(64)));
static float output0_storage[1900544] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1933312, 1);
  fill_f16(input1_storage, 14016512, 2);
  fill_f32(input2_storage, 1900544, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 3776, 3776, 1, input1_storage, input1_storage, 0, 3776, 3712, 3712, 1, input2_storage, input2_storage, 0, 512, 3712, 3712, 1, output0_storage, output0_storage, 0, 512, 3712, 3712, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00499_0e36245e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1900544));
  return 0;
}
