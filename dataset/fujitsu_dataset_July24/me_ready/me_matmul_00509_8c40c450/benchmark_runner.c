...
static float input2_storage[4415488] __attribute__((aligned(64)));
static float output0_storage[4415488] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 4415488, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3136, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1408, 1408, 1, input2_storage, input2_storage, 0, 3136, 1408, 1408, 1, output0_storage, output0_storage, 0, 3136, 1408, 1408, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00509_8c40c450: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4415488));
  return 0;
}
