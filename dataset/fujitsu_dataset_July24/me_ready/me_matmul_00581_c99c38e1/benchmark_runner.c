...
static uint16_t input1_storage[1327104] __attribute__((aligned(64)));
static uint16_t input2_storage[1622016] __attribute__((aligned(64)));
static uint16_t output0_storage[1622016] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1327104, 2);
  fill_f16(input2_storage, 1622016, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 576, 576, 1, input1_storage, input1_storage, 0, 576, 2304, 2304, 1, input2_storage, input2_storage, 0, 704, 2304, 2304, 1, output0_storage, output0_storage, 0, 704, 2304, 2304, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00581_c99c38e1: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1622016));
  return 0;
}
