...
static uint16_t input0_storage[118784] __attribute__((aligned(64)));
static uint16_t input1_storage[253952] __attribute__((aligned(64)));
static uint16_t input2_storage[7364608] __attribute__((aligned(64)));
static uint16_t output0_storage[7364608] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 118784, 1);
  fill_f16(input1_storage, 253952, 2);
  fill_f16(input2_storage, 7364608, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1856, 64, 64, 1, input1_storage, input1_storage, 0, 64, 3968, 3968, 1, input2_storage, input2_storage, 0, 1856, 3968, 3968, 1, output0_storage, output0_storage, 0, 1856, 3968, 3968, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00653_58fc37f2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 7364608));
  return 0;
}
