...
static uint16_t input2_storage[12288] __attribute__((aligned(64)));
static uint16_t output0_storage[12288] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 12288, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 64, 64, 1, input2_storage, input2_storage, 0, 192, 64, 64, 1, output0_storage, output0_storage, 0, 192, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00650_d3ffadf1: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 12288));
  return 0;
}
