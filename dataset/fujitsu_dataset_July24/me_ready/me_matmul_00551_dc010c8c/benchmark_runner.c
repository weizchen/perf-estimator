...
static uint16_t input0_storage[2228224] __attribute__((aligned(64)));
static uint16_t input1_storage[974848] __attribute__((aligned(64)));
static uint16_t input2_storage[458752] __attribute__((aligned(64)));
static uint16_t output0_storage[458752] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2228224, 1);
  fill_f16(input1_storage, 974848, 2);
  fill_f16(input2_storage, 458752, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 448, 448, 1, input2_storage, input2_storage, 0, 1024, 448, 448, 1, output0_storage, output0_storage, 0, 1024, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00551_dc010c8c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 458752));
  return 0;
}
