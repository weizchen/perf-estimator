...
static uint16_t input1_storage[3244032] __attribute__((aligned(64)));
static float input2_storage[720896] __attribute__((aligned(64)));
static float output0_storage[720896] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 3244032, 2);
  fill_f32(input2_storage, 720896, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1152, 1152, 1, input1_storage, input1_storage, 0, 1152, 2816, 2816, 1, input2_storage, input2_storage, 0, 256, 2816, 2816, 1, output0_storage, output0_storage, 0, 256, 2816, 2816, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00448_3f7a3341: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 720896));
  return 0;
}
