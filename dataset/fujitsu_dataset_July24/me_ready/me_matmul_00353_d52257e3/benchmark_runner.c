...
static float input2_storage[266240] __attribute__((aligned(64)));
static float output0_storage[266240] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 512000, 1);
  fill_f16(input1_storage, 1331200, 2);
  fill_f32(input2_storage, 266240, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 832, 832, 1, input2_storage, input2_storage, 0, 320, 832, 832, 1, output0_storage, output0_storage, 0, 320, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00353_d52257e3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 266240));
  return 0;
}
