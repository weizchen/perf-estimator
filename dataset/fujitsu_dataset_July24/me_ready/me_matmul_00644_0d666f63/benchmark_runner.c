...
static uint16_t input0_storage[1069056] __attribute__((aligned(64)));
static uint16_t input1_storage[1548288] __attribute__((aligned(64)));
static uint16_t input2_storage[4988928] __attribute__((aligned(64)));
static uint16_t output0_storage[4988928] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1069056, 1);
  fill_f16(input1_storage, 1548288, 2);
  fill_f16(input2_storage, 4988928, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1856, 576, 576, 1, input1_storage, input1_storage, 0, 576, 2688, 2688, 1, input2_storage, input2_storage, 0, 1856, 2688, 2688, 1, output0_storage, output0_storage, 0, 1856, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00644_0d666f63: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 4988928));
  return 0;
}
