...
static uint8_t input0_storage[557056] __attribute__((aligned(64)));
static uint8_t input1_storage[1048576] __attribute__((aligned(64)));
static float input2_storage[8912896] __attribute__((aligned(64)));
static float output0_storage[8912896] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 557056, 1);
  fill_f8(input1_storage, 1048576, 2);
  fill_f32(input2_storage, 8912896, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 256, 256, 1, input1_storage, input1_storage, 0, 256, 4096, 4096, 1, input2_storage, input2_storage, 0, 2176, 4096, 4096, 1, output0_storage, output0_storage, 0, 2176, 4096, 4096, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00071_2cfcbae3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8912896));
  return 0;
}
