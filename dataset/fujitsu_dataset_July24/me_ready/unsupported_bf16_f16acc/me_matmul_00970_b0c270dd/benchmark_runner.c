...
static uint16_t input0_storage[8650752] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 8650752, 1);
  fill_bf16(input1_storage, 180224, 2);
  fill_f16(input2_storage, 196608, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3072, 2816, 2816, 1, input1_storage, input1_storage, 0, 2816, 64, 64, 1, input2_storage, input2_storage, 0, 3072, 64, 64, 1, output0_storage, output0_storage, 0, 3072, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00970_b0c270dd: benchmark completed");
