...
static uint16_t input1_storage[401408] __attribute__((aligned(64)));
static uint16_t input2_storage[16384] __attribute__((aligned(64)));
static uint16_t output0_storage[16384] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 401408, 1);
  fill_bf16(input1_storage, 401408, 2);
  fill_f16(input2_storage, 16384, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 128, 128, 1, input2_storage, input2_storage, 0, 128, 128, 128, 1, output0_storage, output0_storage, 0, 128, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00994_46ec968f: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 16384));
  return 0;
}
