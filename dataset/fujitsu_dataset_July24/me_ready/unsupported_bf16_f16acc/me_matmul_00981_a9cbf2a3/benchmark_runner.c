...
static uint16_t input0_storage[950272] __attribute__((aligned(64)));
static uint16_t input1_storage[6176768] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 950272, 1);
  fill_bf16(input1_storage, 6176768, 2);
  fill_f16(input2_storage, 1703936, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 3328, 3328, 1, input2_storage, input2_storage, 0, 512, 3328, 3328, 1, output0_storage, output0_storage, 0, 512, 3328, 3328, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00981_a9cbf2a3: benchmark completed");
