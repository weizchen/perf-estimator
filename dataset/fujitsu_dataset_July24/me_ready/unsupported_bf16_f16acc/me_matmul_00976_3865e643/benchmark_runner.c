...
static uint16_t input0_storage[53248] __attribute__((aligned(64)));
static uint16_t input1_storage[200704] __attribute__((aligned(64)));
static uint16_t input2_storage[2609152] __attribute__((aligned(64)));
static uint16_t output0_storage[2609152] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 53248, 1);
  fill_bf16(input1_storage, 200704, 2);
  fill_f16(input2_storage, 2609152, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 832, 64, 64, 1, input1_storage, input1_storage, 0, 64, 3136, 3136, 1, input2_storage, input2_storage, 0, 832, 3136, 3136, 1, output0_storage, output0_storage, 0, 832, 3136, 3136, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00976_3865e643: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2609152));
  return 0;
}
