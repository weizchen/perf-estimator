...
static uint16_t input1_storage[151552] __attribute__((aligned(64)));
static uint16_t input2_storage[3637248] __attribute__((aligned(64)));
static uint16_t output0_storage[3637248] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 98304, 1);
  fill_bf16(input1_storage, 151552, 2);
  fill_f16(input2_storage, 3637248, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 64, 64, 1, input1_storage, input1_storage, 0, 64, 2368, 2368, 1, input2_storage, input2_storage, 0, 1536, 2368, 2368, 1, output0_storage, output0_storage, 0, 1536, 2368, 2368, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00965_1255f072: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3637248));
  return 0;
}
