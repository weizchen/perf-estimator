...
static uint16_t input1_storage[3870720] __attribute__((aligned(64)));
static uint16_t input2_storage[516096] __attribute__((aligned(64)));
static uint16_t output0_storage[516096] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 122880, 1);
  fill_bf16(input1_storage, 3870720, 2);
  fill_f16(input2_storage, 516096, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 960, 960, 1, input1_storage, input1_storage, 0, 960, 4032, 4032, 1, input2_storage, input2_storage, 0, 128, 4032, 4032, 1, output0_storage, output0_storage, 0, 128, 4032, 4032, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_01000_06bb431a: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 516096));
  return 0;
}
