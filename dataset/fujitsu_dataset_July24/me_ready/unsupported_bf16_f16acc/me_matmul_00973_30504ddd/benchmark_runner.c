...
static uint16_t input0_storage[1056768] __attribute__((aligned(64)));
static uint16_t input1_storage[221184] __attribute__((aligned(64)));
static uint16_t input2_storage[1585152] __attribute__((aligned(64)));
static uint16_t output0_storage[1585152] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1056768, 1);
  fill_bf16(input1_storage, 221184, 2);
  fill_f16(input2_storage, 1585152, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 384, 384, 1, input1_storage, input1_storage, 0, 384, 576, 576, 1, input2_storage, input2_storage, 0, 2752, 576, 576, 1, output0_storage, output0_storage, 0, 2752, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00973_30504ddd: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1585152));
  return 0;
}
