...
static uint16_t input1_storage[2076672] __attribute__((aligned(64)));
static uint16_t input2_storage[212992] __attribute__((aligned(64)));
static uint16_t output0_storage[212992] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 638976, 1);
  fill_bf16(input1_storage, 2076672, 2);
  fill_f16(input2_storage, 212992, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2496, 2496, 1, input1_storage, input1_storage, 0, 2496, 832, 832, 1, input2_storage, input2_storage, 0, 256, 832, 832, 1, output0_storage, output0_storage, 0, 256, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00999_1529eda5: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 212992));
  return 0;
}
