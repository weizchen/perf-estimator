...
static uint16_t input1_storage[1114112] __attribute__((aligned(64)));
static uint16_t input2_storage[851968] __attribute__((aligned(64)));
static uint16_t output0_storage[851968] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 3620864, 1);
  fill_bf16(input1_storage, 1114112, 2);
  fill_f16(input2_storage, 851968, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 512, 512, 1, input2_storage, input2_storage, 0, 1664, 512, 512, 1, output0_storage, output0_storage, 0, 1664, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00989_41e0e1ab: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 851968));
  return 0;
}
