...
static uint16_t input0_storage[512000] __attribute__((aligned(64)));
static uint16_t input1_storage[1536000] __attribute__((aligned(64)));
static uint16_t input2_storage[307200] __attribute__((aligned(64)));
static uint16_t output0_storage[307200] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 512000, 1);
  fill_bf16(input1_storage, 1536000, 2);
  fill_f16(input2_storage, 307200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 960, 960, 1, input2_storage, input2_storage, 0, 320, 960, 960, 1, output0_storage, output0_storage, 0, 320, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_01001_d2df9c41: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 307200));
  return 0;
}
