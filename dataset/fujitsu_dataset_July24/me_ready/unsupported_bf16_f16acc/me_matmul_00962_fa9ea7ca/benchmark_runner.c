...
static uint16_t input1_storage[3440640] __attribute__((aligned(64)));
static uint16_t input2_storage[573440] __attribute__((aligned(64)));
static uint16_t output0_storage[573440] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 3440640, 2);
  fill_f16(input2_storage, 573440, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 1280, 1280, 1, input2_storage, input2_storage, 0, 448, 1280, 1280, 1, output0_storage, output0_storage, 0, 448, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00962_fa9ea7ca: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 573440));
  return 0;
}
