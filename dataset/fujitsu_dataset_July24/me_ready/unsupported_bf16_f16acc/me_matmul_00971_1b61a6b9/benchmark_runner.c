...
static uint16_t input0_storage[4145152] __attribute__((aligned(64)));
static uint16_t input1_storage[188416] __attribute__((aligned(64)));
static uint16_t input2_storage[360448] __attribute__((aligned(64)));
static uint16_t output0_storage[360448] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 4145152, 1);
  fill_bf16(input1_storage, 188416, 2);
  fill_f16(input2_storage, 360448, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2816, 1472, 1472, 1, input1_storage, input1_storage, 0, 1472, 128, 128, 1, input2_storage, input2_storage, 0, 2816, 128, 128, 1, output0_storage, output0_storage, 0, 2816, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00971_1b61a6b9: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 360448));
  return 0;
}
