...
static uint16_t input0_storage[184320] __attribute__((aligned(64)));
static uint16_t input1_storage[1511424] __attribute__((aligned(64)));
static uint16_t input2_storage[839680] __attribute__((aligned(64)));
static uint16_t output0_storage[839680] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 184320, 1);
  fill_bf16(input1_storage, 1511424, 2);
  fill_f16(input2_storage, 839680, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 576, 576, 1, input1_storage, input1_storage, 0, 576, 2624, 2624, 1, input2_storage, input2_storage, 0, 320, 2624, 2624, 1, output0_storage, output0_storage, 0, 320, 2624, 2624, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00992_1b7d569e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 839680));
  return 0;
}
