...
static uint16_t input2_storage[45056] __attribute__((aligned(64)));
static uint16_t output0_storage[45056] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 315392, 2);
  fill_f16(input2_storage, 45056, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 448, 448, 1, input1_storage, input1_storage, 0, 448, 704, 704, 1, input2_storage, input2_storage, 0, 64, 704, 704, 1, output0_storage, output0_storage, 0, 64, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00995_f3230f7e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 45056));
  return 0;
}
