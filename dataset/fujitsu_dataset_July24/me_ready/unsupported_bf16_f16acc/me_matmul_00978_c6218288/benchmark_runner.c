...
static uint16_t input0_storage[2064384] __attribute__((aligned(64)));
static uint16_t input1_storage[1966080] __attribute__((aligned(64)));
static uint16_t input2_storage[1720320] __attribute__((aligned(64)));
static uint16_t output0_storage[1720320] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 2064384, 1);
  fill_bf16(input1_storage, 1966080, 2);
  fill_f16(input2_storage, 1720320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1344, 1536, 1536, 1, input1_storage, input1_storage, 0, 1536, 1280, 1280, 1, input2_storage, input2_storage, 0, 1344, 1280, 1280, 1, output0_storage, output0_storage, 0, 1344, 1280, 1280, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00978_c6218288: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1720320));
  return 0;
}
