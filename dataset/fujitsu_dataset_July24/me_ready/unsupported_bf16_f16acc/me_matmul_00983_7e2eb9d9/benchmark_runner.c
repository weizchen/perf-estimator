...
static uint16_t input1_storage[544768] __attribute__((aligned(64)));
static uint16_t input2_storage[630784] __attribute__((aligned(64)));
static uint16_t output0_storage[630784] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1712128, 1);
  fill_bf16(input1_storage, 544768, 2);
  fill_f16(input2_storage, 630784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 448, 448, 1, input2_storage, input2_storage, 0, 1408, 448, 448, 1, output0_storage, output0_storage, 0, 1408, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00983_7e2eb9d9: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 630784));
  return 0;
}
