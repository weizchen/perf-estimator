...
static uint16_t input2_storage[512000] __attribute__((aligned(64)));
static uint16_t output0_storage[512000] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 102400, 1);
  fill_bf16(input1_storage, 20480, 2);
  fill_f16(input2_storage, 512000, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 64, 64, 1, input1_storage, input1_storage, 0, 64, 320, 320, 1, input2_storage, input2_storage, 0, 1600, 320, 320, 1, output0_storage, output0_storage, 0, 1600, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00961_4d90c7ad: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 512000));
  return 0;
}
