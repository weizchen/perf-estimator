...
static uint16_t input0_storage[1146880] __attribute__((aligned(64)));
static uint16_t input1_storage[2637824] __attribute__((aligned(64)));
static uint16_t input2_storage[3768320] __attribute__((aligned(64)));
static uint16_t output0_storage[3768320] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1146880, 1);
  fill_bf16(input1_storage, 2637824, 2);
  fill_f16(input2_storage, 3768320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 896, 896, 1, input1_storage, input1_storage, 0, 896, 2944, 2944, 1, input2_storage, input2_storage, 0, 1280, 2944, 2944, 1, output0_storage, output0_storage, 0, 1280, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00964_b37a3822: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3768320));
  return 0;
}
