...
static uint16_t input0_storage[8126464] __attribute__((aligned(64)));
static uint16_t input1_storage[2666496] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 8126464, 1);
  fill_bf16(input1_storage, 2666496, 2);
  fill_f16(input2_storage, 5505024, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4096, 1984, 1984, 1, input1_storage, input1_storage, 0, 1984, 1344, 1344, 1, input2_storage, input2_storage, 0, 4096, 1344, 1344, 1, output0_storage, output0_storage, 0, 4096, 1344, 1344, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00968_2c17bcbf: benchmark completed");
