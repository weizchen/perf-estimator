...
static uint8_t input0_storage[1048576] __attribute__((aligned(64)));
static uint8_t input1_storage[1572864] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 1048576, 1);
  fill_f8(input1_storage, 1572864, 2);
  fill_f32(input2_storage, 1572864, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 1536, 1536, 1, input2_storage, input2_storage, 0, 1024, 1536, 1536, 1, output0_storage, output0_storage, 0, 1024, 1536, 1536, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00231_473995e5: benchmark completed");
