...
static uint16_t input0_storage[3813376] __attribute__((aligned(64)));
static uint16_t input1_storage[1806336] __attribute__((aligned(64)));
static float input2_storage[700416] __attribute__((aligned(64)));
static float output0_storage[700416] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3813376, 1);
  fill_f16(input1_storage, 1806336, 2);
  fill_f32(input2_storage, 700416, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 576, 576, 1, input2_storage, input2_storage, 0, 1216, 576, 576, 1, output0_storage, output0_storage, 0, 1216, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00454_17193f69: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 700416));
  return 0;
}
