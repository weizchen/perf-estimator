...
static uint16_t input0_storage[1126400] __attribute__((aligned(64)));
static uint16_t input1_storage[811008] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1126400, 1);
  fill_f16(input1_storage, 811008, 2);
  fill_f32(input2_storage, 1843200, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 704, 704, 1, input1_storage, input1_storage, 0, 704, 1152, 1152, 1, input2_storage, input2_storage, 0, 1600, 1152, 1152, 1, output0_storage, output0_storage, 0, 1600, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00271_300904ef: benchmark completed");
