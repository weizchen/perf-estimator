...
static uint8_t input0_storage[720896] __attribute__((aligned(64)));
static uint8_t input1_storage[524288] __attribute__((aligned(64)));
static float input2_storage[1441792] __attribute__((aligned(64)));
static float output0_storage[1441792] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 720896, 1);
  fill_f8(input1_storage, 524288, 2);
  fill_f32(input2_storage, 1441792, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 512, 512, 1, input1_storage, input1_storage, 0, 512, 1024, 1024, 1, input2_storage, input2_storage, 0, 1408, 1024, 1024, 1, output0_storage, output0_storage, 0, 1408, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00209_c8a1162b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1441792));
  return 0;
}
