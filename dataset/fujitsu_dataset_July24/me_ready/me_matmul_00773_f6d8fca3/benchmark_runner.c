...
static uint16_t input2_storage[221184] __attribute__((aligned(64)));
static uint16_t output0_storage[221184] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 221184, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 576, 576, 1, input2_storage, input2_storage, 0, 384, 576, 576, 1, output0_storage, output0_storage, 0, 384, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00773_f6d8fca3: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 221184));
  return 0;
}
