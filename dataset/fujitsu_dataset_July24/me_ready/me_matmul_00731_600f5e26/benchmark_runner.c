...
static uint16_t input2_storage[778240] __attribute__((aligned(64)));
static uint16_t output0_storage[778240] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 778240, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 576, 576, 1, input1_storage, input1_storage, 0, 576, 1216, 1216, 1, input2_storage, input2_storage, 0, 640, 1216, 1216, 1, output0_storage, output0_storage, 0, 640, 1216, 1216, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00731_600f5e26: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 778240));
  return 0;
}
