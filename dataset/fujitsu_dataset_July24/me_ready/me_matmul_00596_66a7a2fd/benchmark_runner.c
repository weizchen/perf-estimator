...
static uint16_t input0_storage[241664] __attribute__((aligned(64)));
static uint16_t input1_storage[15224832] __attribute__((aligned(64)));
static uint16_t input2_storage[258048] __attribute__((aligned(64)));
static uint16_t output0_storage[258048] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 241664, 1);
  fill_f16(input1_storage, 15224832, 2);
  fill_f16(input2_storage, 258048, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3776, 3776, 1, input1_storage, input1_storage, 0, 3776, 4032, 4032, 1, input2_storage, input2_storage, 0, 64, 4032, 4032, 1, output0_storage, output0_storage, 0, 64, 4032, 4032, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00596_66a7a2fd: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 258048));
  return 0;
}
