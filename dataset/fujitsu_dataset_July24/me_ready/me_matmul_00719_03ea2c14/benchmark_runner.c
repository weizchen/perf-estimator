...
static uint16_t input2_storage[5677056] __attribute__((aligned(64)));
static uint16_t output0_storage[5677056] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 5677056, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 576, 576, 1, input1_storage, input1_storage, 0, 576, 4032, 4032, 1, input2_storage, input2_storage, 0, 1408, 4032, 4032, 1, output0_storage, output0_storage, 0, 1408, 4032, 4032, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00719_03ea2c14: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5677056));
  return 0;
}
