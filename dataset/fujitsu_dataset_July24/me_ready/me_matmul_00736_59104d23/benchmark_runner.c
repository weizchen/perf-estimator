...
static uint16_t input0_storage[217088] __attribute__((aligned(64)));
static uint16_t input1_storage[2605056] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 217088, 1);
  fill_f16(input1_storage, 2605056, 2);
  fill_f16(input2_storage, 49152, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3392, 3392, 1, input1_storage, input1_storage, 0, 3392, 768, 768, 1, input2_storage, input2_storage, 0, 64, 768, 768, 1, output0_storage, output0_storage, 0, 64, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00736_59104d23: benchmark completed");
