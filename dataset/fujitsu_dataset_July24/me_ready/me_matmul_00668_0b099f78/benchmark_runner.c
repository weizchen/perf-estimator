...
static uint16_t input0_storage[483328] __attribute__((aligned(64)));
static uint16_t input1_storage[3624960] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 483328, 1);
  fill_f16(input1_storage, 3624960, 2);
  fill_f16(input2_storage, 122880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3776, 3776, 1, input1_storage, input1_storage, 0, 3776, 960, 960, 1, input2_storage, input2_storage, 0, 128, 960, 960, 1, output0_storage, output0_storage, 0, 128, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00668_0b099f78: benchmark completed");
