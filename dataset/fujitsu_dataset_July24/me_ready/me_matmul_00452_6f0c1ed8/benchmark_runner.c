...
static uint16_t input0_storage[7483392] __attribute__((aligned(64)));
static uint16_t input1_storage[2019328] __attribute__((aligned(64)));
static float input2_storage[4386816] __attribute__((aligned(64)));
static float output0_storage[4386816] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 7483392, 1);
  fill_f16(input1_storage, 2019328, 2);
  fill_f32(input2_storage, 4386816, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4032, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 1088, 1088, 1, input2_storage, input2_storage, 0, 4032, 1088, 1088, 1, output0_storage, output0_storage, 0, 4032, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00452_6f0c1ed8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4386816));
  return 0;
}
