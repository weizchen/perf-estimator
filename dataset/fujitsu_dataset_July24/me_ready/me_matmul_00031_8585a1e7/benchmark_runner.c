...
static uint8_t input0_storage[5242880] __attribute__((aligned(64)));
static uint8_t input1_storage[12582912] __attribute__((aligned(64)));
static float input2_storage[3932160] __attribute__((aligned(64)));
static float output0_storage[3932160] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 5242880, 1);
  fill_f8(input1_storage, 12582912, 2);
  fill_f32(input2_storage, 3932160, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 4096, 4096, 1, input1_storage, input1_storage, 0, 4096, 3072, 3072, 1, input2_storage, input2_storage, 0, 1280, 3072, 3072, 1, output0_storage, output0_storage, 0, 1280, 3072, 3072, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00031_8585a1e7: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3932160));
  return 0;
}
