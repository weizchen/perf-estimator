...
static uint8_t input0_storage[786432] __attribute__((aligned(64)));
static uint8_t input1_storage[1114112] __attribute__((aligned(64)));
static float input2_storage[3342336] __attribute__((aligned(64)));
static float output0_storage[3342336] __attribute__((aligned(64)));
...
int main(void) {
  fill_f8(input0_storage, 786432, 1);
  fill_f8(input1_storage, 1114112, 2);
  fill_f32(input2_storage, 3342336, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 512, 512, 1, input1_storage, input1_storage, 0, 512, 2176, 2176, 1, input2_storage, input2_storage, 0, 1536, 2176, 2176, 1, output0_storage, output0_storage, 0, 1536, 2176, 2176, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00012_31bcedba: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3342336));
  return 0;
}
