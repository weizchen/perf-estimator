...
static uint16_t input2_storage[1671168] __attribute__((aligned(64)));
static uint16_t output0_storage[1671168] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 1671168, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1536, 1536, 1, input2_storage, input2_storage, 0, 1088, 1536, 1536, 1, output0_storage, output0_storage, 0, 1088, 1536, 1536, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00677_86ec91eb: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1671168));
  return 0;
}
