...
static uint16_t input1_storage[8146944] __attribute__((aligned(64)));
static uint16_t input2_storage[1253376] __attribute__((aligned(64)));
static uint16_t output0_storage[1253376] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 8146944, 2);
  fill_f16(input2_storage, 1253376, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 2496, 2496, 1, input1_storage, input1_storage, 0, 2496, 3264, 3264, 1, input2_storage, input2_storage, 0, 384, 3264, 3264, 1, output0_storage, output0_storage, 0, 384, 3264, 3264, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00616_cfbc7045: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1253376));
  return 0;
}
