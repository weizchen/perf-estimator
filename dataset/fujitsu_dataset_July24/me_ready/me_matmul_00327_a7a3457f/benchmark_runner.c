...
static uint16_t input1_storage[6266880] __attribute__((aligned(64)));
static float input2_storage[860160] __attribute__((aligned(64)));
static float output0_storage[860160] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 6266880, 2);
  fill_f32(input2_storage, 860160, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 3264, 3264, 1, input1_storage, input1_storage, 0, 3264, 1920, 1920, 1, input2_storage, input2_storage, 0, 448, 1920, 1920, 1, output0_storage, output0_storage, 0, 448, 1920, 1920, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00327_a7a3457f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 860160));
  return 0;
}
