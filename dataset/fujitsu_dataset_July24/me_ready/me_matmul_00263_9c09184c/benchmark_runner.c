...
static uint16_t input0_storage[1323008] __attribute__((aligned(64)));
static uint16_t input1_storage[1245184] __attribute__((aligned(64)));
static float input2_storage[1114112] __attribute__((aligned(64)));
static float output0_storage[1114112] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1323008, 1);
  fill_f16(input1_storage, 1245184, 2);
  fill_f32(input2_storage, 1114112, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 1024, 1024, 1, input2_storage, input2_storage, 0, 1088, 1024, 1024, 1, output0_storage, output0_storage, 0, 1088, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("me_matmul_00263_9c09184c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1114112));
  return 0;
}
