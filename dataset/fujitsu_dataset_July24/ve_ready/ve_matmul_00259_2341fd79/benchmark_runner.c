...
static uint16_t input0_storage[720896] __attribute__((aligned(64)));
static uint16_t input1_storage[294912] __attribute__((aligned(64)));
static uint16_t input2_storage[811008] __attribute__((aligned(64)));
static uint16_t output0_storage[811008] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 720896, 1);
  fill_f16(input1_storage, 294912, 2);
  fill_f16(input2_storage, 811008, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 512, 512, 1, input1_storage, input1_storage, 0, 512, 576, 576, 1, input2_storage, input2_storage, 0, 1408, 576, 576, 1, output0_storage, output0_storage, 0, 1408, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00259_2341fd79: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 811008));
  return 0;
}
