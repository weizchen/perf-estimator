...
static uint16_t input0_storage[282624] __attribute__((aligned(64)));
static uint16_t input1_storage[712704] __attribute__((aligned(64)));
static uint16_t input2_storage[5464064] __attribute__((aligned(64)));
static uint16_t output0_storage[5464064] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 282624, 1);
  fill_f16(input1_storage, 712704, 2);
  fill_f16(input2_storage, 5464064, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1472, 192, 192, 1, input1_storage, input1_storage, 0, 192, 3712, 3712, 1, input2_storage, input2_storage, 0, 1472, 3712, 3712, 1, output0_storage, output0_storage, 0, 1472, 3712, 3712, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00237_69d435e0: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 5464064));
  return 0;
}
