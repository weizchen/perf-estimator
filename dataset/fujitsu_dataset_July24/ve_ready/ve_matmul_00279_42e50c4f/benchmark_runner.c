...
static uint16_t input0_storage[696320] __attribute__((aligned(64)));
static uint16_t input1_storage[2088960] __attribute__((aligned(64)));
static uint16_t input2_storage[3551232] __attribute__((aligned(64)));
static uint16_t output0_storage[3551232] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 696320, 1);
  fill_f16(input1_storage, 2088960, 2);
  fill_f16(input2_storage, 3551232, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 640, 640, 1, input1_storage, input1_storage, 0, 640, 3264, 3264, 1, input2_storage, input2_storage, 0, 1088, 3264, 3264, 1, output0_storage, output0_storage, 0, 1088, 3264, 3264, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00279_42e50c4f: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3551232));
  return 0;
}
