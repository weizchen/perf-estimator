...
static uint16_t input2_storage[524288] __attribute__((aligned(64)));
static uint16_t output0_storage[524288] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 524288, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4096, 192, 192, 1, input1_storage, input1_storage, 0, 192, 128, 128, 1, input2_storage, input2_storage, 0, 4096, 128, 128, 1, output0_storage, output0_storage, 0, 4096, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00281_b3901029: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 524288));
  return 0;
}
