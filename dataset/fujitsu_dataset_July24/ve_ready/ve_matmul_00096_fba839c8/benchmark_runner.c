...
static int8_t input0_storage[3440640] __attribute__((aligned(64)));
static int8_t input1_storage[1261568] __attribute__((aligned(64)));
static int32_t input2_storage[5406720] __attribute__((aligned(64)));
static int32_t output0_storage[5406720] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 3440640, 1);
  fill_i8(input1_storage, 1261568, 2);
  fill_i32(input2_storage, 5406720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3840, 896, 896, 1, input1_storage, input1_storage, 0, 896, 1408, 1408, 1, input2_storage, input2_storage, 0, 3840, 1408, 1408, 1, output0_storage, output0_storage, 0, 3840, 1408, 1408, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00096_fba839c8: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 5406720));
  return 0;
}
