...
static uint16_t input0_storage[2973696] __attribute__((aligned(64)));
static uint16_t input1_storage[1622016] __attribute__((aligned(64)));
static float input2_storage[2433024] __attribute__((aligned(64)));
static float output0_storage[2433024] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2973696, 1);
  fill_f16(input1_storage, 1622016, 2);
  fill_f32(input2_storage, 2433024, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2112, 1408, 1408, 1, input1_storage, input1_storage, 0, 1408, 1152, 1152, 1, input2_storage, input2_storage, 0, 2112, 1152, 1152, 1, output0_storage, output0_storage, 0, 2112, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00116_14a11ef4: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2433024));
  return 0;
}
