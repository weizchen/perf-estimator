...
static uint16_t input1_storage[798720] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 798720, 2);
  fill_f16(input2_storage, 1277952, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 320, 320, 1, input1_storage, input1_storage, 0, 320, 2496, 2496, 1, input2_storage, input2_storage, 0, 512, 2496, 2496, 1, output0_storage, output0_storage, 0, 512, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00283_f8ecdbf3: benchmark completed");
