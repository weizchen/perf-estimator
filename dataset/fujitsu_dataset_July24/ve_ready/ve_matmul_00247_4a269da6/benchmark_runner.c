...
static uint16_t input0_storage[3207168] __attribute__((aligned(64)));
static uint16_t input1_storage[4157440] __attribute__((aligned(64)));
static uint16_t input2_storage[3870720] __attribute__((aligned(64)));
static uint16_t output0_storage[3870720] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3207168, 1);
  fill_f16(input1_storage, 4157440, 2);
  fill_f16(input2_storage, 3870720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1728, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 2240, 2240, 1, input2_storage, input2_storage, 0, 1728, 2240, 2240, 1, output0_storage, output0_storage, 0, 1728, 2240, 2240, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00247_4a269da6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3870720));
  return 0;
}
