...
static float input0_storage[4669440] __attribute__((aligned(64)));
static float input1_storage[311296] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 4669440, 1);
  fill_f32(input1_storage, 311296, 2);
  fill_f32(input2_storage, 983040, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3840, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 256, 256, 1, input2_storage, input2_storage, 0, 3840, 256, 256, 1, output0_storage, output0_storage, 0, 3840, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00423_cd439b26: benchmark completed");
