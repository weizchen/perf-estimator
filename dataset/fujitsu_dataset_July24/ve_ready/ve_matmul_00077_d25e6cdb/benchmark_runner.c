...
static int8_t input0_storage[3538944] __attribute__((aligned(64)));
static int8_t input1_storage[9732096] __attribute__((aligned(64)));
static int32_t input2_storage[2883584] __attribute__((aligned(64)));
static int32_t output0_storage[2883584] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 3538944, 1);
  fill_i8(input1_storage, 9732096, 2);
  fill_i32(input2_storage, 2883584, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 3456, 3456, 1, input1_storage, input1_storage, 0, 3456, 2816, 2816, 1, input2_storage, input2_storage, 0, 1024, 2816, 2816, 1, output0_storage, output0_storage, 0, 1024, 2816, 2816, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00077_d25e6cdb: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2883584));
  return 0;
}
