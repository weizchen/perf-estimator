...
static uint16_t input0_storage[10973184] __attribute__((aligned(64)));
static uint16_t input1_storage[1167360] __attribute__((aligned(64)));
static float input2_storage[962560] __attribute__((aligned(64)));
static float output0_storage[962560] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 10973184, 1);
  fill_f16(input1_storage, 1167360, 2);
  fill_f32(input2_storage, 962560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3008, 3648, 3648, 1, input1_storage, input1_storage, 0, 3648, 320, 320, 1, input2_storage, input2_storage, 0, 3008, 320, 320, 1, output0_storage, output0_storage, 0, 3008, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00147_b36c65dd: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 962560));
  return 0;
}
