...
static int8_t input0_storage[1015808] __attribute__((aligned(64)));
static int8_t input1_storage[10158080] __attribute__((aligned(64)));
static int32_t input2_storage[655360] __attribute__((aligned(64)));
static int32_t output0_storage[655360] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1015808, 1);
  fill_i8(input1_storage, 10158080, 2);
  fill_i32(input2_storage, 655360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 3968, 3968, 1, input1_storage, input1_storage, 0, 3968, 2560, 2560, 1, input2_storage, input2_storage, 0, 256, 2560, 2560, 1, output0_storage, output0_storage, 0, 256, 2560, 2560, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00013_c93d46ac: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 655360));
  return 0;
}
