...
static uint16_t input1_storage[6533120] __attribute__((aligned(64)));
static float input2_storage[118784] __attribute__((aligned(64)));
static float output0_storage[118784] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 6533120, 2);
  fill_f32(input2_storage, 118784, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 3520, 3520, 1, input1_storage, input1_storage, 0, 3520, 1856, 1856, 1, input2_storage, input2_storage, 0, 64, 1856, 1856, 1, output0_storage, output0_storage, 0, 64, 1856, 1856, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00119_cc8a315f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 118784));
  return 0;
}
