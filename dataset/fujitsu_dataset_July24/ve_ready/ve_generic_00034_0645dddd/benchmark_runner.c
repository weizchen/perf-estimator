...
static uint16_t input1_storage[768] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 768, 1);
  fill_bf16(input1_storage, 768, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 768, 1, input1_storage, input1_storage, 0, 768, 1, output0_storage, output0_storage, 0, 768, 1);
  ofa_dma_memory_fence();
  puts("ve_generic_00034_0645dddd: benchmark completed");
