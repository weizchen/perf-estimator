...
static float input0_storage[76032] __attribute__((aligned(64)));
static float input1_storage[76032] __attribute__((aligned(64)));
static float output0_storage[76032] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 76032, 1);
  fill_f32(input1_storage, 76032, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 11, 8, 24, 36, 6912, 864, 36, 1, input1_storage, input1_storage, 0, 11, 24, 8, 36, 6912, 288, 36, 1, output0_storage, output0_storage, 0, 11, 24, 8, 36, 6912, 288, 36, 1);
  ofa_dma_memory_fence();
  puts("ve_transpose_00054_099865f3: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 76032));
  return 0;
}
