...
int main(void) {
  fill_bf16(input0_storage, 5, 1);
  fill_bf16(input1_storage, 5, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 5, 1, input1_storage, input1_storage, 0, 5, 1, output0_storage, output0_storage, 0, 5, 1);
  ofa_dma_memory_fence();
  puts("ve_elementwise_00026_b215ae6e: benchmark completed");
