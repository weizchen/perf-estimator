...
static int8_t input0_storage[85] __attribute__((aligned(64)));
static int8_t input1_storage[85] __attribute__((aligned(64)));
static int8_t output0_storage[85] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 85, 1);
  fill_i8(input1_storage, 85, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00523_a4a155cf: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 85));
  return 0;
}
