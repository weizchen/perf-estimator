...
static uint16_t input0_storage[6758400] __attribute__((aligned(64)));
static uint16_t input1_storage[2928640] __attribute__((aligned(64)));
static uint16_t input2_storage[1597440] __attribute__((aligned(64)));
static uint16_t output0_storage[1597440] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 6758400, 1);
  fill_f16(input1_storage, 2928640, 2);
  fill_f16(input2_storage, 1597440, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1920, 3520, 3520, 1, input1_storage, input1_storage, 0, 3520, 832, 832, 1, input2_storage, input2_storage, 0, 1920, 832, 832, 1, output0_storage, output0_storage, 0, 1920, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00211_cb6e90d3: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1597440));
  return 0;
}
