...
static float input0_storage[67620] __attribute__((aligned(64)));
static float input1_storage[67620] __attribute__((aligned(64)));
static float output0_storage[67620] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 67620, 1);
  fill_f32(input1_storage, 67620, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 23, 21, 35, 4, 2940, 140, 4, 1, input1_storage, input1_storage, 0, 23, 4, 35, 21, 2940, 735, 21, 1, output0_storage, output0_storage, 0, 23, 4, 35, 21, 2940, 735, 21, 1);
  ofa_dma_memory_fence();
  puts("ve_transpose_00034_c8ccfe37: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 67620));
  return 0;
}
