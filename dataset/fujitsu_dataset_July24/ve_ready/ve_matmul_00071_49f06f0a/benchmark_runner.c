...
static int8_t input1_storage[3784704] __attribute__((aligned(64)));
static int32_t input2_storage[540672] __attribute__((aligned(64)));
static int32_t output0_storage[540672] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 3784704, 2);
  fill_i32(input2_storage, 540672, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 1408, 1408, 1, input2_storage, input2_storage, 0, 384, 1408, 1408, 1, output0_storage, output0_storage, 0, 384, 1408, 1408, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00071_49f06f0a: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 540672));
  return 0;
}
