...
static int8_t output0_storage[2548] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 52, 1);
  fill_i8(input1_storage, 2548, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 13, 4, 4, 1, input1_storage, input1_storage, 0, 13, 4, 49, 196, 49, 1, output0_storage, output0_storage, 0, 13, 4, 49, 196, 49, 1);
  ofa_dma_memory_fence();
  puts("ve_broadcast_00090_fc61af84: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 2548));
  return 0;
}
