...
static int8_t input0_storage[180480] __attribute__((aligned(64)));
static int8_t input1_storage[180480] __attribute__((aligned(64)));
static int8_t output0_storage[180480] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 180480, 1);
  fill_i8(input1_storage, 180480, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 12, 20, 47, 11280, 940, 47, 1, input1_storage, input1_storage, 0, 16, 12, 47, 20, 11280, 940, 20, 1, output0_storage, output0_storage, 0, 16, 12, 47, 20, 11280, 940, 20, 1);
  ofa_dma_memory_fence();
  puts("ve_transpose_00014_29dd3703: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 180480));
  return 0;
}
