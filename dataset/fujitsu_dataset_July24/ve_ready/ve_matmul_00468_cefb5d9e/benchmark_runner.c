...
static float input0_storage[1232896] __attribute__((aligned(64)));
static float input1_storage[917504] __attribute__((aligned(64)));
static float input2_storage[5636096] __attribute__((aligned(64)));
static float output0_storage[5636096] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1232896, 1);
  fill_f32(input1_storage, 917504, 2);
  fill_f32(input2_storage, 5636096, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 448, 448, 1, input1_storage, input1_storage, 0, 448, 2048, 2048, 1, input2_storage, input2_storage, 0, 2752, 2048, 2048, 1, output0_storage, output0_storage, 0, 2752, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00468_cefb5d9e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5636096));
  return 0;
}
