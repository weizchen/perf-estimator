...
static int8_t input0_storage[156] __attribute__((aligned(64)));
static int8_t input1_storage[156] __attribute__((aligned(64)));
static int8_t output0_storage[156] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 156, 1);
  fill_i8(input1_storage, 156, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 156, 1, input1_storage, input1_storage, 0, 156, 1, output0_storage, output0_storage, 0, 156, 1);
  ofa_dma_memory_fence();
  puts("ve_generic_00777_7b737a04: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 156));
  return 0;
}
