...
static uint16_t input1_storage[5709824] __attribute__((aligned(64)));
static float input2_storage[671744] __attribute__((aligned(64)));
static float output0_storage[671744] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 5709824, 2);
  fill_f32(input2_storage, 671744, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 2624, 2624, 1, input2_storage, input2_storage, 0, 256, 2624, 2624, 1, output0_storage, output0_storage, 0, 256, 2624, 2624, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00102_20ae50ac: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 671744));
  return 0;
}
