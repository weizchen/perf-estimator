...
static int8_t input0_storage[376832] __attribute__((aligned(64)));
static int8_t input1_storage[1507328] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 376832, 1);
  fill_i8(input1_storage, 1507328, 2);
  fill_i32(input2_storage, 65536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 512, 512, 1, input2_storage, input2_storage, 0, 128, 512, 512, 1, output0_storage, output0_storage, 0, 128, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00090_cd0c5705: benchmark completed");
