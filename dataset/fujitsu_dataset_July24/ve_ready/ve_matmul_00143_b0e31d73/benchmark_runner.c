...
static uint16_t input0_storage[7008256] __attribute__((aligned(64)));
static uint16_t input1_storage[1662976] __attribute__((aligned(64)));
static float input2_storage[3383296] __attribute__((aligned(64)));
static float output0_storage[3383296] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 7008256, 1);
  fill_f16(input1_storage, 1662976, 2);
  fill_f32(input2_storage, 3383296, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3776, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 896, 896, 1, input2_storage, input2_storage, 0, 3776, 896, 896, 1, output0_storage, output0_storage, 0, 3776, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00143_b0e31d73: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3383296));
  return 0;
}
