...
static uint16_t input0_storage[4997120] __attribute__((aligned(64)));
static uint16_t input1_storage[2621440] __attribute__((aligned(64)));
static uint16_t input2_storage[7995392] __attribute__((aligned(64)));
static uint16_t output0_storage[7995392] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4997120, 1);
  fill_f16(input1_storage, 2621440, 2);
  fill_f16(input2_storage, 7995392, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3904, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 2048, 2048, 1, input2_storage, input2_storage, 0, 3904, 2048, 2048, 1, output0_storage, output0_storage, 0, 3904, 2048, 2048, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00252_e168d3b3: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 7995392));
  return 0;
}
