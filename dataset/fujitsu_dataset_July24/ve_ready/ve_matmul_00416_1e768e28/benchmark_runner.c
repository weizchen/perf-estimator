...
static float input0_storage[1474560] __attribute__((aligned(64)));
static float input1_storage[1327104] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1474560, 1);
  fill_f32(input1_storage, 1327104, 2);
  fill_f32(input2_storage, 1474560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1280, 1152, 1152, 1, input1_storage, input1_storage, 0, 1152, 1152, 1152, 1, input2_storage, input2_storage, 0, 1280, 1152, 1152, 1, output0_storage, output0_storage, 0, 1280, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00416_1e768e28: benchmark completed");
