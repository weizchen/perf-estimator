...
static float input0_storage[1204224] __attribute__((aligned(64)));
static float input1_storage[3870720] __attribute__((aligned(64)));
static float input2_storage[2580480] __attribute__((aligned(64)));
static float output0_storage[2580480] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1204224, 1);
  fill_f32(input1_storage, 3870720, 2);
  fill_f32(input2_storage, 2580480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 1344, 1344, 1, input1_storage, input1_storage, 0, 1344, 2880, 2880, 1, input2_storage, input2_storage, 0, 896, 2880, 2880, 1, output0_storage, output0_storage, 0, 896, 2880, 2880, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00409_62096f31: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2580480));
  return 0;
}
