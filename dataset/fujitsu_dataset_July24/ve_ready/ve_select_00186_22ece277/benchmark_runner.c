...
static float input1_storage[81] __attribute__((aligned(64)));
static float input2_storage[81] __attribute__((aligned(64)));
static float input3_storage[81] __attribute__((aligned(64)));
static float output0_storage[81] __attribute__((aligned(64)));
...
int main(void) {
  fill_i1(input0_storage, 81, 1);
  fill_f32(input1_storage, 81, 2);
  fill_f32(input2_storage, 81, 3);
  fill_f32(input3_storage, 81, 4);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 81, 1, input1_storage, input1_storage, 0, 81, 1, input2_storage, input2_storage, 0, 81, 1, input3_storage, input3_storage, 0, 81, 1, output0_storage, output0_storage, 0, 81, 1);
  ofa_dma_memory_fence();
  puts("ve_select_00186_22ece277: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 81));
  return 0;
}
