...
static int8_t input1_storage[6881280] __attribute__((aligned(64)));
static int32_t input2_storage[1376256] __attribute__((aligned(64)));
static int32_t output0_storage[1376256] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 6881280, 2);
  fill_i32(input2_storage, 1376256, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 768, 3840, 3840, 1, input1_storage, input1_storage, 0, 3840, 1792, 1792, 1, input2_storage, input2_storage, 0, 768, 1792, 1792, 1, output0_storage, output0_storage, 0, 768, 1792, 1792, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00026_d452b1a8: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 1376256));
  return 0;
}
