...
static uint16_t input0_storage[2023424] __attribute__((aligned(64)));
static uint16_t input1_storage[1712128] __attribute__((aligned(64)));
static uint16_t input2_storage[2342912] __attribute__((aligned(64)));
static uint16_t output0_storage[2342912] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2023424, 1);
  fill_f16(input1_storage, 1712128, 2);
  fill_f16(input2_storage, 2342912, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 1408, 1408, 1, input2_storage, input2_storage, 0, 1664, 1408, 1408, 1, output0_storage, output0_storage, 0, 1664, 1408, 1408, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00266_27fbbfb6: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2342912));
  return 0;
}
