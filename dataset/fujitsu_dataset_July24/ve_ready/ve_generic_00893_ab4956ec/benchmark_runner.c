...
static uint16_t input0_storage[49] __attribute__((aligned(64)));
static uint16_t input1_storage[49] __attribute__((aligned(64)));
static uint16_t output0_storage[49] __attribute__((aligned(64)));
...
int main(void) {
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00893_ab4956ec: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 49));
  return 0;
}
