...
static uint16_t input2_storage[196] __attribute__((aligned(64)));
static uint16_t output0_storage[196] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 196, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 28, 7, 7, 1, input1_storage, input1_storage, 0, 28, 7, 7, 1, input2_storage, input2_storage, 0, 28, 7, 7, 1, output0_storage, output0_storage, 0, 28, 7, 7, 1);
  ofa_dma_memory_fence();
  puts("ve_elementwise_00222_7e622a06: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 196));
  return 0;
}
