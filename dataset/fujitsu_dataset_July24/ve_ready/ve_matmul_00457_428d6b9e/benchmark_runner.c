...
static float input0_storage[1146880] __attribute__((aligned(64)));
static float input1_storage[8028160] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1146880, 1);
  fill_f32(input1_storage, 8028160, 2);
  fill_f32(input2_storage, 1835008, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 2240, 2240, 1, input1_storage, input1_storage, 0, 2240, 3584, 3584, 1, input2_storage, input2_storage, 0, 512, 3584, 3584, 1, output0_storage, output0_storage, 0, 512, 3584, 3584, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00457_428d6b9e: benchmark completed");
