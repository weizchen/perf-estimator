...
static uint16_t input0_storage[2809856] __attribute__((aligned(64)));
static uint16_t input1_storage[172032] __attribute__((aligned(64)));
static float input2_storage[602112] __attribute__((aligned(64)));
static float output0_storage[602112] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 2809856, 1);
  fill_bf16(input1_storage, 172032, 2);
  fill_f32(input2_storage, 602112, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3136, 896, 896, 1, input1_storage, input1_storage, 0, 896, 192, 192, 1, input2_storage, input2_storage, 0, 3136, 192, 192, 1, output0_storage, output0_storage, 0, 3136, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00354_1aeefb72: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 602112));
  return 0;
}
