...
static uint16_t input0_storage[69] __attribute__((aligned(64)));
static uint16_t input1_storage[69] __attribute__((aligned(64)));
static uint16_t output0_storage[69] __attribute__((aligned(64)));
...
int main(void) {
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00244_e76cfb69: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 69));
  return 0;
}
