...
static uint16_t input2_storage[389120] __attribute__((aligned(64)));
static uint16_t output0_storage[389120] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 389120, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 1216, 1216, 1, input2_storage, input2_storage, 0, 320, 1216, 1216, 1, output0_storage, output0_storage, 0, 320, 1216, 1216, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00276_11d14f22: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 389120));
  return 0;
}
