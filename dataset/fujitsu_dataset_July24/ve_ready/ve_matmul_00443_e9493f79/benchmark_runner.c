...
static float input0_storage[126976] __attribute__((aligned(64)));
static float input1_storage[253952] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 126976, 1);
  fill_f32(input1_storage, 253952, 2);
  fill_f32(input2_storage, 8192, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 1984, 1984, 1, input1_storage, input1_storage, 0, 1984, 128, 128, 1, input2_storage, input2_storage, 0, 64, 128, 128, 1, output0_storage, output0_storage, 0, 64, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00443_e9493f79: benchmark completed");
