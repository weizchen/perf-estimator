...
static uint16_t input0_storage[1261568] __attribute__((aligned(64)));
static uint16_t input1_storage[1531904] __attribute__((aligned(64)));
static uint16_t input2_storage[974848] __attribute__((aligned(64)));
static uint16_t output0_storage[974848] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1261568, 1);
  fill_f16(input1_storage, 1531904, 2);
  fill_f16(input2_storage, 974848, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 1408, 1408, 1, input1_storage, input1_storage, 0, 1408, 1088, 1088, 1, input2_storage, input2_storage, 0, 896, 1088, 1088, 1, output0_storage, output0_storage, 0, 896, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00228_1891136f: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 974848));
  return 0;
}
