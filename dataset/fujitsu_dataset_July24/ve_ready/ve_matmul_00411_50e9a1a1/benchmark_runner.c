...
static float input0_storage[6766592] __attribute__((aligned(64)));
static float input1_storage[10633216] __attribute__((aligned(64)));
static float input2_storage[5046272] __attribute__((aligned(64)));
static float output0_storage[5046272] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 6766592, 1);
  fill_f32(input1_storage, 10633216, 2);
  fill_f32(input2_storage, 5046272, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 3776, 3776, 1, input1_storage, input1_storage, 0, 3776, 2816, 2816, 1, input2_storage, input2_storage, 0, 1792, 2816, 2816, 1, output0_storage, output0_storage, 0, 1792, 2816, 2816, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00411_50e9a1a1: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5046272));
  return 0;
}
