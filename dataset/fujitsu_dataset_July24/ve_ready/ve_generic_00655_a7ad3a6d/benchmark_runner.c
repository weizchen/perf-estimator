...
static uint16_t input0_storage[43] __attribute__((aligned(64)));
static uint16_t input1_storage[43] __attribute__((aligned(64)));
static uint16_t output0_storage[43] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 43, 1);
  fill_f16(input1_storage, 43, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00655_a7ad3a6d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 43));
  return 0;
}
