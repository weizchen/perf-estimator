...
static uint16_t input0_storage[8085504] __attribute__((aligned(64)));
static uint16_t input1_storage[6545408] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 8085504, 1);
  fill_f16(input1_storage, 6545408, 2);
  fill_f16(input2_storage, 5849088, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2688, 3008, 3008, 1, input1_storage, input1_storage, 0, 3008, 2176, 2176, 1, input2_storage, input2_storage, 0, 2688, 2176, 2176, 1, output0_storage, output0_storage, 0, 2688, 2176, 2176, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00297_b9991e5c: benchmark completed");
