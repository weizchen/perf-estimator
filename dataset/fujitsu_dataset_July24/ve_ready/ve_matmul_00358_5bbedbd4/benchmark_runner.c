...
static uint16_t input0_storage[1818624] __attribute__((aligned(64)));
static uint16_t input1_storage[757760] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1818624, 1);
  fill_bf16(input1_storage, 757760, 2);
  fill_f32(input2_storage, 245760, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 768, 2368, 2368, 1, input1_storage, input1_storage, 0, 2368, 320, 320, 1, input2_storage, input2_storage, 0, 768, 320, 320, 1, output0_storage, output0_storage, 0, 768, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00358_5bbedbd4: benchmark completed");
