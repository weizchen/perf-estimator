...
static float input0_storage[35] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 35, 1);
  fill_f32(input1_storage, 35, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 35, 1, input1_storage, input1_storage, 0, 35, 1, output0_storage, output0_storage, 0, 35, 1);
  ofa_dma_memory_fence();
  puts("ve_generic_00837_5f6467b1: benchmark completed");
