...
static uint16_t input1_storage[1761280] __attribute__((aligned(64)));
static uint16_t input2_storage[2994176] __attribute__((aligned(64)));
static uint16_t output0_storage[2994176] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1761280, 2);
  fill_f16(input2_storage, 2994176, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 640, 640, 1, input1_storage, input1_storage, 0, 640, 2752, 2752, 1, input2_storage, input2_storage, 0, 1088, 2752, 2752, 1, output0_storage, output0_storage, 0, 1088, 2752, 2752, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00269_7019562d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2994176));
  return 0;
}
