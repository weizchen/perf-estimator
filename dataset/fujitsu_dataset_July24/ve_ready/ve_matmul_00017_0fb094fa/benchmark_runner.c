...
static int8_t input0_storage[2752512] __attribute__((aligned(64)));
static int8_t input1_storage[6537216] __attribute__((aligned(64)));
static int32_t input2_storage[2490368] __attribute__((aligned(64)));
static int32_t output0_storage[2490368] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 2752512, 1);
  fill_i8(input1_storage, 6537216, 2);
  fill_i32(input2_storage, 2490368, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1024, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 2432, 2432, 1, input2_storage, input2_storage, 0, 1024, 2432, 2432, 1, output0_storage, output0_storage, 0, 1024, 2432, 2432, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00017_0fb094fa: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2490368));
  return 0;
}
