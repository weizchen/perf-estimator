...
static int8_t input0_storage[6127616] __attribute__((aligned(64)));
static int8_t input1_storage[4685824] __attribute__((aligned(64)));
static int32_t input2_storage[3620864] __attribute__((aligned(64)));
static int32_t output0_storage[3620864] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 6127616, 1);
  fill_i8(input1_storage, 4685824, 2);
  fill_i32(input2_storage, 3620864, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2176, 2816, 2816, 1, input1_storage, input1_storage, 0, 2816, 1664, 1664, 1, input2_storage, input2_storage, 0, 2176, 1664, 1664, 1, output0_storage, output0_storage, 0, 2176, 1664, 1664, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00031_00360879: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 3620864));
  return 0;
}
