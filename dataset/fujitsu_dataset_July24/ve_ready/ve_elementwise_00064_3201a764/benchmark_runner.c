...
static float input0_storage[60] __attribute__((aligned(64)));
static float input1_storage[60] __attribute__((aligned(64)));
static float input2_storage[60] __attribute__((aligned(64)));
static float output0_storage[60] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 60, 1);
  fill_f32(input1_storage, 60, 2);
  fill_f32(input2_storage, 60, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 10, 6, 6, 1, input1_storage, input1_storage, 0, 10, 6, 6, 1, input2_storage, input2_storage, 0, 10, 6, 6, 1, output0_storage, output0_storage, 0, 10, 6, 6, 1);
  ofa_dma_memory_fence();
  puts("ve_elementwise_00064_3201a764: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 60));
  return 0;
}
