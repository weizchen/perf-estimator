...
static float input2_storage[9977856] __attribute__((aligned(64)));
static float output0_storage[9977856] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 237568, 1);
  fill_bf16(input1_storage, 172032, 2);
  fill_f32(input2_storage, 9977856, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3712, 64, 64, 1, input1_storage, input1_storage, 0, 64, 2688, 2688, 1, input2_storage, input2_storage, 0, 3712, 2688, 2688, 1, output0_storage, output0_storage, 0, 3712, 2688, 2688, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00396_bc107cb4: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 9977856));
  return 0;
}
