...
static float input0_storage[1700] __attribute__((aligned(64)));
static float input1_storage[1700] __attribute__((aligned(64)));
static float input2_storage[1700] __attribute__((aligned(64)));
static float output0_storage[1700] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1700, 1);
  fill_f32(input1_storage, 1700, 2);
  fill_f32(input2_storage, 1700, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 34, 50, 50, 1, input1_storage, input1_storage, 0, 34, 50, 50, 1, input2_storage, input2_storage, 0, 34, 50, 50, 1, output0_storage, output0_storage, 0, 34, 50, 50, 1);
  ofa_dma_memory_fence();
  puts("ve_elementwise_00006_52799b6c: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1700));
  return 0;
}
