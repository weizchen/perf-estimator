...
static int8_t input0_storage[1081344] __attribute__((aligned(64)));
static int8_t input1_storage[2523136] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 1081344, 1);
  fill_i8(input1_storage, 2523136, 2);
  fill_i32(input2_storage, 344064, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 2816, 2816, 1, input1_storage, input1_storage, 0, 2816, 896, 896, 1, input2_storage, input2_storage, 0, 384, 896, 896, 1, output0_storage, output0_storage, 0, 384, 896, 896, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00083_62171685: benchmark completed");
