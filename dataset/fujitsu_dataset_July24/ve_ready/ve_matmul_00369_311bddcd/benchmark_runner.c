...
static float input2_storage[3133440] __attribute__((aligned(64)));
static float output0_storage[3133440] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 139264, 1);
  fill_bf16(input1_storage, 368640, 2);
  fill_f32(input2_storage, 3133440, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2880, 2880, 1, input2_storage, input2_storage, 0, 1088, 2880, 2880, 1, output0_storage, output0_storage, 0, 1088, 2880, 2880, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00369_311bddcd: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3133440));
  return 0;
}
