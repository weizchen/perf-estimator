...
static uint16_t input0_storage[2375680] __attribute__((aligned(64)));
static uint16_t input1_storage[475136] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2375680, 1);
  fill_f16(input1_storage, 475136, 2);
  fill_f16(input2_storage, 81920, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 640, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 128, 128, 1, input2_storage, input2_storage, 0, 640, 128, 128, 1, output0_storage, output0_storage, 0, 640, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00293_074af035: benchmark completed");
