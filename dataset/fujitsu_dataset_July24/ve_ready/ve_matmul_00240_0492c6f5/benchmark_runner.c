...
static uint16_t input0_storage[831488] __attribute__((aligned(64)));
static uint16_t input1_storage[143360] __attribute__((aligned(64)));
static uint16_t input2_storage[593920] __attribute__((aligned(64)));
static uint16_t output0_storage[593920] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 831488, 1);
  fill_f16(input1_storage, 143360, 2);
  fill_f16(input2_storage, 593920, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1856, 448, 448, 1, input1_storage, input1_storage, 0, 448, 320, 320, 1, input2_storage, input2_storage, 0, 1856, 320, 320, 1, output0_storage, output0_storage, 0, 1856, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00240_0492c6f5: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 593920));
  return 0;
}
