...
static float input0_storage[475136] __attribute__((aligned(64)));
static float input1_storage[6295552] __attribute__((aligned(64)));
static float input2_storage[868352] __attribute__((aligned(64)));
static float output0_storage[868352] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 475136, 1);
  fill_f32(input1_storage, 6295552, 2);
  fill_f32(input2_storage, 868352, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 1856, 1856, 1, input1_storage, input1_storage, 0, 1856, 3392, 3392, 1, input2_storage, input2_storage, 0, 256, 3392, 3392, 1, output0_storage, output0_storage, 0, 256, 3392, 3392, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00447_bb2cebcc: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 868352));
  return 0;
}
