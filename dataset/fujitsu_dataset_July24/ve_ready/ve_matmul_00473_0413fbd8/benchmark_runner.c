...
static float input0_storage[307200] __attribute__((aligned(64)));
static float input1_storage[880640] __attribute__((aligned(64)));
static float input2_storage[2641920] __attribute__((aligned(64)));
static float output0_storage[2641920] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 307200, 1);
  fill_f32(input1_storage, 880640, 2);
  fill_f32(input2_storage, 2641920, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 960, 320, 320, 1, input1_storage, input1_storage, 0, 320, 2752, 2752, 1, input2_storage, input2_storage, 0, 960, 2752, 2752, 1, output0_storage, output0_storage, 0, 960, 2752, 2752, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00473_0413fbd8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2641920));
  return 0;
}
