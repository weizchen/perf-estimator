...
static uint16_t input0_storage[1990656] __attribute__((aligned(64)));
static uint16_t input1_storage[479232] __attribute__((aligned(64)));
static float input2_storage[2875392] __attribute__((aligned(64)));
static float output0_storage[2875392] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1990656, 1);
  fill_bf16(input1_storage, 479232, 2);
  fill_f32(input2_storage, 2875392, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3456, 576, 576, 1, input1_storage, input1_storage, 0, 576, 832, 832, 1, input2_storage, input2_storage, 0, 3456, 832, 832, 1, output0_storage, output0_storage, 0, 3456, 832, 832, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00332_87da1b3d: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2875392));
  return 0;
}
