...
static uint16_t input0_storage[1605632] __attribute__((aligned(64)));
static uint16_t input1_storage[1892352] __attribute__((aligned(64)));
static float input2_storage[3784704] __attribute__((aligned(64)));
static float output0_storage[3784704] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1605632, 1);
  fill_bf16(input1_storage, 1892352, 2);
  fill_f32(input2_storage, 3784704, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 896, 896, 1, input1_storage, input1_storage, 0, 896, 2112, 2112, 1, input2_storage, input2_storage, 0, 1792, 2112, 2112, 1, output0_storage, output0_storage, 0, 1792, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00357_7a9b2484: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3784704));
  return 0;
}
