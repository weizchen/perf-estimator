...
static float input2_storage[884736] __attribute__((aligned(64)));
static float output0_storage[884736] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 589824, 2);
  fill_f32(input2_storage, 884736, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 384, 384, 1, input1_storage, input1_storage, 0, 384, 1536, 1536, 1, input2_storage, input2_storage, 0, 576, 1536, 1536, 1, output0_storage, output0_storage, 0, 576, 1536, 1536, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00328_3f4a4560: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 884736));
  return 0;
}
