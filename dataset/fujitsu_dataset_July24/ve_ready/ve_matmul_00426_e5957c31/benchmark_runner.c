...
static float input0_storage[40960] __attribute__((aligned(64)));
static float input1_storage[245760] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 40960, 1);
  fill_f32(input1_storage, 245760, 2);
  fill_f32(input2_storage, 614400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 128, 128, 1, input1_storage, input1_storage, 0, 128, 1920, 1920, 1, input2_storage, input2_storage, 0, 320, 1920, 1920, 1, output0_storage, output0_storage, 0, 320, 1920, 1920, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00426_e5957c31: benchmark completed");
