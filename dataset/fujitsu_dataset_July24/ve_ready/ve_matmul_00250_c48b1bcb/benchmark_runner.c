...
static uint16_t input0_storage[5644288] __attribute__((aligned(64)));
static uint16_t input1_storage[2387968] __attribute__((aligned(64)));
static uint16_t input2_storage[1171456] __attribute__((aligned(64)));
static uint16_t output0_storage[1171456] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5644288, 1);
  fill_f16(input1_storage, 2387968, 2);
  fill_f16(input2_storage, 1171456, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1664, 3392, 3392, 1, input1_storage, input1_storage, 0, 3392, 704, 704, 1, input2_storage, input2_storage, 0, 1664, 704, 704, 1, output0_storage, output0_storage, 0, 1664, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00250_c48b1bcb: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1171456));
  return 0;
}
