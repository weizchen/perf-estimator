...
static uint16_t input0_storage[835584] __attribute__((aligned(64)));
static uint16_t input1_storage[11698176] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 835584, 1);
  fill_bf16(input1_storage, 11698176, 2);
  fill_f32(input2_storage, 917504, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 3264, 3264, 1, input1_storage, input1_storage, 0, 3264, 3584, 3584, 1, input2_storage, input2_storage, 0, 256, 3584, 3584, 1, output0_storage, output0_storage, 0, 256, 3584, 3584, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00313_4ab3dc33: benchmark completed");
