...
static uint16_t input1_storage[278528] __attribute__((aligned(64)));
static float input2_storage[5570560] __attribute__((aligned(64)));
static float output0_storage[5570560] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 327680, 1);
  fill_bf16(input1_storage, 278528, 2);
  fill_f32(input2_storage, 5570560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2560, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2176, 2176, 1, input2_storage, input2_storage, 0, 2560, 2176, 2176, 1, output0_storage, output0_storage, 0, 2560, 2176, 2176, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00372_0bf1ef15: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5570560));
  return 0;
}
