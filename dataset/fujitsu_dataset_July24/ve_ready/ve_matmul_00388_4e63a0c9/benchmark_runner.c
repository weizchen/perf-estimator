...
static uint16_t input1_storage[638976] __attribute__((aligned(64)));
static float input2_storage[7188480] __attribute__((aligned(64)));
static float output0_storage[7188480] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 737280, 1);
  fill_bf16(input1_storage, 638976, 2);
  fill_f32(input2_storage, 7188480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2880, 256, 256, 1, input1_storage, input1_storage, 0, 256, 2496, 2496, 1, input2_storage, input2_storage, 0, 2880, 2496, 2496, 1, output0_storage, output0_storage, 0, 2880, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00388_4e63a0c9: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7188480));
  return 0;
}
