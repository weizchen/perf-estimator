...
static float input0_storage[1712128] __attribute__((aligned(64)));
static float input1_storage[450560] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1712128, 1);
  fill_f32(input1_storage, 450560, 2);
  fill_f32(input2_storage, 1556480, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2432, 704, 704, 1, input1_storage, input1_storage, 0, 704, 640, 640, 1, input2_storage, input2_storage, 0, 2432, 640, 640, 1, output0_storage, output0_storage, 0, 2432, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00434_09888309: benchmark completed");
