...
static float input0_storage[3940352] __attribute__((aligned(64)));
static float input1_storage[2662400] __attribute__((aligned(64)));
static float input2_storage[3788800] __attribute__((aligned(64)));
static float output0_storage[3788800] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 3940352, 1);
  fill_f32(input1_storage, 2662400, 2);
  fill_f32(input2_storage, 3788800, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2368, 1664, 1664, 1, input1_storage, input1_storage, 0, 1664, 1600, 1600, 1, input2_storage, input2_storage, 0, 2368, 1600, 1600, 1, output0_storage, output0_storage, 0, 2368, 1600, 1600, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00478_e75d9764: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 3788800));
  return 0;
}
