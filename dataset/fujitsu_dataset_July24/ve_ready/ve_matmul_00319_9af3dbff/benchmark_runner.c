...
static uint16_t input1_storage[1409024] __attribute__((aligned(64)));
static float input2_storage[528384] __attribute__((aligned(64)));
static float output0_storage[528384] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 1409024, 2);
  fill_f32(input2_storage, 528384, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 512, 512, 1, input1_storage, input1_storage, 0, 512, 2752, 2752, 1, input2_storage, input2_storage, 0, 192, 2752, 2752, 1, output0_storage, output0_storage, 0, 192, 2752, 2752, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00319_9af3dbff: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 528384));
  return 0;
}
