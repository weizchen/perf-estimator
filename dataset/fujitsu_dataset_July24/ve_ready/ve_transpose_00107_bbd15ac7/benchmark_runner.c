...
static float input0_storage[2822] __attribute__((aligned(64)));
static float input1_storage[2822] __attribute__((aligned(64)));
static float output0_storage[2822] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 2822, 1);
  fill_f32(input1_storage, 2822, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_transpose_00107_bbd15ac7: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2822));
  return 0;
}
