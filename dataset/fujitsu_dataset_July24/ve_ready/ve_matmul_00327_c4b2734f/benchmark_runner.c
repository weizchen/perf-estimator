...
static uint16_t input0_storage[10780672] __attribute__((aligned(64)));
static uint16_t input1_storage[688128] __attribute__((aligned(64)));
static float input2_storage[577536] __attribute__((aligned(64)));
static float output0_storage[577536] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 10780672, 1);
  fill_bf16(input1_storage, 688128, 2);
  fill_f32(input2_storage, 577536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3008, 3584, 3584, 1, input1_storage, input1_storage, 0, 3584, 192, 192, 1, input2_storage, input2_storage, 0, 3008, 192, 192, 1, output0_storage, output0_storage, 0, 3008, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00327_c4b2734f: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 577536));
  return 0;
}
