...
static uint16_t input0_storage[471040] __attribute__((aligned(64)));
static uint16_t input1_storage[659456] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 471040, 1);
  fill_f16(input1_storage, 659456, 2);
  fill_f16(input2_storage, 143360, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 1472, 1472, 1, input1_storage, input1_storage, 0, 1472, 448, 448, 1, input2_storage, input2_storage, 0, 320, 448, 448, 1, output0_storage, output0_storage, 0, 320, 448, 448, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00299_e667d6f9: benchmark completed");
