...
static uint16_t input0_storage[5275648] __attribute__((aligned(64)));
static uint16_t input1_storage[229376] __attribute__((aligned(64)));
static uint16_t input2_storage[376832] __attribute__((aligned(64)));
static uint16_t output0_storage[376832] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5275648, 1);
  fill_f16(input1_storage, 229376, 2);
  fill_f16(input2_storage, 376832, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2944, 1792, 1792, 1, input1_storage, input1_storage, 0, 1792, 128, 128, 1, input2_storage, input2_storage, 0, 2944, 128, 128, 1, output0_storage, output0_storage, 0, 2944, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00264_9bfbdbfe: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 376832));
  return 0;
}
