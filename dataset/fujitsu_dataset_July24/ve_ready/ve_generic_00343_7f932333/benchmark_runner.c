...
static uint16_t input0_storage[1128] __attribute__((aligned(64)));
static uint16_t input1_storage[1128] __attribute__((aligned(64)));
static uint16_t output0_storage[1128] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1128, 1);
  fill_bf16(input1_storage, 1128, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1128, 1, input1_storage, input1_storage, 0, 1128, 1, output0_storage, output0_storage, 0, 1128, 1);
  ofa_dma_memory_fence();
  puts("ve_generic_00343_7f932333: benchmark completed");
  printf("output0_bf16bits_checksum=%lld\n", checksum_u16(output0_storage, 1128));
  return 0;
}
