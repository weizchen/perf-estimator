...
static uint16_t input0_storage[2580480] __attribute__((aligned(64)));
static uint16_t input1_storage[368640] __attribute__((aligned(64)));
static float input2_storage[2322432] __attribute__((aligned(64)));
static float output0_storage[2322432] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 2580480, 1);
  fill_bf16(input1_storage, 368640, 2);
  fill_f32(input2_storage, 2322432, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4032, 640, 640, 1, input1_storage, input1_storage, 0, 640, 576, 576, 1, input2_storage, input2_storage, 0, 4032, 576, 576, 1, output0_storage, output0_storage, 0, 4032, 576, 576, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00312_186aa597: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2322432));
  return 0;
}
