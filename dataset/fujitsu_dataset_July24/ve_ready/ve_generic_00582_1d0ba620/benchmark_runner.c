...
static uint16_t input1_storage[109] __attribute__((aligned(64)));
static uint16_t output0_storage[109] __attribute__((aligned(64)));
...
int main(void) {
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00582_1d0ba620: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 109));
  return 0;
}
