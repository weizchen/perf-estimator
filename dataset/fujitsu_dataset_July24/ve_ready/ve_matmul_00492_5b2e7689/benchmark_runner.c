...
static float input0_storage[401408] __attribute__((aligned(64)));
static float input1_storage[602112] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 401408, 1);
  fill_f32(input1_storage, 602112, 2);
  fill_f32(input2_storage, 24576, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 192, 192, 1, input2_storage, input2_storage, 0, 128, 192, 192, 1, output0_storage, output0_storage, 0, 128, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00492_5b2e7689: benchmark completed");
