...
static float input0_storage[1404928] __attribute__((aligned(64)));
static float input1_storage[2007040] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1404928, 1);
  fill_f32(input1_storage, 2007040, 2);
  fill_f32(input2_storage, 286720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 640, 640, 1, input2_storage, input2_storage, 0, 448, 640, 640, 1, output0_storage, output0_storage, 0, 448, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00474_ae774814: benchmark completed");
