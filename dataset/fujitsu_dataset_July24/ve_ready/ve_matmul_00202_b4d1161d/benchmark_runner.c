...
static uint16_t input0_storage[1400832] __attribute__((aligned(64)));
static uint16_t input1_storage[1105920] __attribute__((aligned(64)));
static uint16_t input2_storage[10506240] __attribute__((aligned(64)));
static uint16_t output0_storage[10506240] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 1400832, 1);
  fill_f16(input1_storage, 1105920, 2);
  fill_f16(input2_storage, 10506240, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3648, 384, 384, 1, input1_storage, input1_storage, 0, 384, 2880, 2880, 1, input2_storage, input2_storage, 0, 3648, 2880, 2880, 1, output0_storage, output0_storage, 0, 3648, 2880, 2880, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00202_b4d1161d: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 10506240));
  return 0;
}
