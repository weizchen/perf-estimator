...
static uint16_t input0_storage[4177920] __attribute__((aligned(64)));
static uint16_t input1_storage[983040] __attribute__((aligned(64)));
static float input2_storage[278528] __attribute__((aligned(64)));
static float output0_storage[278528] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 4177920, 1);
  fill_bf16(input1_storage, 983040, 2);
  fill_f32(input2_storage, 278528, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1088, 3840, 3840, 1, input1_storage, input1_storage, 0, 3840, 256, 256, 1, input2_storage, input2_storage, 0, 1088, 256, 256, 1, output0_storage, output0_storage, 0, 1088, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00347_f8b23a02: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 278528));
  return 0;
}
