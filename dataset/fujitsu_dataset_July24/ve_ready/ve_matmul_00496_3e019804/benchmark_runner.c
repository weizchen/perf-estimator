...
static float input0_storage[9027584] __attribute__((aligned(64)));
static float input1_storage[7364608] __attribute__((aligned(64)));
static float input2_storage[4825088] __attribute__((aligned(64)));
static float output0_storage[4825088] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 9027584, 1);
  fill_f32(input1_storage, 7364608, 2);
  fill_f32(input2_storage, 4825088, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2432, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 1984, 1984, 1, input2_storage, input2_storage, 0, 2432, 1984, 1984, 1, output0_storage, output0_storage, 0, 2432, 1984, 1984, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00496_3e019804: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4825088));
  return 0;
}
