...
static float input0_storage[1662976] __attribute__((aligned(64)));
static float input1_storage[712704] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1662976, 1);
  fill_f32(input1_storage, 712704, 2);
  fill_f32(input2_storage, 86016, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 448, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 192, 192, 1, input2_storage, input2_storage, 0, 448, 192, 192, 1, output0_storage, output0_storage, 0, 448, 192, 192, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00420_3eabcf1d: benchmark completed");
