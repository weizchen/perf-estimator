...
static uint16_t input0_storage[1740800] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1740800, 1);
  fill_bf16(input1_storage, 69632, 2);
  fill_f32(input2_storage, 102400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 1088, 1088, 1, input1_storage, input1_storage, 0, 1088, 64, 64, 1, input2_storage, input2_storage, 0, 1600, 64, 64, 1, output0_storage, output0_storage, 0, 1600, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00350_b51a8e8b: benchmark completed");
