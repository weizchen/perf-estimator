...
static uint16_t input0_storage[405504] __attribute__((aligned(64)));
static uint16_t input1_storage[2793472] __attribute__((aligned(64)));
static uint16_t input2_storage[2285568] __attribute__((aligned(64)));
static uint16_t output0_storage[2285568] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 405504, 1);
  fill_f16(input1_storage, 2793472, 2);
  fill_f16(input2_storage, 2285568, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 704, 704, 1, input1_storage, input1_storage, 0, 704, 3968, 3968, 1, input2_storage, input2_storage, 0, 576, 3968, 3968, 1, output0_storage, output0_storage, 0, 576, 3968, 3968, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00265_6514b642: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 2285568));
  return 0;
}
