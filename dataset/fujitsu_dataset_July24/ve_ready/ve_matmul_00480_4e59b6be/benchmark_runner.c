...
static float input1_storage[90112] __attribute__((aligned(64)));
static float input2_storage[495616] __attribute__((aligned(64)));
static float output0_storage[495616] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 90112, 2);
  fill_f32(input2_storage, 495616, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 128, 128, 1, input1_storage, input1_storage, 0, 128, 704, 704, 1, input2_storage, input2_storage, 0, 704, 704, 704, 1, output0_storage, output0_storage, 0, 704, 704, 704, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00480_4e59b6be: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 495616));
  return 0;
}
