...
static float input1_storage[3014656] __attribute__((aligned(64)));
static float input2_storage[2637824] __attribute__((aligned(64)));
static float output0_storage[2637824] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 3014656, 2);
  fill_f32(input2_storage, 2637824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 1024, 1024, 1, input1_storage, input1_storage, 0, 1024, 2944, 2944, 1, input2_storage, input2_storage, 0, 896, 2944, 2944, 1, output0_storage, output0_storage, 0, 896, 2944, 2944, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00456_6f70e584: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2637824));
  return 0;
}
