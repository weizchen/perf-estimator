...
static float input0_storage[638976] __attribute__((aligned(64)));
static float input1_storage[1224704] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 638976, 1);
  fill_f32(input1_storage, 1224704, 2);
  fill_f32(input2_storage, 1130496, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 768, 832, 832, 1, input1_storage, input1_storage, 0, 832, 1472, 1472, 1, input2_storage, input2_storage, 0, 768, 1472, 1472, 1, output0_storage, output0_storage, 0, 768, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00445_1adfce12: benchmark completed");
