...
static uint16_t input1_storage[1003520] __attribute__((aligned(64)));
static float input2_storage[122880] __attribute__((aligned(64)));
static float output0_storage[122880] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_bf16(input1_storage, 1003520, 2);
  fill_f32(input2_storage, 122880, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 320, 320, 1, input2_storage, input2_storage, 0, 384, 320, 320, 1, output0_storage, output0_storage, 0, 384, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00379_6ca42198: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 122880));
  return 0;
}
