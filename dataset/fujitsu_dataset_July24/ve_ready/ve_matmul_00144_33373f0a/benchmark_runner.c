...
static uint16_t input0_storage[552960] __attribute__((aligned(64)));
static uint16_t input1_storage[9953280] __attribute__((aligned(64)));
static float input2_storage[663552] __attribute__((aligned(64)));
static float output0_storage[663552] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 552960, 1);
  fill_f16(input1_storage, 9953280, 2);
  fill_f32(input2_storage, 663552, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 2880, 2880, 1, input1_storage, input1_storage, 0, 2880, 3456, 3456, 1, input2_storage, input2_storage, 0, 192, 3456, 3456, 1, output0_storage, output0_storage, 0, 192, 3456, 3456, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00144_33373f0a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 663552));
  return 0;
}
