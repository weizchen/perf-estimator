...
static uint16_t input0_storage[2293760] __attribute__((aligned(64)));
static uint16_t input1_storage[2457600] __attribute__((aligned(64)));
static uint16_t input2_storage[3440640] __attribute__((aligned(64)));
static uint16_t output0_storage[3440640] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2293760, 1);
  fill_f16(input1_storage, 2457600, 2);
  fill_f16(input2_storage, 3440640, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1792, 1280, 1280, 1, input1_storage, input1_storage, 0, 1280, 1920, 1920, 1, input2_storage, input2_storage, 0, 1792, 1920, 1920, 1, output0_storage, output0_storage, 0, 1792, 1920, 1920, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00280_288955a2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3440640));
  return 0;
}
