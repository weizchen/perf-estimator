...
static uint16_t input0_storage[36864] __attribute__((aligned(64)));
static uint16_t input1_storage[114688] __attribute__((aligned(64)));
static uint16_t input2_storage[1032192] __attribute__((aligned(64)));
static uint16_t output0_storage[1032192] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 36864, 1);
  fill_f16(input1_storage, 114688, 2);
  fill_f16(input2_storage, 1032192, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 64, 64, 1, input1_storage, input1_storage, 0, 64, 1792, 1792, 1, input2_storage, input2_storage, 0, 576, 1792, 1792, 1, output0_storage, output0_storage, 0, 576, 1792, 1792, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00244_9ffa8c36: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1032192));
  return 0;
}
