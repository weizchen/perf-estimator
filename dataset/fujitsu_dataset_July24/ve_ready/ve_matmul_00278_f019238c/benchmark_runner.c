...
static uint16_t input0_storage[6213632] __attribute__((aligned(64)));
static uint16_t input1_storage[1007616] __attribute__((aligned(64)));
static uint16_t input2_storage[909312] __attribute__((aligned(64)));
static uint16_t output0_storage[909312] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 6213632, 1);
  fill_f16(input1_storage, 1007616, 2);
  fill_f16(input2_storage, 909312, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2368, 2624, 2624, 1, input1_storage, input1_storage, 0, 2624, 384, 384, 1, input2_storage, input2_storage, 0, 2368, 384, 384, 1, output0_storage, output0_storage, 0, 2368, 384, 384, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00278_f019238c: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 909312));
  return 0;
}
