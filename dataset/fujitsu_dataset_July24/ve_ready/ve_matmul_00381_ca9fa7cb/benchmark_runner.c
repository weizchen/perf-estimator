...
static float input2_storage[4096] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 77824, 1);
  fill_bf16(input1_storage, 77824, 2);
  fill_f32(input2_storage, 4096, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 64, 64, 1, input2_storage, input2_storage, 0, 64, 64, 64, 1, output0_storage, output0_storage, 0, 64, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00381_ca9fa7cb: benchmark completed");
