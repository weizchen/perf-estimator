...
static uint16_t input1_storage[2162688] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 2162688, 2);
  fill_f16(input2_storage, 720896, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 768, 768, 1, input1_storage, input1_storage, 0, 768, 2816, 2816, 1, input2_storage, input2_storage, 0, 256, 2816, 2816, 1, output0_storage, output0_storage, 0, 256, 2816, 2816, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00217_f376194a: benchmark completed");
