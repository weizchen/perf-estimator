...
static uint16_t input0_storage[2408448] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 2408448, 1);
  fill_bf16(input1_storage, 573440, 2);
  fill_f32(input2_storage, 1720320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2688, 896, 896, 1, input1_storage, input1_storage, 0, 896, 640, 640, 1, input2_storage, input2_storage, 0, 2688, 640, 640, 1, output0_storage, output0_storage, 0, 2688, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00361_44b59ff0: benchmark completed");
