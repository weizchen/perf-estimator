...
static int8_t input0_storage[13650] __attribute__((aligned(64)));
static int8_t input1_storage[13650] __attribute__((aligned(64)));
static int8_t output0_storage[13650] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 13650, 1);
  fill_i8(input1_storage, 13650, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 25, 6, 13, 7, 546, 91, 7, 1, input1_storage, input1_storage, 0, 25, 7, 13, 6, 546, 78, 6, 1, output0_storage, output0_storage, 0, 25, 7, 13, 6, 546, 78, 6, 1);
  ofa_dma_memory_fence();
  puts("ve_transpose_00059_db346aba: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 13650));
  return 0;
}
