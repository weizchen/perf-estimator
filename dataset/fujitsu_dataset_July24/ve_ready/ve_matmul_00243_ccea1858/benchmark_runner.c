...
static uint16_t input0_storage[3035136] __attribute__((aligned(64)));
static uint16_t input1_storage[159744] __attribute__((aligned(64)));
static uint16_t input2_storage[77824] __attribute__((aligned(64)));
static uint16_t output0_storage[77824] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3035136, 1);
  fill_f16(input1_storage, 159744, 2);
  fill_f16(input2_storage, 77824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1216, 2496, 2496, 1, input1_storage, input1_storage, 0, 2496, 64, 64, 1, input2_storage, input2_storage, 0, 1216, 64, 64, 1, output0_storage, output0_storage, 0, 1216, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00243_ccea1858: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 77824));
  return 0;
}
