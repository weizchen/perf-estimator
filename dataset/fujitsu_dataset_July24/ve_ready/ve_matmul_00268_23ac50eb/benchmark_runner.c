...
static uint16_t input1_storage[1691648] __attribute__((aligned(64)));
static uint16_t input2_storage[1208320] __attribute__((aligned(64)));
static uint16_t output0_storage[1208320] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 1691648, 2);
  fill_f16(input2_storage, 1208320, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 448, 448, 1, input1_storage, input1_storage, 0, 448, 3776, 3776, 1, input2_storage, input2_storage, 0, 320, 3776, 3776, 1, output0_storage, output0_storage, 0, 320, 3776, 3776, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00268_23ac50eb: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1208320));
  return 0;
}
