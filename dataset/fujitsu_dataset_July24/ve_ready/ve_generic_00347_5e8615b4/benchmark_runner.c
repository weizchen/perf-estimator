...
static uint16_t input0_storage[45] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 45, 1);
  fill_bf16(input1_storage, 45, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 45, 1, input1_storage, input1_storage, 0, 45, 1, output0_storage, output0_storage, 0, 45, 1);
  ofa_dma_memory_fence();
  puts("ve_generic_00347_5e8615b4: benchmark completed");
