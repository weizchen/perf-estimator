...
static float input0_storage[450560] __attribute__((aligned(64)));
static float input1_storage[348160] __attribute__((aligned(64)));
static float input2_storage[1531904] __attribute__((aligned(64)));
static float output0_storage[1531904] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 450560, 1);
  fill_f32(input1_storage, 348160, 2);
  fill_f32(input2_storage, 1531904, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 320, 320, 1, input1_storage, input1_storage, 0, 320, 1088, 1088, 1, input2_storage, input2_storage, 0, 1408, 1088, 1088, 1, output0_storage, output0_storage, 0, 1408, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00437_7dcaad3a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1531904));
  return 0;
}
