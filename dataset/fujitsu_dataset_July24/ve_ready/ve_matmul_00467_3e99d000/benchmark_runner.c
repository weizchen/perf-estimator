...
static float input0_storage[6041600] __attribute__((aligned(64)));
static float input1_storage[102400] __attribute__((aligned(64)));
static float input2_storage[241664] __attribute__((aligned(64)));
static float output0_storage[241664] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 6041600, 1);
  fill_f32(input1_storage, 102400, 2);
  fill_f32(input2_storage, 241664, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3776, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 64, 64, 1, input2_storage, input2_storage, 0, 3776, 64, 64, 1, output0_storage, output0_storage, 0, 3776, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00467_3e99d000: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 241664));
  return 0;
}
