...
static uint16_t input0_storage[10813440] __attribute__((aligned(64)));
static uint16_t input1_storage[2703360] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 10813440, 1);
  fill_bf16(input1_storage, 2703360, 2);
  fill_f32(input2_storage, 3686400, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3840, 2816, 2816, 1, input1_storage, input1_storage, 0, 2816, 960, 960, 1, input2_storage, input2_storage, 0, 3840, 960, 960, 1, output0_storage, output0_storage, 0, 3840, 960, 960, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00384_38f33678: benchmark completed");
