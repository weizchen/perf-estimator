...
static uint16_t input0_storage[2236416] __attribute__((aligned(64)));
static uint16_t input1_storage[212992] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 2236416, 1);
  fill_f16(input1_storage, 212992, 2);
  fill_f32(input2_storage, 688128, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2688, 832, 832, 1, input1_storage, input1_storage, 0, 832, 256, 256, 1, input2_storage, input2_storage, 0, 2688, 256, 256, 1, output0_storage, output0_storage, 0, 2688, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00199_12caf7ae: benchmark completed");
