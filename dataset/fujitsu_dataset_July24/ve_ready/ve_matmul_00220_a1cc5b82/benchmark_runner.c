...
static uint16_t input2_storage[225280] __attribute__((aligned(64)));
static uint16_t output0_storage[225280] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input2_storage, 225280, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 704, 2944, 2944, 1, input1_storage, input1_storage, 0, 2944, 320, 320, 1, input2_storage, input2_storage, 0, 704, 320, 320, 1, output0_storage, output0_storage, 0, 704, 320, 320, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00220_a1cc5b82: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 225280));
  return 0;
}
