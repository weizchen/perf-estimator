...
static uint16_t input0_storage[70] __attribute__((aligned(64)));
static uint16_t input1_storage[70] __attribute__((aligned(64)));
static uint16_t output0_storage[70] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 70, 1);
  fill_f16(input1_storage, 70, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_elementwise_00262_33f9b1e3: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 70));
  return 0;
}
