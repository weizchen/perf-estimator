...
static float input0_storage[633600] __attribute__((aligned(64)));
static float input1_storage[633600] __attribute__((aligned(64)));
static float output0_storage[633600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 633600, 1);
  fill_f32(input1_storage, 633600, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 18, 40, 20, 44, 35200, 880, 44, 1, input1_storage, input1_storage, 0, 18, 44, 20, 40, 35200, 800, 40, 1, output0_storage, output0_storage, 0, 18, 44, 20, 40, 35200, 800, 40, 1);
  ofa_dma_memory_fence();
  puts("ve_transpose_00080_bb015a11: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 633600));
  return 0;
}
