...
static uint16_t input0_storage[8228864] __attribute__((aligned(64)));
static uint16_t input1_storage[2686976] __attribute__((aligned(64)));
static uint16_t input2_storage[3211264] __attribute__((aligned(64)));
static uint16_t output0_storage[3211264] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 8228864, 1);
  fill_f16(input1_storage, 2686976, 2);
  fill_f16(input2_storage, 3211264, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3136, 2624, 2624, 1, input1_storage, input1_storage, 0, 2624, 1024, 1024, 1, input2_storage, input2_storage, 0, 3136, 1024, 1024, 1, output0_storage, output0_storage, 0, 3136, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00296_ac06392a: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 3211264));
  return 0;
}
