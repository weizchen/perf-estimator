...
static float input2_storage[798720] __attribute__((aligned(64)));
static float output0_storage[798720] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 204800, 1);
  fill_bf16(input1_storage, 1597440, 2);
  fill_f32(input2_storage, 798720, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 640, 640, 1, input1_storage, input1_storage, 0, 640, 2496, 2496, 1, input2_storage, input2_storage, 0, 320, 2496, 2496, 1, output0_storage, output0_storage, 0, 320, 2496, 2496, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00377_ca87305e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 798720));
  return 0;
}
