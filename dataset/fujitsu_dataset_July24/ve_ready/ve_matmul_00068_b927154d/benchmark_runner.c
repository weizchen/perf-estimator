...
static int8_t input1_storage[1523712] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 1523712, 2);
  fill_i32(input2_storage, 1015808, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 384, 384, 1, input1_storage, input1_storage, 0, 384, 3968, 3968, 1, input2_storage, input2_storage, 0, 256, 3968, 3968, 1, output0_storage, output0_storage, 0, 256, 3968, 3968, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00068_b927154d: benchmark completed");
