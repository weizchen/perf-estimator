...
static uint16_t input0_storage[153] __attribute__((aligned(64)));
static uint16_t input1_storage[153] __attribute__((aligned(64)));
static uint16_t output0_storage[153] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 153, 1);
  fill_bf16(input1_storage, 153, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00782_f159f8c2: benchmark completed");
  printf("output0_bf16bits_checksum=%lld\n", checksum_u16(output0_storage, 153));
  return 0;
}
