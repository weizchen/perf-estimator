...
static uint16_t input1_storage[94208] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 94208, 2);
  fill_f32(input2_storage, 12288, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 1472, 1472, 1, input1_storage, input1_storage, 0, 1472, 64, 64, 1, input2_storage, input2_storage, 0, 192, 64, 64, 1, output0_storage, output0_storage, 0, 192, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00152_110ab955: benchmark completed");
