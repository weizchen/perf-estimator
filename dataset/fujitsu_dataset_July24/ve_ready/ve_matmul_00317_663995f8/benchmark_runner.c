...
static uint16_t input0_storage[3727360] __attribute__((aligned(64)));
static uint16_t input1_storage[1810432] __attribute__((aligned(64)));
static float input2_storage[2437120] __attribute__((aligned(64)));
static float output0_storage[2437120] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 3727360, 1);
  fill_bf16(input1_storage, 1810432, 2);
  fill_f32(input2_storage, 2437120, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2240, 1664, 1664, 1, input1_storage, input1_storage, 0, 1664, 1088, 1088, 1, input2_storage, input2_storage, 0, 2240, 1088, 1088, 1, output0_storage, output0_storage, 0, 2240, 1088, 1088, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00317_663995f8: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2437120));
  return 0;
}
