...
static float input0_storage[6856704] __attribute__((aligned(64)));
static float input1_storage[3649536] __attribute__((aligned(64)));
static float input2_storage[8380416] __attribute__((aligned(64)));
static float output0_storage[8380416] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 6856704, 1);
  fill_f32(input1_storage, 3649536, 2);
  fill_f32(input2_storage, 8380416, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 2112, 2112, 1, input2_storage, input2_storage, 0, 3968, 2112, 2112, 1, output0_storage, output0_storage, 0, 3968, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00472_1aafc8b5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 8380416));
  return 0;
}
