...
static float input0_storage[4291] __attribute__((aligned(64)));
static float input1_storage[4291] __attribute__((aligned(64)));
static float output0_storage[4291] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 4291, 1);
  fill_f32(input1_storage, 4291, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4291, 1, input1_storage, input1_storage, 0, 4291, 1, output0_storage, output0_storage, 0, 4291, 1);
  ofa_dma_memory_fence();
  puts("ve_generic_00722_7e89ca8b: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4291));
  return 0;
}
