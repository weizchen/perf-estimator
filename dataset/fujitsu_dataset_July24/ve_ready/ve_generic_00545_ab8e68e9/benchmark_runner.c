...
static uint16_t input0_storage[590] __attribute__((aligned(64)));
static uint16_t input1_storage[590] __attribute__((aligned(64)));
static uint16_t output0_storage[590] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 590, 1);
  fill_bf16(input1_storage, 590, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00545_ab8e68e9: benchmark completed");
  printf("output0_bf16bits_checksum=%lld\n", checksum_u16(output0_storage, 590));
  return 0;
}
