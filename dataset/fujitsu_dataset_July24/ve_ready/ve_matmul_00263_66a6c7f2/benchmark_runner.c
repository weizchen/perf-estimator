...
static uint16_t input0_storage[6881280] __attribute__((aligned(64)));
static uint16_t input1_storage[5775360] __attribute__((aligned(64)));
static uint16_t input2_storage[10780672] __attribute__((aligned(64)));
static uint16_t output0_storage[10780672] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 6881280, 1);
  fill_f16(input1_storage, 5775360, 2);
  fill_f16(input2_storage, 10780672, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3584, 1920, 1920, 1, input1_storage, input1_storage, 0, 1920, 3008, 3008, 1, input2_storage, input2_storage, 0, 3584, 3008, 3008, 1, output0_storage, output0_storage, 0, 3584, 3008, 3008, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00263_66a6c7f2: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 10780672));
  return 0;
}
