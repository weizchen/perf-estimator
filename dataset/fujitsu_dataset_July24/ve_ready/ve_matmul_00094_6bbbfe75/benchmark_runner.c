...
static int8_t input0_storage[11403264] __attribute__((aligned(64)));
static int8_t input1_storage[13778944] __attribute__((aligned(64)));
static int32_t input2_storage[11403264] __attribute__((aligned(64)));
static int32_t output0_storage[11403264] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 11403264, 1);
  fill_i8(input1_storage, 13778944, 2);
  fill_i32(input2_storage, 11403264, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3072, 3712, 3712, 1, input1_storage, input1_storage, 0, 3712, 3712, 3712, 1, input2_storage, input2_storage, 0, 3072, 3712, 3712, 1, output0_storage, output0_storage, 0, 3072, 3712, 3712, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00094_6bbbfe75: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 11403264));
  return 0;
}
