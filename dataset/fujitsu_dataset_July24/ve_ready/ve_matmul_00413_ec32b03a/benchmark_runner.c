...
static float input0_storage[5017600] __attribute__((aligned(64)));
static float input1_storage[10838016] __attribute__((aligned(64)));
static float input2_storage[5529600] __attribute__((aligned(64)));
static float output0_storage[5529600] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 5017600, 1);
  fill_f32(input1_storage, 10838016, 2);
  fill_f32(input2_storage, 5529600, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1600, 3136, 3136, 1, input1_storage, input1_storage, 0, 3136, 3456, 3456, 1, input2_storage, input2_storage, 0, 1600, 3456, 3456, 1, output0_storage, output0_storage, 0, 1600, 3456, 3456, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00413_ec32b03a: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 5529600));
  return 0;
}
