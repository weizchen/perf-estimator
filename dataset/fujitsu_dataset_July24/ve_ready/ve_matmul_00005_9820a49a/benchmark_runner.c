...
static int32_t input2_storage[5111808] __attribute__((aligned(64)));
static int32_t output0_storage[5111808] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i32(input2_storage, 5111808, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1536, 256, 256, 1, input1_storage, input1_storage, 0, 256, 3328, 3328, 1, input2_storage, input2_storage, 0, 1536, 3328, 3328, 1, output0_storage, output0_storage, 0, 1536, 3328, 3328, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00005_9820a49a: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 5111808));
  return 0;
}
