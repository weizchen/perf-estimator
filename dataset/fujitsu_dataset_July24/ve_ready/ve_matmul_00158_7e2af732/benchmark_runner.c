...
static float input2_storage[1867776] __attribute__((aligned(64)));
static float output0_storage[1867776] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input2_storage, 1867776, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3648, 640, 640, 1, input1_storage, input1_storage, 0, 640, 512, 512, 1, input2_storage, input2_storage, 0, 3648, 512, 512, 1, output0_storage, output0_storage, 0, 3648, 512, 512, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00158_7e2af732: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1867776));
  return 0;
}
