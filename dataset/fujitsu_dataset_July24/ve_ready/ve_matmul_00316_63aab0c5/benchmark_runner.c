...
static float input2_storage[331776] __attribute__((aligned(64)));
static float output0_storage[331776] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 86016, 1);
  fill_bf16(input1_storage, 774144, 2);
  fill_f32(input2_storage, 331776, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 192, 448, 448, 1, input1_storage, input1_storage, 0, 448, 1728, 1728, 1, input2_storage, input2_storage, 0, 192, 1728, 1728, 1, output0_storage, output0_storage, 0, 192, 1728, 1728, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00316_63aab0c5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 331776));
  return 0;
}
