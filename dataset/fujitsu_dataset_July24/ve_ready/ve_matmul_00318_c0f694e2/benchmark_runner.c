...
int main(void) {
  fill_bf16(input0_storage, 20480, 1);
  fill_bf16(input1_storage, 327680, 2);
  fill_f32(input2_storage, 65536, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 320, 320, 1, input1_storage, input1_storage, 0, 320, 1024, 1024, 1, input2_storage, input2_storage, 0, 64, 1024, 1024, 1, output0_storage, output0_storage, 0, 64, 1024, 1024, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00318_c0f694e2: benchmark completed");
