...
static float input0_storage[6352896] __attribute__((aligned(64)));
static float input1_storage[270336] __attribute__((aligned(64)));
static float input2_storage[385024] __attribute__((aligned(64)));
static float output0_storage[385024] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 6352896, 1);
  fill_f32(input1_storage, 270336, 2);
  fill_f32(input2_storage, 385024, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3008, 2112, 2112, 1, input1_storage, input1_storage, 0, 2112, 128, 128, 1, input2_storage, input2_storage, 0, 3008, 128, 128, 1, output0_storage, output0_storage, 0, 3008, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00403_53755b1e: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 385024));
  return 0;
}
