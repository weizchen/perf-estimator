...
static float input0_storage[1302528] __attribute__((aligned(64)));
static float input1_storage[2605056] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 1302528, 1);
  fill_f32(input1_storage, 2605056, 2);
  fill_f32(input2_storage, 294912, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 384, 3392, 3392, 1, input1_storage, input1_storage, 0, 3392, 768, 768, 1, input2_storage, input2_storage, 0, 384, 768, 768, 1, output0_storage, output0_storage, 0, 384, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00464_8beb9c37: benchmark completed");
