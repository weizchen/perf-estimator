...
static float input0_storage[720] __attribute__((aligned(64)));
static float input1_storage[33120] __attribute__((aligned(64)));
static float output0_storage[33120] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 720, 1);
  fill_f32(input1_storage, 33120, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 20, 36, 36, 1, input1_storage, input1_storage, 0, 20, 46, 36, 1656, 36, 1, output0_storage, output0_storage, 0, 20, 46, 36, 1656, 36, 1);
  ofa_dma_memory_fence();
  puts("ve_broadcast_00284_6c854be5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 33120));
  return 0;
}
