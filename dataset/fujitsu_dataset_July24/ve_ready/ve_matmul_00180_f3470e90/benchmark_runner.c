...
static uint16_t input0_storage[802816] __attribute__((aligned(64)));
static uint16_t input1_storage[2809856] __attribute__((aligned(64)));
static float input2_storage[2809856] __attribute__((aligned(64)));
static float output0_storage[2809856] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 802816, 1);
  fill_f16(input1_storage, 2809856, 2);
  fill_f32(input2_storage, 2809856, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 896, 896, 1, input1_storage, input1_storage, 0, 896, 3136, 3136, 1, input2_storage, input2_storage, 0, 896, 3136, 3136, 1, output0_storage, output0_storage, 0, 896, 3136, 3136, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00180_f3470e90: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2809856));
  return 0;
}
