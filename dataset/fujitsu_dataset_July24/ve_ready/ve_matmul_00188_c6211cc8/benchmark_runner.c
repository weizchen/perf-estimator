...
static uint16_t input1_storage[3760128] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 3760128, 2);
  fill_f32(input2_storage, 589824, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 512, 3264, 3264, 1, input1_storage, input1_storage, 0, 3264, 1152, 1152, 1, input2_storage, input2_storage, 0, 512, 1152, 1152, 1, output0_storage, output0_storage, 0, 512, 1152, 1152, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00188_c6211cc8: benchmark completed");
