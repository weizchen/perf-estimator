...
static float input0_storage[4797] __attribute__((aligned(64)));
static float input1_storage[4797] __attribute__((aligned(64)));
static float input2_storage[4797] __attribute__((aligned(64)));
static float output0_storage[4797] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 4797, 1);
  fill_f32(input1_storage, 4797, 2);
  fill_f32(input2_storage, 4797, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 123, 39, 39, 1, input1_storage, input1_storage, 0, 123, 39, 39, 1, input2_storage, input2_storage, 0, 123, 39, 39, 1, output0_storage, output0_storage, 0, 123, 39, 39, 1);
  ofa_dma_memory_fence();
  puts("ve_elementwise_00117_0f0ec8e9: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 4797));
  return 0;
}
