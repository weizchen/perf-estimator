...
static float input0_storage[176128] __attribute__((aligned(64)));
static float input1_storage[704512] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 176128, 1);
  fill_f32(input1_storage, 704512, 2);
  fill_f32(input2_storage, 16384, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 64, 2752, 2752, 1, input1_storage, input1_storage, 0, 2752, 256, 256, 1, input2_storage, input2_storage, 0, 64, 256, 256, 1, output0_storage, output0_storage, 0, 64, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00481_619849be: benchmark completed");
