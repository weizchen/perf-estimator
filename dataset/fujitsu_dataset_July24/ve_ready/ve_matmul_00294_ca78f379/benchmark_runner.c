...
static uint16_t input0_storage[3612672] __attribute__((aligned(64)));
static uint16_t input1_storage[5419008] __attribute__((aligned(64)));
static uint16_t input2_storage[1204224] __attribute__((aligned(64)));
static uint16_t output0_storage[1204224] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 3612672, 1);
  fill_f16(input1_storage, 5419008, 2);
  fill_f16(input2_storage, 1204224, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 4032, 4032, 1, input1_storage, input1_storage, 0, 4032, 1344, 1344, 1, input2_storage, input2_storage, 0, 896, 1344, 1344, 1, output0_storage, output0_storage, 0, 896, 1344, 1344, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00294_ca78f379: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 1204224));
  return 0;
}
