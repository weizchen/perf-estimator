...
static float input1_storage[286720] __attribute__((aligned(64)));
static float input2_storage[2007040] __attribute__((aligned(64)));
static float output0_storage[2007040] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 286720, 2);
  fill_f32(input2_storage, 2007040, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 896, 128, 128, 1, input1_storage, input1_storage, 0, 128, 2240, 2240, 1, input2_storage, input2_storage, 0, 896, 2240, 2240, 1, output0_storage, output0_storage, 0, 896, 2240, 2240, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00448_3727e3e5: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 2007040));
  return 0;
}
