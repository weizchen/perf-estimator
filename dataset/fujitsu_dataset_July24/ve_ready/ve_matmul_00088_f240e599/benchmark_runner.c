...
static int8_t input0_storage[2031616] __attribute__((aligned(64)));
static int8_t input1_storage[65536] __attribute__((aligned(64)));
static int32_t input2_storage[507904] __attribute__((aligned(64)));
static int32_t output0_storage[507904] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 2031616, 1);
  fill_i8(input1_storage, 65536, 2);
  fill_i32(input2_storage, 507904, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3968, 512, 512, 1, input1_storage, input1_storage, 0, 512, 128, 128, 1, input2_storage, input2_storage, 0, 3968, 128, 128, 1, output0_storage, output0_storage, 0, 3968, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00088_f240e599: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 507904));
  return 0;
}
