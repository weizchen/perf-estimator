...
static float input0_storage[637] __attribute__((aligned(64)));
static float input1_storage[164346] __attribute__((aligned(64)));
static float output0_storage[164346] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 637, 1);
  fill_f32(input1_storage, 164346, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 7, 91, 91, 1, input1_storage, input1_storage, 0, 258, 7, 91, 637, 91, 1, output0_storage, output0_storage, 0, 258, 7, 91, 637, 91, 1);
  ofa_dma_memory_fence();
  puts("ve_broadcast_00160_71ed5a77: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 164346));
  return 0;
}
