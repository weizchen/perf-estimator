...
static uint16_t input1_storage[241664] __attribute__((aligned(64)));
static uint16_t input2_storage[966656] __attribute__((aligned(64)));
static uint16_t output0_storage[966656] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 241664, 2);
  fill_f16(input2_storage, 966656, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 256, 64, 64, 1, input1_storage, input1_storage, 0, 64, 3776, 3776, 1, input2_storage, input2_storage, 0, 256, 3776, 3776, 1, output0_storage, output0_storage, 0, 256, 3776, 3776, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00261_8f4e3835: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 966656));
  return 0;
}
