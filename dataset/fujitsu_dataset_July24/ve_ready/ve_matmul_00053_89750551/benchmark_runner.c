...
static int8_t input0_storage[7602176] __attribute__((aligned(64)));
static int8_t input1_storage[1572864] __attribute__((aligned(64)));
static int32_t input2_storage[2850816] __attribute__((aligned(64)));
static int32_t output0_storage[2850816] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 7602176, 1);
  fill_i8(input1_storage, 1572864, 2);
  fill_i32(input2_storage, 2850816, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3712, 2048, 2048, 1, input1_storage, input1_storage, 0, 2048, 768, 768, 1, input2_storage, input2_storage, 0, 3712, 768, 768, 1, output0_storage, output0_storage, 0, 3712, 768, 768, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00053_89750551: benchmark completed");
  printf("output0_i32_checksum=%lld\n", checksum_i32(output0_storage, 2850816));
  return 0;
}
