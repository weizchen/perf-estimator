...
static uint16_t input0_storage[4595712] __attribute__((aligned(64)));
static uint16_t input1_storage[557056] __attribute__((aligned(64)));
static float input2_storage[540672] __attribute__((aligned(64)));
static float output0_storage[540672] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 4595712, 1);
  fill_bf16(input1_storage, 557056, 2);
  fill_f32(input2_storage, 540672, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2112, 2176, 2176, 1, input1_storage, input1_storage, 0, 2176, 256, 256, 1, input2_storage, input2_storage, 0, 2112, 256, 256, 1, output0_storage, output0_storage, 0, 2112, 256, 256, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00371_34d5dfc6: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 540672));
  return 0;
}
