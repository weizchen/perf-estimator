...
static uint16_t input0_storage[700416] __attribute__((aligned(64)));
static uint16_t input1_storage[778240] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 700416, 1);
  fill_bf16(input1_storage, 778240, 2);
  fill_f32(input2_storage, 368640, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 1216, 1216, 1, input1_storage, input1_storage, 0, 1216, 640, 640, 1, input2_storage, input2_storage, 0, 576, 640, 640, 1, output0_storage, output0_storage, 0, 576, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00342_402d2041: benchmark completed");
