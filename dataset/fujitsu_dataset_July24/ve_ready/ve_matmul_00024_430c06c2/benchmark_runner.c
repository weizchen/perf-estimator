...
static int8_t input1_storage[5406720] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_i8(input1_storage, 5406720, 2);
  fill_i32(input2_storage, 491520, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1408, 1408, 1, input1_storage, input1_storage, 0, 1408, 3840, 3840, 1, input2_storage, input2_storage, 0, 128, 3840, 3840, 1, output0_storage, output0_storage, 0, 128, 3840, 3840, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00024_430c06c2: benchmark completed");
