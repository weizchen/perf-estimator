...
static uint16_t input0_storage[5136384] __attribute__((aligned(64)));
static uint16_t input1_storage[2334720] __attribute__((aligned(64)));
static uint16_t input2_storage[901120] __attribute__((aligned(64)));
static uint16_t output0_storage[901120] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 5136384, 1);
  fill_f16(input1_storage, 2334720, 2);
  fill_f16(input2_storage, 901120, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1408, 3648, 3648, 1, input1_storage, input1_storage, 0, 3648, 640, 640, 1, input2_storage, input2_storage, 0, 1408, 640, 640, 1, output0_storage, output0_storage, 0, 1408, 640, 640, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00288_c3730f66: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 901120));
  return 0;
}
