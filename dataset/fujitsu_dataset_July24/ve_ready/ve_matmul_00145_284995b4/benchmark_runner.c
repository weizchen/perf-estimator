...
static uint16_t input1_storage[3379200] __attribute__((aligned(64)));
static float input2_storage[270336] __attribute__((aligned(64)));
static float output0_storage[270336] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 3379200, 2);
  fill_f32(input2_storage, 270336, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1600, 1600, 1, input1_storage, input1_storage, 0, 1600, 2112, 2112, 1, input2_storage, input2_storage, 0, 128, 2112, 2112, 1, output0_storage, output0_storage, 0, 128, 2112, 2112, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00145_284995b4: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 270336));
  return 0;
}
