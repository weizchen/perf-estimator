...
static uint16_t input0_storage[1679360] __attribute__((aligned(64)));
static uint16_t input1_storage[81920] __attribute__((aligned(64)));
static float input2_storage[335872] __attribute__((aligned(64)));
static float output0_storage[335872] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 1679360, 1);
  fill_bf16(input1_storage, 81920, 2);
  fill_f32(input2_storage, 335872, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2624, 640, 640, 1, input1_storage, input1_storage, 0, 640, 128, 128, 1, input2_storage, input2_storage, 0, 2624, 128, 128, 1, output0_storage, output0_storage, 0, 2624, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00324_18f85719: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 335872));
  return 0;
}
