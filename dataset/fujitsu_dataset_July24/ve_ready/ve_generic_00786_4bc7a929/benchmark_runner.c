...
static uint16_t input0_storage[87] __attribute__((aligned(64)));
static uint16_t input1_storage[87] __attribute__((aligned(64)));
static uint16_t output0_storage[87] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 87, 1);
  fill_bf16(input1_storage, 87, 2);
  ...
  ofa_dma_memory_fence();
  puts("ve_generic_00786_4bc7a929: benchmark completed");
  printf("output0_bf16bits_checksum=%lld\n", checksum_u16(output0_storage, 87));
  return 0;
}
