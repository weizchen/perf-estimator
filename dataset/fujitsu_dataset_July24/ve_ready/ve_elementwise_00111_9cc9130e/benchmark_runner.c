...
static uint16_t input0_storage[624] __attribute__((aligned(64)));
static uint16_t input1_storage[624] __attribute__((aligned(64)));
static uint16_t input2_storage[624] __attribute__((aligned(64)));
static uint16_t output0_storage[624] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 624, 1);
  fill_f16(input1_storage, 624, 2);
  fill_f16(input2_storage, 624, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 16, 39, 39, 1, input1_storage, input1_storage, 0, 16, 39, 39, 1, input2_storage, input2_storage, 0, 16, 39, 39, 1, output0_storage, output0_storage, 0, 16, 39, 39, 1);
  ofa_dma_memory_fence();
  puts("ve_elementwise_00111_9cc9130e: benchmark completed");
  printf("output0_f16bits_checksum=%lld\n", checksum_u16(output0_storage, 624));
  return 0;
}
