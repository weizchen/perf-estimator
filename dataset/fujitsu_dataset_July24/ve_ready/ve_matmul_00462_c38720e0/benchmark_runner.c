...
static float input0_storage[8470528] __attribute__((aligned(64)));
static float input1_storage[180224] __attribute__((aligned(64)));
static float input2_storage[192512] __attribute__((aligned(64)));
static float output0_storage[192512] __attribute__((aligned(64)));
...
int main(void) {
  fill_f32(input0_storage, 8470528, 1);
  fill_f32(input1_storage, 180224, 2);
  fill_f32(input2_storage, 192512, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 3008, 2816, 2816, 1, input1_storage, input1_storage, 0, 2816, 64, 64, 1, input2_storage, input2_storage, 0, 3008, 64, 64, 1, output0_storage, output0_storage, 0, 3008, 64, 64, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00462_c38720e0: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 192512));
  return 0;
}
