...
static uint16_t input0_storage[221184] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 221184, 1);
  fill_bf16(input1_storage, 983040, 2);
  fill_f32(input2_storage, 1474560, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 576, 384, 384, 1, input1_storage, input1_storage, 0, 384, 2560, 2560, 1, input2_storage, input2_storage, 0, 576, 2560, 2560, 1, output0_storage, output0_storage, 0, 576, 2560, 2560, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00322_63e52b69: benchmark completed");
