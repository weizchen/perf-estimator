...
static float input1_storage[10] __attribute__((aligned(64)));
static float input2_storage[10] __attribute__((aligned(64)));
static float input3_storage[10] __attribute__((aligned(64)));
static float output0_storage[10] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f32(input1_storage, 10, 2);
  fill_f32(input2_storage, 10, 3);
  fill_f32(input3_storage, 10, 4);
  ...
  ofa_dma_memory_fence();
  puts("ve_select_00264_ad058245: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 10));
  return 0;
}
