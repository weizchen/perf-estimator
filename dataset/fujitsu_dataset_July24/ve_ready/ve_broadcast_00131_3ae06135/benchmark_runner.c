...
static int8_t input0_storage[324] __attribute__((aligned(64)));
static int8_t input1_storage[1296] __attribute__((aligned(64)));
static int8_t output0_storage[1296] __attribute__((aligned(64)));
...
int main(void) {
  fill_i8(input0_storage, 324, 1);
  fill_i8(input1_storage, 1296, 2);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 4, 81, 81, 1, input1_storage, input1_storage, 0, 4, 4, 81, 324, 81, 1, output0_storage, output0_storage, 0, 4, 4, 81, 324, 81, 1);
  ofa_dma_memory_fence();
  puts("ve_broadcast_00131_3ae06135: benchmark completed");
  printf("output0_i8_checksum=%lld\n", checksum_i8(output0_storage, 1296));
  return 0;
}
