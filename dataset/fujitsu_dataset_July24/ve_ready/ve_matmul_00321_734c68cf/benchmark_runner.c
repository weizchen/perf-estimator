...
static uint16_t input0_storage[880640] __attribute__((aligned(64)));
static uint16_t input1_storage[352256] __attribute__((aligned(64)));
...
int main(void) {
  fill_bf16(input0_storage, 880640, 1);
  fill_bf16(input1_storage, 352256, 2);
  fill_f32(input2_storage, 40960, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 320, 2752, 2752, 1, input1_storage, input1_storage, 0, 2752, 128, 128, 1, input2_storage, input2_storage, 0, 320, 128, 128, 1, output0_storage, output0_storage, 0, 320, 128, 128, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00321_734c68cf: benchmark completed");
