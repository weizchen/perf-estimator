...
static uint16_t input0_storage[4931584] __attribute__((aligned(64)));
static uint16_t input1_storage[4931584] __attribute__((aligned(64)));
static float input2_storage[7573504] __attribute__((aligned(64)));
static float output0_storage[7573504] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 4931584, 1);
  fill_f16(input1_storage, 4931584, 2);
  fill_f32(input2_storage, 7573504, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 2752, 1792, 1792, 1, input1_storage, input1_storage, 0, 1792, 2752, 2752, 1, input2_storage, input2_storage, 0, 2752, 2752, 2752, 1, output0_storage, output0_storage, 0, 2752, 2752, 2752, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00115_0ccc1c51: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 7573504));
  return 0;
}
