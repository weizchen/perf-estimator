...
static uint16_t input1_storage[3956736] __attribute__((aligned(64)));
static float input2_storage[1978368] __attribute__((aligned(64)));
static float output0_storage[1978368] __attribute__((aligned(64)));
...
int main(void) {
  ...
  fill_f16(input1_storage, 3956736, 2);
  fill_f32(input2_storage, 1978368, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 1344, 2688, 2688, 1, input1_storage, input1_storage, 0, 2688, 1472, 1472, 1, input2_storage, input2_storage, 0, 1344, 1472, 1472, 1, output0_storage, output0_storage, 0, 1344, 1472, 1472, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00177_bcdf6f61: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 1978368));
  return 0;
}
