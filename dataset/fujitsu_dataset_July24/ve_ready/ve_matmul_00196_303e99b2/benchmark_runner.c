...
static uint16_t input1_storage[4091904] __attribute__((aligned(64)));
static float input2_storage[303104] __attribute__((aligned(64)));
static float output0_storage[303104] __attribute__((aligned(64)));
...
int main(void) {
  fill_f16(input0_storage, 221184, 1);
  fill_f16(input1_storage, 4091904, 2);
  fill_f32(input2_storage, 303104, 3);
  memset(output0_storage, 0, sizeof(output0_storage));
  ofa_dma_memory_fence();
  kernel(input0_storage, input0_storage, 0, 128, 1728, 1728, 1, input1_storage, input1_storage, 0, 1728, 2368, 2368, 1, input2_storage, input2_storage, 0, 128, 2368, 2368, 1, output0_storage, output0_storage, 0, 128, 2368, 2368, 1);
  ofa_dma_memory_fence();
  puts("ve_matmul_00196_303e99b2: benchmark completed");
  printf("output0_f32_checksum=%.10e\n", checksum_f32(output0_storage, 303104));
  return 0;
}
